...
/** Parse "개발/Frontend" → { parent: "개발", child: "Frontend" } */
export function parseCategory(category: string): { parent: string; child: string | null; full: string } {
  const parts = category.split('/');
  return { parent: parts[0], child: parts.length > 1 ? parts.slice(1).join('/') : null, full: category };
}

/** Build a hierarchical category tree from post category strings */
...
export function buildCategoryTree(categories: string[]): Map<string, CategoryNode> {
  ...
  for (const cat of categories) {
    ...
    if (!tree.has(parent)) tree.set(parent, { name: parent, count: 0, children: new Map() });
    const parentNode = tree.get(parent)!;
    parentNode.count++;
    if (child) {
      if (!parentNode.children.has(child)) parentNode.children.set(child, { name: child, count: 0, children: new Map() });
