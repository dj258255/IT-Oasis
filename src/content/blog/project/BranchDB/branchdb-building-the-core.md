...
`Tree`가 `Option<Arc<Node>>`인 게 전부다. `None`이 빈 트리, `Some(arc)`가 루트를 가진 트리. 그리고 **브랜치란 이 `Tree` 하나**다. 포크는 이 `Arc`를 복제하는 것이고, O(1)이며 데이터 복사가 0이다.
...
다음 글에서는 `diff`와 `merge`를 붙인다. 두 브랜치가 무엇이 다른지 알아내고, 좋은 갈래를 main에 합치는 것, 이것이 에이전트가 "여러 우주를 돌려보고 제일 좋은 우주만 남긴다"의 마지막 조각이다.
