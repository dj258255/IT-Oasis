...
그렇게 만든 게 **minidb** 다. PostgreSQL·MySQL 같은 관계형 DB가 내부에서 어떻게 동작하는지를, C로 한 겹씩 직접 구현하며 해부한 학습 프로젝트다. 이 시리즈는 페이저부터 트랜잭션·조인·집계까지 밑바닥부터 쌓아 올린 전 과정을 다섯 편으로 나눠 담는다(전 계층 테스트 196개). 그 첫 편인 이 글은 맨 아래 — 바이트를 디스크에 얹는 **저장 계층** 을 짓는다.
