...
인덱스도 페이지 단위로 디스크에 살아요. 이진 트리는 노드 하나에 키가 하나라, 높이가 log₂(n)으로 깊습니다. 한 단계 내려갈 때마다 페이지를 한 번 읽어야 하니 디스크 접근이 그만큼 많아져요 — 100만 행이면 높이가 약 20단이라, 최악의 경우 높이만큼(약 20번) 페이지를 읽어야 합니다(물론 버퍼 풀·page cache에 이미 올라온 페이지는 디스크를 다시 안 때려요).

**B+Tree**는 **노드 하나(=페이지 하나)에 키를 수십~수백 개** 담아 부채살처럼(high fan-out, f) 갈라져요. 그래서 높이가 BST의 log₂N이 아니라 **log_f N**으로 확 낮아집니다 — 같은 100만 행이라도 BST는 log₂(10⁶)≈20단인데, fan-out이 수백이면 몇 단에 그쳐요(키 길이·페이지 크기·fan-out에 따라 다르지만, 흔한 8KB 페이지·fan-out 수백이면 대략 3~4단). "한 번의 디스크 I/O로 가능한 많은 키를 본다"가 B+Tree의 전부입니다.
...
여기서 B+Tree가 "균형(Balanced)"을 유지하는 비밀이 나와요. AVL처럼 회전으로 균형을 맞추는 게 아니라, **위에서가 아니라 아래(리프)에서 자라기 때문에** 모든 리프가 항상 같은 깊이를 유지합니다. 한쪽으로 절대 기울지 않아요 — 모든 리프가 같은 깊이라서 "Balanced tree"인 거예요.
...
내부 노드 분할은 미묘하게 달라요. 가운데 키를 **복사가 아니라 위로 올려보냅니다(push up)** — 내부 노드의 키는 길잡이일 뿐이라 양쪽에 둘 필요가 없거든요. 리프는 키를 복사(copy up), 내부는 키를 올림(push up) — 이 차이가 B+Tree(값은 리프에만, 리프끼리 옆으로 연결)와 B-Tree(값이 내부 노드에도 들어감)를 가르는 지점이기도 합니다.
...
> **실무/면접 포인트**: "WAL이 있으면 **커밋 시점엔** fsync를 로그에만 하면 된다"가 핵심이에요. 데이터 페이지 수십 개에 무작위 fsync를 하는 대신, 커밋 땐 순차 로그 하나에만 fsync하니 빠릅니다(데이터 파일 fsync는 나중에 체크포인트에서 모아서 합니다). 진짜 DB의 `synchronous_commit`(PG)·`innodb_flush_log_at_trx_commit`(MySQL)이 바로 이 "커밋 시 fsync를 얼마나 빡세게 할까" 다이얼이에요.
...
- **B+Tree 인덱스** — 인덱스는 "키 -> RID" 정렬 구조로 O(n) 풀 스캔을 O(log n)으로 줄인다. 이진 트리가 아니라 B+Tree인 건 **디스크** 때문 — 노드 하나에 키 수백 개를 담아(high fan-out) 높이가 보통 수 단(흔히 3~4단)으로 낮다.
...
The index lives on disk in pages too. A binary tree holds one key per node, so its height is log₂(n) — deep. Each step down reads one page, so disk accesses pile up. A million rows is a height of about 20, so worst case you read about as many pages as the height (~20) — though pages already in the buffer pool / page cache do not hit disk again.

A **B+Tree** packs **tens to hundreds of keys per node (= per page)**, fanning out like a paper fan (high fan-out, f). So its height becomes **log_f N** instead of a BST's log₂N — for the same million rows, a BST is log₂(10⁶)≈20 levels while a fan-out of hundreds drops it to a few levels (it depends on key length, page size, and fan-out, but with a typical 8KB page and fan-out in the hundreds it is roughly 3-4). "See as many keys as possible per disk I/O" is the whole of the B+Tree.
...
Here is the secret of how a B+Tree stays "Balanced." Not by rotations like an AVL tree, but **because it grows from the bottom (the leaves), not the top**, all leaves always stay at the same depth. It never leans to one side — all leaves at equal depth is what "Balanced tree" means here.
...
> **Practical/interview note**: the key is "with a WAL, **at commit time** you only fsync the log." Instead of random fsyncs across dozens of data pages, a commit fsyncs one sequential log — fast (the data file is fsynced later, batched at a checkpoint). Real DBs' `synchronous_commit` (PG) and `innodb_flush_log_at_trx_commit` (MySQL) are exactly this "how hard to fsync at commit" dial.
...
- **B+Tree index** — an index is a sorted "key -> RID" structure that cuts an O(n) full scan to O(log n). It is a B+Tree, not a binary tree, because of **disk** — hundreds of keys per node (high fan-out) keep the height to a few levels (often 3-4).
