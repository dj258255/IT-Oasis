...
그 차이가 이 편의 전부예요. 이번 편은 새 계층을 짓는 1~5편과 달리, **"기능을 받으면 먼저 이게 기존 것의 조합으로 표현되나?를 묻는 습관"** 을 BETWEEN·LIKE·IN 세 기능으로 따라가 봅니다.
...
이러면 [2편에서 만든 파서](/blog/project/minidb/minidb-2-sql-engine)가 뱉는 AST는 `id BETWEEN 2 AND 5`나 `id >= 2 AND id <= 5`나 **완전히 똑같아요.** 실행기, WHERE 평가, DNF 처리 — 그 아래 전부가 BETWEEN이라는 단어를 영영 모른 채로 동작합니다. 새 코드 경로가 0개라는 건 곧 새 버그 가능성도 0개라는 뜻이에요.
...
> **주의 — 공짜엔 솔직한 한계 하나**: minidb의 인덱스 경로는 ["조건이 딱 하나일 때"만](/blog/project/minidb/minidb-3-index-wal) 발동하는데, BETWEEN은 조건 두 개로 풀리니 그 경로를 안 타고 풀 스캔으로 떨어진다. 결과는 정확하지만 인덱스 가속은 못 받는다.
...
먼저 정한 것 하나: LIKE는 정규식이 아니에요. SQL의 `LIKE`는 와일드카드가 딱 두 개(`%`, `_`)뿐인 축소판 글롭(glob)입니다. 정규식 엔진을 통째로 들고 오는 건 학습용 DB엔 과하고, SQL 표준 의미와도 안 맞아요.
...
| **백트래킹 two-pointer** (minidb 선택) | O(n·m) | **O(1)** | 인위적 최악 패턴에서 출렁임 |
...
이 방식은 재귀 없이 시간 O(n·m), 공간 O(1)이에요. 위키피디아 "Matching wildcards" 항목이나 여러 글롭 구현이 정확히 이 골격을 씁니다 — 인위적인 최악 케이스(`%`가 잔뜩 박힌 패턴)에선 백트래킹이 출렁이지만, 현실의 패턴에선 단순 반복문이 NFA 같은 정교한 방식보다 대체로 빨라요. 학습용으로도, 실용으로도 합리적인 타협점이었습니다.
...
minidb의 `LIKE`는 풀 스캔이에요. 모든 행을 꺼내 `like_match`를 돌립니다. 그런데 이건 학습용의 한계가 아니라 — **`LIKE '%키워드%'`는 진짜 DB에서도 인덱스를 못 써요.** 이게 백엔드에서 자주 데이는 지점입니다.
...
사실 저는 이걸 머리가 아니라 몸으로 배웠어요. [WikiEngine](/blog/project/WikiEngine/lucene-decision)에서 1,215만 건에 `LIKE '%검색어%'`를 날렸다가 [5초 만에 타임아웃](/blog/project/WikiEngine/fulltext-ngram-index)이 나고, 커넥션 풀이 마르며 검색과 무관한 API까지 503으로 무너졌습니다. EXPLAIN을 찍으니 `type=ALL`, 2,700만 행 풀 스캔이었어요. 결국 MySQL FULLTEXT ngram을 거쳐 임베디드 Lucene으로 갈아엎었습니다.
...
멤버십을 검사하는 실행기 코드도, 메모리를 정리하는 코드도, 인덱스 플래너도 한 줄을 안 고쳤어요. 서브쿼리가 없으면(`sub`가 NULL이면) 실행 직전 prepare 단계가 알아서 건너뛰도록 이미 짜여 있었기 때문입니다.
...
기능을 받으면 먼저 "이게 기존 것의 조합으로 표현되나?"를 묻는 습관 — 이게 이 셋을 붙이며 굳어진 가장 실용적인 교훈이에요. 여기까지 더해 minidb의 테스트는 223개가 됐습니다. 작은 DB를 키우며 가장 자주 써먹은 사고가 바로 이거였어요: 새 기능 앞에서 "이건 진짜 새 코드가 필요한가, 아니면 있는 걸 다시 쓰면 되나"를 먼저 묻는 것.
...
That contrast is the whole of this part. Unlike Parts 1-5, which build a new layer each time, this one follows a habit through three features (BETWEEN, LIKE, IN): **when a feature lands, first ask "is this expressible as a combination of what already exists?"**
...
Now the AST emitted by [the parser from Part 2](/blog/project/minidb/minidb-2-sql-engine) is **identical** for `id BETWEEN 2 AND 5` and `id >= 2 AND id <= 5`. The executor, WHERE evaluation, DNF handling — everything below runs forever unaware of the word BETWEEN. Zero new code paths means zero new bug surface.
...
> **Caution — one honest limit on the free lunch**: minidb's index path fires [only when there is exactly one condition](/blog/project/minidb/minidb-3-index-wal), but BETWEEN unfolds into two, so it skips that path and falls to a full scan. The result is correct but gets no index speedup.
...
One decision up front: LIKE is not regex. SQL's `LIKE` is a reduced glob with exactly two wildcards (`%`, `_`). Dragging in a whole regex engine is overkill for a learning DB and doesn't match SQL standard semantics.
...
| **Backtracking two-pointer** (minidb's choice) | O(n·m) | **O(1)** | wobbles on artificial worst-case patterns |
...
This runs in O(n·m) time and O(1) space, no recursion. Wikipedia's "Matching wildcards" entry and many glob implementations use exactly this skeleton — on artificial worst cases (patterns crammed with `%`) the backtracking wobbles, but on real-world patterns a plain loop is generally faster than fancier NFA-style approaches. A reasonable compromise for both learning and practice.
...
minidb's `LIKE` is a full scan. It pulls every row and runs `like_match`. But this is not a learning-DB limitation — **`LIKE '%term%'` can't use an index in real DBs either.** This is a common burn on the backend.
...
Not a line changed in the executor code that checks membership, the code that frees memory, or the index planner. Because it was already written so that when there is no subquery (`sub` is NULL), the prepare step right before execution skips it on its own.
...
The habit of asking, when a feature lands, "is this expressible as a combination of what exists?" — that is the most practical lesson that set in while adding these three. With them, minidb's tests reached 223. The thought I reached for most often while growing a small DB was exactly this: before a new feature, first ask "does this really need new code, or can I reuse what's there?"
