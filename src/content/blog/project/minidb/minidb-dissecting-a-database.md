...
그렇게 만든 게 **minidb** 다. PostgreSQL·MySQL 같은 관계형 DB가 내부에서 어떻게 동작하는지를, C로 한 겹씩 직접 구현하며 해부한 학습 프로젝트다. 이 글은 페이저부터 트랜잭션까지 아홉 계층을 밑바닥부터 쌓고, 그 위에 다중 테이블·조인·집계까지 올린 전 과정을 한 번에 담는다. 각 계층은 전부 테스트로 검증했다(총 182개).
