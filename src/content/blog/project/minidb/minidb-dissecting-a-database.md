...
그리고 이 인덱스를 실행기에 연결했다. INSERT는 `(PK -> RID)` 를 인덱스에 등록하고, `WHERE id = 2` 처럼 인덱스된 컬럼을 쓰면 풀 스캔 대신 `btree_search` -> `heap_get` 한 줄만 읽는다. "쓸 수 있으면 인덱스를 쓴다"는 이 분기가 **쿼리 플래너의 가장 단순한 형태** 다. `WHERE`는 `=` 외에 `<` `>` `<=` `>=` `!=` 도 받는데, `=` 일 때만 인덱스로 한 줄을 집고 범위·부등 조건은 풀 스캔으로 떨어진다 — 연산자에 따라 실행 계획이 갈리는 게 곧 플래너가 하는 일이다. 조건은 `AND`로 여러 개를 묶을 수도 있다(`WHERE id > 1 AND name != 'kim'`).
