---
title: '테스트가 통과해도 DB 상태를 믿을 수 없는 이유'
description: '테스트는 다 통과하는데 띄우니 결제가 DB에 없었다. 이벤트는 두 번 인코딩되고, 배치는 아무도 안 부르고, 금고는 비어 있었고, 이스케이프 함수는 안 통하는 자리에 있었다.'
date: 2026-04-06
category: study/be-commerce
coverImage: "/uploads/project/be-commerce/thumbs/be-commerce-ch2.svg"
draft: true
series: "BE-commerce"
seriesOrder: 4
tags:
...
그래서 이 버그는 단위 테스트의 사각지대에 정확히 숨어 있었습니다. [부하테스트(k6)](/blog/project/be-commerce/be-commerce-ch2-concurrency-and-load)도 HTTP 200만 봤지 DB 상태는 안 봤습니다. 에스크로가 "주문이 PAID여야 한다"는 조건을 실제로 요구하면서, 실기동에서 처음 터진 것입니다.
...
> 결제 확정 버그는 실기동 E2E가, [outbox 데드락](/blog/project/be-commerce/be-commerce-ch2-concurrency-and-load)은 스파이크 실측이, 이번 이중 인코딩은 실제 소비자를 붙이는 것이 드러냈습니다. 소비하는 쪽이 생기기 전까지 만드는 쪽은 자기가 옳다고 믿습니다. 계약의 검증은 언제나 반대편 끝에서 옵니다.
...
이 시스템은 [Modulith의 Event Publication Registry](/blog/project/be-commerce/be-commerce-ch1-what-to-trust)를 outbox로 씁니다(`event_publication` 테이블). 이벤트를 발행하면 여기 기록되고, 리스너가 처리하면 완료 표시됩니다.
...
> 참고로 Modulith 1.3엔 `completion-mode`로 `DELETE`(완료 즉시 삭제)나 `ARCHIVE`(별도 테이블로 이동) 모드도 있습니다. 감사 이력을 잠깐이라도 남기고 싶어서 "완료 유지 + 주기 purge"를 택했습니다. outbox의 목적은 유실 방지지 영구 보관이 아닙니다. **이 선택은 나중에 뒤집혔습니다.** 완료 이벤트가 안 지워져 부하 실험 여섯 번 만에 15만 행이 됐고, 완료 처리 쿼리가 그 전체를 풀스캔했습니다. 지금은 `ARCHIVE`입니다([측정이 틀렸던 편](/blog/project/be-commerce/be-commerce-ch7-measuring-wrong)).
...
자체 모델링한 PG·내부 테스트 진입점의 웹훅 서명 검증은 HMAC 시크릿으로 "이 테스트 계약을 만족하는 요청인지"를 확인합니다. 이 설명을 실제 토스 일반 결제 웹훅 전체에 적용하면 안 됩니다.
...
당시 코드가 `test-webhook-secret`이라는 약한 기본값을 조용히 쓰고 있었습니다. 운영에서 환경변수를 깜빡 안 넣으면? 앱은 아무 경고 없이 뜨고, 공개된 문자열로 서명을 검증합니다. 공격자가 그 문자열로 서명을 위조해 가짜 웹훅을 보낼 수 있습니다. 현재 코드는 기본 시크릿을 운영 계약으로 사용하지 않고, 미설정·약한 값이면 기동을 거부하도록 바뀌었습니다.
...
[rate limiter](/blog/project/be-commerce/be-commerce-ch2-concurrency-and-load)나 토큰 저장소와 같은 문제였고, 해법도 같습니다. Redis 공유 카운터.
