---
title: 'Spring 백엔드에서 동시성 제어를 어디에 둘 것인가'
description: '같은 자원에 몰린 요청을 어디서 기다리게 할지 실측으로 골랐다. 한 번은 재는 방법이 틀려 결론을 되돌렸다. 문제는 동시 요청이 아니라 쌓인 일감과 스캔 폭이었다.'
date: 2026-03-19
...
`thresholds`를 걸어서 p95가 1,500ms를 넘거나 오류율이 1%를 넘으면 k6가 실패로 종료합니다. PR마다 소규모 부하로 성능 회귀를 커밋 단위로 잡습니다. 개선 스토리는 카카오페이 프레임(목표→한계→병목→개선→전후 수치)으로 [총정리 편](/blog/project/be-commerce/be-commerce-0-overview)에 정리했습니다.

### 버린 것

경합을 앱에서 다룰 손잡이. 조건부 UPDATE는 대기열도 재시도도 없애는 대신, **셋 다 결국 같은 hot row의 배타적 락에서 직렬화된다**는 사실은 그대로입니다. 그 행의 처리량이 곧 그 상품의 판매 속도 상한이 됩니다. 규모가 커지면 검토할 것은 분산락이 아니라 상품별 대기열·재고 예약·hot 상품 분리입니다.
...
![폭주 제어 데모: 연타 3건이 429 RATE_LIMITED로 쳐내지고, 한정판은 입장권 없이 429 → 대기열 입장 → 201](/uploads/project/be-commerce/demo/demo-overload.png)
...
초과 요청 전부. 97.5%를 429로 문 앞에서 버렸고, 그 안에는 정상 사용자도 섞여 있습니다. 지킨 것은 **통과한 요청의 속도**이지 요청 자체가 아닙니다. 그리고 이 실험은 계정 하나로 돌아 **전역 층이 부하를 받은 적이 없었습니다** — 반쪽 실험이라는 걸 나중에 다시 재면서 알았습니다.
...
5xx가 사라졌습니다. 다만 정직하게 적어둘 게 있습니다. 로그를 확인하니 **이번 런은 데드락 자체가 0건**이었습니다. 데드락은 커밋 타이밍이 겹쳐야 나는 확률적 현상이라 매번 발생하지 않습니다. 그래서 정확한 문장은 **"재시도는 단위 테스트 3종으로 검증된 안전망으로 들어갔고, 다음에 데드락이 나면 카운터에 찍히며 조용히 흡수된다"**까지입니다. "재시도가 113건을 흡수했다"고 쓰면 과장이 됩니다. 측정 결과를 서사에 맞춰 부풀리지 않는 것, [영속 버그 때](/blog/project/be-commerce/be-commerce-ch2-runtime-truths)처럼 이번에도 지켰습니다.
...
재시도 지연과 그 상한. 데드락이면 최대 3회 다시 도는데 그동안 응답이 늦어지고, 3회를 넘기면 그대로 실패합니다. 그리고 이 런은 **데드락이 0건이라 재시도가 실제로 흡수한 것이 없습니다** — 들어간 것은 단위 테스트로 고정된 안전망이지 개선 수치가 아닙니다.
...
낙관적 락의 감시. 원자 증가는 그 감시를 벗어나므로 `version`을 함께 올려 다른 경로의 안전장치를 유지했습니다(안 올리면 lost update). 이벤트 비동기 분리도 이번엔 하지 않아 **적립 실패가 결제 응답까지 번지는 경로는 그대로**입니다. 사용·환불 경로가 충돌을 더 자주 맞을 수 있는데 **그 경합 비용은 재지 않았습니다.**
...
앞의 넷은 **동시에 들어오는 요청**이 문제였습니다. 이건 **쌓인 일감**이 문제였고,
찾은 경위가 달랐습니다.

카카오페이 데이터 조직의 기술 블로그를 훑다가 같은 주제가 반복해 나오는 것을 봤습니다.

- 원장 통계 배치가 수억 건을 한 번에 읽다 OOM. 파티셔닝으로 97분에서 8분 55초
- 정산팀 조회가 하루 25만 번에서 **1억 번**으로 늘면서 Limit-Offset 이 5천만 번째부터
...
**실패한 건이 대상에 남기 때문**입니다. 이 배치들은 한 건이 실패해도 멈추지 않고 다음
...
틱당 처리량 천장. 상한을 걸어 메모리를 지키는 대신 **한 틱이 처리할 수 있는 양에 천장이 생깁니다.** 이 대가는 정산 배치에서 나중에 드러납니다 — 상한 하나 × 하루 한 틱 = 하루 처리량 상한이 되고, 확정이 그보다 많으면 지연이 무한히 쌓입니다. 커서도 안 들였습니다. 필요가 증명되기 전에 무거운 장치를 들이지 않는다는 판단이고, 그 판단을 적어 둬야 나중에 무엇이 바뀌었는지 알 수 있습니다.
...
쓰기 속도 3% 안쪽과 **안 걸기로 한 4개**. 조회 107배를 쓰기 3% 안쪽으로 사는 교환이라 받았지만 공짜는 아닙니다. 어떤 조회는 일부러 인덱스를 안 걸었고, 그 근거를 마이그레이션과 테스트 주석 양쪽에 남겼습니다 — 빠진 것과 일부러 뺀 것을 구별할 수 있어야 나중에 보는 사람이 판단할 수 있습니다.
...
> **2026-09-26 정정.** 이 표를 만든 측정은 **빈 목록**을 쟀습니다. 부하 스크립트가 실행마다 새 계정을 만들어
> 로그인했고, 주문은 시드 단계가 만든 다른 계정에만 들어 있었습니다. 조회한 40계정의 주문은 0건이었고 응답은
> 빈 배열이었는데 상태 코드가 200이라 성공으로 셌습니다. 스크립트를 고쳐 주문이 30건씩 있는 계정으로 다시 재니
> p95가 4.4 · 6.7 · 10.1ms(400 · 1,000 · 1,800/s)로 이 표와 같은 크기였습니다. 수치는 그대로 두고, 그 수치를
> 만든 측정이 틀려 있었다는 것을 적어 둡니다([#352](https://github.com/dj258255/BE-commerce/issues/352)).
...
**못 잰 것도 적었습니다.** 부하를 건 것은 목록 조회 한 종류였고, 타임라인 조립과 데이터가 메모리를 넘는
경우는 재지 않았습니다. 2026-09-26에 둘 다 쟀습니다. 타임라인은 요청당 SELECT 17개로 무릎이 400/s였고
쓰기 경로보다 높았습니다([#350](https://github.com/dj258255/BE-commerce/issues/350)). 자주 조회되는 사용자의
데이터가 버퍼 풀보다 크면 무릎이 400~1,000/s 사이로 내려가 DB가 병목이 됐습니다
([#352](https://github.com/dj258255/BE-commerce/issues/352)). 그때는 캐시를 넣기 전에 버퍼 풀부터 봅니다.
부하기와 서버가 같은 노트북이라 2,200/s 위는 이 구성으로 못 잽니다.
