...
- **문제**: 자막과 요약은 같은 transcript 에서 나오지만 **비용 단위가 다릅니다.** STT 는 오디오 시간 과금, LLM 은 토큰 과금입니다.
- **판단**: **자막 hot path 에서는 LLM 을 부르지 않습니다.**
    - 부르면 모델 지연과 장애점이 **자막 조각마다** 붙습니다
    - 대신 결정적 용어 사전으로 `python → 파이썬` 같은 표기만 보정합니다. **비용 0원, 지연 0ms, 결과가 항상 같습니다**
    - **→ LLM 은 지연을 허용하는 경로(회의 후 요약·검색 색인)로 뺐습니다**
- **partial 은 저장하지 않습니다**: 화면에서 계속 바뀌는 중간 결과를 저장하면 같은 발화가 여러 번 들어가 **요약 토큰을 낭비**하고, transcript 가 "사용자가 본 화면" 과 "실제 발화" 의 뒤섞임이 됩니다.
- **저장은 발행 경로 밖으로**: 시험이 **"요청 반환 시점에 아직 DB 에 없어야 한다"** 를 단언합니다. 있으면 동기 저장이라는 뜻이고, 자막 표시가 DB 를 기다린다는 뜻입니다.
- **순서를 무엇으로 세우나**: 비동기라 저장 순서와 발화 순서가 다릅니다. `sequence` NULL 후순위 → `sequence` → `spokenAt` → `id`. **여기서 순서가 흔들리면 요약 결과도 흔들립니다.**
- **정직하게 적어 둔 것**: CLOVA batch STT 라 **지금 이것은 실시간 자막이 아닙니다.** 결과에 `realtime: false`, `approximate_timing: true` 를 값으로 박아 뒀습니다.
...
나중에 이 한 줄이 결정을 하나 더 만들게 됩니다. MCP 도구로 자막을 꺼내 쓰면서 DB 직접 조회를 기각한 이유가 정확히 이 문장입니다.
