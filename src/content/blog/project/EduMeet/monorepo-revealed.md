...
- **문제**: 저장소를 합치는 건 정리 작업이라고 생각했는데, 합치고 나서 **AI 연동이 한 번도 동작한 적 없다는 것**을 알았습니다. URL 의 `{meetingId}` 가 치환조차 안 됐고 `X-Internal-Token` 이 없었습니다.
    - **문서는 이미 알고 있었습니다** — *"파이썬 저장소가 이 리포에 없어서 클라이언트 쪽 변경은 미반영이다"*
    - **→ 몰라서 안 고친 게 아니라 고칠 수 없는 위치에 있었습니다. 그리고 문서는 CI 를 실패시키지 못합니다**
- **이력을 잃지 않고 합치기**: `git subtree add` 는 `git blame` 은 되는데 `git log -- frontend/...` 가 0건이었습니다. 병합 이전 커밋의 **경로가 루트**라 필터가 안 걸립니다.
    - `filter-repo --to-subdirectory-filter` 로 경로를 재작성해 **3건 → 224건**
    - **백엔드는 SHA 를 바꾸지 않았습니다** — 같은 필터를 걸면 머지된 PR 90여 개의 참조가 전부 깨집니다
- **해결**: 계약을 `contracts/internal-api.json` 으로 옮겨 **Java 테스트와 파이썬 테스트가 같은 파일을 읽게** 했습니다. 계약만 바꿨을 때 Gradle 이 테스트를 UP-TO-DATE 로 건너뛰던 구멍도 `inputs.file` 로 막고 되돌려 확인했습니다.
- **곁가지**: FastAPI 가 `async def` 를 이벤트 루프에서 직접 도는데 그 안에서 최대 10분 동기 블로킹을 하고 있었습니다. 동시 2요청 **1.02초(직렬) → 0.52초**.
