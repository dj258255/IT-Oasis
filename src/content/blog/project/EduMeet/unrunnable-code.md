...
방송 모드 세 개에 테스트를 붙이고 있었습니다. `BROADCAST` 세션은 HLS 요청을 올바르게 만드는가, `AUDIO_BROADCAST` 는 비디오 발행을 막는가. 전부 초록불이었습니다.

그런데 한 가지를 아무도 안 물어봤습니다.

> **그 세션을 만들 수는 있는가?**

만들 수 없었습니다. 세션 생성 요청 DTO에 `sessionType` 필드가 아예 없어서 모든 세션이 `INTERACTIVE` 로 저장되고 있었습니다. **기능 네 개가 통째로 도달 불가였고, 테스트는 픽스처를 직접 만들어서 그 사실을 영영 안 물어보고 있었습니다.**

테스트가 **부품을 검증했지, 부품이 연결되어 있는지는 검증하지 않은** 것입니다. 같은 모양을 이 글에서 여섯 번 만납니다.

## 요약

- **문제**: 방송 모드 세 개에 테스트를 붙이고 전부 초록불이었는데, **그 세션을 만들 수 있는지는 아무도 안 물어봤습니다.** 생성 요청 DTO 에 `sessionType` 이 없어 모든 세션이 `INTERACTIVE` 로 저장되고 있었습니다.
    - **→ 기능 네 개가 통째로 도달 불가. 테스트는 픽스처를 직접 만들어 그 사실을 영영 안 물어봤습니다**
- **같은 모양이 여섯 번**: 오디오 전용 정책이 클라이언트 관례로만 존재, egress 를 띄울 수 없음, WebRTC 계층이 운영에 연결된 적 없음 등
- **판단**: 검증 기준을 **"의도"에서 "실제로 나간 것"** 으로 바꿨습니다.
    - `service.canPublish == false` 확인 → **발급된 JWT 를 열어 `video.canPublishSources` 를 확인**
    - 테스트가 엔티티를 직접 생성 → **생성 API 로 만들어 저장된 타입을 확인**
    - `@EnumSource(SessionType.class)` — **모드를 추가하면 그 모드도 자동으로 검사됩니다**
- **곁가지**: 실패 지점이 원인에서 20줄 떨어져 있었습니다. `getId()` 는 프록시가 DB 를 안 타고 답하므로, 초기화가 필요한 첫 필드(`email`)에서야 터집니다.
