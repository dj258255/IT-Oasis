...
- **문제**: 파이썬 요약 함수가 **471줄에 테스트 0개**였습니다. 반년 전에 *"쪼개려면 테스트가 필요한데 471줄이라 테스트를 쓸 수 없다 — 닭-달걀"* 이라고 코드에 적어 두고 미뤘습니다.
- **뒷 문장이 틀렸습니다**: 함수 안에 이미 경계가 있었고 **그중 셋은 순수 함수라 옮기기만 하면 바로 테스트할 수 있었습니다.**
    - **→ 쪼갤 수 없어서가 아니라 쪼갤 순서를 안 정했던 것입니다**
- **결과**: 5개로 분리 — **471 → 88줄**, 파이썬 테스트 **30 → 98**
    - 파싱과 그리기를 가른 것이 가장 컸습니다. 전에는 `in_code`·`callout` 상태를 **PDF 그리는 코드가 직접 들고 있어** 블록 판정을 물어볼 자리가 없었습니다
    - LLM 클라이언트를 인자로 받게 바꿔 **유료 API 키 없이도 요약 로직을 테스트**할 수 있게 했습니다
- **테스트를 쓰자마자 한 번도 실행된 적 없는 코드 세 곳이 드러났습니다**
    - 원본은 *"폰트가 없으면 글자가 네모로 나온다"* 를 전제했는데 fpdf2 2.7 부터는 **예외**입니다 → 되돌림 PDF 가 **존재 이유인 그 조건에서 같이 죽습니다**
    - 그 예외가 **이미 만들어진 요약 본문까지 버렸습니다**
    - 되돌림 PDF 는 `multi_cell` 뒤 x 를 안 되돌려 **한 줄짜리 문서에서만** 동작했습니다
- **교훈**: 되돌림 경로는 정의상 잘 안 돕니다. **잘 안 도는 길일수록 테스트가 유일한 관측 수단입니다.**
