...
레이어드 아키텍처에서 인터페이스는 Application 레이어에, 그 구현체는 Infrastructure 레이어에 위치해야 한다. 이렇게 해야 의존성 역전 원칙(DIP)이 지켜지고, Application 레이어가 특정 기술(QueryDSL, JPA 등)에 직접 의존하지 않는 구조가 된다.
...
이 구조의 문제를 인식하고, 두 가지 작업을 진행했다.

**1단계: 이름 정리**

`BoardSearch`와 `BoardSearchImpl`이라는 이름은 역할이 불명확했다. QueryDSL을 사용하는 Repository 구현체인 만큼, 이름을 아래처럼 변경했다.
...
`@Component`로 선언해도 빈 등록은 되지만, `@Repository`를 선택한 이유는 두 가지다.
...
결과적으로 `BoardJpaRepository`는 JPA 엔티티 관리만, `BoardSearchRepositoryImpl`은 QueryDSL 기반 복잡 쿼리만 담당하게 되어 **관심사 분리(SRP)**가 달성됐다. 인터페이스 기반의 의존성 역전도 유지된다.
...
AI가 제안한 방법이 틀린 건 아닐 수 있지만, 현재 프로젝트의 레이어 구조와 의존성 방향을 고려했을 때 내 판단으로는 분리하는 것이 더 적절했다.

AI의 답변을 무조건 수용하기보다, 현재 아키텍처의 맥락에서 검증하고 판단하는 과정이 중요하다. AI가 제시하는 것이 모든 방법의 전부가 아니라는 전제로, 스스로 근거를 갖고 선택하는 것이 더 나은 결과를 만든다.
