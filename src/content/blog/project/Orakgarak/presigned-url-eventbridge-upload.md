...
서버를 거치는 업로드 방식이 OOM과 이중 전송 문제를 일으킨다는 걸 파악하고, Presigned URL로 S3 직접 업로드 + EventBridge로 완료 감지하는 구조로 전환했습니다.

---

## 배경

오락가락은 사용자의 음성을 분석해 음악을 추천하는 서비스입니다.
사용자가 브라우저에서 녹음한 음성 파일을 서버에 올리고, 이걸 WAV로 변환한 뒤 AI 서버가 분석하는 구조입니다.

처음 설계할 때는 "Presigned URL로 S3에 바로 올리면 되겠다"고 생각했습니다.
그런데 AI 분석 담당 팀원이 "추론 서버가 WAV 포맷만 처리할 수 있다"고 했습니다.
브라우저 녹음은 WebM으로 나오니까, 어딘가에서 WAV 변환이 필요했습니다.

그래서 처음엔 파일을 서버로 받아서 변환 후 S3에 저장하는 방식으로 갔습니다.
동작은 했지만, 여기서 문제가 터졌습니다.
...
서버 업로드 방식에서는 파일 데이터가 서버 메모리를 경유합니다.
Spring Boot의 `StandardMultipartResolver`는 `file-size-threshold`(기본값 0) 이상이면 디스크 임시 파일에 쓰지만, 업로드 수신 중 서블릿 컨테이너가 입력 스트림을 버퍼링하고, 이후 S3로 전송하기 위해 다시 읽는 과정에서 메모리를 사용합니다.

음성 파일 하나가 대략 30-50MB였습니다.
동시 업로드가 10건만 겹쳐도 디스크 I/O + 메모리 버퍼링으로 수백 MB가 순간 점유되고, S3 전송까지 포함하면 요청당 30-40초간 리소스를 점유합니다.
실제로 OOM이 발생한 건 아니지만, 동시 업로드가 증가하면 JVM 힙 압박 → Full GC → 응답 지연 → 스레드 점유 누적으로 이어지는 구조적 위험이 있었습니다.

### 이중 네트워크 전송

![](/uploads/project/Orakgarak/presigned-url-eventbridge-upload/double-handling-network-cost.png)

파일이 네트워크를 두 번 탑니다.
클라이언트에서 서버로 한 번, 서버에서 S3로 한 번.
업로드 시간이 두 배고, 서버-S3 구간에서 실패하면 클라이언트부터 다시 보내야 합니다.

### 서버 상태에 종속

서버가 다운되면 업로드도 불가능합니다.
배포 중에도 업로드가 끊깁니다.
서버를 수평 확장해도 각 서버가 파일을 버퍼링해야 하니, 스케일아웃의 이점이 상쇄됩니다.

---

## WAV 변환을 어디서 할 것인가

Presigned URL을 쓰기로 결정하면, 서버가 파일을 직접 받지 않으니 변환 시점이 달라집니다.
프론트엔드에서 변환할지, 백엔드에서 변환할지 선택해야 했습니다.

프론트엔드 변환의 문제는 용량이었습니다.
WebM은 압축 포맷이라 5MB 정도인데, WAV는 비압축이라 50MB까지 늘어납니다.
모바일에서 50MB 업로드는 사용성이 나빠집니다.

실패 복구도 다릅니다.
프론트에서 변환 중 실패하면 사용자가 다시 녹음해야 합니다.
백엔드에서 변환에 실패하면 S3에 원본이 남아있으니 재시도가 가능합니다.

변환 품질도 백엔드가 유리했습니다.
FFmpeg로 샘플레이트와 비트뎁스를 정밀하게 제어할 수 있습니다.
브라우저의 Web Audio API는 이 부분에서 제약이 있습니다.

원본(WebM)을 S3에 직접 업로드하고, 백엔드에서 FFmpeg로 변환하는 방식을 택했습니다.

---

## 최종 아키텍처

![](/uploads/project/Orakgarak/presigned-url-eventbridge-upload/event-driven-architecture.png)
...
![](/uploads/project/Orakgarak/presigned-url-eventbridge-upload/state-flow-diagram.png)

---

## 왜 클라이언트 콜백이 아닌 서버 이벤트인가

처음에는 "S3 업로드 후 클라이언트가 완료 API를 호출하면 되지 않나?"라고 생각했습니다.

세 가지가 문제였습니다.
브라우저를 닫으면 완료 처리가 안 됩니다.
네트워크 오류 시 DB와 S3 상태가 어긋납니다.
업로드 없이 완료 API를 호출하는 악의적 요청도 가능합니다.

S3 이벤트 기반이면 실제로 파일이 올라왔을 때만 트리거됩니다.
서버 상태와 무관하게 동작하고, 신뢰성이 보장됩니다.

---

## 왜 S3 Lambda Trigger가 아닌 EventBridge인가

S3 이벤트를 받는 방법은 두 가지가 있습니다.
S3 Event Notification(Lambda)과 EventBridge입니다.

S3 Event Notification은 버킷에 직접 설정합니다.
간단하지만, 같은 prefix에 Lambda 하나만 연결할 수 있습니다.
타겟을 추가하려면 버킷 설정을 고쳐야 합니다.
한 번 실패하면 복구도 어렵습니다.

EventBridge는 S3 버킷 설정과 이벤트 처리 로직이 분리됩니다.
같은 이벤트를 Kafka, Step Functions 등 여러 서비스로 라우팅할 수 있고, 이벤트 아카이브에서 재생도 가능합니다.
새로운 처리 로직을 추가할 때 버킷 설정을 건드릴 필요가 없습니다.

느슨한 결합과 확장성에서 EventBridge가 명확하게 나았습니다.

---

## 고아 파일(Orphan Files) 처리

Presigned URL 방식에서 피할 수 없는 문제가 고아 파일입니다.

발생 시나리오는 세 가지입니다.
- Presigned URL을 발급받고 업로드하지 않으면 DB에 PENDING 레코드만 남습니다.
- 업로드는 성공했는데 EventBridge가 실패하면 S3에 파일만 있고 DB는 PENDING입니다.
- WAV 변환 중 서버가 크래시하면 S3에 WebM과 WAV가 둘 다 있을 수 있습니다.

이걸 DB 상태 기반 배치 스캔으로 자동 정리합니다.
...
![](/uploads/project/Orakgarak/presigned-url-eventbridge-upload/orphan-file-batch-scan.svg)

![](/uploads/project/Orakgarak/presigned-url-eventbridge-upload/orphan-file-cleanup-flow.svg)
...
![](/uploads/project/Orakgarak/presigned-url-eventbridge-upload/presigned-url-issue-flow.svg)

URL 발급 시 DB에 Upload 레코드를 먼저 만듭니다.
S3 이벤트가 올 때 "누구의 어떤 파일인지" 알아야 하기 때문입니다.
S3 키에 UUID를 포함시키고(`recordings/{uuid}_{filename}`), EventBridge 이벤트에서 UUID를 추출해 레코드를 조회합니다.

### S3 이벤트 핸들러

![](/uploads/project/Orakgarak/presigned-url-eventbridge-upload/s3-event-handler.svg)
...
Presigned URL 방식도 완벽하지 않습니다.

업로드 전에 파일을 검증할 수 없다는 게 가장 큰 단점입니다.
서버를 거치면 파일 포맷, 크기, 악성코드 등을 업로드 전에 체크할 수 있지만, S3 직접 업로드에서는 올라온 뒤에야 확인 가능합니다.
Presigned POST 정책(POST Policy)에서 `content-length-range` 조건으로 파일 크기 상한을, `Content-Type` 조건으로 허용 포맷을 제한했습니다. (일반 Presigned PUT URL은 파일 크기 제한이 불가능하므로 Presigned POST를 사용했습니다.) 하지만 파일 내용 자체의 검증은 업로드 후에만 가능합니다.

구현 복잡도도 올라갑니다.
Presigned URL 발급, EventBridge 설정, 고아 파일 처리 배치잡까지 관리 포인트가 늘어납니다.
클라이언트에서 S3로 직접 보내니 디버깅도 어렵습니다.

다만 우리 서비스에서는 음성 파일 특성상 업로드 전 검증이 크게 필요 없었고, 서버 메모리 부담 제거가 더 중요했습니다.
그래서 이 트레이드오프를 감수했습니다.
