...
서버를 거치는 업로드 방식이 OOM과 이중 전송 문제를 일으킨다는 걸 파악하고, Presigned URL로 S3 직접 업로드 + EventBridge로 완료 감지하는 구조로 전환했어요.

---

## 배경

오락가락은 사용자의 음성을 분석해 음악을 추천하는 서비스예요.
사용자가 브라우저에서 녹음한 음성 파일을 서버에 올리고, 이걸 WAV로 변환한 뒤 AI 서버가 분석하는 구조예요.

처음 설계할 때는 "Presigned URL로 S3에 바로 올리면 되겠다"고 생각했어요.
그런데 AI 분석 담당 팀원이 "추론 서버가 WAV 포맷만 처리할 수 있다"고 했거든요.
브라우저 녹음은 WebM으로 나오니까, 어딘가에서 WAV 변환이 필요했어요.

그래서 처음엔 파일을 서버로 받아서 변환 후 S3에 저장하는 방식으로 갔어요.
동작은 했지만, 여기서 문제가 터졌어요.
...
업로드 시간이 두 배고, 서버-S3 구간에서 실패하면 클라이언트부터 다시 보내야 해요.

### 서버 상태에 종속

서버가 다운되면 업로드도 불가능해요.
배포 중에도 업로드가 끊기고요.
서버를 수평 확장해도 각 서버가 파일을 버퍼링해야 하니, 스케일아웃의 이점이 상쇄돼요.

---

## WAV 변환을 어디서 할 것인가

Presigned URL을 쓰기로 결정하면, 서버가 파일을 직접 받지 않으니 변환 시점이 달라져요.
프론트엔드에서 변환할지, 백엔드에서 변환할지 선택해야 했어요.

프론트엔드 변환의 문제는 용량이었어요.
WebM은 압축 포맷이라 5MB 정도인데, WAV는 비압축이라 50MB까지 늘어나거든요.
모바일에서 50MB 업로드는 사용성이 나빠져요.

실패 복구도 달라요.
프론트에서 변환 중 실패하면 사용자가 다시 녹음해야 해요.
백엔드에서 변환에 실패하면 S3에 원본이 남아있으니 재시도가 가능하거든요.

변환 품질도 백엔드가 유리했어요.
FFmpeg로 샘플레이트와 비트뎁스를 정밀하게 제어할 수 있거든요.
브라우저의 Web Audio API는 이 부분에서 제약이 있거든요.

결론: 원본(WebM)을 S3에 직접 업로드하고, 백엔드에서 FFmpeg로 변환하는 방식을 택했어요.
...
처음에는 "S3 업로드 후 클라이언트가 완료 API를 호출하면 되지 않나?"라고 생각했어요.

세 가지가 문제였어요.
브라우저를 닫으면 완료 처리가 안 돼요.
네트워크 오류 시 DB와 S3 상태가 어긋나고요.
업로드 없이 완료 API를 호출하는 악의적 요청도 가능하고요.

S3 이벤트 기반이면 실제로 파일이 올라왔을 때만 트리거돼요.
서버 상태와 무관하게 동작하고, 신뢰성이 보장돼요.

---

## 왜 S3 Lambda Trigger가 아닌 EventBridge인가

S3 이벤트를 받는 방법은 두 가지가 있어요.
S3 Event Notification(Lambda)과 EventBridge예요.

S3 Event Notification은 버킷에 직접 설정해요.
간단하지만, 같은 prefix에 Lambda 하나만 연결할 수 있어요.
타겟을 추가하려면 버킷 설정을 고쳐야 하고요.
한 번 실패하면 복구도 어렵고요.

EventBridge는 S3 버킷 설정과 이벤트 처리 로직이 분리돼요.
같은 이벤트를 Kafka, Step Functions 등 여러 서비스로 라우팅할 수 있고, 이벤트 아카이브에서 재생도 가능해요.
새로운 처리 로직을 추가할 때 버킷 설정을 건드릴 필요가 없거든요.

느슨한 결합과 확장성에서 EventBridge가 명확하게 나았어요.

---

## 고아 파일(Orphan Files) 처리

Presigned URL 방식에서 피할 수 없는 문제가 고아 파일이에요.

발생 시나리오는 세 가지예요.
- Presigned URL을 발급받고 업로드하지 않으면 DB에 PENDING 레코드만 남아요.
- 업로드는 성공했는데 EventBridge가 실패하면 S3에 파일만 있고 DB는 PENDING이에요.
- WAV 변환 중 서버가 크래시하면 S3에 WebM과 WAV가 둘 다 있을 수 있어요.

이걸 DB 상태 기반 배치 스캔으로 자동 정리해요.
...
URL 발급 시 DB에 Upload 레코드를 먼저 만들어요.
S3 이벤트가 올 때 "누구의 어떤 파일인지" 알아야 하기 때문이거든요.
S3 키에 UUID를 포함시키고(`recordings/{uuid}_{filename}`), EventBridge 이벤트에서 UUID를 추출해 레코드를 조회해요.
