...
[5편](/blog/project/dbtower/dbtower-5-production-safety)에서 운영 안전을 닫고 나니, 성격이 다른 두 가지가 남았어요. 하나는 기능의 질문이에요. "DBA가 장애 때 실제로 뭘 보는가?" 지금까지 DBTower의 쿼리 통계는 load%, 즉 "어떤 쿼리가 시간을 쓰나"를 답했는데, 현업 DBA가 장애 순간 가장 먼저 여는 건 그다음 질문이에요. **"그 시간에 무엇을 기다렸나"**, 곧 CPU냐 IO냐 Lock이냐 Latch냐를 가리는 일이죠. 이게 Wait Event 분석입니다.
...
그런데 여기서 중요한 건, 기종마다 **"대기"의 의미론 자체가 다르다**는 거였어요. 같은 이름을 달고 있어도 답하는 내용은 제각각이에요.
...
누적 카운터(MySQL·MSSQL·Oracle)와 현재 순간 스냅샷(PostgreSQL·MongoDB)이 섞여 있어요. PostgreSQL을 무부하 구간에 조회하면 빈 배열이 나오는데, 버그처럼 보여도 스냅샷 방식의 정직한 결과입니다.
...
**Lombok은 JPA 엔티티에만.** 값 객체는 `record`로 불변을 유지하고 Lombok은 JPA 엔티티 6묶음에만 `@Getter`와 `@NoArgsConstructor(PROTECTED)`로 넣었어요. `@Data`·`@ToString`·`@EqualsAndHashCode`는 엔티티의 lazy 연관과 hashCode에 지뢰가 되니 일부러 뺐습니다. 이렇게 해서 손으로 쓴 게터 43개를 지웠어요. "Lombok을 쓴다"보다 "어디에 쓰면 안전한가"를 정한 거예요.
...
그래서 `JpaSpecificationExecutor`를 상속하고 각 빌더는 파라미터가 비면 `null`을 반환("이 필터 없음")하게 했어요. 컨트롤러가 null을 걸러 AND로 reduce하면, 필터가 늘어도 메서드는 그대로고 조각 하나만 늘 뿐입니다.
...
돌아보면 이 프로젝트를 관통한 문장은 하나였어요. "이건 기종마다, 기술마다 어떻게 다른가"를 먼저 묻고, 그 차이를 경계 뒤로 밀어 넣는 것. 기종 차이는 `DbmsOperator` 뒤로, 소비자 차이는 채널 뒤로, 그리고 기술 차이는 각자의 제자리로. 깔끔함은 프레임워크로 전부 통일하는 데서 나오지 않아요. 각 도구가 제일 잘하는 자리에 있게 두는 것, 그게 깔끔함이라는 걸 이번 정리에서 다시 배웠습니다.
