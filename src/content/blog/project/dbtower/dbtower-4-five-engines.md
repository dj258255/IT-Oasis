...
- **Oracle**은 JDBC는 맞지만 통계 소스(V$SQL)와 권한 모델, 백업(Data Pump)이 전부 다른 상용 DB예요.
- **MongoDB**는 SQL도 JDBC도 없고, 실행계획이 JSON이며, 통계 소스가 시스템 뷰 대신 컬렉션이에요.
...
explain 입력이 SQL이 아니라 `{"find": "users", "filter": {"name": {"$regex": "user123"}}}` 같은 JSON이 되는데도 인터페이스 시그니처는 그대로예요. 경계를 "SQL을 실행한다"에 긋지 않고 **"실행계획을 받아온다"라는 운영 행위**에 그어뒀기 때문입니다. 규칙 분석기도 마찬가지예요. MySQL의 `access_type=ALL`이 하던 역할을 MongoDB에서는 `COLLSCAN` 스테이지가 합니다. 신호 문자열만 다르지 "인덱스 없이 전부 훑는다"는 판정은 같아요.

한 가지 성질 차이는 문서에 박아뒀습니다. 다른 기종의 통계는 서버 기동 이후 무한 누적 카운터인데, `system.profile`은 **capped collection**이라 가득 차면 오래된 기록을 덮어써요. 합계가 줄어들 수 있다는 뜻인데, 시점 비교가 카운터 리셋 대비로 이미 갖고 있던 음수 클램프가 이 경우도 흡수합니다. 설계할 때 넣은 방어가 예상 못 한 기종에서 제값을 하는 순간이었어요.
...
원인은 코드가 아니었습니다. **메타데이터 DB에 남아 있던 과거**였어요. Hibernate가 처음 테이블을 만들 때 enum 컬럼에 `CHECK (type IN ('MYSQL','POSTGRESQL','MSSQL'))` 제약을 같이 생성해뒀는데, `ddl-auto: update`는 컬럼은 추가해도 **기존 CHECK 제약은 갱신하지 않아요**. 코드의 enum에 값을 추가하는 순간, 배포로 끝날 일이 스키마 마이그레이션이 필요한 변경으로 바뀌는 겁니다.

수동 ALTER로 풀었지만 교훈은 명확했어요. enum을 DDL 제약으로 내려보내는 순간 enum 확장은 코드 배포만으로 끝나지 않는다. Flyway 같은 마이그레이션 도구가 왜 필요한지를 교과서에서 읽기 전에 내 프로젝트 안에서 먼저 만났습니다.
