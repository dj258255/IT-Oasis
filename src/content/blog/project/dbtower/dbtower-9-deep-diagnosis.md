...
1. **암시적 형변환** — 문자열 컬럼에 숫자 리터럴(`code = 12345`). 비교 규칙상 문자열 쪽이 숫자로 캐스팅돼(컬럼에 함수를 씌운 꼴) 인덱스 무력화 — 게다가 `'012345'` 같은 다른 문자열까지 같은 숫자로 매칭되는 **정합성 위험**
...
발행 후 받은 외부 리뷰가 이 화면을 세 군데 고치게 했어요. **하나, 카드 순서.** 처음엔 "카디널리티 오추정"이 첫 카드였는데, 괴리는 형변환의 **증상**이지 원인이 아닙니다 — 첫 카드만 읽은 사용자가 "통계 문제구나" 하고 ANALYZE만 돌리고 끝낼 수 있다는 지적이 정확했어요. 지금은 근본 원인이 먼저 오고, 괴리는 "위 원인의 부산물 — 통계 갱신으로는 안 풀린다"로 자리를 옮겼습니다. **둘, 정합성 경고.** 형변환의 더 무서운 얼굴은 `'012345'`·`'12345 '`·`'12345abc'`까지 전부 숫자 12345로 매칭되는 것 — 조회면 오답, UPDATE/DELETE면 데이터 사고인데, 원래 판정문은 성능 얘기만 하는 반쪽짜리였어요. **셋, loops 환산 안내는 loops>1일 때만** — loops=1인 계획에서는 노이즈였습니다.

그리고 리뷰가 제안한 것 하나를 새로 만들었어요 — **수정안 원클릭 재진단**. 형변환 케이스는 "숫자 리터럴에 따옴표"라는 기계적으로 안전한 수정이 가능해서, 판정이 수정안 SQL을 함께 돌려주고 버튼 한 번으로 재진단해 before/after를 비교합니다.

![수정 전후 비교 — 괴리 300배에서 없음으로, 근본원인 1건에서 0건으로, Index lookup 전환](/uploads/project/dbtower/deep-before-after.png)

여기서 정합성 경고가 실측으로 증명되는 덤이 있었습니다. 수정 전의 "실제 1행"은 사실 `'012345'`가 숫자 캐스팅으로 **잘못 매칭된 행**이었고, 문자열 `'12345'`로 고치자 정확히 0행이 나왔어요 — 형변환은 느린 게 문제가 아니라 틀린 게 문제라는 걸, 진단 도구가 스스로 보여준 셈입니다.
