...
**로그인이 무제한으로 뚫려 있었습니다.** 실패를 감사 로그로 기록은 하는데, 정작 그 실패에 반응해 문을 잠그지는 않았습니다. 계정별로 연속 실패를 세다가 임계(기본 10회)를 넘으면 일정 시간(기본 15분) 잠그는 `LoginAttemptGuard`와, 잠긴 계정의 시도를 인증 앞에서 되돌리는 필터를 넣었습니다. 새 라이브러리 없이 인메모리로 — 감사가 "기록"이라면 이건 "반응"입니다. 대소문자를 바꿔 우회하는 것도 막았고, 잠겼을 땐 로그인 화면이 남은 시간을 알려줍니다.
