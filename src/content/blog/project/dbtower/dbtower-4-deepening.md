...
descriptionEn: 'The deepening chapter of DBTower, a management platform for heterogeneous DBMS engines. After tagging v1.0.0, I went back to the leftovers I had honestly listed in the docs and dug deeper. Three of them I closed: plan-flip detection for the classic incident where the query and data are unchanged but the plan suddenly slows down, built on PostgreSQL 16 GENERIC_PLAN; completing the 3-2-1 rule by shipping backups to S3-compatible offsite storage; and connecting to TLS-enforced managed services while deliberately declining to add a certificate-verification bypass. Four deepening arcs followed. I extended plan-flip across all five engines by unifying their very different acquisition paths behind one shape-normalization layer, and raised p95''s honesty grade from cumulative to recent-window and from unsupported to estimated while labeling Oracle where it cannot be raised. I read deadlocks across three engines without any config change, and controlled scale along five axes so the watchtower never becomes the load itself. I closed by auditing what I built, sweeping concurrency, correctness, security, and lifecycle, then cross-checking against OWASP, CWE, and vendor docs to separate FIX from SKIP.'
date: 2026-07-06
tags:
  - Java
  - Spring Boot
