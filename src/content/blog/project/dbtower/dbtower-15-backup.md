...
네 걸음을 다 걷고 나서 다섯 번째가 나왔습니다. 이번엔 코드가 아니라 질문에서 발견됐습니다. "백업 정책이랑 로그 백업 정책은 따로 걸 수 있는 건가?" 확인해보니 정책 테이블의 유니크 제약이 인스턴스 단위였습니다. 즉 인스턴스당 정책이 딱 하나라서, FULL과 LOG를 동시에 자동 스케줄로 걸 수가 없었습니다.

이게 왜 문제냐면, 시점 복구의 실무 표준이 정확히 그 병행이기 때문입니다. SQL Server의 정석 구성(주간 FULL, 일간 DIFF, 15분 LOG)도, MySQL의 일간 물리 백업과 binlog 상시 수집도, PostgreSQL의 basebackup과 WAL 아카이빙도 전부 같은 구조입니다. 이유는 두 지표가 서로 다른 주기를 요구해서입니다. 얼마나 잃어도 되는가(RPO)는 로그 주기가 정하니 로그는 촘촘해야 하고, 얼마나 빨리 복구되는가(RTO)는 앵커의 최신성이 정하니 전체 백업은 무겁지만 주기적으로 갱신돼야 합니다. 인스턴스당 정책 하나면 둘 중 하나를 사람의 수동 실행에 의존하게 됩니다. 시리즈 내내 배격해온 "사람이 기억해야 작동하는 운영"이 백업의 심장부에 남아 있던 셈입니다.

수정은 제약을 (인스턴스, 타입) 복합으로 푸는 것이 본체였습니다. 폴러는 원래 기한이 된 정책 목록을 순회하며 각자의 타입으로 실행하는 구조여서 손댈 게 없었고, 스키마와 API의 단수 의미론만 열면 됐습니다. 마이그레이션에서 함정이 하나 있었는데, 기존 유니크 제약의 이름이 환경마다 다릅니다(스크립트가 명명한 것과 하이버네이트가 생성한 것). 이름을 박아 DROP하는 대신 카탈로그에서 유니크 제약을 찾아 동적으로 지우게 했습니다. 덤으로 드러난 것도 있습니다. 정책 타입의 CHECK 제약에 PHYSICAL이 빠져 있어서, 물리 백업은 즉시 실행만 가능했고 정책 스케줄은 원천 차단돼 있었습니다.

신선도도 같이 고쳤습니다. 기존 판정은 "종류 무관 최근 성공"이라, LOG가 15분마다 성공하면 FULL이 몇 주째 깨져 있어도 화면이 초록이었습니다. 앵커가 없으면 로그 체인만으로는 복원이 안 되는데도요. 신선도의 기준을 앵커(FULL/PHYSICAL)로 바꾸고, LOG 축의 건강은 복원 가능 창이 따로 말하게 뒀습니다.

실측은 이렇게 했습니다. 한 인스턴스에 FULL 360분과 LOG 1분을 동시에 걸자 정책 목록에 두 행이 공존하고, 폴러가 같은 틱에 FULL과 LOG를 각각 실행한 뒤 다음 분에는 LOG만 돌았습니다. 각자의 주기가 독립으로 살아 있는 겁니다. 그리고 신선도를 조회하니 기준 시각이 최신 성공(방금의 LOG)이 아니라 앵커(그 전의 FULL)로 잡혔습니다. 가림 왜곡이 사라진 것까지 확인하고서야 이 절을 닫았습니다.
