---
title: '아무도 못 쓰던 프로젝트를 셀프호스트 제품으로 끌어올리고, 화면 패리티까지 맞추다'
...
descriptionEn: 'DBTower productionization and screen-parity part. The features had already surpassed the reference platform, but when I asked whether a stranger could clone this and actually run it, the answer was no. There were four blockers. No license meant nobody could legally use it. The encryption fail-closed guard happened to miss exactly the docker profile self-host uses, so target-DB passwords were stored in plaintext. An old H2 file with a password column was still committed, and the AI judgment-rules file was excluded from the image, leaving it to run on an empty prompt. I removed the four Phase 0 deployment blockers, made the control tower defend itself (login lockout, meta-DB backup, web HTTPS), and attached referenced-table schema to inquiries. Then I put 11 slides of the reference platform side by side and compared them column by column to close the table-column parity gap. Along the way I hit three traps. 382 unit tests were green while the entire web console sat white-screened in a committed state. The "catalog reconstruction (approximate)" badge turned out to be a label for laziness. And the activity chart had been querying an empty window nine hours in the future. All recorded with live measurement.'
date: 2026-07-15
tags:
  - Java
  - Spring Boot
