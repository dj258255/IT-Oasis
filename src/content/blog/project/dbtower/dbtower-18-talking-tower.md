...
17편의 디스크 포화 예측에도 접점이 늘었습니다. Prometheus에 직결돼 있던 지표 소스를 인터페이스로 갈라 CloudWatch(RDS FreeStorageSpace) 구현을 추가했습니다. RDS는 총 용량 메트릭이 없어 여유 %를 지어내는 대신 null로 두고, 판정이 "있는 축만으로" 동작하게 확장했습니다. 라이브 검증은 에뮬레이터(LocalStack)와 SDK의 프로토콜 세대 차이로 실 AWS가 필요해 정직하게 남겨뒀습니다.
