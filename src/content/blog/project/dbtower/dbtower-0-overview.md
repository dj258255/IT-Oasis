...
전부 개선 전 수치를 먼저 재고, 구조적 원인을 분석하고, 고친 뒤 다시 쟀습니다. 커넥션 풀과 배치 개선은 한 기종이 아니라 여러 기종에서 각각 측정했습니다.

| 문제 | 원인 | 해결 | 전/후 (기종별 실측) |
|---|---|---|---|
| 수집(폴링)마다 지연 | 매 수집 새 커넥션 생성으로 TCP+인증 핸드셰이크 반복 | 인스턴스별 HikariCP 풀(대상 DB 보호를 위해 max 2) | MySQL 47.1→11.8ms(4.0배) · PG 34.1→14.4ms(2.4배) · MSSQL 49.5→14.6ms(3.4배) |
| 스냅샷 대량 저장이 느림 | JPA saveAll이 행별 INSERT 발행 | JDBC batch + reWriteBatchedInserts | PG 행당 1.51→0.11ms(13.8배) · MySQL 행당 2.33→0.95ms(2.5배) |
...
| 로그 백업이 매번 전체를 다시 뜸 | 증분 없이 풀덤프 반복 | oplog ts 마커 이후만 --query로 추출(MongoDB) | 2차 LOG 7,686바이트, 직전 풀 대비 28분의 1 |
...
## 18. Lessons Learned: 밟은 기술 이슈와 해결

기능을 붙이는 내내 기종 내부 동작 때문에 밟은 함정들이 있었습니다. 대표적인 것 다섯 개를 문제 → 원인(원리) → 해결 → 검증 순으로 남깁니다. 전부 실측으로 재현하고 고쳤습니다.

### 이슈 1. MySQL digest 절단 — 서로 다른 쿼리가 하나로 뭉개진다

**문제.** 상위 SQL에 뜨는 쿼리는 파라미터 자리를 `?`로 정규화한 digest입니다. 그런데 앞부분이 같은 긴 쿼리 둘이 같은 digest로 합쳐지면, 통계가 뭉쳐 "둘 중 어느 쿼리가 문제인지"를 식별할 수 없습니다.

**원인(원리).** MySQL은 쿼리 전체가 아니라 `max_digest_length`(기본 1024 byte)까지만 잘라서 정규화합니다. 그 경계를 넘는 부분은 버려지고, 앞 1024 byte가 같으면 뒤가 달라도 한 digest가 됩니다.

**해결·검증.** docker-compose에서 처음부터 4096으로 올렸고, 기본값 1024 서버를 임시로 붙여 같은 쿼리 쌍을 side-by-side로 재현했습니다.

```
쿼리 쌍: 동일 프리픽스(AND name = ? 반복 200회) + 꼬리만 다름, 원문 4,260자
...
부수 발견도 있었습니다. 절단 기준은 정규화 텍스트 길이가 아니라 토큰 버퍼 바이트라, 원문 1,740자여도 안 잘리고 반복 200회로 늘려서야 1024 서버에서 절단됐습니다. 메모리 영향은 `performance_schema_max_digest_length` 상향이 digest 테이블 행당 텍스트 컬럼을 키우는 것이라, 기본 1만 행 기준 약 3KB × 10,000 = 수십 MB 수준입니다. 식별 불가 리스크와 맞바꿀 만한 비용이라 판단했습니다. **PostgreSQL은 이 이슈가 없습니다** — 텍스트가 아니라 쿼리 파싱 결과로 digest를 만들기 때문입니다. 같은 "쿼리 통계"도 기종 내부가 이렇게 다르고, 이게 `DbmsOperator` 추상화가 필요한 또 하나의 근거입니다.

### 이슈 2. EXPLAIN ANALYZE의 actual rows는 "루프당 평균"이다

**문제.** 심층 진단은 추정 행수와 실제 행수의 괴리로 옵티마이저의 오판을 잡는데, 실제 행수를 잘못 읽으면 괴리 계산 자체가 틀립니다.

**원인·해결.** MySQL/PostgreSQL의 실행계획에서 `actual rows`는 그 노드가 실행된 loops당 평균입니다. 총 처리 행수는 `actual rows × loops`라, 이걸 곱하지 않으면 조인 안쪽 노드의 실제 규모를 과소평가합니다. 곱셈 로직을 단위 테스트로 고정했습니다.

### 이슈 3. pg_stat_statements는 클러스터 전역이다

**문제·원인.** 도그푸딩 중 `sample` DB와 `dbtower` DB가 서로의 쿼리를 자기 화면에서 보고 있었습니다. `pg_stat_statements`는 데이터베이스별이 아니라 클러스터 전역 뷰라, `dbid` 필터 없이 조회하면 같은 클러스터의 다른 DB 쿼리까지 섞입니다.

**해결.** 조회를 현재 DB의 `dbid`로 필터링했습니다. DB 내부를 알아야 보이는 지점이었습니다.

### 이슈 4. 권한이 부족하면 PostgreSQL은 에러 대신 HTTP 200으로 조용히 저하한다

**문제.** 최소 권한 계정을 확정하는 과정에서, 권한이 부족한데도 API가 200을 반환하며 정상처럼 보였습니다.

**원인·해결.** PostgreSQL은 통계 뷰 권한이 없으면 에러를 던지는 대신 해당 컬럼에 `insufficient privilege`를 채워 행을 돌려줍니다. 그래서 조용한 저하가 됩니다. 권한 0 계정으로 시작해 DB가 돌려주는 권한 에러 원문을 수집하며 필요한 것만 추가해, `pg_monitor` 전체가 아니라 `pg_read_all_stats`로 좁혀 확정했습니다.

### 이슈 5. MongoDB의 CSOT가 명령별 타임아웃을 덮어쓴다

**문제·판단.** 진단이 부하가 되지 않게 명령마다 시간 상한(`maxTimeMS`)을 걸었는데, 드라이버 전역 타임아웃인 CSOT(Client-Side Operation Timeout)를 함께 켜면 CSOT가 명령별 상한을 덮어써 심층 진단의 의도가 깨집니다. 정석은 CSOT이지만 이 경우엔 일부러 배제하고, 이유를 코드 주석으로 남겼습니다.
