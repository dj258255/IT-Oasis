...
**구조적 원인** — 문자열 컬럼과 숫자 리터럴을 비교하면 MySQL은 **컬럼 쪽을 숫자로 변환**합니다. '1'도 '1.'도 '1a'도 전부 1로 변환되니 컬럼 전체를 변환해야 하고, 인덱스는 원본 문자열 순서로 정렬돼 있으므로 탐색 시작점을 잡을 수 없어요(암시적 형변환). 문제는 EXPLAIN(추정)만으로는 옵티마이저가 "왜 속았는지"까지 안 보인다는 것 — 그래서 **추정 행수와 실제 행수의 괴리**를 봐야 하고, 그러려면 쿼리를 실제로 실행하는 EXPLAIN ANALYZE가 필요합니다.
