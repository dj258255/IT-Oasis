...
descriptionEn: 'Multi-tenancy and the host dimension of DBTower, a heterogeneous DBMS operations platform. The first half is about teams sharing one console. Team users see only their instances plus global ones, and probing another team''s instance returns 404, not 403, hiding its very existence. Enforcement lives in exactly one place (RegistryService). Sessions moved to the meta DB so logins survive restarts, though that path hid a trap where Boot''s autoconfiguration silently fell back to in-memory. Adding a node then changed nothing, because one distributed lock meant one collector. Per-shard locks let two nodes split the work. Kill one and the survivor takes over every shard with zero configuration, and the same cookie keeps you logged in. The login lockout counter moved to the meta DB too. Two failures on node A plus one on B locked the fourth attempt, measured. Monthly partitioning on the largest table turned retention from a 1.9-second DELETE over two million rows into a 12.8ms partition DROP with zero bloat, and connections-on-demand brought an isolated target''s idle connections from one-forever to zero. The second half is the host dimension. Disk saturation forecasting watches the rate of shrinkage rather than the remaining space. A CRITICAL fires at 76.8% free after I drove real write load. At 17MB/s of shrinkage that disk dies in 20 hours, so the alert is right. Here too there were traps: node-exporter silently watching only its own container without a rootfs mount, and pinning mountpoint="/" when real DB data lives on dedicated mounts. Last comes server-sharing awareness. Registration units are databases while physical units are servers, so two DBs on one server made every session, replication, and deadlock alert fire twice. I deduped these to once per group with an explicit note of who they apply to, while leaving health scores (risk attribution) undeduped on purpose: both databases really are at risk.'
date: 2026-07-16
tags:
  - Java
  - Spring Boot
