...
Phase 1에서 actuator 메트릭 엔드포인트의 토큰 인가와, 재시작마다 바뀌는 API 토큰을 메타 DB에 영속시키는 일은 아직 남겨뒀습니다. 정직하게 다음 계단으로 넘깁니다. 여기까지, 관제탑은 이제 로그인을 잠그고, 자기 자신을 백업하고, TLS 뒤에 설 준비를 마쳤습니다. 다음은 여러 팀이 하나의 관제탑을 나눠 쓰는 이야기(Phase 3, 멀티테넌시)로 이어집니다.
