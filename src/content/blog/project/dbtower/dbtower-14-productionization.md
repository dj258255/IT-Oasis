...
여기까지, 관제탑은 이제 로그인을 잠그고, 자기 자신을 백업하고, TLS 뒤에 설 준비를 마쳤고, 토큰이 재시작에도 흔들리지 않습니다. 자기 자신을 향한 절반을 채운 셈입니다. 다음은 기능의 깊이를 더하는 이야기(Phase 2 — 문의에 스키마를 붙이고, 로그를 가리고, 로그 백업을 다섯 기종으로)로 이어집니다.
