...
description: 'DBTower는 활성 세션 조회를 5기종 전부 구현해 두고도 어디에도 남기지 않았다. 화면이 지금만 보여주고 지나가서, 장애가 끝나면 누가 누구를 막았는지 되짚을 수 없었다. 영속되는 것은 5분 주기 이벤트별 집계뿐인데 세션도 쿼리도 블로커도 원천 쿼리의 GROUP BY에서 이미 사라진 뒤였다. Oracle ASH가 1초마다 활성 세션을 샘플링해 순환 버퍼에 담고 AWR로 10대 1로 줄여 내리는 구조를 확인하고, 짧은 보존과 긴 보존을 나누는 그 형태가 곧 DBTower 7일과 이 창고임을 알았다. 그래서 1초 세션 샘플러를 붙이고 계약부터 썼는다. event time과 processing time을 나눠 늦은 도착을 관측 가능하게 하고, 결측은 결번으로만 드러나므로 시퀀스를 넣고, 봤다와 남겼다와 버렸다를 나눠 떨어뜨린 사실이 지표에 남게 했다. 유입 앞에 로그 버퍼가 필요하다는 제 논거는 장애 중에 관측이 굶는다는 것이었는데, 60인스턴스에 락 폭풍을 주입해 커넥션을 고갈시켜 보니 부하 세션들이 접속에 실패하는 동안 샘플러는 2,400틱을 전부 성공했다. 결측률 0.0000, 드롭률 0.0000, 수집 시간 최대 18.34ms이다. 굶어 죽을 줄 알았던 쪽이 이미 잡아 둔 풀을 쥔 생존자였다. 그래서 버퍼를 넣지 않았다. 대신 실제 2단계 락 컨보이가 잡혔고, 계측 도구 자신이 0의 함정에 빠져 9시간을 헛짚은 이야기도 함께 적었다.'
date: 2026-08-29
tags:
  - Data Engineering
  - Airflow
