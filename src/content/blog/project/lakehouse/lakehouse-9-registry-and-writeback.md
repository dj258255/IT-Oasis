...
똑같이 보존 정책의 지배를 받습니다. "이번 분기 백업 실패율 추세"나 "이 쿼리의 플랜이
...
**둘째, plan_snapshot은 보존이 카운트 기반입니다.** 쿼리당 최신 20개를 넘으면 한
...
안 잰 것을 잰 척하지 않는 것 — 3편에서 게이트를 만들 때 세운 원칙이 프로필에도 그대로
...
그래서 시간대별 팩트(`fct_query_hourly` — 일간 팩트와 같은 양 끝 차분을 (dt, hour)
...
32,498행이 단일 트랜잭션으로 들어갔고 행수 대조가 통과했습니다. 그리고 봉인 검증 —
...
정직하게 다룰 수 있게 됐습니다. 그리고 되쓰기 — 분석계가 처음으로 원천에 무언가를
주면서도, 봉인을 여는 게 아니라 옆문을 하나 만들고 그 문에만 열쇠를 준 것. 방향이
