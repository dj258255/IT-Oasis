...
그래서 순서를 뒤집었어요. **먼저 1년치를 실제로 만들어 어디가 먼저 무너지는지 재고,
...
있어야 하죠. 그래서 **마지막 7일**의 `total_time_ms`에만 쿼리별 악화 계수를
곱했어요(hash 버킷으로 60%는 안정, 일부는 +50%, 일부는 +150%). 한 파티션 전체에
...
- **CHECKPOINT는 0.47초.** 1년치 366커밋이 366개 소파일을 쌓았는데, CHECKPOINT가
  그걸 1파일로 컴팩션하는 데 반 초입니다. Phase 6에서 붙인 유지보수가 소파일 누적을
  값싸게 흡수하고 있었어요.

그리고 소파일. 파일 평균이 **177KB**입니다. 실무에서 합의된 타깃이 128MB~1GB/파일인데,
그거의 **1/741**이에요. 소파일 폭증은 분명히 있습니다. 다만 그 고통이 어디로
...
런타임에 평가되는데, 파티션 경로 프루닝은 플랜 타임에 상수가 있어야 걸리거든요.
그래서 predicate가 행은 걸러도 2,190파일을 다 스캔했어요. 증분의 이득이 통째로
날아간 거죠.
...
축이에요 — 지금 상태를 한 화면으로. Phase 8에서 이미 알림 payload에 `dashboard_url`
...
거였습니다. 며칠치로는 아무것도 증명 못 해요. 1년치를 실제로 만들어 재보니 병목은
