...
[앞의 아홉 단계 지도](#상황이-이끈-아홉-단계)에서 그림을 그렸습니다. 계정계는 고정길이 전문(電文)과 TCP로만 말하고, 앱은 JSON과 HTTP로만 말합니다. 둘 사이에 통역기를 세우기로 했는데, 그 통역기가 제일 먼저 할 줄 알아야 하는 일이 이것입니다.

> 계정계가 보낸 `"0210" + "12345678901234" + "IN01" + ...` 같은 **바이트 덩어리**를 `{"accountNo":"12345678901234", ...}` **JSON**으로 바꾸고, 반대로도 바꾼다.

네트워크(TCP 소켓)는 그다음입니다. 소켓으로 아무리 잘 받아와도, 그 바이트를 해석하지 못하면 통역은 시작조차 안 되기 때문입니다. 그래서 이번 장은 오직 **전문 ↔ DTO ↔ JSON 변환 엔진** 하나만 만듭니다.
...
이제 통역기가 소켓 앞에 섰습니다. 바이트가 스트림으로 쪼개져 와도 전문으로 다시 세우고, 그 전문을 REST 세계로 넘기고, 답을 다시 전문으로 만들어 돌려보냅니다. 하지만 이 통로는 아직 **아무나 드나들 수 있습니다.** 다음 장은 이 앞에 문지기를 세웁니다. 인증과 라우팅, 유량제어를 역시 손으로 짭니다.
...
#### 문제, 통로를 외부에 열면 아무나 들어온다

[1장](#1-전문-파서와-tcp-프레이밍) 2부에서 통로를 열었습니다. 문제는 이 통로가 무방비라는 것입니다. `POST /api/gateway/balance`에 계좌번호만 넣으면 누구나 계정계에 질의를 밀어넣을 수 있습니다. 외부에 열리는 순간 셋이 필요해집니다. 누가 보냈는지 확인하는 인증, 어디로 갈지 정하는 라우팅, 얼마나 자주 쳐도 되는지 제한하는 유량제어입니다. 각 백엔드가 따로 처리하면 중복이고 관리가 안 되니 앞단에서 한 번에 걸러야 합니다.

인증과 라우팅은 정직합니다. 헤더를 보고 맞으면 통과, 아니면 상태코드입니다. 진짜 함정은 유량제어에 있습니다. 클라이언트별 분당 N건을 토큰버킷으로 직접 짜면 두 곳에서 조용히 샙니다.

하나는 시계입니다. 토큰버킷의 핵심은 지난번 이후 흐른 시간만큼 토큰을 채우는 것인데, 벽시계인 `System.currentTimeMillis()`를 쓰면 NTP 보정이나 서머타임으로 시계가 뒤로 점프할 수 있습니다. 그러면 흐른 시간이 음수가 되고 토큰이 폭증하거나 음수로 꺼집니다. 로컬에서는 안 보이다가 운영 서버의 시간 동기화 한 번에 터지는 종류입니다.

다른 하나는 동시성입니다. 한 클라이언트가 동시에 여러 요청을 보내면 여러 스레드가 같은 버킷의 토큰을 동시에 소비하려 듭니다. `if (남은 토큰 > 0) 토큰--` 같은 검사 후 실행은 원자적이지 않아서, 두 스레드가 동시에 토큰 있음을 보고 둘 다 통과합니다. 용량 5인데 6건이 지나가는 초과 소비입니다.

#### 해법, 필터 체인은 인터페이스로 짜고 시계는 단조로, 버킷은 잠근다

| 정한 것 | 어떻게 | 왜 |
|---|---|---|
| 필터 체인을 직접 추상화 | `GatewayFilter` 인터페이스와 체인 실행기. 각 필터는 통과 아니면 차단을 결정 | 서블릿 API에 묶이지 않는 순수 자바라 필터 로직만 단독 테스트 가능. 서블릿 연결은 얇은 브릿지 하나로 격리 |
| 단조 시계 사용 | `System.nanoTime()`을 주입 가능한 `LongSupplier`로 | 뒤로 가지 않음이 보장되고, 테스트에서 가짜 시계로 보충 로직을 결정론적으로 검증 |
| 버킷 단위 잠금 | 클라이언트마다 버킷 하나를 `ConcurrentHashMap`에 두고 소비 연산만 `synchronized` | 락 범위가 그 버킷 하나라 다른 클라이언트끼리는 안 막힘 |
...
문지기(인증→라우팅→유량제어)를 다 통과한 뒤에야 1장의 전문 왕복이 실행됩니다. 문 통과 후 통역입니다.
...
모듈 경계 검증까지 합쳐 `./gradlew test`는 **52개 전부 그린**입니다(1장의 37개 + 이번 15개). `ModularityTest`의 `verify()`가 그린이라는 건, 위에 그린 모듈 DAG가 코드로 지켜지고 있다는 뜻입니다.


이제 통로 앞에 문지기가 섰습니다. 누가 보냈는지 확인하고 갈 곳이 있는지 보고 너무 자주 치면 잠시 막습니다. 그 관문을 통과한 요청만 1장의 전문 왕복에 닿습니다. 하지만 운영 중 "잔액조회가 가끔 실패한다"는 말이 나오면, 지금 구조로는 **어디서 깨졌는지**가 안 보입니다. 인증에서? 라우팅에서? 백엔드 타임아웃에서? 3장은 그 거래의 경로를 남기고 실패 지점을 드러냅니다.

그 전에, 1장 2부가 잔여로 남겨 둔 통로의 구멍 둘, 곧 가변 전문과 소켓 재사용부터 2부에서 메웁니다.

### 2부. 가변 프레이밍과 커넥션 풀

#### 문제, 1장 2부가 남겨 둔 두 개의 구멍

[1장](#1-전문-파서와-tcp-프레이밍) 2부에서 통로를 열 때, 잔여로 두 가지를 정직하게 적어 뒀습니다.

> 커넥션 풀 없음(요청당 소켓), 길이 헤더(가변 전문) 미구현.

1장 2부의 프레이밍은 "한 전문 = 고정 61byte"만 다뤘습니다. 프레임 길이를 상수로 아니까, 그 길이만큼 모이면 잘라 내보내면 됐습니다. 하지만 실제 거래에는 **길이가 매번 다른 전문**이 있습니다. 대표적인 게 거래내역 조회 응답입니다. 계좌에 거래가 3건이면 레코드 3개, 12건이면 12개가 붙어 전문 전체 길이가 조회할 때마다 달라집니다. 프레임 길이 상수를 못 씁니다.

두 번째 구멍은 소켓입니다. 1장 2부의 클라이언트는 `exchange()`가 불릴 때마다 소켓을 새로 열고 응답을 받고 닫았습니다. 요청 한 건에 TCP 3-way 핸드셰이크 한 번, 소켓 자원 한 벌을 매번 지불한 셈입니다. 한두 건이면 티가 안 나지만 초당 수백 건이 흐르면 이 반복 비용이 그대로 지연이 됩니다.

이번 장의 목표는 그 두 구멍을 메우는 것입니다.
...
문제는 TCP가 바이트 스트림이라는 점이 여기서 한 겹 더 깊어진다는 것입니다. 1장 2부에서 "고정 61byte도 한 번에 안 온다"를 다뤘는데, 가변에서는 **길이 헤더조차 반쪽으로 옵니다.** 4byte 헤더 중 2byte만 먼저 오면, 본문 길이를 아직 읽을 수조차 없습니다. 그러니 읽기가 2단계여야 합니다.

1. **1단계**: 헤더 4byte가 다 모일 때까지 기다린다(헤더 반쪽 방어).
2. **2단계**: 헤더로 본문 길이 L을 안 뒤, 본문 L byte가 다 모일 때까지 더 기다린다(본문 반쪽 방어).

여기에 1장 2부의 함정들이 그대로 얹힙니다. 여러 전문이 붙어 오는 뭉침과 한 전문 반쪽만 남는 경계 말입니다.
...
#### 해법, 2단계 누적기와 검증하는 헤더, 잠그고 거절하는 풀

**하나, 가변 프레이밍은 고정 프레이밍과 나란히 둔다.** 1장 2부의 `FixedLengthFramer` 옆에 `LengthPrefixedFramer`를 새로 만듭니다. 둘 다 "소켓 조각을 누적하다가 한 전문이 완성되면 잘라 내보낸다"는 같은 골격이되, 경계를 잡는 방법만 다릅니다(상수 길이 vs 헤더가 알려주는 길이).
...
**셋, 가변 전문 코덱은 고정 코덱을 두 번 쓰는 것으로 만든다.** 응답 본문은 "고정 헤더 + 고정 레코드 × N"입니다. 1장의 고정 코덱을 앞 헤더에 한 번, 뒤 레코드에 건수만큼 쓰면 됩니다. 경계는 이미 전송 계층의 길이 헤더가 확정해 주므로, 본문 전체 길이에서 역산합니다.
...
읽는 쪽(`LengthPrefixedConnection`)은 1장 2부의 `FramedConnection`과 같은 골격입니다. read 버퍼를 일부러 작게(16byte) 잡아, 헤더·본문이 한 번에 안 오는 상황을 코드가 구조적으로 다루게 합니다.
...
`CoreBankingClient`(1장 2부, 고정 61byte)와 새 `TransactionHistoryClient`(가변)가 각자 이 풀을 하나씩 들고 `exchange()`/`query()`가 빌려 쓰고 반납합니다. 계정계가 keep-alive라 한 소켓으로 여러 전문을 주고받을 수 있어(서버측 처리 루프가 프레임이 더 안 올 때까지 읽습니다) 풀이 성립합니다.
...
`txType`(입금/출금)과 `summary`(급여이체·카드결제…)가 한글입니다. 레코드 슬라이스도 byte 오프셋으로 잘라 EUC-KR로 디코딩하므로, 2byte 한글이 경계에서 안 깨집니다(1장의 원칙이 레코드에도 그대로).
...
`created`가 1로 고정입니다. 소켓을 딱 하나만 열어 여섯 번 재사용했습니다. 1장 2부라면 여기서 소켓을 여섯 번 열고 닫았을 겁니다.
...
가장 중요한 경계들은 화면 데모로는 부족해 테스트로 못 박았습니다. `./gradlew test`는 **80개 전부 그린**입니다(1장과 1부의 52개 + 이번 28개).
...
1장 2부가 남긴 두 구멍을 메웠습니다. 이제 길이가 매번 다른 전문도 프레이밍으로 정확히 자르고 소켓은 요청마다 새로 열지 않고 재사용합니다. 통로가 더 튼튼해졌습니다.
...
#### 문제, 통로는 튼튼해졌는데 아무것도 기억하지 못한다

[2장](#2-필터-체인과-가변-프레이밍-커넥션-풀)까지로 통로는 단단해졌습니다. 전문을 정확히 자르고, 필터 체인이 문을 지키고, 커넥션 풀이 소켓을 재사용합니다. 그런데 이런 질문에는 답이 없습니다.

> "어제 14시쯤 들어온 그 잔액조회, 계정계까지 갔나요? 응답은 받았나요?"

게이트웨이는 거래를 흘려보내기만 하고 아무것도 기억하지 못합니다. 앱 로그는 있지만 어떤 라인이 어떤 요청의 것인지 묶을 열쇠가 없고, 성공인지 실패인지의 판정 자체가 어디에도 없습니다. 평소에는 문제가 아닙니다. 호출자는 응답을 못 받았다고 하고 계정계는 처리했다고 하는 장애 상황에서, 중간에 선 게이트웨이에 기록이 없으면 중재가 불가능합니다.

기억을 붙이는 데 함정이 셋 있었습니다.

첫째, 응답을 못 받은 거래를 실패로 적으면 안 됩니다. 상태를 성공과 실패 2값으로 두면 예외가 났으니 FAILED로 짜게 되는데, 타임아웃에서는 그게 거짓말입니다. read 타임아웃은 정해진 시간 안에 응답이 안 왔다는 뜻이지 계정계가 처리를 안 했다는 뜻이 아닙니다. 요청 전문은 이미 나갔고 계정계는 처리한 뒤 응답을 보냈는데 늦게 도착하는 중일 수도 있습니다. 이걸 FAILED로 적으면 다시 보내게 되고 계정계에서 같은 거래가 두 번 실행됩니다. 잔액조회면 무해하지만 이체라면 사고입니다.

그래서 3값이어야 합니다. SUCCESS는 응답을 정상 수신했고 응답코드도 정상입니다. FAILED는 명확한 실패로, 오류 응답을 받았거나 입력이 틀렸거나 연결이 거부돼 요청이 나가기 전에 죽은 경우입니다. 계정계에서 처리됐을 가능성이 없습니다. UNKNOWN은 모른다는 뜻입니다. 요청은 나갔는데 응답을 못 받았고, 처리됐을 수도 안 됐을 수도 있습니다. 모르는 것을 모른다고 적는 편이 아는 척 틀리게 적는 것보다 안전합니다. UNKNOWN만 골라 나중에 확인하면 되기 때문입니다.

둘째, 기록이 거래를 느리게 하면 안 됩니다. 원장 적재를 거래 경로에 동기로 끼우면 insert 시간만큼 모든 거래가 느려지고, 더 나쁘게는 원장 DB가 죽을 때 거래까지 죽습니다. 관측하려고 붙인 것이 장애의 원인이 되는 구조는 뒤집혀 있습니다.

셋째, 원장이 민감정보 창고가 됩니다. 거래 기록에는 계좌번호가 들어갑니다. 원문 그대로 쌓으면 DB 백업과 조회 화면, 로그로 흘러나가는 모든 경로가 유출 표면이 됩니다. 마스킹을 보여줄 때 하면 늦습니다.

#### 해법, 새 모듈, 예외 사슬 판정, 저장 직전 마스킹

원장은 새 모듈 `ledger`로 세웠습니다. 관측은 통역과 전송, 검문 어디의 부속도 아니라 경계를 따로 그었습니다. 의존은 ledger에서 message로 가는 한 줄뿐이고 `ApplicationModules.verify()`가 이 경계를 계속 강제합니다.

거래고유번호는 자기설명 구조로 잡았습니다. 오픈뱅킹 거래고유번호가 이용기관코드와 생성구분, 일련번호로 짜여 ID만 보고 출처가 읽히는 걸 참조해 `GWMN` + `U` + 날짜 8자리 + 일련번호 9자리로 22자입니다. 유일성은 두 겹입니다. 동시 요청은 `AtomicLong`이 막고, 재기동은 자정 이후 흐른 밀리초에 10을 곱한 값을 시드로 써서 막습니다.

3값 판정은 예외의 원인 사슬로 합니다. 응답을 받았으면 응답코드로 가르고, 못 받았으면 사슬을 훑어 `SocketTimeoutException`과 `EOFException`이면 UNKNOWN, `ConnectException` 같으면 FAILED입니다.

적재는 전용 스레드와 유한 큐로 비동기 처리합니다. 거래 스레드는 큐에 넣고 즉시 돌아갑니다. 큐가 차거나 DB가 죽으면 WARN 로그만 남기고 거래는 진행합니다. `record()`는 예외를 던지지 않는 메서드로 계약했습니다.

마스킹은 저장 직전 한 곳에서 합니다. 앞 6자리와 뒤 4자리만 노출하고 짧으면 더 보수적으로 자릅니다. 적재 경로가 어디든 이 지점을 지나므로 원장에 원문이 남을 수 없습니다.

요청마다 correlation ID를 답니다. 수신 헤더에 있으면 형식 검증 후 승계하고 없으면 만들어 MDC에 넣습니다. 로그 패턴이 모든 라인에 찍고 응답 헤더로 돌려주고 원장에도 저장합니다. 이 502가 어느 요청이었나를 호출자와 앱 로그, 원장 사이에서 꿰는 실입니다.

저장소는 PostgreSQL 컨테이너로 두되 로컬 개발과 테스트는 H2 인메모리로 DB 없이 돌게 했습니다.
...
읽는 재미가 있습니다. `destroyed_total=1`은 타임아웃 난 소켓을 풀이 폐기한 것이고, `opened_total=2 · reused_total=6`은 소켓 2개로 8왕복을 처리했다는 뜻입니다. 2장 1부의 토큰버킷과 2부의 커넥션 풀이 이제 숫자로 자신을 보고합니다. 헬스는 liveness/readiness가 분리돼 있고(`/actuator/health/liveness`·`/readiness` 각각 UP), SIGTERM을 보내면 graceful shutdown이 처리 중 요청을 마저 끝낸 뒤 풀과 원장 스레드를 정리하는 것도 로그로 확인했습니다.
...
`./gradlew test`는 **104개 전부 그린**입니다(1~2장의 80개 + 이번 24개). 채번 동시성·재기동, 3값 판정(진짜 소켓 타임아웃 포함), 마스킹 규칙, "적재 실패가 거래를 안 막는 것", HTTP→소켓→비동기 적재 전 구간 통합까지. 새 `ledger` 모듈을 포함해도 `ApplicationModules.verify()`는 계속 그린입니다(5모듈 단방향 DAG는 web → gateway·message·core·ledger, ledger → message).
...
#### 실측, 서킷이 열리고 닫히는 것, 해소 두 경로가 원장에 남는 것
...
같은 UNKNOWN인데 하나는 CANCELED, 하나는 FAILED입니다. **"모름"의 확정을 조회로 한다**는 이번 장의 문장이 원장 두 줄로 남았습니다.
...
`./gradlew test`는 **129개 전부 그린**입니다(1·2장과 1부의 104개 + 이번 25개). 서킷 상태 전이(임계 도달 OPEN·즉시 거절·HALF_OPEN 탐침 정원·탐침 성공/실패), **변경성 재시도 금지(재시도 설정이 있어도 호출은 정확히 1회)**, 데드라인이 마지막 시도의 read 타임아웃을 깎는 것, 상태조회/망취소 전문 왕복, HTTP→소켓→원장 전 구간의 UNKNOWN→CANCELED/FAILED 해소까지. 서킷과 실행기는 시계와 sleep을 주입받게 만들어 실제 대기 없이 시간 흐름을 검증합니다.
...
[3장](#3-3값-원장과-손으로-짠-서킷브레이커)까지로 로드맵의 기능은 다 돌았습니다. 전문 코덱, 프레이밍, 필터 체인, 커넥션 풀, 원장, 서킷브레이커, UNKNOWN 해소까지 매 편 실측으로 "돈다"를 증명해 왔습니다. 그래서 이번 장은 방향을 뒤집었습니다. 질문 하나를 들고 **전체 코드 감사**를 돌렸습니다.
...
**경로 1. 서킷브레이커가 내부 사정을 백엔드 장애로 계수한다.** 3장의 실행기는 시도 실패를 이렇게 나눴습니다.
...
이 장의 수리는 한 문장으로 수렴합니다. 타입이 계약입니다. 예외 타입은 무슨 일이 났는지를 실어 나르는 계약인데, 계약서에 없는 타입이 흐르는 순간 그 위에 쌓은 서킷과 원장, 상태 판정이 전부 동시에 헛돕니다. 그래서 고치는 방식도 타입 단위로 정했습니다.

| 고친 것 | 어떻게 | 왜 |
|---|---|---|
| 서킷은 백엔드 실패만 센다 | `PoolExhaustedException` 전용 catch로 계수 없이 타입 그대로 올리고 재시도도 안 함 | 풀 고갈은 내부 사정이지 계정계 장애가 아님. 풀이 이미 borrow-timeout만큼 기다렸고 과부하에서 재시도는 부하 증폭 |
| 원장 공백 금지 | 왕복 이후 실패는 클라이언트 예외로 감싸 기록 경로에 태우고, 풀 고갈은 원장 FAILED와 503으로, 마지막에 `catch (RuntimeException)` 최후 방어 | 어떤 미분류 예외도 원장에 구멍을 못 내게 |
| 죽은 유휴 소켓 거르기 | 유휴 TTL(반납 후 30초 지난 연결 폐기) | `isValid()` 생존 검사는 그 자체가 왕복 비용이고 검사와 사용 사이 틈이 남음. 계정계 keep-alive보다 짧게 잡으면 낡은 소켓이 변경성 전문을 태울 창이 사라짐 |
| 날짜 전이 시 재시드 | 빠른 경로는 그대로 두고 `next()`가 날짜 전이를 감지했을 때만 synchronized 재시드 | 일련번호 충돌 방지 |
| 원장 유실 계수 | 적재 실패를 `gwanmun.ledger.dropped{reason}`으로 계수 | 적재가 거래를 막지 않는다는 원칙대로 삼키되, 유실은 보이는 유실이어야 알람을 걺 |
| 인코딩 fail-closed | `CharsetEncoder` + `CodingErrorAction.REPORT`, NUMERIC 필드의 비숫자도 빌드가 거절 | 상대 계정계 파서가 물음표를 어떻게 읽을지에 운명을 맡기지 않음 |
| 실거래 경로는 관문 안으로 | `/api/history`를 필터 체인에 편입. 반면 `/api/ledger`·`/api/pool/stats`·`/api/circuit/stats`는 일부러 밖에 남김 | 관측 경로는 계정계 호출이 없고, 서킷이 열린 상황을 보는 요청이 유량제어에 막히면 관측 자체가 안 됨 |
| 외부 응답 일반화 | 에러 바디에서 내부 host:port와 예외 원문, 입력 에코를 걷어내고 사유와 거래ID, correlationId만 | 상세는 서버 로그와 원장에 있고 correlationId가 둘을 잇는 열쇠 |

첫 줄에 디테일이 하나 숨어 있습니다. 실행기는 시도 전에 `breaker.acquire()`로 허가를 받는데, HALF_OPEN에서 탐침 정원 1을 받아 놓고 풀 고갈로 백엔드까지 가 보지도 못하면 성공도 실패도 아닙니다. 정원을 안 돌려놓으면 나가지도 않은 탐침이 자리를 영원히 차지해 서킷이 영영 안 닫힙니다. `onAborted()`를 그래서 추가했습니다.

3값 판정은 그대로입니다. 풀 고갈은 요청이 계정계로 나가기 전의 실패라 처리됐을 가능성이 0이고, 그래서 FAILED가 맞습니다.

실행기의 실패 분류가 이번 장의 핵심 diff입니다.
...
9행 완결. elapsed_ms=1008이 "borrow 대기 1초 후 거절"이라는 정책까지 숫자로 증언합니다. 성공 4건의 5초와 나란히 놓고 보면, 어느 요청이 풀을 쥐었고 어느 요청이 문 앞에서 돌아갔는지가 원장만으로 재구성됩니다. 3장에서 원장을 만든 이유가 바로 이런 날을 위해서였습니다.
...
3장까지가 "장애가 나도 무너지지 않는 게이트웨이"였다면, 1부는 "**자기 자신이 만든 예외에도** 무너지지 않는 게이트웨이"입니다. 감사가 가르쳐 준 건 결국 하나였습니다. 안전장치는 계약(타입) 위에 쌓이고, 계약에 없는 타입 하나가 흐르면 서킷도 원장도 판정도 한꺼번에 헛돈다는 것. 그래서 이번 장의 수리는 catch 블록을 고친 것처럼 보이지만, 실은 전부 계약서를 고친 일이었습니다.
...
서킷브레이커의 결과 보고 코드는 3장부터 이렇게 생겼습니다. `onSuccess()`와 `onFailure()`는 **현재 상태만** 봅니다.
...
**서킷이 있으면** 3연속 실패로 열린 뒤 나머지는 계정계 호출 없이 튕겨 냅니다. 처리량은 27배로 뛰고(351 → 9,425 req/s), p50 지연은 8.11초에서 0.68ms로 약 1.2만 배 떨어집니다. 빠른 실패는 **스레드를 죽은 백엔드에 헌납하지 않으려는** 장치입니다. 처리량이 오르는 건 그 부산물입니다. 이 숫자가 3장에서 서킷을 손으로 짠 이유를 뒤늦게 정당화해 줍니다.
...
[4장](#4-감사가-찾은-결함과-부하가-드러낸-레이스)까지 원장은 모든 거래를 3값(SUCCESS/FAILED/**UNKNOWN**)으로 적었고 UNKNOWN을 상태조회·망취소로 해소할 수 있었습니다. 하지만 3장부터 4장까지 "잔여"에 계속 적어 둔 두 문장이 있었습니다.

> 멱등키 없음(같은 요청의 재시도를 게이트웨이가 구분하지 못한다).
> 해소는 수동 트리거만(주기 대사 배치는 확장 지점).

이 둘은 같은 뿌리에서 나옵니다. **타임아웃은 실패가 아니라 미확인(UNKNOWN)이다**. 3장의 이 규칙이 옳으려면, 미확인을 받은 호출자가 그 다음에 무엇을 하느냐를 다뤄야 합니다.
...
그래서 이번 장의 두 축이 정해졌습니다. **재전송을 게이트웨이가 막고(멱등키), 매일 장부를 대조한다(EOD 대사).**
...
처음엔 인메모리 맵 + 락으로 짜려다 멈췄습니다. 4장에서 부하가 A3 서킷 레이스를 드러냈던 걸 떠올리면, 인메모리 락은 **단일 노드에서만** 맞습니다. 두 요청이 같은 키로 동시에 들어올 때, 이걸 하나로 묶는 일은 애플리케이션 락으로는 안 됩니다. **DB 유니크 제약**이어야 하고, 그건 노드가 몇 개든 안 깨집니다.
...
그리고 대조 **전에** UNKNOWN을 자동 해소합니다. 3장의 해소 절차(상태조회 → 처리됐으면 망취소 → CANCELED, 미처리면 FAILED)를 그대로 돌립니다. 미확인이 남은 채로 대조하면 진짜 불일치인지 "아직 모름"인지가 뒤섞이기 때문입니다.
...
왜 원장 PG인가. 부하가 커지면 모든 거래 경로가 지나는 원장 insert에서 경합·슬로우쿼리·락이 생깁니다. 4장의 `gwanmun.ledger.dropped` 카운터는 유실을 세지만 **왜** 느린지는 못 봅니다. 그건 gwanmun 자신이 안에서 못 보는 것(자기 커넥션 지연은 알아도 DB 서버 관점의 경합·플랜은 모름)이고 관제가 밖에서 `pg_stat_statements`로 봅니다.
...
테스트는 **168건**(기존 150 + 이번 장 18)이 그린이고 모듈 경계 `verify()`도 그대로 그린입니다. 각 단계의 함정·판단·검증은 저장소의 [ROADMAP](https://github.com/dj258255/gwanmun/blob/main/docs/ROADMAP.md)·[VERIFICATION](https://github.com/dj258255/gwanmun/blob/main/docs/VERIFICATION.md)에 있습니다.

3장에서 "타임아웃은 실패가 아니라 미확인"이라고 적었을 때, 그 규칙이 온전해지려면 미확인 이후의 세계, 곧 호출자의 재전송과 장부의 대조를 다뤄야 한다는 걸 이번에야 마무리했습니다. 재전송을 막고 매일 장부를 대조합니다.
...
여러 단계에 걸쳐 같은 경계가 반복됩니다. 그건 한 번만 적습니다.

실제 은행 표준 전문을 베끼지 않았습니다. 여기 쓴 전문은 구조만 현실적으로 흉내 낸 학습용 합성 스펙이고, 표준 전수 구현이 아닙니다.

전문 암호화와 전용선은 인프라 영역이라 손대지 않았습니다. 여기는 평문 로컬 소켓이고 API 키도 평문 보관입니다. 학습판의 경계입니다.

채번과 대사, rate limit, 서킷은 모두 단일 노드 전제입니다. 인스턴스를 여럿으로 늘리면 각자 따로 세고 각자 열고 닫습니다. 공유하려면 외부 저장소가 필요한데 그 순간 네트워크 왕복과 원자 연산, 장애 시 동작이라는 새 문제가 붙습니다.

JWT와 OAuth 단독 인증은 안 했습니다. 인증은 정적 API 키 검증까지이고, 토큰 만료와 서명 검증, 발급 위임 흐름은 범위 밖입니다. 인터페이스는 같은 자리라 나중에 교체할 수 있게만 뒀습니다.
...
**3장. 3값 원장과 손으로 짠 서킷브레이커**

- **UNKNOWN을 해소하는 흐름이 없습니다.** 이번 장은 UNKNOWN을 *기록*하는 것까지입니다. 실무라면 UNKNOWN 거래를 취소 전문(망취소)으로 무효화하거나 상태 조회(대사)로 확정 짓는 후속 흐름이 붙어야 합니다. 그게 2부의 재료입니다.
