...
## 1. 문제 정의 — 두 시스템이 말이 안 통한다

은행에 모바일 잔액조회를 붙여야 한다고 해봅시다. 문제는 양쪽이 쓰는 언어가 완전히 다르다는 거예요.

- **계정계(코어뱅킹)** 는 오래된 시스템이라 이렇게 말합니다: `"0200" + 계좌번호[14] + 거래코드[4] + 공백패딩...` — 띄어쓰기와 자릿수가 곧 의미인 **고정길이 전문**, 그것도 **TCP 소켓**으로.
...
지어낸 상황이 아니라 지금 실무의 문제 공간입니다. ISO 20022 전환기에도 다수 은행이 코어를 못 고쳐 미들웨어로 신규 포맷과 레거시 고정길이를 변환하는 전략을 택했고(변환 계층은 사라지기는커녕 두꺼워지는 중), 국내 대외계·코어뱅킹은 여전히 고정길이 전문 + TCP 연계가 흔합니다.

## 2. 설계의 경계 — 두 층, 그리고 안 섞는 정체성

통역기가 하는 일은 두 층으로 나뉩니다.

- **연계(통역) 층** — JSON을 계정계가 알아듣는 전문으로 바꾸고, TCP로 보내고, 돌아온 전문을 다시 JSON으로. 프로토콜(HTTP↔TCP)과 포맷(JSON↔전문)을 동시에 변환합니다.
- **API 게이트웨이 층** — 이 통로가 외부(핀테크·마이데이터)에 열리면 아무나 들어오면 안 되니, 앞단에서 인증·라우팅·유량제어로 지킵니다. 신분을 확인하고, 어디로 갈지 안내하고, 방문 횟수를 제한하되, 집 안 살림(잔액 계산)엔 관여하지 않아요 — **흐름은 통제하되 비즈니스 로직은 계정계에 위임**한다는 원칙입니다.
...
이 프로젝트는 기능 나열이 아니라 **"어떤 상황에서 무엇이 깨지고, 그래서 무엇을 만드는가"** 로 씁니다. 각 단계는 앞 단계가 남긴 구멍에서 출발했어요. 전/후가 있는 건 전후로 실측했습니다.
...
1. **서킷이 잘못 열렸다.** 실행기가 이걸 백엔드 실패로 계수했습니다. 계정계는 멀쩡한데 내부 고갈 3연속이면 서킷이 OPEN — 이후 멀쩡한 계정계로 가는 모든 거래가 503으로 차단됩니다(오보).
2. **거래가 장부에서 증발했다.** 서비스·컨트롤러가 IOException·GatewayException만 잡아서, 이 예외는 500으로 관통하며 **원장에 아무것도 안 남겼습니다.** 8건을 보냈는데 원장엔 5행 — 4건이 사라졌어요.
...
고갈 4건은 HTTP 500이 아니라 **503**("포화 상태" + 거래ID + correlationId)으로 나가고, 서킷은 **CLOSED**를 유지하며, 직후 멀쩡한 계좌는 200입니다. `elapsed_ms=1008 ≈ borrow 대기 1000ms`라, 고갈 거절이 "대기 후 거절"이라는 것까지 숫자로 남아요. 이 회귀는 `AuditRegressionIntegrationTest`로 고정해, 다시 깨지면 CI가 잡습니다.
...
**부하 수치도 경계가 있습니다.** k6·앱·목업이 한 머신에서 CPU를 나눠 쓰므로, 무릎 ~10–12k req/s는 **결합 시스템의 천장**이지 순수 서버 성능이 아닙니다. 그래도 상대 비교(게이트웨이 vs 직접, 서킷 on vs off)는 유효해요. "금융권 표준 P95" 같은 공개 근거 없는 수치는 지어내지 않았습니다.

## 6. 안 하기로 한 것과 그 이유

"다 넣었다"가 아니라 **"왜 안 넣었는지 안다"** 가 이 프로젝트의 성격입니다. 감사 백로그에서 안 하기로 가른 것과 근거를 그대로 옮깁니다.

- **JWT/OAuth 단독 인증 — 안 함.** 서버 간 게이트웨이 인증은 API 키+mTLS가 관례이고, 토큰 발급 인프라는 전문·연계라는 이 프로젝트의 주제와 겹치지 않습니다.
- **분산 rate limit(Redis 등) — 안 함.** 인스턴스 로컬 토큰버킷으로 개념은 이미 증명했고, 분산화는 저장소 의존만 늘립니다. 실무 답도 "로컬 근사 + 임계 근처만 외부 저장소 검증" 하이브리드예요.
- **HA·이중화·중앙 채번 — 안 함.** 실무에서 절체는 L4/인프라 계층 장치입니다. 앱이 담당하는 부분집합(graceful shutdown·재접속)은 이미 커버했고, 단일 노드 경계를 코드 주석·문서에 명시하는 쪽을 택했어요.
- **MQ 어댑터(Kafka 등) — 안 함.** 동기 TCP 전문 연계가 주제입니다. 비동기 채널은 응답 전문의 거래ID 매칭 등 전제가 달라져 별개 프로젝트 규모가 됩니다.
...
1. **차이는 경계 뒤로** — 프로토콜·포맷 차이는 연계층 뒤로, 외부 위협은 게이트웨이층 뒤로, 두 정체성(인라인 중계 vs 아웃오브밴드 관제)은 별도 저장소로.
