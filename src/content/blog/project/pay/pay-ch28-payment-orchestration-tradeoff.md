...
description: '두 개 이상의 PG를 연결하면 장애 우회와 승인율 최적화를 얻을 수 있지만, 상태·환불·토큰·정산·대사의 복잡성이 함께 생긴다. pay에서 다중 PG를 바로 확대하지 않고 계약과 실패 경계를 먼저 남긴 이유를 정리했다.'
date: 2026-09-22
category: study/pay
...
Adyen도 orchestration이 장애를 없애는 기능이 아니라 복잡성을 다른 위치로 이동시키는 선택이라고 설명합니다. 그래서 현재 pay에서는 provider 추상화가 있다고 해서 live failover가 구현됐다고 쓰지 않습니다.
