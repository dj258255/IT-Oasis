...
description: '원장 잔액이 맞는 것만으로는 부족하다. 자금 이동의 completeness·timeliness·accuracy·explainability를 기준으로, pay에 이미 구현한 것과 아직 외부 계약이 필요한 것을 나눠 정리했다.'
date: 2026-09-22
category: study/pay
...
이 기준을 그대로 가져와 “pay도 대규모 결제 플랫폼이다”라고 주장하지는 않았습니다. 대신 각 기준을
현재 규모에서 관찰할 수 있는 질문으로 줄였습니다.

| 품질 축 | pay에서 묻는 질문 | 현재 확인한 것 |
|---|---|---|
...
환불, 차지백, 수수료, 지급 보류가 올 수 있습니다. 그래서 pay는 원거래를 덮어쓰지 않고
각 사건을 별도 분개로 남깁니다.
...
현재 pay는 승인·취소·차지백 패소·정산 지급을 별도 원장 분개로 남기고, 차변과 대변이 맞지 않는
...
현재 pay는 원본 이벤트를 멱등하게 저장하고, 결제 행이 없으면 `PENDING_PAYMENT`로 남기며,
...
이런 결정을 혼자 진행하면 기능을 생각나는 순서대로 추가하기 쉽습니다. 그래서 pay에는 작업 보드와
