...
PG 정산 파일에는 있는데 우리 장부에는 없는 결제를 **모델이 찾아내게 했습니다.** 규칙이 원인 후보를 하나도 못 낸 건만 모델에게 넘기고 이 원인 하나만 판정하도록 범위를 좁혔더니, 한 유형은 세 모델이 91~100%를 맞혔습니다.

**최종 상태는 "껐습니다"입니다.** 표본을 넓혀 한 유형만 켜니 잘 맞았지만, 그 유형은 **규칙 분류기가 이미 같은 답을 내고 있어 규칙 대비 개선이 0**이었습니다. 홀드아웃에서 다시 재고 껐고, 기본값은 `template`(모델을 부르지 않음)입니다. 한 줄로 적으면 **켰다 → 홀드아웃에서 재측정 → 껐다(기본값 `template`)** 입니다.

**이 글이 남긴 셋은 이렇습니다.** 재는 방법을 두 번 잘못 잡았고(모델이 아니라 표본 문제), 잘 되는 유형 하나를 켰지만 규칙 대비 개선이 0이라 껐으며, 프롬프트로 시키는 검증은 실패하고 코드가 막아야 했습니다.
...
### 나머지 사례는 가드 표의 근거로 흡수했다

기권을 고위험 분류의 표준 장치로 두는 **선택적 예측(selective prediction)**은 가드 4의 근거가 됐습니다. 규칙이 못 가른 건은 애초에 어려운 건이라, 모델이 아무거나 고르면 확인하는 사람의 일이 오히려 늡니다. 토스가 공개한 에러 분석 자동화에서 얻은 **"더 많은 정보를 제공할수록 더 많은 분석을 이끌어낸다"**는 주문 이력(11곳에서 모아둔 그 이력)을 통째로 넘기는 근거가 됐고, 카드사 FDS 서술의 **"기본 분류·대조는 AI가, 담당자는 판단에 집중"**은 "제안까지만"의 근거가 됐습니다. LINE의 평가 자동화 사례는 모델보다 **평가 하네스를 먼저** 두는 순서를 뒷받침합니다 — 이 프로젝트가 [11편에서 잣대를 먼저 만든 것](/blog/project/pay/pay-ch10-ruler-first)과 같은 판단입니다.
...
**분류는 셋 다 쓸 만한데 기권은 아닙니다.** llama는 기권해야 하는 네 건 중 **셋을 단정**했습니다. 14B도 아래 H-15처럼, 차액 1,000원이 수수료율과 맞는다는 근거가 없고 **취소 이력을 못 봤다는 사실이 목록에 적혀 있는데도** 단정했습니다.
...
| 표본 | 결과 |
|---|---|
| 손으로 만든 홀드아웃 15건 (일곱 원인 골고루) | 14B가 **14/15** |
...
잔여가 40건에서 **272건**이 됐습니다. 유형별로 그 유형의 건을 모아 다시 쟀고, 가장 큰 유형(외부에만 있음)이 **60건**이었습니다.
...
그래서 고를 수 있는 값을 그 하나로 줄였습니다. 그랬더니 **60건 중 대부분을 그 하나로 찍었습니다.** 정확도가 15%로 떨어졌습니다.
...
이 시점에 기본값을 `ollama`로 바꿔 켰습니다(뒤의 홀드아웃 재측정에서 다시 끕니다). **켠 유형은 하나이고 전체 잔여 272건 중 60건, 약 22%를 덮습니다.** 나머지 78%는 기권으로 사람에게 갑니다.
...
반대 사례가 Klarna입니다(참고 사례). AI 상담으로 **한 달에 230만 건**, 전체 대화의 **3분의 2**를 자동화하고 해결 시간을 **11분에서 2분 미만**으로 줄였습니다. 그런데 **2025년에 되돌렸습니다.** 사람 상담원을 다시 뽑았고 CEO가 "비용 주도 자동화는 품질이 낮다"고 인정했습니다.
...
**규칙이 못 가른 예외 중 한 유형을 AI에게 맡겼습니다. 일곱 유형을 다 맡기면 못 쓰는데, PG엔 있고 우리 장부엔 없는 건 하나만 켜니 세 모델이 91~100%였습니다. 그 수치는 기준을 고친 그 표본에서 나온 값이고요.**
...
표본을 여섯 번 만들었습니다. 크기와 결론이 서로 다릅니다.

| 표본 | 크기 | 어떻게 만들었나 |
|---|---|---|
| 개발 | 12건 | 손으로 작성(정답 9 · 기권 3) |
| 홀드아웃 | 15건 | 손으로 작성(정답 11 · 기권 4) |
| 엔진 잔여 | 40건 | 엔진이 만든 실제 잔여 분포 |
| 확장 잔여 | 272건 | 코퍼스·가드 수정 후 |
| 유형 표본 | 60건 | 확장 잔여 중 한 유형(외부에만 있음) |
| 재홀드아웃 | 45건 | 프롬프트 동결 후 새로 만든 표본(유형당 15) |

**앞의 둘은 제가 문장을 썼고, 나머지는 엔진이 실제 분포에서 만들었습니다.** 그리고 표본마다 결론이 서로 다릅니다.
