...
PG 정산 파일에는 있는데 우리 장부에는 없는 결제를 모델이 찾아내게 했습니다. 규칙이 후보를 하나도 못 낸 건만 넘기고 그 하나만 판정하게 좁혔더니 한 유형은 91~100%를 맞혔지만, 홀드아웃에서 다시 재니 **규칙 분류기가 이미 같은 답을 내 개선이 0**이라 껐습니다. 기본값은 `template`입니다.

대사는 내 결제 기록과 PG 정산 파일을 하루치씩 대조해 안 맞는 건을 찾고 사람이 원인을 골라 확정하는 작업입니다. 그 화면엔 11곳에서 모은 이력·산수로 후보 내기·안내 초안 쓰기를 붙였고, 이 글은 **규칙이 아무 후보도 못 내는 건**에만 모델을 붙인 기록입니다.
...
1차에서 **틀린 자리가 모델을 넘어 겹쳤습니다.** `TIMEZONE_BOUNDARY`와 `PG_FILE_DELAY`는 둘 다 "다른 날짜 파일에 있다"라 방향을 안 주면 못 가릅니다 — 전날이면 거래일 경계, 다음날이면 파일 지연입니다.
...
**모델을 키운 것보다 정의를 붙인 게 컸습니다** — 8B→14B가 +1, 같은 8B에 정의를 붙인 게 +3, 계열이 다른 llama도 3→8. 뜻을 안 적은 원인이 있으면 테스트가 걸리게 `ResidualPromptBuilder`에 박았습니다.
...
갈린 지점: 손으로 쓴 홀드아웃으로 판단할 것인가, 엔진이 실제 잔여 분포를 만들게 할 것인가. 외부 리뷰가 "고친 프롬프트를 같은 12건으로 다시 쟀다"를 짚어, 프롬프트를 손대지 않고 안 본 15건(정답 11 · 기권 4)을 한 번만 쟀습니다.
...
**14B는 유지됐습니다.** 하지만 두 표본 다 문장을 내가 썼습니다.

> 정확도는 **250~500개**에서 안정화되기 시작한다. 다만 **1,000개가 의미상 비슷하면 실제로는 수백 개만큼의 검정력밖에 없다.**

그래서 조건 축만 정하고 **사실 문장은 대사 엔진이 만들게** 하니(금액 8종 × 조건 8종 × 변형 5개 = 320건) **규칙이 아무 후보도 못 낸 건은 40건**뿐이었고, 규칙이 못 내는 문은 "내부에만 있음" 한 줄뿐이었습니다(금액 불일치는 배제법으로, 외부에만 있음은 `INTERNAL_RECORD_LOST`). 손으로 만든 27건은 실제로 오지 않을 건들이었습니다.

그 40건에 세 모델 모두 **0/40**, 전부 `INTERNAL_RECORD_LOST`, 신뢰도 95 · 96 · **100**. 뜻은 "PG에는 있는데 내부에 기록이 없음"이라 적어 뒀는데 사실은 정반대였습니다 — 그때는 정의가 없어서, 이번엔 정의가 있는데도 틀렸습니다.
...
**신뢰도 임계는 모델마다 다르게 듭니다.** 원인을 실제로 고른 건만 모아 임계별 통과/정답/오답을 셌습니다.
...
세 모델 다 무시했습니다. 그래서 기권을 부탁하는 대신 **코드가 막게** 했습니다 — 조회가 한 곳이라도 실패하면 원인을 가릴 수 없으니 모델을 아예 안 부릅니다. 같은 15건을 다시 돌려 14B가 15/15가 됐습니다(위 표). **모델도 프롬프트도 안 바꿨습니다. 코드가 막았습니다.**
...
"정보가 모자라다"가 아니라 **"있는 정보로 틀린 추론을 한다"**라서 조건문으로 못 막습니다. 인용한 7,000원이 실제로 있는 값인데도 가드를 전부 통과한 오답이 있었습니다 — **출처가 맞는 숫자로 틀린 주장을 할 수 있습니다.** 그건 [네 번 나를 속인 채점 기준](/blog/project/pay/pay-ch11-ruler-fooled-me)이 잡을 일이라, 출처를 재는 일과 주장이 맞는지 재는 일은 다른 층입니다.

## ④ 좁혀서 켰다가, 홀드아웃에서 껐다

갈린 지점: 전체 정확도로 볼 것인가, 유형별로 갈라 볼 것인가. 전체 정확도로만 보고 있었는데, 코퍼스(대사 기록이 안 만들어지던 것)와 가드 1(후보가 하나라도 → **결정적 후보가 있으면**)을 고치니 잔여가 40 → **272건**이 됐습니다.
...
**전체는 25~40%인데 한 유형은 60건 중 60건입니다.** 맨 위는 사실이 "파일에 있고 내부에 없다"로 단순하고 안 겹치고, 맨 아래는 셋 다 차액이 있으면 무조건 수수료로 답합니다. 값을 그 하나로 줄였더니 **60건 중 대부분을 그 하나로 찍어** 정확도가 15%로 떨어졌습니다.
...
**켠 유형은 전체 잔여 272건 중 60건, 약 22%**를 덮고 나머지 78%는 사람에게 갑니다. 프롬프트를 한 글자도 안 바꾸고 새 표본 45건으로 다시 재니:
...
**그리고 껐습니다.** 모델이 낼 수 있는 값은 그 하나 또는 기권뿐이고, 그 값이 통과하는 조건인 "외부에만 있음"에서 **규칙 분류기가 이미 같은 답**(`INTERNAL_RECORD_LOST`)을 냅니다. 왕복은 값을 만들지 않고 지연과 비용만 만듭니다. 유형별로는 갈라 쟀지만 **규칙 대비로는 안 쟀습니다.** 기본값을 `template`으로 되돌렸습니다.
...
**남은 구멍.** 나머지 유형은 세 모델 모두 0~50%라 열 수 없고, 실제 업무 개선을 말할 표본은 아직 없으며, 사실이 완전한데 틀리는 추론은 여전히 못 막습니다.
