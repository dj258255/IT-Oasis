---
title: '백오피스 AI: 잘 되는 줄 알았는데 표본이 내가 만든 것이었다'
description: '규칙이 못 가른 대사 건에만 모델을 붙이고 가드를 일곱 개 걸었습니다. 손으로 만든 표본에서는 잘 됐는데, 대사 엔진으로 실제 잔여 분포를 만들어 재니 세 모델이 40건 모두 틀렸습니다. 차이를 만든 것은 모델이 아니라 표본이었습니다.'
...
**규칙이 못 가른 예외에 모델을 안전하게 붙여 봤고, 손으로 만든 표본에서는 분류가 쓸 만해 보였습니다. 그런데 엔진으로 실제 분포를 만들어 재니 0/40이었고, 켜지 않았습니다.**

무엇이 됐고 무엇이 안 됐는지 갈라 적으면 이렇습니다.

| | 상태 |
|---|---|
| 원인 분류 | 손으로 만든 표본에서는 14B가 11/11. **엔진이 만든 실제 잔여 분포에서는 0/40** |
