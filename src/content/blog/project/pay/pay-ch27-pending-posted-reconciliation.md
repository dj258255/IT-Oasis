...
description: '결제 예외 큐의 PENDING과 외부 금융 거래의 pending은 같은 말이 아니다. 잠정 대사를 빠르게 보여주는 것과 최종 금액을 확정하는 것 사이의 트레이드오프를 pay의 현재 모델과 함께 정리했다.'
date: 2026-09-22
category: study/pay
...
pay의 `PENDING`은 내부 기록과 외부 기록이 맞지 않아 사람이 확인해야 하는 예외 상태입니다. 그런데 외부 PG나 은행의 `pending`은 아직 최종 게시되지 않은 거래를 뜻할 수 있습니다. 둘을 하나의 상태로 취급하면 “현재 보이는 값”과 “최종적으로 확정된 값”을 구분할 수 없습니다.
...
현재 pay는 이 둘을 섞지 않는다. 정규화된 지급 report를 입력으로 받는
