...
> 토스페이먼츠는 서명 스펙을 공개하지 않으므로 자체 Mock PG 기준으로 구현한다.
...
토스는 서명을 주지 않으므로 이 문에는 위조 요청도 들어옵니다. 막는 것은 페이로드를 믿지 않는 설계입니다. 해석이 조회 API로 실상태를 다시 물으므로, 위조가 할 수 있는 최대치는 **쓸모없는 조회를 부르는 것**이고 장부는 바뀌지 않습니다. 발신 IP 제한은 아직 걸지 않았습니다.
