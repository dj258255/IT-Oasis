...
수수료율은 **basis point(bps)** 정수로 뒀습니다. 270 bps가 2.7%입니다. `0.027`(double)로 두고 `gross * 0.027`을 하는 쪽이 편해 보이지만, 이 프로젝트에는 [KRW를 소수점 없는 정수(long)로만 다룬다](/blog/project/pay/pay-ch1-what-to-trust)는 원칙이 있습니다. 부동소수를 돈 계산에 끼우면 반올림 오차가 쌓입니다.
...
그런데 [정산 모듈](/blog/project/pay/pay-ch1-what-to-trust)은 그걸 몰랐습니다. `PaymentSettlementListener`가 결제 승인 이벤트를 받아, 승인되자마자 정산 항목을 쌓고 있었습니다.
...
전액취소는 "이미 CANCELED면 무시" 가드가 있어서 [at-least-once 재배달](/blog/project/pay/pay-ch1-what-to-trust)에 안전합니다. 부분취소는 다릅니다. `reduce`로 델타(취소분)를 빼기만 합니다.
...
> 대사에서는 취소를 **별도 음수 행**으로 쌓도록 바꿨습니다(10편). **정산은 "지금 얼마를 지급할 것인가"라는
