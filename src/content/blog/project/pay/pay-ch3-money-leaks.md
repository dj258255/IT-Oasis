---
title: '성공했는데 돈이 틀렸다: 정산과 취소의 조용한 자금 버그'
description: '정산이 총액의 3%만 떼고, 집계 키가 승인일이라 가맹점에 돈을 안 주고, 구매확정 전에 정산되고, 취소가 월렛을 몰랐고, 부분취소는 재배달되면 두 배로 깎였다. 다섯 다 아무도 에러를 안 냈다.'
...
결제에서 제일 무서운 실패는 터지는 실패가 아니다. 예외도 로그도 없이 숫자만 틀리는 실패다.

이 글은 정산과 취소 경로에서 그렇게 조용히 틀리고 있던 것들을 다섯 갈래로 묶었다. 수수료는 계산됐고 정산은 실행됐고 취소는 성공했는데, 금액이 틀렸다.
...
> PG/가맹점 정산은 **수수료(정률)** 에 그 **수수료의 부가세** 를 더 떼고, 지급이 **언제** 이뤄지는지(**지급예정일**)까지 계산한다.
>
> **구체적인 값은 계약마다 다르다.** 결제수단별 수수료율, VAT 별도·포함, 건별 계산인지 총액 계산인지, 원 단위 반올림·절사, 일·주·월 정산, 지급 주기 +N이 전부 계약 사항이다. **이 프로젝트는 "과세 수수료 2.7%, VAT 별도 10%, 일정산 후 2영업일 지급"을 가정했다.** 승인 100,000원이면 수수료 2,700원에 VAT 270원이 붙어 실지급 97,030원이다.
...
거래일은 **`Asia/Seoul` 기준**으로 자른다. UTC로 자르면 한국시간 00:30 결제가 전날 정산으로 들어간다. 이 편의 다른 버그와 정확히 같은 실패 모드(조용히 하루가 밀린다)라 도메인 상수로 못 박았다.
...
이름은 `confirmedDate`("구매확정일")인데 실제로 담긴 건 **승인일**이었다. 항목이 `CONFIRMED`(정산 가능)로 바뀌는 건 에스크로 릴리스(구매확정) 시점인데, 그 전이가 `confirmedDate`를 **재스탬프하지 않았다.**
...
이제 릴리스일 R로 재스탬프되니 **R+1의 `settle(R)`이 이 항목을 집계**한다.

**그런데 이 수정도 반쪽이었다.** 외부 리뷰가 짚었다.
...
그 날짜 정산은 이미 있어 멱등하게 skip되고, 다음 날 배치는 다른 날짜만 본다.
**늦게 확정된 항목이 영영 집계되지 않는다.** 승인일로 집계하던 원래 버그와 같은 실패 모드다.

날짜를 정확히 맞추는 대신 **그 날짜 이하의 미정산 재고를 전부** 보게 고쳤다.
...
늦게 확정된 항목은 다음 실행이 쓸어 담는다. **"그 날짜에 확정된 것"이 아니라 "아직 안 나간 것"을
묻는 게 맞았다.** 필드 이름도 이 수정으로 바로잡혔다. `confirmedDate`가 이름대로 "구매확정일"이 됐다(전엔 이름과 달리 승인일이었다).
...
읽고 나서 "아…" 했다. 에스크로 편에서 분명히 이렇게 만들었다. 결제금을 구매확정 전까지 HELD로 보류하고, 구매자가 확정하면 RELEASED로 풀어 정산 가능하게. 자금이 판매자 것이 되는 건 구매확정 시점이라고.
...
- 너무 일찍 (승인 시점) 정산하면 → 미배송·분쟁 때 이미 나간 돈을 회수 못 한다.
...
> 이미 SETTLED된 항목에 취소가 오면 금액을 건드리지 않고 `settlement.postsettle.cancel` 카운터만 올렸다. "정산은 재구성 가능한 집계이고 취소의 진짜 이력은 원장 역분개가 보유한다"는 판단을 주석에 적어뒀다.

#### 그런데 카운터로는 부족했다

외부 리뷰가 짚었다. **원장은 취소 이력을 갖고 있지만, 그건 근거지 실행할 일이 아니다.**

카운터만 있으면 "몇 건 있었다"는 알지만 **어떤 주문을 얼마 조정해야 하는지는 복구할 수 없다.**
프로세스가 재시작되면 운영이 처리할 목록조차 안 남는다.
...
settlement_adjustments
  ...
  adjustment_amount          ← 음수로 저장. 차기 총액에 그대로 더한다
  status                     PENDING / APPLIED / REVIEW_REQUIRED
```

**과거 정산은 고치지 않는다.** 이미 지급 대상으로 나갔고, 수정하면 그때 무엇을 근거로 얼마를
줬는지 추적할 수 없게 된다. 회계에서 지워진 기록은 기록이 아니다. 대신 **차기 정산이 음수로
반영**한다. 원장이 취소를 역분개로 쌓는 것과 같은 형태다.

여기서 선을 하나 더 그었다. **회수액이 차기 정산 총액보다 크면 자동으로 처리하지 않는다.**
그건 음수 지급, 즉 "돈을 돌려받는" 일이라 지급 파이프라인이 아니라 별도 청구 절차다.
조용히 0으로 깎으면 **회수액이 증발한다.** `REVIEW_REQUIRED`로 남겨 사람이 본다.

그리고 정산 항목에 `settlement_id`를 뒀다. 이게 없으면 "어느 지급에서 잘못 나갔는지"를
못 적는다. 회수를 만들려면 원 정산을 가리킬 수 있어야 한다.

측정을 부풀리지 않았듯(데드락 편 각주처럼), 처리 못 하는 케이스도 "못 한다"고 표시하는 쪽을 택했다. 조용히 틀리게 처리하는 것보다 낫다.

### 4. 경계는 여기서도 단방향

정산이 에스크로 이벤트를 구독하니 `settlement`가 `escrow`에 의존한다. 방향을 확인했다. settlement → escrow 단방향이다. 에스크로는 정산을 모른다(자기 이벤트만 던질 뿐). 모듈 경계를 CI가 강제하니 순환이 생겼으면 빌드가 깨졌을 텐데, 통과했다.
...
결과부터. 회원·분쟁/차지백·월렛 배선까지 확정 기능을 다 만든 뒤 바로 그 신규 코드를 정면으로 감사했고, **치명 2건 포함 11건**이 나왔다. 인증 쪽 셋은 [인증 비용 편](/blog/project/pay/pay-ch6-auth-cost)으로 보내고, 여기서는 돈이 걸린 것만 본다. 지난 감사들이 매번 실 자금 버그를 잡아왔으니, 돈과 인증을 새로 건드린 코드가 무사할 리 없다고 봤는데 역시나였다. 하나씩 보면 전부 "기능은 각자 옳은데, 만나는 지점이 틀린" 종류였다.
...
이제 같은 취소가 세 번 와도 `amount`는 잔액 그대로다. **절대값 세팅은 중복 배달에 멱등**이다.

**다만 순서 역전에는 안전하지 않다.** 이것도 외부 리뷰가 짚었다.
...
2차가 먼저 소비 → 4,000
늦게 온 1차 적용 → 7,000   ← 되돌아간다
```

멱등한 것과 순서 독립적인 것은 다르다. 그래서 **취소 순번을 함께 본다.**
...
순번은 결제 도메인이 **이미 부여하고 있던 값**이다(`PaymentCanceledEvent.cancelSeq`).
정산이 자기 번호를 새로 매기면 같은 취소인지 판단할 근거가 사라진다. 델타(`cancelAmount`)는 원장 역분개와 에스크로 환불이 여전히 쓰기 때문에 같이 실어 보내고, 정산만 절대 잔액을 본다.

> **정정 이력**: 나중에 대사 쪽에서 같은 이름의 메서드를 발견하고 일괄 수정할 뻔했다.
> 대사는 "무슨 일이 있었나"라는 **사건 이력**이라 덮어쓰면 이미 나간 판정이 무효가 된다.
> 대사에서는 취소를 **별도 음수 행**으로 쌓도록 바꿨다(9편). **정산은 "지금 얼마를 지급할 것인가"라는
> 현재 상태**라 덮어쓰는 게 맞고, 두 모듈의 처방이 다른 게 정상이다.
...
다섯 갈래 다 **예외를 던지지 않았다.** 로그도 안 남았다. 수수료는 계산됐고, 정산은 실행됐고, 취소는 성공했다. **숫자만 틀렸다.**
