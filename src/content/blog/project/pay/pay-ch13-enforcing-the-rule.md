...
## 그리고 문서와 코드를 통째로 대조했다

대사 쪽에서도 같은 걸 겪었다(8편). 그때 든 생각은 이거였다.

> **내가 안 본 곳은 내가 세어서는 못 찾는다.**

그래서 밖에서 가져왔다. 여기서 댄 잣대는 **내가 쓴 문서**다.

### 문서가 말하는 것이 코드에 있는가

`TIMEZONE_BOUNDARY`가 문서에만 있고 코드에 없던 걸 찾은 적이 있다. **같은 종류가 더 있는지** 전수로 봤다.
...
**재발을 막는 테스트를 붙였다.** 스펙 표에서 코드를 뽑아 실제로 던지는 문자열과 대조한다. 앞의 ERD 문서 대 마이그레이션과 같은 발상이고, 대사의 원인 분류 수 대 규칙도 같다(8편). **문서가 코드보다 앞서가는 걸 사람이 알아채길 기다리지 않는다.**
---
