...
결제 코어를 만들 때 제일 먼저 정한 것이 신뢰 경계였습니다. 금액·가격·재고처럼 돈이 걸린 값을 누가 정하느냐입니다. 이 글은 금액 검증을 열심히 짜놓고도 그 검증이 보는 기준값이 이미 클라이언트에서 온 값이었던 것을 발견하고 고친 기록입니다.
...
그런데 이 절의 진짜 이야기는 뒤에 있습니다. **커밋 보안 검토가 내 결제 시스템의 심장을 찔렀습니다.** 거기부터 거꾸로 풀어 갑니다.
...
눈여겨볼 건 `UNKNOWN`입니다. 보통 결제 튜토리얼엔 없는 상태입니다. 토스페이먼츠 승인 API를 부르다 타임아웃이 나면, 결제가 된 건지 안 된 건지 우리는 모릅니다. 이걸 "실패"로 단정하면 돈은 빠졌는데 주문은 취소되는 최악의 CS가 생깁니다.

그래서 타임아웃을 실패로 단정하지 않고 `UNKNOWN`으로 보존합니다. (카카오페이 "MSA 네트워크 예외" 글의 성공/실패/Unknown 3-상태 모델입니다.) 이 상태를 실제로 어떻게 확정하는지(조회·망취소·복구 배치)는 다음 절의 주제고, 여기서는 상태를 잃지 않고 보존하는 데까지.
...
금액, 상품 가격, 재고처럼 "돈이 걸린 값"의 원천(source of truth)이 서버인지 클라이언트인지를 먼저 못 박아야 합니다. 그게 틀리면 그 위에 아무리 검증을 쌓아도 극장입니다. 검증 로직은 그다음 문제입니다.
...
**돈이 걸린 값의 원천이 서버인지 클라이언트인지를 먼저 못 박아야 합니다.**
그게 틀리면 그 위에 아무리 검증을 쌓아도 극장입니다.

이 질문은 뒤에서도 계속 나왔습니다. 유입 제한을 붙이면서 "클라이언트가 준 IP"를 믿을 뻔했고(7편),
웹훅을 받으면서 "PG가 보낸 값"을 믿을 뻔했습니다. **방어 코드를 짤 때도 신뢰 경계를 착각하면
그게 곧 구멍이 됩니다.**
