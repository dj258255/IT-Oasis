---
title: '결제 코어: 무엇을 신뢰하고, 실패를 어떻게 다룰지'
description: '검증보다 신뢰 경계가 먼저였고, 타임아웃은 실패가 아니라 미확정이었고, 이미 나간 승인은 롤백 대신 보상으로 되돌렸고, failover는 넘기면 안 되는 경우부터 정했다. 실패를 어떻게 다룰지 초반에 정한 것들이다.'
...
이 넷이 뒤에 나오는 모든 것을 지탱했다. **신뢰 경계**는 매 API 마다 다시 물었고, **타임아웃을 실패로 안 본 것**은 복구 배치·대사·미확정 나이 지표로 이어졌고, **롤백 대신 보상**은 외부 시스템이 낀 모든 자리의 기본형이 됐고, **failover 조건을 좁힌 것**은 "재시도해도 되는 실패"를 가르는 기준이 됐다(2편). 다만 그 라우터를 실제로 꽂는 데는 한참이 걸렸다.
