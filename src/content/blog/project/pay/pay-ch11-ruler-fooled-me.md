...
고객에게 그대로 보낼 수 없는 안내 초안에 채점기가 **평균 4.97/5점**을 줬습니다. 이 글은 초안이 쓸 만한지 재려다 그 잣대가 네 번 나를 속인 기록입니다. 무르면 나쁜 게 통과하고, 빡빡하면 좋은 게 떨어지고, 요구하면 대상이 요구에 맞춰 변형되고, 맞아도 검사 밖은 못 봅니다.
...
이 글의 산출물은 잣대 하나가 아니라 **설계**입니다. 검사를 세 층으로 나누고, 확실한 것만 버리고 나머지는 표시합니다.
...
그리고 모델을 비교할 때는 **가점**을 씁니다. 점수를 **요구하면 대상이 그 요구에 맞춰 변형되므로**, 통과 여부는 그대로 두고 가점만 얹습니다. 아래 두 상황은 이 둘을 어떻게 고르게 됐는지의 기록입니다.
...
첫 결과가 **평균 4.97/5점(5점 만점 기준 99.4%).**

그런데 4건 블라인드 리뷰는 편집률 45%라고 했습니다. **둘 중 하나가 틀렸습니다.** 무엇으로 통과했는지 뜯어봤습니다.
...
¹ 지연은 이 표에서 유일하게 나빠진 값입니다. 이 +23%는 **다른 계열 심판과 가드를 더 얹은 비용**으로, 개선의 대가로 치른 값이라 따로 적어 둡니다.
...
**앱이 구독 자격증명으로 모델을 부르는 건 정책 이전에 기술적으로 안 됩니다.** CLI가 쓰는 건 API 키가 아니라 OAuth 토큰이고, 어댑터가 부르는 Messages API는 `x-api-key`를 받습니다. 로컬이든 아니든 인증이 성립하지 않고, 구독은 API 접근을 포함하지 않습니다.
...
> **남은 트레이드오프.** 앞에서는 결제 데이터를 밖으로 보내는 설계를 못 쓴다고 해놓고, 뒤에서는 사람이 복사해 외부 모델에 넣을 통로를 기능으로 열었습니다. 꺼낸 프롬프트에 주문번호·금액·거래일이 그대로 들어 있는데 "붙여 넣는 건 꺼낸 사람 책임"이라고 주석에 적어둔 게 전부였습니다. **앱이 직접 보내지 않았다는 건 방어가 안 됩니다.** 외부 리뷰에서 지적받았고, 맞는 지적입니다. 지금은 안 돼 있습니다 — 규제를 설계 조건으로 걸어놓고 그 조건을 사람 손에 맡긴 셈이라, **이 편에서 제일 약한 자리**입니다.
...
같은 케이스 4건을 8B 초안과 프런티어 모델 초안으로 나란히 채점했습니다. 루브릭은 항목 6개라 **케이스당 만점이 6**입니다.

| 케이스 4건 | qwen3:8b | 프런티어 |
|---|---:|---:|
...
**내가 고른 축만 봅니다.** 밖에서 분류표를 가져오거나 다른 계열 모델을 심판으로 세운 것도 같은 문제를 다른 방향에서 친 거였습니다.
...
**검사를 만들면 그 검사가 나를 속입니다.** 무르면 나쁜 게 통과하고(평균 4.97/5점인데 발송 불가),
빡빡하면 좋은 게 떨어지고, 맞아도 검사 밖은 못 봅니다.
그리고 하나 더 — **재되, 요구하지 않기.** 루브릭이 "고객 영향을 말하라"고 요구했더니 모델이 점수를 채우려고 알 수 없는 것까지 단언했고, 가점은 요구를 빼고 재는 쪽으로 그 함정을 피했습니다.
