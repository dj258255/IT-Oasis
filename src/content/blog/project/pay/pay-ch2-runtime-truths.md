---
title: '실제로 띄워보기: 테스트는 다 초록불인데 틀렸다'
description: '테스트가 다 통과하는데 실제로 띄우니 결제가 DB에 없었다. 이벤트는 두 번 인코딩되고, 배치는 아무도 부르지 않고, 금고는 비어 있었고, 만들어둔 이스케이프 함수는 안 통하는 자리에 쓰여 있었다.'
...
단위 테스트는 내가 상상한 경계만 검사한다. 상상하지 못한 경계는 실제로 띄워봐야 나온다.

이 글은 테스트가 200개 넘게 전부 초록불인 상태에서 실기동과 전수 감사로 드러난 여섯 가지를 다룬다. 승인 응답은 나갔는데 DB에는 없었고, 이벤트는 두 번 인코딩되고 있었고, 배치는 로직만 있고 부르는 사람이 없었다.
...
**(3) 실패도 분리.** 파싱 안 되는 메시지(포이즌)는 warn 찍고 건너뛰게 했다.

**이건 나중에 뒤집었다.** 파티션이 멈추는 걸 막으려던 건 맞는데, **선택지를 둘로만 봤다** — 던지거나(멈춤) 버리거나(유실). 그 사이에 **DLT**가 있다. 건너뛰면 오프셋까지 커밋되니 그 이벤트는 되돌릴 방법 없이 사라진다.

더 이상한 건, **본체는 이미 알림 실패를 `DeadLetter` 테이블로 격리하고 어드민에서 재처리**하고 있었다는 점이다. 같은 프로젝트 안에서 한쪽은 격리하고 한쪽은 버리고 있었다. 지금은 예외를 던지고 `DefaultErrorHandler`가 2회 재시도 후 `payment.confirmed.DLT`로 보낸다. 원 토픽·파티션·오프셋·예외가 헤더로 같이 실린다. outbox 재발행은 at-least-once라 중복 수신이 가능하다는 것, 그래서 실소비자는 orderNo/paymentId 기반 멱등 처리가 필수라는 것도 코드에 명시했다.
...
`env:`로 시작하지 않으면 마이그레이션 전 평문으로 보고 그대로 돌려준다. 새로 저장되는 값은 항상 암호화되고, 옛 평문은 읽기만 된다. 한 번에 다 재암호화하지 않고 점진적으로 넘어가는 방식이다.

**그런데 이 분기에 끝이 없었다.** 남겨두면 "암호화를 우회하는 형식"이 영구히 허용되고, 손상되거나 바뀐 값도 조용히 평문으로 받아들인다. 몇 개 남았는지도 안 셌다.

**뒤에서 비밀번호 이관에는 "남은 레거시 해시 수" 게이지를 만들어놓고(6편), 여기엔 같은 걸 안 했다.** 지금은 평문으로 읽힌 횟수를 세고, 이관 완료를 선언하면(`fail-on-legacy-plaintext`) 평문을 통과시키지 않고 실패시킨다. **끝낼 수 없는 이관이라면 최소한 끝을 판정할 수 있어야 한다.**
...
전수 감사의 보안 관련 항목 셋을 이번에 마감했다. 성격은 다른데 관통하는 주제가 하나 있다. **보안 상태가 얼마나 오래, 어디까지 사는가.**

### 1. 조용한 기본값: 웹훅 시크릿

[웹훅 서명 검증](/blog/project/pay/pay-ch1-what-to-trust)은 HMAC 시크릿으로 "이 웹훅이 진짜 PG가 보낸 것인지"를 확인한다.

*짚어둘 게 있다. **실제 토스페이먼츠 일반 결제 웹훅에는 서명 헤더가 없다.** 공식 안내는 `paymentKey`로 조회 API를 다시 호출해 검증하는 방식이다. 이 프로젝트의 HMAC은 **자체 모델링한 PG 계약**이고, 실제 토스 연동이라면 조회 재검증이 맞다.* 그런데 그 시크릿이 이렇게 주입되고 있었다.
...
외부 리뷰가 짚었다. **서버가 여러 대면 여전히 안 막힌다.**
...
기동 시 재적재는 **재시작**을 견디게 할 뿐이고, **런타임에 추가한 차단**은 그 인스턴스에만 남는다.
바로 아래에서 velocity 카운터가 같은 이유로 무력해진다고 적어놓고, 차단 목록에는 같은 질문을 안 했다.

Redis 공유 집합으로 옮겼다. 진실 원천은 여전히 DB의 심사 이력이고, Redis는 **인스턴스들이 같은 답을
보게 하는 자리**다. 로컬 `Set`은 Redis가 죽었을 때만 쓰는 마지막 사본으로 내렸다.

**여기서는 fail-open 하지 않는다.** velocity는 "이상 급증 신호"라 못 세면 통과시키는 게 맞지만,
차단은 **이미 부정으로 확정된 카드**다. 통과시키는 쪽이 더 비싸다.
...
여기에도 빈틈이 하나 있었다. `INCR`과 `EXPIRE`를 **나눠 불렀다.** 그 사이에 프로세스가 죽으면
**TTL 없는 키가 영구히 남는다.** 카드마다 분마다 키가 생기므로 그런 키가 쌓이면 Redis가 계속 커진다.
...
한 스크립트로 합쳤다. **"보통은 둘 다 성공한다"에 기대는 코드는 언젠가 그 틈으로 샌다.**
...
대사 근거를 모아 보여주는 어드민 화면을 만들다 보안 리뷰에 두 번 걸렸다(9편). 둘 다 유효했다.
...
여섯 건의 공통점이 있다. **전부 테스트는 통과하고 있었다.**
