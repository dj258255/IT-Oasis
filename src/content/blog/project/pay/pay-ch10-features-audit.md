...
원인은 단순해요 — `CancelService`가 환불 재원을 **포인트와 카드만** 조회했어요. 월렛은 의존성에도 없었죠. 월렛을 결제수단으로 배선할 때 사가(승인 실패 보상)는 챙겼는데, **완료된 결제의 취소 경로**는 빠뜨린 거예요. 전액 월렛 결제 주문은 아예 취소가 불가능했고요.
...
- **amount 무검증**: 웹훅 `amount` 누락/문자열이면 Jackson `asLong()`이 조용히 0을 줘요. amount=0 분쟁이 생기고, 패소 확정 시 원장 분개가 "금액은 양수" 검증에 걸려 **아웃박스 이벤트가 영구 실패**(포이즌). 개시 시점에 양수 가드를 넣었어요 — 라이브로 amount=0 웹훅이 분쟁 0건인 것 확인.
...
- **위험한 기본값**: resolve가 `"WON".equalsIgnoreCase(outcome)` — 오타든 null이든 **"WON"이 아니면 전부 LOST(비가역 역분개)**였어요. WON/LOST 외에는 400으로 거부. 라이브로 `"WIN"` 오타가 400, 상태 OPEN 유지 확인.
...
감사에서 나온 건 자금 손실 2(치명)·원장 오염 3·인증 갭 3, 거기에 제 방어 코드의 스푸핑 구멍까지. 전부 제가 "완성했다"고 생각한 신규 코드에서 나왔어요. 그리고 자금 손실 두 개는 **같은 실수** — 새 결제수단(월렛)을 기존의 검증된 계약(포인트의 예약·보상·취소·멱등)에 대칭시키지 않은 것 — 에서 왔고요.
