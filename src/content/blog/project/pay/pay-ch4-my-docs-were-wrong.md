---
title: '자기 검증: 내가 고치라고 해놓고 두 번 철회했다'
description: '같은 코드베이스의 반례를 못 봤고, 다른 PG의 기준을 이 도메인에 잘못 들이댔고, 공개 장애 사례와 대조하기 전까지 외부 호출의 위험을 작게 봤다. 내가 내린 하드닝 판단을 코드 안팎의 기준으로 다시 감사한 기록이다.'
...
코드를 오래 들여다본다고 내 판단이 맞아지지는 않았다. **같은 코드베이스 안의 반례**를 못 봤고,
**다른 PG의 기준**을 이 도메인에 잘못 들이댔고, **공개된 장애 사례**와 대조하기 전까지
외부 호출의 위험을 작게 보고 있었다.

이 글은 내가 내린 하드닝 판단을 코드 안의 반례와 코드 밖의 공식 문서·장애 사례로 다시 검증한 기록이다.
그리고 그 재검증도 한 번 더 틀려서, 마지막에 또 고쳤다.
