...
이번에도 같은 방법으로 풀었어요. Hibernate한테 엔티티에서 DDL을 파일로 뽑게 하고, 새 테이블만 골라 `V3__extensions.sql`로 만들었죠.
...
27개 테이블, 13개 모듈이 전부 올라와서 실제로 떴어요. 인증(ROLE_USER)도 붙은 상태로 스모크를 확인했고요. 무인증 주문은 401, 인증 주문은 201.
...
가장 큰 원인은 **인증**이었어요. [보안 수정으로](/blog/project/pay/pay-ch2-payment-methods) 결제 엔드포인트에 HTTP Basic 인증을 걸었거든요.
...
그런데 실서비스 관점에서 보면, 취소가 없는 결제는 반쪽이에요. 사람들은 잘못 눌러서, 마음이 바뀌어서, 배송이 안 돼서 주문을 **되돌립니다.** 되돌리는 순간, 결제할 때는 없던 질문들이 한꺼번에 튀어나와요. 이번 편은 그 주석을 지우면서 마주친 판단들의 기록이에요.
...
이제 결제 시스템이 "받는 것"만이 아니라 "되돌리는 것"까지 온전히 다뤄요. 되돌리는 쪽이 실은 더 조심스럽다는 걸, 이중환불 하나가 그대로 손실이 되니까, 만들면서 다시 배웠어요.
...
이번 편은 그 미뤄둔 숙제예요. 왜 미뤄뒀냐면, 이게 결제 시스템에서 **가장 위험한 순간** 중 하나이기 때문이에요.
...
> `@Transactional`이 롤백하는 건 **DB뿐**이에요. 하지만 2번의 카드 승인은 **PG(외부 시스템)에서 이미 일어났어요.** DB를 롤백해도 PG의 승인은 되돌아오지 않아요. 결과는 **고객 카드에서는 돈이 빠져나갔는데, 우리 DB엔 주문이 없는** 상태예요. 고객 입장에선 "결제했는데 아무것도 못 받은" 최악의 경험이죠.
...
**"잡았다"고 없던 일이 되는 게 아니에요.** 참여 트랜잭션 안에서 던져진 예외는, 잡아도 전체를 오염시켜요.
...
그래서 지난 측정과 **똑같은 조건**으로 맞췄어요. 같은 노트북, 비슷한 머신 부하(load avg ~6), 같은 50 VU, 같은 주문→승인 시나리오로요. 딱 하나, **인증 방식만** HTTP Basic→JWT로 바꿨어요. k6 스크립트도 `setup()`에서 한 번 로그인해 토큰을 받고, 이후엔 그 Bearer 토큰을 재사용하게 고쳤죠.
