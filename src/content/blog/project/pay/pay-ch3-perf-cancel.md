...
확장 모듈까지 얹은 시스템을 처음으로 통째로 띄우고 k6 부하를 걸었다. 오류율 0%에 p95 3초. 범인은 앱 로직이 아니라 요청마다 도는 BCrypt였다.
...
측정에서도 이게 딱 보였다. **최소 응답시간이 110ms.** 무인증 때 min이 3.5ms였으니, 이 110ms 바닥은 순수한 BCrypt 비용이다. 앱 로직이 아니라 **인증 방식의 값**인 셈이다.
...
> "동시에 많이 결제하면?"에 대한 답이 "우리 앱은 빠릅니다"가 아니라 **"현재 인증이 무상태 BCrypt라 요청당 ~110ms가 들고, 실서비스는 토큰으로 이걸 없앤다"**가 되는 것. 부하테스트를 실제로 돌려봐야만 나오는 답이다.
...
미뤄둔 숙제 중 취소부터. 붙여 보니 취소는 결제의 역연산이 아니라, 환불 순서·재고 복원·재취소 정산이라는 세 개의 독립적인 판단이 필요한 별개 문제였다.
...
> 실무에서 "수량 단위 부분취소"(3권 중 1권만 반품)를 하려면 취소 API 자체를 금액이 아니라 **주문 라인(item) 단위**로 다시 설계해야 한다. 그건 별개의 기능이라, 지금은 "금액 부분취소 = 재고 불변"으로 경계를 그었다. 할 수 없는 걸 억지로 흉내 내는 것보다 못 하는 이유를 코드가 설명하게 두는 편이 낫다.
...
취소를 "결제의 역연산"으로 보고 시작했지만, 실제로는 무엇부터 되돌릴지(포인트 우선), 되돌릴 수 없는 게 뭔지(부분취소의 재고), 이미 되돌린 걸 어떻게 뺄지(재취소 정산)라는 세 갈래 판단이었다. 결제가 한 방향이라면 취소는 여러 방향으로 갈라진다. 그래서 별도 오케스트레이션이 필요했다. 되돌리는 쪽이 실은 더 조심스럽다. 이중환불 하나가 그대로 손실이니까.
...
이게 분산 트랜잭션의 본질적 문제다. 우리 DB와 PG는 **한 트랜잭션으로 묶을 수 없다.** 그래서 "둘 중 하나가 이미 커밋됐는데 다른 하나가 실패"하는 순간을, 롤백이 아니라 **보상(compensation)**으로 풀어야 한다.

### 2. 해법: 롤백 대신 자동 망취소

방향을 바꿨다. 재고 차감이 실패하면 롤백하지 않는다. 대신 이미 승인된 카드를 **취소(망취소)**하는 걸로 되돌린다.

그런데 망취소도 PG 호출이라 **또 실패할 수 있다.** 네트워크가 끊길 수도, PG가 잠깐 죽었을 수도 있다. 그래서 망취소를 "지금 한 번 시도하고 끝"이 아니라 **durable하게 적재해두고 성공할 때까지 재시도**하는 구조로 만들었다. 이게 `compensation_tasks` 테이블이다. outbox 패턴의 사촌이다.
...
"취소할 결제가 없다"는 건 실패가 아니라 **이미 목적이 달성된 상태**다. 보상 작업에서 멱등성은 이렇게 "재시도해도 같은 결과"를 보장하는 안전장치다.
...
[부하테스트 편](/blog/project/pay/pay-ch3-perf-cancel)에서 p95가 567ms까지 오른 걸 파고들었더니, 범인은 앱 로직이 아니라 **요청마다 도는 BCrypt**였다.
...
BCrypt가 느린 건 버그가 아니라 기능이다. 느려야 무차별 대입 공격이 어렵다. 그러니 BCrypt 자체를 빠르게 만들 게 아니라 **돌리는 횟수를 줄여야** 한다. 로그인 1회로 몰아넣는 게 정답이다.
