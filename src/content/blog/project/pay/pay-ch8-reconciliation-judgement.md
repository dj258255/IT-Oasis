---
title: '대사 확정 화면: 사람이 고르게 하려면 근거와 부분 실패를 먼저 정해야 한다'
...
내 결제 기록과 PG 정산 파일이 안 맞는 건을 규칙으로 종결하면 **틀린 판단이 장부에 남습니다.** 그래서 사람이 근거를 보고 원인을 골라 확정하게 만들었습니다. 그런데 사람이 고르게 하는 순간 **셋을 정해야 했습니다** — 근거를 어디까지 모아 보여줄 것인가, 그중 하나를 못 읽었을 때 어떻게 표시할 것인가, 확정한 뒤에 근거가 바뀌면 누가 책임질 것인가.

정답이 있는 자리였으면 "근거를 다 모아 보여준다"로 끝났을 것입니다. 정답이 없어 갈린 자리는 **부분 실패**였습니다. 조회 하나가 실패했을 때 조용히 빼면 화면에는 "기록이 없다"로 읽히고, 그 상태로 확정하면 틀린 판단이 장부에 남습니다.

그 결정을 지키는지 확인하는 과정에서 둘이 더 나왔습니다. 확정 뒤 취소가 오면 승인 행을 덮어쓰던 것, 그리고 전제를 바꿔놓고 반대편 파서를 안 고친 것입니다. **둘 다 판단이 아니라 버그지만, 판단의 근거를 어디까지 믿을 수 있는지를 보여줍니다.**
...
| **상황 2** | **근거**: 확정한 뒤 근거가 바뀌면 — 승인 행을 덮어쓰고 있었다 | 승인과 취소를 각각의 거래일에 별도 행으로 보존 |
| **상황 3** | **근거**: 전제를 바꾸면 반대편도 바뀌어야 한다 — 파서가 정상 환불을 버렸다 | 음수 환불 행을 읽게 하고 건너뛴 행을 드러냄 |
...
**① 대사 모듈의 허용 의존을 늘린다 → 기각.** `allowedDependencies`에 7개를 더 적으면 검증은 통과하지만 의미가 사라집니다. **"목록에 적으면 뚫린다"가 되면 그 목록은 문서일 뿐입니다.**
...
**이 모양에는 이름이 있습니다. [API Composition(Aggregator)](https://microservices.io/patterns/data/api-composition.html)입니다.** 전담 조립자가 하위 여러 곳을 질의해 하나의 응답으로 합치는 형태입니다. 마이크로서비스에서는 서비스마다 따로 나가던 **네트워크 왕복이 한 번으로 줄어드는 것**이 이 패턴의 큰 이점입니다. 이 프로젝트는 모듈러 모놀리스라 같은 프로세스 안의 호출이어서 그 이점은 크지 않습니다. 여기서 이 패턴이 값을 한 자리는 **모듈 경계를 뚫지 않고 여러 도메인을 한 화면에 모았다는 것**입니다.

패턴 설명에는 **일부가 실패했을 때 부분 결과를 돌려주는 전략**도 같이 나옵니다. 그걸 어떻게 할지가 이 절에서 제일 중요한 결정이 됐습니다.
