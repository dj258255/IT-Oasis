...
미확정 복구·보상·정산처럼 규칙으로 답이 나오는 일은 [앞 편에서 배치로 옮겼습니다](/blog/project/pay/pay-ch9-batch-ownership). 그래도 남는 게 있습니다. 금액이 1원 안 맞는 대사, 카드사가 끝내 답을 안 준 결제, 고객이 건 이의제기. 규칙으로 종결하면 틀린 판단이 장부에 남는 일들입니다.

**대사(reconciliation)** 는 내 결제 기록과 PG가 보내주는 정산 파일을 하루치씩 대조하는 작업입니다. 이 불일치를 **사람이 근거를 보고 원인을 골라 확정**하게 설계했습니다.

백오피스에 확정 버튼을 만들면 끝인 줄 알았습니다. 직접 써 보니 아니었습니다. 판단할 근거가 흩어져 있었고, 확정한 뒤에 그 근거가 바뀌었고, 한쪽 모델을 고쳐놓고 반대쪽 파서는 그대로였습니다. 이 글은 그 세 가지를 다룹니다.
