...
**대사(reconciliation)** 는 내 결제 기록과 PG가 보내주는 정산 파일을 하루치씩 대조하는 작업입니다. 금액이 다르거나 한쪽에만 있으면 불일치로 잡는데, 이걸 자동으로 종결하면 틀린 판단이 장부에 남습니다. 그래서 **사람이 근거를 보고 원인을 골라 확정**하게 설계했습니다.

이 글은 그 수기 판정 경로를 직접 써 보고 검증하며 세 번 놓친 것을 다룹니다. 근거를 안 줬고, 내려둔 판정이 뒤집힌 걸 몰랐고, 고친 전제를 반대편에 알리지 않았습니다.
...
eBay가 공개한 Explainers와 같은 형태입니다. 각 도메인이 자기 조각을 내주고 위에서 조립합니다.
...
> 이 분류기가 **AI가 넘어야 할 기준선**입니다.

그런데 **기준선의 값을 재는 수단이 없었습니다.** 넘었는지 알 수가 없습니다.

정답은 어디서 나오나 생각해보니, **사람이 원인을 고르는 순간이 곧 라벨**이었습니다. 그래서 확정 직전에 분류기를 한 번 더 돌려 대조합니다. 둘을 함께 아는 유일한 순간입니다.

집계는 넷으로 나눴습니다. 1순위 적중 / 후보엔 있었음 / 빗나감 / **제안 못 냄**. 합치면 두 가지가 안 보입니다. 제안을 못 낸 게 많으면 정확도가 높아도 쓸모없고(커버리지), 후보에만 있는 게 많으면 "골라주긴 하는데 순서가 틀린" 것입니다.

**다만 이걸 "정확도"라고 부르지 않기로 했습니다.** 사람이 제안을 **보고** 확정하니 앵커링 편향이 있습니다. 제안에 끌려간 확정이 적중으로 집계됩니다. 실제 정확도의 **상한**에 가깝습니다.

편향을 걷으려면 제안을 감춘 상태의 표본이 필요한데 아직 없습니다. 그래서 **"일치율"**이라고 부릅니다.

### AI는 아직 안 붙였다

붙일 조건이 이제 명확합니다.

- **일치율이 쌓여야 합니다.** 비교 대상이 없으면 "나아졌다"고 말할 수 없습니다
- **blind 표본을 설계해야 합니다.** 앵커링 편향을 걷어내려면
- 그리고 LLM 호출은 **비용과 외부 전송**이 걸린 결정입니다

지금 붙이면 *"AI 붙였습니다"*는 되지만 *"이만큼 나아졌습니다"*는 못 됩니다.

### 얼마나 걸리나

로컬 도커 데몬이 계속 죽어 실측이 밀렸습니다. 업데이트를 발견하면 스스로 종료하는 패턴이라, Homebrew에 있던 MySQL·Redis로 우회해서 쟀습니다.
...
**쿼리 25회는 예상(10~14)보다 많습니다.** 예상이 틀렸습니다. 기여자마다 읽기 전용 트랜잭션을 따로 열고, 감사 기여자가 대상 3종을 각각 조회하기 때문으로 보입니다.
...
이번 건은 **터진 적이 없습니다.** 로컬에서 일부러 만들어야 보였습니다. 취소가 대사 확정 뒤에 오고, 그 날짜를 다시 볼 일이 없어야 드러나는 조건이라서 그렇습니다.

그래서 배운 게 하나 있습니다. **"장애가 안 났으니 괜찮다"는 근거가 못 됩니다.** 안 난 게 아니라 안 보인 거였습니다.

순서 역전은 외부 리뷰가 짚어줘서 고쳤고, 경고를 무시한 확정은 경계에 막혀 아직 못 막았습니다. 둘 다 **한 번도 일어난 적이 없습니다.** 그게 없다는 뜻은 아닙니다.

그리고 남의 사례를 읽는 방식도 조금 달라졌습니다. 처음엔 "우리도 그 문제가 있나" 하고 찾았는데, 그 문제는 없었습니다. 대신 **그걸 확인하려고 코드를 읽다가** 다른 게 나왔습니다. 사례는 답이 아니라 **어디를 들여다볼지 알려주는 것**에 가까웠습니다.
...
쉼표를 지우는 코드가 있습니다. **그런데 그 앞에서 줄을 쉼표로 자릅니다.**

```java
String[] cols = line.split(",", -1);
```

그래서 `"1,234,500"`은 파싱 단계에 도달하기도 전에 세 칸으로 쪼개집니다. **콤마 제거 코드는 이 경로에서 죽은 코드입니다.**

**고치지 않고 현재 동작을 테스트로 고정했습니다.** 실제 PG 파일이 그 형식으로 오는지 확인되기 전에 CSV 파싱을 흔들 이유가 없습니다. 확인되지도 않은 문제를 위해 잘 도는 코드를 건드리는 게 더 위험합니다.

대신 **알고는 있게** 테스트 이름에 박아뒀습니다.
...
원인 분류표가 **빠진 게 없는지**는 내 목록만 봐서는 알 수 없습니다. 밖에서 잣대를 가져와야 합니다.

처음엔 **데이터셋**을 찾았습니다. 국내·해외 다 봤는데 **없습니다.** 라벨된 대사 예외는 결제사 운영 데이터라 공개될 리가 없습니다.

대신 **분류표**가 나왔습니다.

> 대사 예외는 여섯 가지 패턴으로 반복됩니다:
> **타이밍 격차, 금액 불일치, FX 변동, 은행 입금 누락, PSP 기록 누락, 중복**

*표준 규격이 아니라, 여러 PSP·핀테크 운영 자료가 공통으로 드는 분류입니다. 한 곳을 정본으로 특정하지는 못했고, 그래서 "빠진 칸이 있는지" 확인하는 용도로만 썼습니다.*

이게 데이터셋보다 나았습니다. **표본이 없어도 목록에 구멍이 있는지는 확인할 수 있습니다.**
...
엔진은 **이미** 거래 식별자로 중복을 걸러냅니다. 코드 주석에 이렇게 적혀 있습니다.

> 오류: 같은 거래가 중복 기록됨. 합산하면 금액이 부풀어 **불일치를 오히려 감춘다**

경고 로그도 남깁니다. **그런데 예외 큐를 확정하는 사람에게는 고를 어휘가 없었습니다.** `OTHER`로 갈 수밖에 없고, 그러면 **반복돼도 집계에 안 잡힙니다.** 원인을 코드로 받는 이유가 통째로 무너집니다.

두 가지를 채웠습니다.

- `DUPLICATE_RECORD`를 원인에 추가
- **실행 요약이 중복 행 수를 싣습니다.** 로그는 확정하는 사람이 보는 곳이 아닙니다

규칙 제안은 안 붙였습니다. **중복은 매칭 전에 걸러지므로** 남은 불일치만 보고는 판단할 수 없습니다. 사람이 실행 요약을 근거로 고릅니다.

#### 대조 자체를 테스트로 남겼다

매핑표를 테스트가 들고 있고, **비는 칸에는 왜 비었는지를 쓰게 강제합니다.**
...
통화가 늘거나 새 원인이 생기면 여기가 먼저 걸립니다. **"우리는 원화만 하니까"를 코드에 박아두는 것**과 같습니다.
