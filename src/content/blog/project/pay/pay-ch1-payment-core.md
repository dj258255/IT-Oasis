...
## 흔한 PG 연동을 넘어 "결제가 실패했을 때"를 만든다

*원 연재 0편.*

### 0. 들어가며

개인 프로젝트 소재로 결제를 골랐다. 실패가 비싼 도메인이라서다. 타임아웃·중복·정합성·분산 트랜잭션이 실제로 아프게 드러나니, "실패 설계"를 배우기에 이만한 소재가 없다. 시작 전에 조사부터 했다. 실제 결제 시스템이 무엇을 다루는지, 그리고 흔한 결제 예제는 어디까지 가는지.
...
토스페이먼츠는 승인 API만 준다. 타임아웃 나면 어떻게 할지, 중복결제를 어떻게 막을지, 서버가 죽었다 살아나면 미확정 결제를 어떻게 정리할지, PG 정산 파일이 내 기록과 안 맞으면 어떻게 할지는 전부 만드는 사람 몫이다. 흔한 예제는 PG가 해주는 부분(연동)만 하고 끝나고, 진짜 도메인은 이 빈 공간에 있다.
...
- **물리적 MSA**: 유행이라 끌리지만, 1인 개인 프로젝트에서 서비스별 배포·DB·네트워크를 처음부터 깔면 분산 트랜잭션·네트워크 장애 같은 **본질에서 벗어난 인프라 복잡도**에 시간을 다 쓴다. 정작 배우고 싶은 도메인 설계가 묻힌다.
...
핵심은 **모듈끼리 직접 호출하지 않고 도메인 이벤트로만 소통한다**는 규칙이다. 이걸 말로만 두면 흐지부지되니, `package-info.java`에 허용 의존을 선언하고 테스트가 위반을 잡으면 **빌드를 깨뜨린다.**
...
이제 누가 실수로 `payment`에서 `settlement`의 내부 클래스를 가져다 쓰면 **테스트가 빨개진다.** 아키텍처 규칙이 문서에 머물지 않고 코드가 된다.
...
- `open-in-view: false`로 OSIV를 꺼서 긴 트랜잭션/커넥션 점유를 처음부터 차단 (카카오페이 성능 개선기의 교훈을 기본값으로)
...
- **들어오는 웹훅**: PG가 "이 결제 상태가 바뀌었다"고 우리한테 알려주는 것
- **나가는 이벤트**: 결제가 완료됐을 때 원장·정산·알림 같은 다른 모듈에 전파하는 것
...
- 웹훅은 **중복**으로 온다 (PG가 재전송하는 정상 동작이다)
...
> 여기서 얻는 게 있다. 정합성을 눈으로 훑는 대신 수식으로 증명할 수 있다. 어딘가 돈이 새면 차변·대변 합계가 안 맞는다. `imbalance() == 0`이라는 수학적 사실로 검증되니, "느낌상 맞는 것 같다"에 기댈 필요가 없다.

원칙 몇 개를 더 박았다.

- **금액은 항상 양수.** 부호는 방향(DEBIT/CREDIT)으로 표현한다. 음수를 허용하면 방향과 이중 표현이 돼서 버그가 숨는다.
- **append-only.** 취소할 때도 원거래를 지우지 않고 **반대 분개를 추가**한다 (매출 차변 / PG미수금 대변). 이력이 통째로 남는다.
...
핵심은 매칭 엔진이 결정적(deterministic)이라는 점이다. 같은 입력이면 항상 같은 결과. 그래서 MATCHED(대부분)는 자동으로 확정(AUTO_RESOLVED)하고, 어긋난 것만 사람이 보는 큐(PENDING)로 보낸다. 정석은 "결정적으로 맞추고, 못 맞춘 것만 사람"이다 (Modern Treasury의 대사 철학). "AI가 알아서 맞춰준다"는 여기서 통하지 않는다.
...
## 결제는 만들고 나서가 진짜다: 관측성과 백오피스로 마무리
...
## 이제 진짜로 돌린다: Flyway로 실기동하고 라이브 검증
...
이번 건 "검증된 코드"와 "실제 기동" 사이의 구멍을 메운 작업이다. 새로 더한 기능은 없다.
