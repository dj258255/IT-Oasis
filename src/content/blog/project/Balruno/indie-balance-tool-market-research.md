...
인디게임 개발 커뮤니티를 관찰하면서, 그리고 직접 인디게임을 개발해본 경험에서 반복되는 패턴을 발견했습니다.

**패턴**:
1. 개발자가 캐릭터 HP를 1500으로 설정함
2. "이게 적정한 수치인가?" 확신이 없습니다
3. 직접 플레이하며 "감"으로 조정
4. 출시 후 유저 피드백: "이 캐릭터 너무 OP임"
5. 급하게 핫픽스

**왜 이런 일이 발생하는가**:
- 대형 스튜디오에는 전담 밸런스 디자이너와 내부 툴이 있습니다
- 1~2인 인디 개발자에게는 그런 리소스가 없습니다
...
**분석**: 즉각적인 관심이 있다는 것은 "문제를 겪고 있는 사람들이 존재한다"는 증거입니다. 다만 "관심"과 "실제 사용"은 다르므로 설문조사로 더 깊이 파악했습니다.

---

## 3. 설문조사 결과 분석

2,200명 규모 방에 올렸을 때 12명이 응답했습니다. 보상(기프티콘 등) 없이 진행했음에도 12명이 응답한 것은 관심이 있다는 신호로 해석했습니다.
...
**분석**: 응답자의 절반이 현직자입니다. 실제 업무에서 밸런싱을 하는 사람들이 관심을 가졌다는 뜻입니다.

### 3.2 개발 중인 장르

![](/uploads/project/Balruno/indie-balance-tool-market-research/game.png)

**분석**: 로그라이크, RPG, 시뮬레이션이 상위권입니다. 이 장르들의 공통점은 수치 밸런싱이 중요하다는 것입니다. 장르 특성상 타겟 유저가 맞다는 신호입니다.

### 3.3 개발 경험

![](/uploads/project/Balruno/indie-balance-tool-market-research/dev.png)

**분석**: 12명 중 10명이 프로젝트 완성/출시 경험이 있습니다. 실제로 밸런싱 작업을 해본 사람들이 응답한 것입니다. 경험 없는 사람의 의견보다 신뢰도가 높습니다.

### 3.4 현재 사용 도구

![](/uploads/project/Balruno/indie-balance-tool-market-research/tool.png)

**분석**: 대다수가 엑셀/구글 시트를 사용합니다. 이 도구의 한계를 해결하면 전환할 가능성이 있습니다. Unity Inspector를 쓰는 사람도 있는데, 이는 "엔진 내에서 해결하고 싶다"는 니즈를 보여줍니다.

별도 의견: "언리얼 테이블은 기획자가 대규모 작업할 때 짜증난다"
→ 기존 도구에 불만이 있다는 증거입니다

### 3.5 현재 방식의 불편한 점

![](/uploads/project/Balruno/indie-balance-tool-market-research/image-2.png)

**분석**: 이 질문의 응답이 핵심입니다. "어떤 불편함을 해결해야 하는가"를 직접적으로 보여줍니다.

### 3.6 밸런싱 작업에서 어려운 점

![](/uploads/project/Balruno/indie-balance-tool-market-research/image-3.png)

**분석**: "밸런스가 맞는지 확인하기 어렵다"가 핵심 페인 포인트입니다. 이것이 몬테카를로 시뮬레이션 기능의 근거가 됩니다.

### 3.7 정보 습득 경로

![](/uploads/project/Balruno/indie-balance-tool-market-research/image-4.png)

**분석**: 레퍼런스 게임 분석, 커뮤니티, 유튜브가 상위권입니다. 체계적인 학습 경로가 없다는 뜻입니다. 이론적 배경을 제공하면 차별화 포인트가 될 수 있습니다.
...
**분석**: "자동 DPS 계산", "난이도 곡선 시각화"는 이미 구현했습니다. "다른 게임 레퍼런스"는 참고자료 섹션으로 제공할 수 있습니다. 커뮤니티 기능을 추가하면 사용자 간 레퍼런스 공유로 이 니즈를 해소할 수 있습니다. 전반적으로 시각화에 대한 수요가 높았습니다.

### 3.9 사용 의향

![](/uploads/project/Balruno/indie-balance-tool-market-research/image-5.png)

**분석**: 대다수가 "사용할 의향이 있다"고 응답했습니다. 가설 "도구가 있으면 사용할 것이다"가 어느 정도 검증된 셈입니다.

### 3.10 중요하게 생각하는 것

![](/uploads/project/Balruno/indie-balance-tool-market-research/image-6.png)

**분석**: 사용 편의성, 게임 엔진 연동, 시각화가 상위권입니다. 이 세 가지가 핵심 기능이 되어야 합니다.

### 3.11 가격 민감도

![](/uploads/project/Balruno/indie-balance-tool-market-research/311.png)

**분석**: 무료~월 5천원 대가 대다수입니다. 고가 유료화는 어렵고, 프리미엄 모델(기본 무료 + 고급 기능 유료)이 적합하다는 증거입니다.
...
**분석**: 2명이 연락처를 남겼습니다. 초기 사용자 확보 가능성이 있습니다.

---

## 4. 상세 피드백 분석

설문 외에 개인 메시지로 받은 상세 피드백을 분석했습니다.
...
**분석**:
- 전투 종료 시간, 종료 시 HP 추세는 "게임 페이스"를 분석하는 지표입니다
- 치명타 역전 분석은 "운 요소"의 영향을 측정합니다
- 파티플레이는 현재 N:N 팀 전투로 구현돼 있습니다
- 스킬 시스템은 부분 구현(쿨타임, HP 트리거)했고, 무적/부활은 미구현입니다
- 결과 export는 구현 예정입니다
...
**분석**:
- "실제 전투 감각 검증은 어렵다"는 것은 맞는 지적입니다. 이 도구가 "모든 것을 해결해준다"고 주장하면 안 됩니다.
- 핵심 가치는 "이상치 탐지"입니다. Z-score 기반 이상치 탐지 기능이 이미 있지만, 프레임별 스탯 변화 그래프는 추가 검토가 필요합니다.

> Q: 리얼 밸런스를 저 툴로 다 맞춘다하면 그야말로 지옥이라서요. 그건 내려놓고 할 수 있는 부분을 최대한 커버하는 쪽으로 가시면 효율이 좋아질듯. 그것만 해도 엄청난 툴이 됩니다.

**분석**:
- 범위 설정이 중요합니다. "만능 툴"이 아니라 "특정 문제를 확실히 해결하는 툴"로 포지셔닝해야 합니다.
- 명시적으로 "하지 않는 것"을 문서화해야 합니다.
...
**분석**:
- A 케이스: "구간별 성장 곡선"이 필요합니다. 현재는 단일 곡선만 지원하고, 로드맵에 추가했습니다.
- B 케이스: 특수 효과(무적, 부활)는 현재 미지원입니다. 커스텀 스크립트 기능으로 해결할 수 있을 것 같습니다.
...
**분석**:
- "역방향 계산" (Goal Solver) 기능입니다. "원하는 결과를 입력하면 필요한 수치를 역산"하는 기능입니다.
- 이미 Goal Solver 기능이 있지만, "드로잉 → 공식 도출"은 더 직관적인 UX입니다. 장기 로드맵에 추가했습니다.
...
**분석**:
- "반복 수요"가 핵심입니다. 한 번 쓰고 끝나는 기능보다 "매번 업데이트마다 쓰는 기능"이 유료화에 적합합니다.
- 밸런스 검증(패치 전 시뮬레이션), 버전 비교(이전 버전 vs 새 버전) 기능이 반복 수요를 만들 수 있습니다.
