...
처음 이름은 PowerBalance였습니다. 한 번 내놨다가 받은 반응이 지금 구조를 상당 부분 결정했고, 그 이야기는 6장에 있습니다.
...
## 6. 이름이 PowerBalance이던 시절에 배운 것

*원문 발행일 2026-04-02*

지금은 Balruno지만 처음 이름은 PowerBalance였습니다. 그때 한 번 세상에 내놨다가 배운 게 지금 구조에 그대로 남아 있어서, 순서상 여기에 적어둡니다.

처음 만들 때는 꽤 획기적이라고 생각했습니다. 게임 밸런싱은 반복 계산이 많고 엑셀이나 구글 시트만으로는 불편한 지점이 분명했으니, 기획자에게 필요한 수식과 시뮬레이션, 검증을 한곳에 모으면 도움이 될 거라고 믿었습니다. 실제로 주변에서 인디게임 밸런싱을 하던 친구는 만족해했습니다. 문제의식 자체가 틀린 건 아니었고, 적어도 누군가에게는 필요한 도구였습니다.

그런데 홍보를 해보니 반응이 생각보다 훨씬 냉정했습니다. 관심을 보이는 사람은 있었지만 실제로 들어와서 써보는 사람은 많지 않았습니다. 필요해 보인다와 실제로 쓰게 된다는 전혀 다른 문제였습니다.

![](/uploads/project/Balruno/powerbalance-feedback.png)

특히 기억에 남은 건 사용성 피드백이었습니다. 기능이나 방향은 흥미롭지만 처음 켰을 때 무엇부터 해야 할지 막막하고 진입 장벽이 높다는 반응이었습니다. 만드는 쪽에서는 기능을 많이 넣을수록 좋아진다고 생각하기 쉬운데, 쓰는 쪽에서는 처음 5분 안에 감이 오지 않으면 그대로 나가버립니다.

돌이켜보면 사람들에게는 이미 각자 익숙한 방식이 있었습니다. 누군가는 엑셀을, 누군가는 구글 시트를 썼고, 누군가는 머릿속과 메모로도 작업하고 있었습니다. 제 눈에 불편해 보여도 그 사람들에게는 이미 굴러가는 흐름이 있었던 겁니다. 그 흐름을 바꿀 만한 분명한 이유가 없으면 기능이 아무리 많아도 실제 사용으로 이어지지 않습니다.

이 경험은 오래 남았습니다. 나중에 CodingTestKit을 만들 때는 기능이 많은지보다 사람이 지금 당장 왜 이걸 써야 하는지를 훨씬 조심해서 봤습니다. 기존 작업 흐름을 크게 바꾸지 않으면서 바로 편해지는 지점이 있는지, 처음 설치한 사람이 설명 없이도 가치를 느낄 수 있는지를 더 오래 고민했습니다. 그렇게 나온 CodingTestKit은 다운로드 254를 기록했습니다.

그래서 지금 돌아보면 PowerBalance는 실패한 프로젝트라기보다 제품을 보는 눈을 바꾼 프로젝트에 가깝습니다. 좋은 기능을 만드는 일과 사람들이 실제로 쓰는 도구를 만드는 일은 다르다는 걸 이때 확실히 배웠습니다. 그때 이 경험이 없었다면 다음 프로젝트에서도 같은 실수를 반복했을 겁니다.

[1장](#1-시장조사로-방향을-잡다)에서 만들기 전에 설문부터 돌린 것도, [5장](#5-첫-사용자-피드백-24건)에서 첫 피드백 24건을 그대로 받아 고친 것도 여기서 나왔습니다.
