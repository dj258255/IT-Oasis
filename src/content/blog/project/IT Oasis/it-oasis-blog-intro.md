...
카테고리 구조를 제 입맛에 맞게 잡고 싶었고, 한영 전환도 넣고 싶었는데 기존 플랫폼에서는 그게 쉽지 않았습니다.
...
마크다운을 직접 수정해도 되긴 하지만, 비주얼 에디터가 있으면 글쓰기에 훨씬 집중할 수 있습니다.
TinaCMS는 Git 기반이라 별도 DB 없이도 동작하고, 로컬에서든 클라우드에서든 모두 쓸 수 있어서 선택했습니다.
무엇보다 마크다운 파일 자체를 수정하는 구조라 락인 걱정이 없다는 게 결정적이었습니다.
...
2단계 계층 구조를 지원합니다.
`프로젝트/발루노`처럼 부모/자식 형태로 분류할 수 있고, 사이드바에서 펼쳐서 볼 수 있습니다.
카테고리별 아이콘, 정렬 순서, 한/영 이름은 JSON으로 관리하고 있어서 TinaCMS에서도 편집이 가능합니다.

### 한/영 전환 (i18n)

글로벌 토글 하나로 전체 UI와 본문을 한 번에 전환할 수 있습니다.
내부적으로는 세 가지 패턴을 섞어 쓰고 있습니다.

1. `data-ko` / `data-en` 속성: 버튼이나 레이블 같은 짧은 텍스트에 사용합니다.
2. `.lang-ko` / `.lang-en` CSS 클래스: 블록 단위로 통째로 바꿀 때 씁니다.
3. `<!-- EN -->` 구분자: 본문 영역에서 한글과 영문 버전을 나눕니다.

처음엔 좀 복잡해 보였는데, 쓰다 보니 각각 역할이 달라서 나름 합리적인 구성이 됐습니다.

### 다크모드

시스템 설정을 기본으로 따르되, 수동 전환도 가능합니다.
`localStorage`에 사용자 선호값을 저장해두고, 페이지 로드 시 깜빡임(FOUC) 없이 바로 적용되게 처리했습니다.

### 글래스모피즘 UI

전체적으로 글래스모피즘 디자인을 적용해봤습니다.
`backdrop-filter: blur()`와 반투명 배경을 조합해서, 가벼우면서도 정돈된 느낌을 줄 수 있었습니다.
개인적으로 이 스타일이 블로그 분위기에 잘 어울린다고 생각합니다.

### 목차 (TOC)

데스크톱에서는 오른쪽에 플로팅 목차가 표시됩니다.
IntersectionObserver로 현재 읽고 있는 섹션을 하이라이트해주고, 프로그레스바로 전체 진행률도 보여줍니다.
목차가 길어지면 자체 스크롤이 작동하도록 만들었습니다.
