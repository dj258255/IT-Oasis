...
그래서 봇이 그 서비스들의 폴더에는 아예 접근하지 못하도록 격리했습니다. systemd에는 특정 경로를 그 서비스에게 존재하지 않는 것처럼 감추는 설정이 있습니다. 이걸 걸어두면 봇이 높은 권한으로 우회를 시도해도 그 경로만은 보이지 않습니다. 봇은 자기 작업 폴더 안에서만 움직이고 옆에서 돌던 서비스들은 그대로 두었습니다.
...
한계도 분명합니다. 봇이 답하려면 서버가 켜져 있어야 하고 구독 사용량은 노트북의 Claude Code와 한 통을 나눠 쓰기 때문에 한도를 넘기면 봇도 같이 멈춥니다. 무거운 모델로 매 메시지를 처리하면 사용량이 빨리 닳아서 평소에는 가벼운 설정으로 두고 어려운 작업에만 강도를 올리는 식으로 씁니다. 그래도 밖에서 떠오른 걸 폰으로 던져두고 이따 확인하는 원래 목적에는 잘 맞습니다.
