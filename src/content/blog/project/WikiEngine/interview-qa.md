...
description: WikiEngine 프로젝트 전체에서 나올 수 있는 면접 예상 질문과 답변을 정리한다.
...
## Redis 샤딩 + Consistent Hashing

**Q: "왜 Redis Cluster 대신 애플리케이션 레벨 샤딩을 선택했나요?"**

이 단계의 핵심 문제는 범용 샤딩 자체보다, 자동완성·일반 캐시·조회수·블랙리스트처럼 성격이 다른 데이터를 어떻게 분리하고, 노드 추가나 장애 시 키 이동을 얼마나 통제할 수 있느냐였습니다. Redis Cluster는 범용 샤딩에는 강하지만 현재 규모에서는 더 무겁고, 용도별 분리 전략을 세밀하게 표현하기엔 과했습니다. 그래서 이 프로젝트에서는 애플리케이션 레벨 라우팅으로 필요한 분리와 제어를 먼저 확보하는 편이 더 적합하다고 판단했습니다.

**Q: "가상 노드 수를 150으로 설정한 근거는?"**

실측 기준으로 50개일 때는 약 10% 편차, 150개일 때는 약 5% 편차, 500개일 때는 약 2% 편차가 나왔습니다. 3개 물리 노드 × 150 가상 노드 = 450개의 라우팅 엔트리면 메모리 오버헤드는 무시할 수 있는 수준이고, 동시에 5% 이내의 균등 분산을 달성할 수 있었습니다. 즉, 150은 분산 균일성과 운영 단순성 사이에서 가장 무난한 균형점이었습니다.

**Q: "토큰 블랙리스트를 샤딩하지 않는 이유는?"**

블랙리스트는 일반 캐시와 달리 보안상 성격이 완전히 다릅니다. 특정 샤드 장애 시 해당 샤드에 저장된 토큰의 블랙리스트 여부를 확인할 수 없고, 보수적인 정책과 결합하면 일부 장애가 전체 인증 실패로 번질 수 있습니다. 반면 블랙리스트 키 수는 수백~수천 개 수준으로 작아서 굳이 분산할 이유도 크지 않았습니다. 그래서 이 데이터는 성능보다 안전성을 우선해 전용 인스턴스로 분리했습니다.

**Q: "샤딩만으로 핫스팟 문제가 해결되나요?"**

안 됩니다. Consistent Hashing은 데이터 분산이지 부하 분산이 아닙니다. 1글자 prefix처럼 훨씬 많이 조회되는 키는 특정 샤드에 트래픽을 집중시켜 핫스팟을 만들 수 있습니다. 즉, 키를 여러 노드에 나누는 것만으로는 충분하지 않고, 부하가 높은 샤드에는 읽기 전용 복제본을 더 두거나 별도 부하 분산 전략을 추가해야 합니다.

**Q: "Redis 사용량이 60MB인데, 샤딩이 정말 필요한가요?"**

용량만 보면 단일 Redis 유지나 Sentinel이 더 단순한 선택일 수 있습니다. 하지만 이 단계에서 해결하려던 문제는 메모리 부족이 아니라, 배치 작업과 실시간 조회의 간섭, `KEYS` 블로킹, 블랙리스트 같은 보안성 높은 데이터의 영향 범위를 줄이는 것이었습니다. 즉, 샤딩은 용량 문제보다 워크로드 분리와 격리 관점에서 선택된 구조였습니다.

**Q: "프로덕션에서는 어떻게 할 건가요?"**

프로덕션에서는 운영 단순성과 장애 대응을 더 우선해 Redis Cluster, Sentinel, 또는 관리형 서비스를 검토할 것입니다. 다만 이 프로젝트에서는 워크로드별 분리, 키 이동 최소화, 장애 시 영향 범위를 직접 통제하는 것이 더 중요한 과제였고, 현재 구조는 그 판단이 실제로 유효한지 검증하기 위한 선택이었습니다.
...
## 분산 안정성 검증 + Chaos Engineering

**Q: "stress 테스트와 soak 테스트를 왜 분리해서 했나요?"**

목적이 다릅니다. stress는 순간 최대 부하에서 시스템이 어디서 터지는지 찾는 테스트입니다. 실제로 VU를 200까지 올려 한계 지점을 확인했습니다. 반면 soak는 정상 부하를 장시간 유지했을 때 시간에 따라 나타나는 문제를 찾는 테스트입니다. 50 VU 수준의 장시간 부하를 통해 메모리 누수, GC 악화, 커넥션 풀 drift처럼 짧은 테스트로는 보이지 않는 문제를 확인하려는 목적이었습니다.

**Q: "Chaos Engineering에서 가장 위험한 시나리오는 뭐였나요?"**

가장 위험한 시나리오는 토큰 블랙리스트 Redis 장애였습니다. 블랙리스트를 조회할 수 없으면, 보수적 정책을 택할 경우 정상 사용자까지 모두 차단돼 인증이 전면 중단될 수 있고, 반대로 관대한 정책을 택하면 로그아웃된 토큰이 다시 통과하는 보안 구멍이 생깁니다. 즉, 이 시나리오에서 보안과 가용성의 trade-off가 바로 드러났습니다.

**Q: "ARM 2코어 × 2대인데, 200 VU를 견딜 수 있나요?"**

stress 테스트의 목적은 200 VU를 무조건 견디는 것이 아니라, 한계가 어디인지 수치로 확인하는 것입니다. 한계를 알아야 App을 3대로 늘리면 어느 정도까지 버틸 수 있는지, 캐시 히트율을 더 높이면 얼마나 여유가 생기는지 같은 capacity planning이 가능합니다. 즉, 이 테스트는 통과/실패를 보는 용도가 아니라 다음 설계 결정을 위한 기준점을 만드는 과정이었습니다.

**Q: "전체 프로젝트에서 이 글의 의미는?"**

초반 글들이 코드와 쿼리, 검색 구조를 어떻게 빠르게 만들 수 있는지에 집중했다면, 단일 서버 stress 테스트는 그 최적화가 어디서 한계를 맞는지를 보여줬습니다. 이후 Redis L2 캐시, Replication, App 스케일아웃, CDC, Redis 샤딩 글들은 그 한계를 넘기기 위해 구조를 어떻게 바꿨는지를 다뤘습니다. 이 글의 의미는 그렇게 쌓아 올린 분산 구조가 실제 고부하와 장애 상황에서도 안정적으로 동작하는지를 검증했다는 데 있습니다.
...
영어 금칙어는 단어 경계(word boundary) 매칭을 적용합니다. "ass"를 금칙어로 등록해도 "assassination", "class", "Scunthorpe" 같은 정상 단어는 차단하지 않습니다. 한국어는 교착어 특성상 부분 일치가 더 적합합니다. "금칙"을 등록하면 "금칙어", "금칙어목록" 등 합성어까지 모두 잡아야 하기 때문입니다. 따라서 한국어 Trie(부분 일치)와 영어 Trie(단어 경계)를 분리하여 운영합니다.
