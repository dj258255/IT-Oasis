...
가장 위험한 시나리오는 토큰 블랙리스트 Redis 장애였습니다. 블랙리스트를 조회할 수 없으면, 보수적 정책을 택할 경우 정상 사용자까지 모두 차단돼 인증이 전면 중단될 수 있고, 반대로 관대한 정책을 택하면 로그아웃된 토큰이 다시 통과하는 보안 구멍이 생깁니다. 즉, 이 시나리오는 단순한 캐시 미스가 아니라 보안과 가용성의 trade-off가 바로 드러나는 지점이었습니다.
