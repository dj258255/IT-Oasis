...
이론에서는 자동완성 읽기(240ms SLA)와 검색어 수집(쓰기)의 성능 요구사항이 완전히 다르므로, **각각 목적에 맞게 최적화된 별도 데이터 표현**을 사용해야 한다고 합니다.

| 이론 | WikiEngine 구현 |
|------|----------------|
| 읽기 경로: 접두사 → Top-K 매핑을 빠르게 조회 | **Redis flat KV O(1) GET** — `prefix:v{version}:{접두사}` → JSON 배열 |
| 쓰기 경로: 검색어를 수집하여 배치 처리 | **SQL GROUP BY + 배치 빌드** → Redis 버전 네임스페이스 적재 |
| 240ms SLA | Redis GET P95 **2.5ms**, Lucene fallback P95 **68ms** |
| 최종 일관성(Eventual Consistency) | 배치 주기 1시간 → 최대 1시간 지연 허용 |

읽기 경로에서 Redis GET 한 번으로 끝나므로 240ms SLA를 여유 있게 충족합니다. 쓰기 경로는 매시간 배치로 처리하므로 실시간 데이터가 필요하지 않다는 요구사항과 부합합니다.

**관련 글**: [Redis L2 캐시 + 자동완성 flat KV](/blog/project/wikiengine/redis-l2-cache)

### MapReduce 배치 패턴 — 접두사 분해와 Top-K 집계

이론에서는 검색어를 가능한 모든 접두사로 분해(Map)한 뒤, 접두사별로 인기순 Top-K를 집계(Reduce)하는 배치 파이프라인을 권장합니다.

| 이론 | WikiEngine 구현 |
|------|----------------|
| **Map**: 검색어 → 모든 접두사 분해 | SQL GROUP BY → prefix 분해 (원본 + 자모 + 초성) |
| **Reduce**: 접두사별 Top-K 정렬 | PriorityQueue Top-10 집계 |
| **Write**: 결과를 키/값 저장소에 적재 | Redis 버전 네임스페이스 적재 → 포인터 원자적 전환 |
| **파티셔닝**: 해시 함수로 키 분산 | Redis 키 해시 기반 3노드 분산 (Consistent Hashing) |
| **실행 주기**: 30분~1시간 | `@Scheduled` → **Spring Batch Job+Tasklet** (이력 관리, 실패 복구) |

WikiEngine에서는 "bat" → ["b", "ba", "bat"] 분해와 동일한 패턴으로, "삼성전자" → ["ㅅ", "사", "삼", "삼ㅅ", "삼성", ...] 식으로 한국어 자모까지 분해합니다. 초기에는 `@Scheduled`로 구현했지만, Spring Batch Tasklet으로 전환하여 JobRepository에 시작/종료/상태/처리 건수가 자동 기록되고, FAILED 상태에서 재시작이 가능해졌습니다.
...
이론에서는 Trie가 수십억 건 규모에서 메모리/성능 요구사항을 충족하지 못한다고 합니다. 단일 컴퓨터 메모리에 저장하기 어렵고, 1~2글자 접두사에 대해 모든 분기를 순회하면 시간이 너무 오래 걸립니다.

WikiEngine에서는 **실제로 Trie를 먼저 구현한 뒤 한계를 체감**하고 전환했습니다:

| 이론 | WikiEngine 구현 |
|------|----------------|
| Trie는 규모에서 부적합 | [Trie 자동완성](/blog/project/wikiengine/trie-autocomplete)에서 소규모 Trie 구현 — DFS 순회 + Copy-on-Write로 메모리/CPU 부담 |
| 접두사 → Top-K 매핑의 flat 저장소 | **Redis flat KV** — 접두사를 키로, Top-10 제안을 값으로 사전 계산 |
| O(1) 조회 | Trie DFS O(접두사 길이 + 분기 수) → Redis GET O(1) 단일 명령 |

Trie를 Redis flat KV로 전환하면서 인스턴스 간 일관성 문제도 동시에 해결되었습니다. Trie는 JVM 힙에 존재하므로 App 2대에서 각각 독립적인 Trie를 가져야 했지만, Redis는 공유 저장소이므로 어떤 App이 응답해도 동일한 자동완성 결과를 제공합니다.
...
이론에서는 MapReduce 파이프라인의 결과를 자동완성 서비스에 전달할 때, **실제로 변경된 레코드의 이벤트만** 발행하는 CDC가 가장 효율적이라고 합니다.
...
| 이론 | WikiEngine 구현 |
|------|----------------|
| 결과 DB → CDC 프로세스 → 메시지 브로커 → 서비스 | MySQL binlog → **Debezium** → Kafka → Lucene+캐시 동기화 |
| 변경된 레코드의 이벤트만 발행 | Debezium이 INSERT/UPDATE/DELETE 자동 캡처 |
| 메시지 브로커 | **Kafka** (KRaft 단일 브로커, `@ConditionalOnProperty` fallback) |
| 에러 핸들링 | **DLQ** + DefaultErrorHandler + FixedBackOff 9회 + AckMode.RECORD |

기존 dual-write(MySQL UPDATE + Lucene updateDocument를 서비스 코드에서 직접 호출)에서 CDC로 전환하면서, PostService가 MySQL만 업데이트하면 나머지는 이벤트 파이프라인이 자동 처리하는 구조가 되었습니다. OCP(개방-폐쇄 원칙)를 준수하게 된 것입니다.
