...
| 목록 조회 | 15% | OFFSET 페이지네이션            | 깊은 페이지(30% 확률로 page=100~1000) 요청 시 MySQL 부하 확인  |
...
| 메트릭 | 임계값 | 왜 이 기준인가? |
|--------|--------|----------------|
| 전체 P95 | < 3,000ms | 글로벌 SLO 안전망. 목록 조회(OFFSET)가 느리므로 느슨하게 설정 |
...
| 목록 조회 P95 | < 5,000ms | OFFSET 페이지네이션 baseline. keyset 전환 후 500ms로 하향 예정 |
...
> **왜 글로벌과 엔드포인트별을 분리하는가?** `http_req_duration`(글로벌)만 두면 느린 목록 조회가 전체 P95를 오염시켜 검색·자동완성의 실제 성능을 파악할 수 없습니다. 반대로 엔드포인트별만 두면 새 API 추가 시 threshold를 깜빡하면 모니터링 사각지대가 생깁니다.
...
- 목록 조회(OFFSET 페이지네이션)가 평균 2.5초로 가장 느림 — **예상된 병목**

**목록 조회가 느린 이유:**
k6 스크립트에서 30% 확률로 page=100~1000을 요청합니다. page 1000 = `OFFSET 20,000`이므로, MySQL이 `created_at DESC` 인덱스를 타더라도 20,020개 행을 읽고 20,000개를 버려야 합니다. `content`(LONGTEXT)까지 포함된 전체 행을 읽기 때문에 수백 MB의 불필요한 I/O가 발생합니다.

이 문제의 분석과 해결은 [Deferred Join 적용기 — 기대한 40배 vs 현실 13%](/blog/project/wikiengine/deferred-join-optimization)에서 다룹니다. Deferred Join 적용 후 page 1000(OFFSET 20,000) 기준으로 P95가 3,372ms → 2,741ms(-19%)로 개선되었으나, EXPLAIN 분석 결과 병목의 85%가 OFFSET 자체의 인덱스 스캔 비용이었습니다. 실제 사용자는 대부분 앞쪽 페이지만 조회하므로 최대 페이지를 200(OFFSET 4,000)으로 제한하여 worst-case를 ~500ms 수준으로 낮췄습니다. 근본적 해결에는 Keyset Pagination이 필요하며, deep page 요청이 빈번해지는 시점에 도입할 예정입니다.
...
| 메트릭 | 검색 | 자동완성 | 목록 조회 | 상세 조회 | 쓰기 (생성+좋아요) |
|--------|------|---------|----------|----------|-----------------|
...
![k6 시나리오별 응답 시간 — 목록 조회(파란색)가 20초 이상으로 전체 성능을 끌어내림](/uploads/project/WikiEngine/lucene-decision/k6-load-per-scenario.png)

**결과 요약: 전면 실패.** smoke에서 5 VU로 문제없던 서비스가 50~100 VU에서 완전히 무너졌습니다. 특히 **smoke에서 66ms였던 검색이 3,328ms로 50배 느려졌고**, 목록 조회는 평균 19.4초, 에러율 32.53%입니다. 목록 조회만의 문제가 아니라, 목록 조회의 heavy OFFSET이 CPU를 잡아먹으면서 **모든 시나리오가 연쇄적으로 무너진** 패턴입니다.
...
핵심: **InnoDB Buffer Pool 히트율 100%**는 메모리에 데이터가 충분히 올라가 있다는 뜻입니다. 그럼에도 Slow Query가 14.8K건 발생한 이유는 **CPU가 포화되어 쿼리 처리 자체가 느려졌기** 때문입니다. 특히 목록 조회의 `OFFSET 20,000` 같은 deep pagination은 CPU-bound 연산(인덱스 스캔 → 행 건너뛰기)이므로, CPU 1코어 환경에서 치명적입니다.
...
원인 체인:
  목록 조회의 deep OFFSET (OFFSET 20,000) → CPU-bound 인덱스 스캔
...
- 목록 조회 30%가 page 100~200 (OFFSET 2,000~4,000) — 매 요청마다 CPU-bound 인덱스 스캔
