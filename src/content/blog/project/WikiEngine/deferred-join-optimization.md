...
smoke 테스트 결과, 검색(66ms), 자동완성(25ms), 상세 조회(53ms)는 양호했지만 **목록 조회만 평균 2,518ms, P95 3,372ms**로 유독 느렸습니다.
...
| **목록 조회 (MySQL)** | **2,518ms** | **3,372ms** |
| 상세 조회 (MySQL) | 53ms | 93ms |
| 쓰기 (MySQL + Lucene) | 62ms | 124ms |

목록 조회가 검색보다 **38배 느립니다**. Lucene 전체 검색(1,475만 건 BM25 스코어링)보다 단순 목록 정렬이 느리다는 건 쿼리 자체에 구조적 문제가 있다는 뜻입니다.

k6 스크립트에서 목록 조회의 페이지 분포는 다음과 같습니다:
