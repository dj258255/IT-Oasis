...
description: "DB 서버와 클라이언트 사이엔 '프로토콜이라는 계약'이 있다. 그 계약만 지키면 상대는 내가 진짜 PostgreSQL인지 구분하지 못한다. PostgreSQL wire protocol v3를 바이트로 뜯는다: 유일한 변칙인 startup 핸드셰이크와 SSLRequest 거절, [타입 1B][길이 4B][페이로드] 프레임, RowDescription/DataRow/CommandComplete의 세 가지 응답 모양, 그리고 psql 프롬프트(=> / =*> / =!>)를 바꾸는 ReadyForQuery 상태 바이트 한 개까지. 400줄 서버로 실제 psql 14가 접속했고, 좋은 계층 분리 덕에 SQL 실행기는 한 줄도 안 바꿨다. 클라이맥스는 두 psql 터미널이다. 한쪽이 미커밋 UPDATE를 쥔 채로 다른 쪽이 옛 버전을 막힘 없이 읽는, MVCC의 존재 이유가 네트워크 너머에서 증명되는 장면이다. 커넥션당 스레드와 그 뒤에 숨은 굵은 latch의 한계까지 정리한다."
...
[4편](/blog/project/db-hobby/db-internals-04-mvcc)까지의 엔진은 로컬 REPL이었습니다. 진짜 DB는 네트워크 서버입니다. 그리고 서버와 클라이언트 사이엔 **프로토콜이라는 계약**이 있습니다.

이 편의 목표는 하나였습니다: **아무 psql이나 접속하게 하라.** PostgreSQL wire protocol v3를 말하는 서버를 만들면, psql은 상대가 진짜 PG인지 구분하지 못합니다. 클라이언트를 새로 만들지 않고 **계약을 지켜서 기존 생태계(psql, JDBC, 각종 드라이버)를 통째로 빌립니다.** 이게 wire protocol 호환이 강력한 이유고, CockroachDB·YugabyteDB 같은 신생 DB들이 PG wire protocol을 말하는 이유이기도 합니다.

> **흔한 오해 정정**: *"PG wire 호환이면 PostgreSQL 호환이다"* 계약이 보장하는 건 **바이트의 모양**까지입니다. CockroachDB·YugabyteDB는 wire는 PG를 말하지만 격리 수준의 기본값과 동작, SQL 방언, 에러의 결이 PostgreSQL과 다릅니다. 드라이버가 붙는 것과 애플리케이션이 같게 동작하는 건 별개의 문제입니다. 이 글의 400줄 서버가 그 산 증거입니다(psql은 붙지만, 이건 PostgreSQL이 아니죠).

결론부터: **400줄 서버로 실제 psql 14가 붙었고, SQL 실행기는 한 줄도 안 바꿨습니다.**

## 1. 놀라운 건 재사용의 규모다

이 편에서 새로 짠 코드는 딱 두 가지입니다. 이벤트 루프와, 바이트를 빅엔디안으로 조립·해석하는 헬퍼입니다. SQL 실행은 기존 실행기를 **그대로** 부릅니다.
...
실행기는 여전히 `FILE*`에 사람이 읽는 텍스트를 씁니다. 서버는 그 텍스트를 `open_memstream`으로 가로채 wire 메시지로 **번역**할 뿐입니다. 계층이 깨끗하게 분리돼 있던 덕에 네트워크가 실행기 위에 무개조로 얹혔습니다. 좋은 추상화의 배당금입니다.

그리고 결정적으로, **TCP 커넥션을 [4편](/blog/project/db-hobby/db-internals-04-mvcc)의 트랜잭션 세션에 하나씩 매핑**합니다. 커넥션마다 자기 트랜잭션 핸들이 자동 배정됩니다. 다중 트랜잭션 세션이 사실은 이 편을 위한 사전 준비였던 셈입니다.
...
PostgreSQL wire protocol v3는 의외로 소박합니다. 핸드셰이크만 지나면 대부분 **`[타입 1바이트][길이 4바이트][페이로드]`** 프레임입니다(길이는 빅엔디안, 길이 자신을 포함).

**핸드셰이크가 유일한 변칙입니다.** 공식 문서의 첫 문장 그대로, *"To begin a session, a frontend opens a connection to the server and sends a startup message."* 이 startup 메시지엔 타입 바이트가 없습니다(길이부터 옵니다). 그리고 기본 설정(`sslmode=prefer`)의 psql은 접속하자마자 **SSLRequest**(매직 넘버 `80877103`)부터 보내는데, SSL이 없으면 한 바이트로 거절하면 됩니다.
...
(이 "물어보고 거절당하면 평문" 왕복도 진화 중입니다. PostgreSQL 17부터는 첫 바이트부터 TLS로 시작하는 direct TLS(`sslnegotiation=direct`)가 추가됐습니다.)
...
두 가지 정직한 축약이 있습니다. 실제 PostgreSQL은 `ParameterStatus`를 하나가 아니라 여러 개 보냅니다. `server_version`, `standard_conforming_strings`, `DateStyle` 등입니다. 일부 드라이버는 이 값들을 요구하지만, psql만 상대할 땐 `client_encoding` 하나로 충분했습니다. 인증도 마찬가지입니다. 이 서버는 누구든 무조건 `AuthenticationOk`를 받는 trust 상당이고, 실제 PostgreSQL의 기본은 SCRAM-SHA-256 챌린지-응답입니다.

`BackendKeyData`의 pid·key는 **쿼리 취소**용인데, 여기에 프로토콜의 재미있는 설계가 있습니다. 실행 중인 커넥션은 쿼리를 처리하느라 바쁘니, CancelRequest(매직 넘버 `80877102`)는 startup 대신 이 매직을 실은 **별도의 새 커넥션**으로 날아옵니다. db-hobby는 키만 발급하고 취소 처리는 구현하지 않았습니다.

여기서 이 프로토콜의 영리한 디테일이 하나 있습니다. **`ReadyForQuery`의 상태 바이트**입니다. 공식 문서의 정의로 *"'I' if idle (not in a transaction block); 'T' if in a transaction block; or 'E' if in a failed transaction block"*. 서버가 매 응답 끝에 이 한 바이트로 트랜잭션 상태를 알려주면, psql이 프롬프트를 `=>` / `=*>` / `=!>`로 바꿔 그립니다. 커넥션의 세션이 `in_txn`이냐로 이 바이트를 정하니, 엔진의 트랜잭션 상태가 그대로 psql 프롬프트에 반영됩니다.

이 계약의 무게를 보여주는 숫자가 하나 있습니다. 위의 `196608`(v3.0)은 **PostgreSQL 7.4(2003년)부터 지금까지 그대로**입니다. v2 지원이 제거된 게 PostgreSQL 14, 첫 마이너 상향(3.2)이 PostgreSQL 18에서야 나왔습니다. 20년 넘게 안 바뀐 계약이었기에 psql·JDBC·수많은 드라이버 생태계가 그 위에 쌓일 수 있었던 겁니다.
...
실제 애플리케이션 드라이버들의 기본 대화는 simple query가 아니라 **extended query protocol**입니다. 한 문장이 다섯 메시지로 쪼개집니다. `Parse`(SQL 텍스트를 이름 붙은 문장으로), `Bind`(파라미터 값을 채워 portal 생성), `Describe`(결과 모양 조회), `Execute`(portal 실행), `Sync`(경계 표시)입니다. 이렇게 쪼갠 이유는 셋입니다. 파싱·플래닝을 한 번 하고 **플랜을 재사용**할 수 있고(prepared statement), 값을 텍스트가 아닌 **바이너리 포맷**으로 보낼 수 있고, 무엇보다 SQL 텍스트와 값이 **서로 다른 메시지로** 갑니다.

> **흔한 오해 정정**: *"prepared statement는 성능 기능일 뿐이다"* 프로토콜 관점에선 보안 장치입니다. `Parse`에 실리는 SQL엔 `$1` 자리표시자만 있고 값은 `Bind`에 따로 실리니, **값이 SQL로 해석될 경로 자체가 없습니다.** 문자열 이어붙이기가 만드는 SQL 인젝션이 애플리케이션의 이스케이프 실력이 아니라 **프로토콜 수준에서** 차단되는 겁니다.
...
지어낸 트랜스크립트가 아닙니다. **실제 psql 14.19 두 개를 띄워 잡은 출력**입니다. reader는 거부당하지도 기다리지도 않고 스냅샷의 옛 버전을 읽고, 쓰기끼리만 충돌합니다. MVCC의 존재 이유가 네트워크 너머에서 두 클라이언트로 증명됐습니다. 단, 터미널 B가 받은 **즉시 ERROR**는 db-hobby의 이중 단순화, 곧 테이블 단위 락에 기다리지 않는 no-wait가 만든 모양입니다.

> **흔한 오해 정정**: *"MVCC에서 쓰기 충돌은 즉시 에러가 난다"* 실제 PostgreSQL에서 터미널 B의 UPDATE는 에러 없이 **행 락을 기다리며 멈춰 있습니다.** A가 커밋하면 READ COMMITTED에선 최신 버전으로 조건을 재평가해 그냥 진행하고, REPEATABLE READ에서만 `ERROR: could not serialize access due to concurrent update`가 납니다. InnoDB도 기다리는 쪽이라, `innodb_lock_wait_timeout`(기본 50초)을 넘겨야 `ERROR 1205 (HY000): Lock wait timeout exceeded; try restarting transaction`을 봅니다. 기다림을 원치 않으면 그걸 명시하는 게 관행입니다. PostgreSQL의 `lock_timeout`이나 `SELECT ... FOR UPDATE NOWAIT`가 그 방법입니다.

## 5. 커넥션당 스레드, 그리고 굵은 latch

처음 서버는 단일 스레드 `poll()` 루프였습니다. 진짜 병렬이 아니라 인터리브였습니다. 커넥션마다 OS 스레드를 주는 **thread-per-connection**으로 바꾸면 네트워크 I/O는 진짜 병렬이 됩니다. PostgreSQL이 커넥션마다 프로세스를 포크하는 것의 스레드판입니다.
...
- **MVCC가 네트워크 너머에서 증명**: 두 psql, reader는 안 막히고 writer끼리만 충돌(즉시 에러는 db-hobby의 no-wait 단순화이고, 실제 PG는 행 락 대기).

다음 편은 플래너가 멍청해지는 순간을 다룹니다. "인덱스가 있으면 무조건 쓴다"는 규칙이 틀리는 지점에서 **비용 기반 옵티마이저**가 시작됩니다.
