---
title: 'db-hobby 전체 지도 — C로 만든 미니 RDBMS, 30편 읽기 가이드'
titleEn: 'The db-hobby Map — a Mini RDBMS in C, a Reading Guide to 30 Parts'
description: "C로 맨땅에서 만든 미니 관계형 DB db-hobby의 30편 시리즈 전체 지도. 진짜 psql이 붙고, reader가 writer를 안 막는 MVCC 스냅샷 격리, WAL 크래시 복구, VACUUM, 스레드 안전 버퍼 풀, 비용 기반 옵티마이저, 소켓 복제, LSM 엔진, Raft 합의까지 — 페이지 한 장에서 SQL 실행까지 바닥부터 올라간 기록이다. 각 편이 '앞 편이 만든 문제를 다음 편이 푼다'는 장애-해결 사슬로 이어진다. 어디서 시작할지, 무엇이 하이라이트인지, 어떻게 직접 돌려보는지를 한 페이지에 담은 읽기 가이드."
descriptionEn: "The full map of db-hobby, a mini relational database built from scratch in C across 30 parts. A real psql connects to it; MVCC snapshot isolation where readers don't block writers; WAL crash recovery; VACUUM; a thread-safe buffer pool; a cost-based optimizer; socket replication; an LSM engine; Raft consensus — built bottom-up from a single page to running SQL. Each part chains as failure-then-fix: the next part solves the problem the previous one created. A one-page reading guide to where to start, what the highlights are, and how to run it yourself."
date: 2026-07-04
tags:
  - C
  - Database Internals
...
- **테스트 557개 / 33스위트**, 동시성은 ThreadSanitizer로 검증. 코드: [github.com/dj258255/db-hobby](https://github.com/dj258255/db-hobby)
...
### 5부. 네트워크·동시성·최적화·복제·저장엔진·합의 (19~30)
...
- **557 tests / 33 suites**, concurrency verified under ThreadSanitizer. Code: [github.com/dj258255/db-hobby](https://github.com/dj258255/db-hobby)
...
### Part V. Network, Concurrency, Optimization, Replication, Storage Engines, Consensus (19–30)
