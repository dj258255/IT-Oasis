...
description: "관계형 DB를 C로 밑바닥부터 만든 db-hobby 시리즈. 3편의 B+Tree 인덱스는 첫 컬럼(PK)에만 걸렸다. 진짜 DB처럼 아무 컬럼에나 CREATE INDEX를 걸려면 네 단계가 필요했다 - 비유니크 키를 받는 B+Tree(중복이 분할로 흩어지는 문제와 하한 탐색), 기존 행으로 인덱스를 짓고 카탈로그에 영속화, INSERT/UPDATE가 인덱스를 함께 갱신하고 트랜잭션 롤백 시 되돌리는 WAL, 그리고 플래너가 그 인덱스를 골라 find_all + heap_get + WHERE 재검사로 거르는 인덱스 스캔. PK 인덱스가 유니크라서 조용히 기대던 가정들을 하나씩 갚는 과정이다."
...
[3편](/blog/project/db-hobby/db-hobby-3-index-wal)에서 B+Tree 인덱스를 만들었어요. 그런데 그 인덱스는 **첫 컬럼(PK)에만** 걸렸습니다. `WHERE id = 5`는 인덱스를 타도, `WHERE age = 20`은 풀 스캔이었죠. 진짜 DB는 `CREATE INDEX`로 아무 컬럼에나 인덱스를 겁니다. db-hobby에도 그걸 붙이는 데 네 단계가 걸렸는데, 단계가 많았던 이유가 흥미로워요.
...
[2편](/blog/project/db-hobby/db-hobby-2-sql-engine)에서 카탈로그가 스키마 구조체를 통째로 파일에 직렬화한다고 했는데, 거기에 인덱스 정의(이름 + 컬럼)를 덧붙이는 것으로 영속화가 끝났어요.
...
> **실무/면접 포인트**: PostgreSQL도 인덱스를 별도 relation(별도 파일)으로 둔다. `pg_class`/`pg_index` 카탈로그에 인덱스 메타데이터가 적히고, 인덱스 자체는 자기 relfilenode 파일에 산다. db-hobby의 "카탈로그에 정의를 적고 + 별도 .idx 파일에 데이터를 둔다"는 정확히 같은 분리다.
...
db-hobby의 UPDATE는 가변 길이라 제자리 수정이 안 돼서, **옛 행을 지우고(tombstone) 새 행을 삽입**합니다. 즉 RID가 바뀌어요.

처음엔 "바뀐 컬럼의 인덱스만 갱신하면 되겠지" 했는데, 아니었어요. RID가 통째로 바뀌니, **인덱싱한 컬럼이 안 바뀌었어도** 그 인덱스가 옛 RID(이제 tombstone)를 가리키게 됩니다. 그러면 그 행이 검색에서 사라져요.

> **핵심**: 보조 인덱스는 "컬럼값 -> RID"의 매핑이다. RID가 **물리 위치**인 db-hobby(힙 모델)에선 UPDATE가 RID를 바꾸면 컬럼값이 그대로여도 그 매핑의 오른쪽(RID)이 낡는다. 그래서 UPDATE는 바뀐 컬럼과 무관하게 **모든 보조 인덱스에 새 RID를 다시 등록**해야 한다(InnoDB처럼 PK 값을 가리키는 구조라면 안 그래도 된다 — 아래 표).
...
이건 db-hobby가 RID를 물리 위치로 쓰는(힙 모델) 데서 오는 비용이에요. InnoDB처럼 보조 인덱스가 PK 값을 가리킨다면, UPDATE로 행이 옮겨져도 PK가 안 바뀌는 한 보조 인덱스를 건드릴 필요가 없죠. 두 모델의 트레이드오프가 여기서 드러납니다.

| | db-hobby·PostgreSQL (RID 모델) | InnoDB (PK 모델) |
|---|---|---|
...
DELETE는 db-hobby에선 인덱스를 거의 안 건드려도 됐어요 — 정확히는 **지연 삭제(lazy deletion)** 를 택한 거예요. 삭제는 힙 행을 tombstone 처리할 뿐 인덱스 항목은 그냥 두고, 나중에 그 stale 항목을 따라가도 `heap_get`이 tombstone을 만나 걸러요(4단계). 다만 이건 보조 인덱스의 일반 성질이 아니라 구현 선택이에요 — 진짜 DB는 그 죽은 항목을 언젠가 VACUUM/purge로 정리해 공간을 회수합니다.

> **주의**: 이게 작동하는 건 **힙을 항상 최종 진실로 삼는 구조**(tombstone + 4단계 재검사) 덕분이다. 인덱스에 stale 항목이 남아 있어도 최종 판정을 인덱스가 아니라 실제 행으로 하니 결과가 정확하다. 그래서 db-hobby는 **B+Tree 물리 삭제를 구현하지 않아도 됐다**(PK 인덱스가 줄곧 그래 온 것과 같다) — 어디까지나 db-hobby의 구현 선택이고, 진짜 DB는 죽은 엔트리를 결국 정리·회수한다.

### WAL — 인덱스도 함께 묶기

마지막으로 **WAL**이에요. [3편·4편](/blog/project/db-hobby/db-hobby-4-transactions)에서 데이터와 PK 인덱스를 WAL로 묶어 트랜잭션이 원자적으로 커밋·롤백되게 했는데, 보조 인덱스도 자기 WAL로 똑같이 묶었습니다.
...
> **실무/면접 포인트**: "인덱스도 트랜잭션의 원자성에 포함된다"가 핵심이다. 실제 DB에서 인덱스는 데이터와 같은 WAL(또는 redo log) 흐름에 들어가, 데이터와 인덱스가 따로 노는 순간이 없다. db-hobby는 데이터·PK·보조 인덱스가 각자 파일·WAL을 갖되, 커밋 시점에 함께 묶이는 구조로 이걸 흉내 냈다.
...
- **NULL은 색인 안 함.** db-hobby는 단순화를 위해 NULL을 인덱싱하지 않습니다 — B+Tree 자체가 NULL을 못 담는 건 아니에요(실제 PostgreSQL·InnoDB는 NULL도 인덱스에 넣고, NULL 정렬 위치만 따로 정합니다).
...
In [Part 3](/blog/project/db-hobby/db-hobby-3-index-wal) I built a B+Tree index. But that index only covered the **first column (the PK)**. `WHERE id = 5` used the index, while `WHERE age = 20` was a full scan. A real DB indexes any column with `CREATE INDEX`. Adding that to db-hobby took four stages, and why it took so many is the interesting part.
...
In [Part 2](/blog/project/db-hobby/db-hobby-2-sql-engine) I said the catalog serializes the whole schema struct to a file; appending the index definition (name + column) finished the persistence.
...
> **Practical/interview note**: PostgreSQL also keeps an index as a separate relation (a separate file). Index metadata is written in the `pg_class`/`pg_index` catalogs, and the index itself lives in its own relfilenode file. db-hobby's "write the definition in the catalog + keep the data in a separate .idx file" is exactly the same split.
...
db-hobby's UPDATE cannot edit in place (variable length), so it **deletes the old row (tombstone) and inserts a new one** — meaning the RID changes.
...
This is the cost of db-hobby using the RID as a physical location (the heap model). If a secondary index pointed at the PK value like InnoDB, there would be no need to touch it on an UPDATE-induced move as long as the PK is unchanged. The trade-off of the two models shows here.

| | db-hobby·PostgreSQL (RID model) | InnoDB (PK model) |
|---|---|---|
...
DELETE, in db-hobby, barely touches the index — more precisely, it's a **lazy-deletion** choice. A delete just tombstones the heap row and leaves the index entry alone, and even if you later follow that stale entry, `heap_get` meets the tombstone and filters it (Stage 4). But this is an implementation choice, not a general property of secondary indexes — a real DB eventually cleans up those dead entries via VACUUM/purge and reclaims the space.

> **Caution**: this works thanks to a **structure that always treats the heap as the final truth** (tombstone + the Stage-4 recheck). The result stays correct even with stale entries because the final verdict is made on the real row, not the index. That is why db-hobby **did not have to implement physical B+Tree deletion** (exactly as the PK index has done all along) — but that is db-hobby's implementation choice; a real DB eventually cleans up and reclaims dead entries.

### WAL — Bind the Index In Too

Finally, **WAL**. In [Parts 3 and 4](/blog/project/db-hobby/db-hobby-4-transactions) I bound data and the PK index with WAL so transactions commit/rollback atomically; I bound the secondary index the same way with its own WAL.
...
And I surfaced this in [Part 8's EXPLAIN](/blog/project/db-hobby/db-hobby-8-explain).
...
- **NULL is not indexed.** db-hobby skips indexing NULL for simplicity — it's not that a B+Tree can't hold NULL (real PostgreSQL/InnoDB do index NULLs, just deciding where NULL sorts).
