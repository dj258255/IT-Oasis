...
SQL은 선언형 언어입니다. "**무엇을**"만 말하고 "어떻게"는 안 말합니다. `SELECT * FROM users WHERE id = 1`이라는 문자열이 힙의 실제 행이 되려면 세 단계가 필요하고, 이 편에서 그 세 단계를 직접 만듭니다.

이 시리즈의 [1~10편](/blog/project/db-hobby/db-internals-01-storage)은 저장·복구·격리·최적화·분산을 다뤘는데, 정작 **SQL 텍스트가 실제 행이 되기까지의 실행기 파이프라인** 이 비어 있었습니다. 이 편이 그 축을 완결합니다.
