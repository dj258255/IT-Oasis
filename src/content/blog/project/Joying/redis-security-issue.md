...
평소처럼 개발하고 있던 어느 날, 보안 뉴스 하나가 눈에 띄었습니다.

> **"Redis에서 CVSS 10.0 만점의 원격 코드 실행 취약점 발견"**

CVSS 9.9? 사실상 최고 위험도입니다. (NVD 공식 9.9, 일부 벤더는 10.0으로 보도. PR:L, 즉 인증이 필요하기 때문에 0.1 차감) 게다가 **13년 동안 숨어있었다**고 합니다.

우리 프로젝트는 Redis를 세션 관리와 캐싱에 사용 중이었습니다. 심장이 철렁했습니다.
...
CVSS 9.9는 **사실상 최고 위험도**입니다. (10.0이 아닌 이유: 공격에 인증이 필요하기 때문. 하지만 인증 없이 노출된 인스턴스가 6만 개라 실질적 위험은 10.0과 동일)
...
Redis는 기본적으로 Lua 스크립팅을 지원하는데, 이 기능의 메모리 관리에 버그가 있었던 것입니다.
...
하지만 최근 들어 Redis가 **인증 없이** 인터넷에 노출되는 사례가 급증하면서 위험도가 폭발적으로 증가했습니다.
...
현재 GitHub에 **공개된 POC가 8개 이상** 존재합니다:
...
다행히 우리는 **패스워드 인증을 활성화**해둔 상태였습니다.

하지만 인증만으로는 충분하지 않습니다. **인증된 사용자도 악용 가능**하기 때문입니다.
...
Redis는 **내부 네트워크에만** 노출. 백엔드만 접근 가능하도록 했습니다.
...
7.2.11 패치에는 CVE-2025-49844 외에도 추가 취약점 수정이 포함되어 있습니다:
...
Redis 공식 이미지는 **기본적으로 인증 비활성화** 상태입니다. 많은 개발자가 이 사실을 모르고 배포합니다.

### 2. "보안 업데이트는 최우선"

CVSS 9.9는 **즉시 패치**해야 합니다. "나중에..."는 없습니다.
