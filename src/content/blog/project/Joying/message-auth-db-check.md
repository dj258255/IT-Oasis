...
채팅방 목록 조회를 최적화하고 나니, 이번엔 메시지 전송이 느렸습니다.
...
메시지 10개를 연속으로 보내면 1-2초가 걸렸습니다. 특히 권한 확인(30-50ms)은 채팅방 참여자가 거의 변하지 않는 1:1 채팅에서 매 요청마다 MySQL을 조회하는 것이 비효율적이었습니다.

---

## 왜 권한 확인이 필요한가

악의적인 사용자가 다른 사람 채팅방에 메시지를 보내려고 할 수 있습니다.
...
그래서 메시지 전송할 때마다 MySQL에서 확인합니다.

![](/uploads/project/Joying/message-auth-db-check/why-permission-check.svg)


문제는 메시지를 보낼 때마다 조회한다는 점입니다.

채팅방 참여자 정보는 거의 변하지 않습니다. 1:1 채팅이라 구매자와 판매자가 고정되어 있습니다. 매번 MySQL을 조회하는 건 낭비였습니다.

---

## 권한 체크 방법 검토

권한 확인을 빠르게 하기 위한 방법을 검토했습니다.

### 1. 매번 MySQL 조회 (현재)

항상 정확하지만, 메시지마다 30ms + DB 커넥션을 소모합니다.

### 2. Application 메모리 캐시

![](/uploads/project/Joying/message-auth-db-check/app-memory-cache.svg)

0.01ms로 빠르지만, 서버가 여러 대일 때 동기화가 안 되고 재시작 시 소실됩니다.

### 3. Redis 캐시 (선택)

이미 Redis Pub/Sub을 사용 중이라 추가 인프라 없이 적용 가능했습니다. 1:1 채팅이라 권한 정보가 거의 바뀌지 않아서 캐시 히트율이 높고, TTL로 자동 만료도 됩니다.

---

## 해결: Redis 권한 캐싱

Redis에 권한 정보를 캐싱했습니다.
...
실제 사용 패턴에서는 첫 메시지만 MySQL을 조회하고, 이후 TTL(1시간) 동안은 Redis에서 처리합니다. 메시지를 많이 보낼수록 차이가 벌어집니다.
...
채팅방 참여자는 거의 바뀌지 않으므로 TTL 1시간이면 충분했습니다.

---

## Cache Warming

첫 메시지부터 빠르게 처리하기 위해 채팅방 생성 시 미리 캐싱했습니다.

![](/uploads/project/Joying/message-auth-db-check/cache-warming.svg)

---

## lastMessage 비동기 처리

권한 캐싱 외에 lastMessage 업데이트도 병목이었습니다.
...
메시지를 보낼 때마다 MySQL의 `chat_room` 테이블에 lastMessage를 업데이트했습니다. 문제는 이게 메시지 전송 응답에 꼭 필요하지 않다는 점입니다.
...
채팅방 목록 조회는 메시지 전송보다 덜 빈번합니다. 사람들이 계속 채팅은 하지만, 목록을 새로고침하는 건 가끔입니다.

lastMessage가 1-2초 늦게 업데이트되어도 사용자가 거의 못 느낍니다. 메시지 전송 속도가 더 중요했습니다.
...
![](/uploads/project/Joying/message-auth-db-check/chat-room-mysql.svg)


### 메시지 전송 서비스

![](/uploads/project/Joying/message-auth-db-check/message-send-service.svg)
...
권한 캐싱은 CQRS(Command Query Responsibility Segregation) 패턴을 적용한 것입니다.
...
읽기와 쓰기를 분리하면 각각을 독립적으로 최적화할 수 있습니다.
