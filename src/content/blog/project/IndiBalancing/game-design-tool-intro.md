...
게임 밸런싱 작업에는 반복적으로 필요한 것들이 있어요.

- DAMAGE, SCALE, TTK 같은 게임 공식을 쉽게 적용할 수 있어야 해요
- 캐릭터 스탯 변경 시 관련 데이터가 자동으로 재계산되어야 하고요
- 성장 곡선, 능력치 비교 등을 차트로 빠르게 확인할 수 있어야 해요
- JSON, CSV 연동으로 게임 엔진에서 바로 쓸 수 있는 형태로 데이터를 추출할 수 있어야 합니다

기존 도구(엑셀, 구글 시트)로도 가능하지만, 게임 특화 함수가 없어서 매번 수식을 직접 작성하거나 VBA 스크립트를 유지보수해야 하거든요.
...
게임마다 공통으로 쓰이는 수식(데미지 감소율, 성장 곡선, TTK)이 존재하고, 통계적 검증 방법으로 Wilson Score Interval, 몬테카를로 시뮬레이션을 적용할 수 있어요.
...
시트 간 자동 참조를 통해 데이터가 자동으로 재계산돼요. 순환 참조가 감지되면 경고를 표시합니다.
...
Unity, Godot, Unreal 코드를 자동으로 생성해요.
...
최종 목표는 경제 시스템, 가챠/뽑기 시스템 설계, 로그라이크 아이템 시너지까지 지원하는 거예요.

---

## 수익 모델

기본 기능은 무료로 제공해요. 추후 클라우드 저장, 협업 기능 같은 부분을 유료화할 계획입니다.
...
A: 가능해요. 다만 스크립트 작성과 유지보수 시간을 고려하면 게임 개발에 집중하는 편이 나을 수 있습니다.

**Q: 전투 시뮬레이터가 있는가?**
A: 몬테카를로 시뮬레이션으로 1,000~100,000회 전투를 돌릴 수 있고, Wilson 신뢰구간 95%로 통계적 검증이 가능해요. 1:1, 1:N, N:N 팀 전투를 지원합니다.

**Q: 브라우저 캐시를 지우면 데이터가 날아가는가?**
A: IndexedDB를 사용하므로 일반 캐시 삭제로는 날아가지 않아요. 다만 브라우저 데이터 전체 삭제 시에는 소실될 수 있으므로 백업이 필수입니다.

**Q: 게임 엔진 코드 내보내기가 되는가?**
A: Unity ScriptableObject(.cs + .json), Godot Resource(.gd + .json), Unreal USTRUCT(.h + .csv) 형태로 자동 생성해요.
...
A: 엑셀에는 `SCALE(base, level, rate, "exponential")` 같은 게임 특화 함수가 없어요. 순환 참조 감지 경고, 몬테카를로 시뮬레이션, 게임 엔진 코드 자동 생성도 엑셀에는 없는 기능입니다.

**Q: AI한테 시뮬레이션 돌리는 것보다 장점이 무엇인가?**
A: AI는 매번 컨텍스트를 설명해야 하고, 대화가 끊기면 다시 설명해야 하며, 데이터가 축적되지 않아요. 수치를 바꿀 때마다 다시 물어봐야 하고요. 간단한 일회성 계산은 AI가 빠르지만, 지속적인 데이터 관리와 수정이 필요한 상황에서는 전용 툴이 유리합니다.

**Q: 웹 기반이면 오프라인에서 못 쓰는가?**
A: IndexedDB는 로컬 저장이라 인터넷 없이도 작동해요.
