...
description: 2,200명 규모 인디게임 개발 커뮤니티에서 설문조사와 피드백 분석을 통해 게임 밸런스 툴의 시장 수요를 검증한 과정을 정리했어요.
...
인디게임 개발 커뮤니티를 관찰하면서, 그리고 직접 인디게임을 개발해본 경험에서 반복되는 패턴을 발견했어요.

**패턴**:
1. 개발자가 캐릭터 HP를 1500으로 설정함
2. "이게 적정한 수치인가?" 확신이 없어요
3. 직접 플레이하며 "감"으로 조정
4. 출시 후 유저 피드백: "이 캐릭터 너무 OP임"
5. 급하게 핫픽스

**왜 이런 일이 발생하는가**:
- 대형 스튜디오에는 전담 밸런스 디자이너와 내부 툴이 있어요
- 1~2인 인디 개발자에게는 그런 리소스가 없고요
- 혹은 대형 스튜디오에서는 엑셀/구글 시트를 사용하지만, 게임 특화 기능이 없어서 기획자가 힘들어 하는 경우가 많습니다
...
![](/uploads/project/IndiBalancing/indie-balance-tool-market-research/21-research.png)
...
**분석**: 즉각적인 관심이 있다는 것은 "문제를 겪고 있는 사람들이 존재한다"는 증거예요. 다만 "관심"과 "실제 사용"은 다르므로 설문조사로 더 깊이 파악했어요.

---

## 3. 설문조사 결과 분석

2,200명 규모 방에 올렸을 때 12명이 응답했어요. 보상(기프티콘 등) 없이 진행했음에도 12명이 응답한 것은 관심이 있다는 신호로 해석했어요.

### 3.1 응답자 프로필

![](/uploads/project/IndiBalancing/indie-balance-tool-market-research/image.png)
...
**분석**: 응답자의 절반이 현직자예요. 즉, 실제 업무에서 밸런싱을 하는 사람들이 관심을 가진 거죠.

### 3.2 개발 중인 장르

![](/uploads/project/IndiBalancing/indie-balance-tool-market-research/game.png)

**분석**: 로그라이크, RPG, 시뮬레이션이 상위권이에요. 이 장르들의 공통점은 "수치 밸런싱이 중요하다"는 거예요. 장르 특성상 타겟 유저가 맞다는 신호예요.

### 3.3 개발 경험

![](/uploads/project/IndiBalancing/indie-balance-tool-market-research/dev.png)

**분석**: 12명 중 10명이 프로젝트 완성/출시 경험이 있어요. 즉, "실제로 밸런싱 작업을 해본 사람들"이 응답한 거예요. 경험 없는 사람의 의견보다 신뢰도가 높죠.

### 3.4 현재 사용 도구

![](/uploads/project/IndiBalancing/indie-balance-tool-market-research/tool.png)

**분석**: 대다수가 엑셀/구글 시트를 사용해요. 이 도구의 한계를 해결하면 전환할 가능성이 있어요. Unity Inspector를 쓰는 사람도 있는데, 이는 "엔진 내에서 해결하고 싶다"는 니즈를 보여줘요.
...
![](/uploads/project/IndiBalancing/indie-balance-tool-market-research/image-2.png)

**분석**: 이 질문의 응답이 핵심이에요. "어떤 불편함을 해결해야 하는가"를 직접적으로 보여주거든요.

### 3.6 밸런싱 작업에서 어려운 점

![](/uploads/project/IndiBalancing/indie-balance-tool-market-research/image-3.png)

**분석**: "밸런스가 맞는지 확인하기 어렵다"가 핵심 페인 포인트예요. 이것이 몬테카를로 시뮬레이션 기능의 근거가 돼요.

### 3.7 정보 습득 경로

![](/uploads/project/IndiBalancing/indie-balance-tool-market-research/image-4.png)

**분석**: 레퍼런스 게임 분석, 커뮤니티, 유튜브가 상위권이에요. 즉, "체계적인 학습 경로가 없다"는 뜻이죠. 이론적 배경을 제공하면 차별화 포인트가 될 수 있어요.

### 3.8 원하는 기능 (주관식)

![](/uploads/project/IndiBalancing/indie-balance-tool-market-research/feature.png)
...
**분석**: "자동 DPS 계산", "난이도 곡선 시각화"는 이미 구현했어요. "다른 게임 레퍼런스"는 참고자료 섹션으로 제공할 수 있고요. 커뮤니티 기능을 추가하면 사용자 간 레퍼런스 공유로 이 니즈를 해소할 수 있어요. 전반적으로 시각화에 대한 수요가 높았어요.

### 3.9 사용 의향

![](/uploads/project/IndiBalancing/indie-balance-tool-market-research/image-5.png)

**분석**: 대다수가 "사용할 의향이 있다"고 응답했어요. 가설 "도구가 있으면 사용할 것이다"가 어느 정도 검증된 셈이에요.

### 3.10 중요하게 생각하는 것

![](/uploads/project/IndiBalancing/indie-balance-tool-market-research/image-6.png)

**분석**: 사용 편의성, 게임 엔진 연동, 시각화가 상위권이에요. 이 세 가지가 핵심 기능이 되어야 해요.

### 3.11 가격 민감도

![](/uploads/project/IndiBalancing/indie-balance-tool-market-research/311.png)

**분석**: 무료~월 5천원 대가 대다수예요. 고가 유료화는 어렵고, 프리미엄 모델(기본 무료 + 고급 기능 유료)이 적합하다는 증거예요.

### 3.12 자유 의견

![](/uploads/project/IndiBalancing/indie-balance-tool-market-research/image-7.png)

### 3.13 베타테스트 참여 의향

![](/uploads/project/IndiBalancing/indie-balance-tool-market-research/image-8.png)

**분석**: 2명이 연락처를 남겼어요. 초기 사용자 확보 가능성이 있어요.

---

## 4. 상세 피드백 분석

설문 외에 개인 메시지로 받은 상세 피드백을 분석했어요.
...
**분석**:
- 전투 종료 시간, 종료 시 HP 추세는 "게임 페이스"를 분석하는 지표예요
- 치명타 역전 분석은 "운 요소"의 영향을 측정하는 거고요
- 파티플레이는 현재 N:N 팀 전투로 구현돼 있어요
- 스킬 시스템은 부분 구현(쿨타임, HP 트리거)했고, 무적/부활은 미구현이에요
- 결과 export는 구현 예정이에요
...
**분석**:
- "실제 전투 감각 검증은 어렵다"는 것은 맞는 지적이에요. 이 도구가 "모든 것을 해결해준다"고 주장하면 안 돼요.
- 핵심 가치는 "이상치 탐지"예요. Z-score 기반 이상치 탐지 기능이 이미 있지만, 프레임별 스탯 변화 그래프는 추가 검토가 필요해요.

> Q: 리얼 밸런스를 저 툴로 다 맞춘다하면 그야말로 지옥이라서요. 그건 내려놓고 할 수 있는 부분을 최대한 커버하는 쪽으로 가시면 효율이 좋아질듯. 그것만 해도 엄청난 툴이 됩니다.

**분석**:
- 범위 설정이 중요해요. "만능 툴"이 아니라 "특정 문제를 확실히 해결하는 툴"로 포지셔닝해야 해요.
- 명시적으로 "하지 않는 것"을 문서화해야 해요.
...
**분석**:
- A 케이스: "구간별 성장 곡선"이 필요해요. 현재는 단일 곡선만 지원하고, 로드맵에 추가했어요.
- B 케이스: 특수 효과(무적, 부활)는 현재 미지원이에요. 커스텀 스크립트 기능으로 해결할 수 있을 것 같아요.
...
**분석**:
- "역방향 계산" (Goal Solver) 기능이에요. "원하는 결과를 입력하면 필요한 수치를 역산"하는 기능이죠.
- 이미 Goal Solver 기능이 있지만, "드로잉 → 공식 도출"은 더 직관적인 UX예요. 장기 로드맵에 추가했어요.
...
**분석**:
- "반복 수요"가 핵심이에요. 한 번 쓰고 끝나는 기능보다 "매번 업데이트마다 쓰는 기능"이 유료화에 적합하거든요.
- 밸런스 검증(패치 전 시뮬레이션), 버전 비교(이전 버전 vs 새 버전) 기능이 반복 수요를 만들 수 있어요.
...
다만 인디 개발자/팀에서 출발하되, 장기적으로는 대형 스튜디오에서도 사용할 수 있는 수준으로 확장하는 것이 목표예요.

---

## 6. 설문 결과 기반 의사결정

설문조사와 피드백을 바탕으로 다음 결정을 내렸어요.
...
- 게임을 만들어주는 도구가 아니에요
- AI가 밸런스를 자동으로 잡아주지 않아요 (이상치 탐지만 해요)
- 모든 게임의 모든 케이스를 커버하지 않아요 (80% 솔루션)
- 게임 엔진 플러그인이 아니에요 (데이터 내보내기만 제공)

---

## 7. 추가 반응: 러브콜

프로젝트를 홍보하면서 예상치 못한 반응도 있었어요:

- "프로젝트 상용화를 위해 뛰어다니는 게 맘에 들었다"
- "사원 수 100명 이상, 캐주얼 게임 운영 중인 회사에서 본격적으로 만들어보는 건 어떻겠냐"

이런 반응은 "시장에서 이 문제를 인식하고 있다"는 신호로 해석했어요.
...
1. **"관심"과 "실제 사용"은 달라요**: 출시 후 실제 사용률을 봐야 해요
2. **유료 전환은 어려워요**: 무료 기능이 충분하면 굳이 돈을 안 내거든요
3. **범위 설정이 중요해요**: "만능 툴"을 목표로 하면 안 돼요
