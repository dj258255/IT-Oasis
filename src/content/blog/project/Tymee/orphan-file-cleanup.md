...
새로운 파일 타입이 추가돼도 정리 로직을 수정할 필요가 없습니다.
...
![immediate-delete-complexity](/uploads/project/Tymee/orphan-file-cleanup/immediate-delete-complexity.png)
...
즉시 삭제는 단순하지만 트랜잭션 불일치와 API 지연, 복구 불가 문제가 있습니다.
비동기 삭제로 응답 속도는 해결되지만 트랜잭션 문제는 여전합니다.

Soft Delete + 배치 방식은 트랜잭션 안전하고 응답이 빠르며 7일간 복구도 가능합니다.
스토리지를 7일간 추가로 사용하는 비용이 있지만, 삭제 실패가 핵심 비즈니스에 영향을 주지 않고 트래픽 적은 새벽에 일괄 처리할 수 있어서 현업에서도 일반적인 방식입니다.

**스토리지 비용 관점:**
Cloudflare R2 기준으로 저장 비용은 월 $0.015/GB입니다. 프로필 이미지가 평균 500KB이고 일일 교체가 100건이면 7일 유예 기간 동안 추가 스토리지는 약 350MB($0.005/월)로 무시할 수 있는 수준입니다. 반면 즉시 삭제 실패로 인한 재처리 로직, 보상 트랜잭션 구현 비용은 개발 시간으로 환산하면 훨씬 비쌉니다.
...
현재 SpringBoot 서버 1대에서 User 모듈과 Upload 모듈이 같은 JVM에서 실행되므로 서버 간 통신이 필요 없습니다.
soft delete가 실패해도 배치에서 처리되니 치명적이지 않고, 결제처럼 "반드시 처리되어야 하는" 작업도 아닙니다.
RabbitMQ를 쓰려면 ConnectionFactory, Exchange/Queue/Binding 설정, 직렬화 로직, 재연결 처리 등이 추가로 필요한데, Spring Event는 `@EventListener` 하나면 됩니다.
ARM 1 OCPU / 6GB RAM 환경에서 불필요한 네트워크 hop도 낭비입니다.
...
![rabbitmq-scaleout](/uploads/project/Tymee/orphan-file-cleanup/rabbitmq-scaleout.png)
...
![transactional-event-listener](/uploads/project/Tymee/orphan-file-cleanup/transactional-event-listener.png)
...
## 아키텍처
![architecture-diagram](/uploads/project/Tymee/orphan-file-cleanup/architecture-diagram.png)
...
![profile-image-changed-event](/uploads/project/Tymee/orphan-file-cleanup/profile-image-changed-event.png)
...
![user-service-event-publish](/uploads/project/Tymee/orphan-file-cleanup/user-service-event-publish.png)
...
![event-listener](/uploads/project/Tymee/orphan-file-cleanup/event-listener.png)
...
![orphan-cleanup-scheduler](/uploads/project/Tymee/orphan-file-cleanup/orphan-cleanup-scheduler.png)
...
![enable-scheduling](/uploads/project/Tymee/orphan-file-cleanup/enable-scheduling.png)
...
![batch-scheduler-generic](/uploads/project/Tymee/orphan-file-cleanup/batch-scheduler-generic.png)
...
![post-image-delete-example](/uploads/project/Tymee/orphan-file-cleanup/post-image-delete-example.png)
...
가장 크게 배운 건 "트랜잭션의 경계"입니다.
R2/S3 같은 외부 스토리지는 DB 트랜잭션에 포함되지 않습니다.
프로필 업데이트 중 R2 파일을 먼저 삭제했는데 DB 커밋에서 예외가 터지면, 파일은 삭제됐는데 DB는 롤백되는 불일치가 생깁니다.
`@TransactionalEventListener(AFTER_COMMIT)`으로 DB 커밋 성공 후에만 파일 작업을 하도록 해결했습니다.

AFTER_COMMIT 시점의 함정도 있었습니다.
`@Transactional` 메서드를 호출하면 새 트랜잭션이 시작될 줄 알았는데, DB 트랜잭션은 종료됐지만 스프링 트랜잭션 컨텍스트는 아직 정리 전이라 기본 전파 속성(REQUIRED)이 "이미 종료된 트랜잭션에 참여"를 시도했습니다.
