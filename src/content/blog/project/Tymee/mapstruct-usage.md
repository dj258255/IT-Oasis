...
이 프로젝트는 레이어별로 객체 특성이 다릅니다.

- **Domain**: 불변 객체. 생성 후 상태 변경 시 새 객체 반환하거나 명시적 메서드로만 변경합니다.
- **Entity**: 가변 객체. JPA가 프록시로 감싸서 dirty checking 하려면 setter나 필드 직접 접근이 필요합니다.
- **DTO (Request)**: 불변 객체. Java record 사용. 클라이언트에서 받은 값 그대로 유지합니다.
- **DTO (Response)**: 불변 객체. Java record 사용.

문제는 이 객체들 사이에서 변환이 필요하다는 것입니다.
...
필드 순서 틀리거나 하나 빠뜨리면 컴파일 에러도 안 나고 런타임에 이상한 값이 들어갑니다.
MapStruct는 컴파일 타임에 매핑 코드를 생성해서 이런 실수를 방지합니다.

### 부분 업데이트 (PATCH)

설정 변경 API는 보통 전체가 아니라 일부만 바꿉니다.
...
MapStruct의 `@BeanMapping(nullValuePropertyMappingStrategy = IGNORE)`나 default 메서드로 이걸 깔끔하게 처리할 수 있습니다.
...
UserSettings는 더 까다롭습니다.
푸시 알림, 개인정보, 플래너 설정 등 20개가 넘는 필드가 있고, 부분 업데이트(PATCH)를 지원해야 합니다.
null인 필드는 무시하고 기존 값을 유지하는 로직이 필요한데, 이걸 수동으로 하면 코드가 100줄 넘어갑니다.
...
upload은 MapStruct 안 씁니다. 이유는:

1. **추가 파라미터 필요**: `url`과 `thumbnailUrl`은 도메인 객체에 없습니다. R2StorageService에서 생성한 값을 같이 넘겨야 하는데, MapStruct는 단순 객체 → 객체 변환에 최적화되어 있어서 이런 케이스가 어색합니다.

2. **필드가 적음**: Upload 도메인은 필드가 10개 안 됩니다. 수동으로 해도 코드 몇 줄이라 MapStruct 설정하는 비용이 더 큽니다.

3. **VO 없음**: user처럼 Email, Nickname 같은 VO를 안 씁니다. 그냥 primitive 타입이라 변환 로직이 단순합니다.
...
결국 보일러플레이트가 얼마나 많이 줄어드냐의 문제입니다.
user처럼 VO도 많고 필드도 많고 부분 업데이트도 있으면 MapStruct가 확실히 낫습니다.
upload처럼 단순하면 굳이 의존성 추가할 필요 없습니다.
...
Lombok과 같이 쓰면 annotationProcessor 순서가 중요합니다.
MapStruct가 Lombok이 생성한 getter/setter를 사용하기 때문에 Lombok이 먼저 처리되어야 합니다.
...
`componentModel = "spring"`으로 설정하면 Spring Bean으로 등록됩니다.
...
불변 객체 변환은 매번 새 객체를 만들어야 하니까 가변 객체보다 느리지 않을까요? 결론부터 말하면 **거의 차이 없습니다**.

JVM의 단기 객체(short-lived object) 생성 비용은 약 **3.6 나노초**입니다.
API 요청 하나 처리하는 데 보통 수십~수백 밀리초가 걸리는데, 객체 몇 개 더 만든다고 체감되는 성능 저하는 없습니다.

게다가 JVM은 **Escape Analysis**라는 최적화를 합니다.

- 메서드 밖으로 안 나가는 객체는 힙 대신 스택에 할당
- 아예 객체를 만들지 않고 필드만 변수로 쪼개는 **Scalar Replacement**
- 단일 스레드에서만 쓰이면 동기화 제거

결국 불변 객체를 자주 만들어도 GC 부담이 크게 늘지 않습니다. 오히려 불변 객체는:
...
MapStruct가 수동 매핑과 거의 동등한 성능을 냅니다.
**컴파일 타임에 최적화된 코드를 생성**하기 때문입니다.

반면 Dozer나 ModelMapper는 리플렉션 기반이라 런타임 오버헤드가 큽니다. 150배 이상 차이 납니다.
...
레이어드 아키텍처에서 불변 객체를 쓰면 필연적으로 마주치는 문제가 있습니다. **필드 하나만 바꾸고 싶은데 전체를 복사해야 한다**는 것입니다.
...
기존 객체의 모든 필드를 복사한 Builder를 반환합니다. 바꿀 필드만 덮어쓰면 됩니다.
...
단일 필드 변경에 가장 깔끔합니다. 여러 필드 변경하려면 체이닝:
...
Record는 Lombok 없이 써야 할 때 적합합니다. 필드 적으면 괜찮은데, 많으면 보일러플레이트가 늘어납니다.
...
검증 로직 넣거나, 캐싱하거나, 서브타입 반환할 때 유용합니다.
단순 필드 변경엔 과합니다.

### 비교

`toBuilder`는 여러 필드를 동시에 바꿀 때 좋고, Wither는 단일 필드 변경에 깔끔합니다.
Factory 메서드는 검증이나 캐싱 로직이 필요할 때 유용하고, Record wither는 Lombok 없이 쓸 때 적합합니다.
성능은 다 비슷합니다. 결국 새 객체를 만드는 건 똑같고 JVM이 최적화해줍니다.
선택 기준은 코드 가독성과 유지보수성입니다.

---

## 실제 프로젝트: UserSettings 사례

이론은 여기까지고, 실제 이 프로젝트에서 어떻게 했는지 보겠습니다.
...
`toBuilder` 대신 명시적 `updateXxx()` 메서드를 씁니다. 장점은:
