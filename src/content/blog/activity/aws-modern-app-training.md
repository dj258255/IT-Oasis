...
description: '원광대학교에서 진행된 AWS 모던 애플리케이션 교육에 참가하여 AWS JAM 실습으로 3등을 하고, IaC, 배포 전략, 멀티 클라우드 트렌드를 배운 이틀간의 교육 일지를 정리했습니다.'
...
오전 6시에 일어나서 6시 50분부터 버스를 타고 익산에서 서울까지 이동했습니다.
...
첫날에는 AWS JAM에 대한 간략한 설명을 듣고 바로 실습으로 이어졌습니다.

**AWS JAM이란?**

* AWS 실습으로 JAM이라는 점수를 받아서 랭킹을 매기는 시스템
* 같이 강의를 들은 수강생들과 선의의 경쟁을 했습니다

![](/uploads/activity/aws-modern-app-training/jam-3rd-place.png)

결국 JAM에서 **999포인트로 3등**을 했습니다!

열심히 풀었는데 갈피가 안 잡히는 건 힌트를 마구 열어서 풀어버렸습니다. 힌트를 열어도 네트워크에 대해선 갈피가 잘 안 잡혔는데, 강사님이 친절하게 알려주셨습니다.
...
3등 상품으로 가습기(+공기청정기)를 받았습니다!

***

## 2일차

2일차에는 오전 9시 30분부터 교육을 받았습니다. 배운 내용을 정리해보겠습니다.

***

### Two Pizza Team

국내 용어로 "목적형 조직"이라고 합니다.
...
클라우드의 설계도를 그림 형태로 이용할 수 있지만, 실제로 환경을 동일하게 생성하려면 새로운 작업이 필요합니다. 이를 해결하기 위해 설계도를 코드 형태로 작성하는 방법론이 **IaC**입니다.
...
* **규모가 크면 클수록 블루/그린이 가장 이상적**입니다 (활용도가 높습니다)
...
다양한 클라우드의 조합으로 서비스를 구성하는 것입니다. 전략적인 클라우드 인프라 구성이 가능하다는 점에서 하나의 트렌드로 각광받고 있습니다.
...
아쉽게 환경변수 오류 때문에 Azure 변환부터는 못했지만, TFC로 배포하는 것까지는 성공했습니다!
...
열심히 했는데 확실히 시간이 많이 없어서 더 자세하게 못 배운 게 아쉬웠습니다. 하지만 개념적인 여러 용어들을 눈에 익히고 많이 배워서 너무 좋았습니다.

**진짜로 좋았던 건 시야가 확 트인 느낌이었습니다.**

자세하게 배우는 건 나의 의지에 따르는 거라 나중에 한번 깊게 공부해야 할 것 같습니다. AWS에서 요즘 모던 애플리케이션이 왜 중요한지, 사용할 때 주의할 점도 많이 알려주셨습니다.
...
같이 강의를 들은 수강생들과 강사님, 멘토님들과의 사진 한 컷입니다!
