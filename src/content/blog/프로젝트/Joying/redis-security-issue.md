...
평소처럼 개발하고 있던 어느 날, 보안 뉴스 하나가 눈에 띄었다.

> **"Redis에서 CVSS 10.0 만점의 원격 코드 실행 취약점 발견"**

CVSS 10.0? 최고 위험도다. 게다가 **13년 동안 숨어있었다**고 한다.

우리 프로젝트는 Redis를 세션 관리와 캐싱에 사용 중이었다. 심장이 철렁했다.
...
CVSS 10.0은 **가능한 최고 위험도**다.

실제로 이 취약점은 공격자가 **원격에서 코드를 실행**할 수 있게 만든다.
...
Redis는 기본적으로 Lua 스크립팅을 지원하는데, 이 기능의 메모리 관리에 버그가 있었던 것이다.
...
하지만 최근 들어 Redis가 **인증 없이** 인터넷에 노출되는 사례가 급증하면서 위험도가 폭발적으로 증가했다.
...
현재 GitHub에 **공개된 POC가 8개 이상** 존재한다:
...
POC가 공개되었다는 것은 **누구나 공격할 수 있다**는 뜻이다.
...
다행히 우리는 **패스워드 인증을 활성화**해둔 상태였다.

하지만 인증만으로는 충분하지 않다. **인증된 사용자도 악용 가능**하기 때문이다.
...
Redis는 **내부 네트워크에만** 노출. 백엔드만 접근 가능하도록 했다.
...
7.2.11 패치에는 CVE-2025-49844 외에도 추가 취약점 수정이 포함되어 있다:
...
Redis 공식 이미지는 **기본적으로 인증 비활성화** 상태다. 많은 개발자가 이 사실을 모르고 배포한다.

### 2. "보안 업데이트는 최우선"

CVSS 10.0은 **즉시 패치**해야 한다. "나중에..."는 없다.
