...
This post bundles the previous four ([Generational Heap Structure on the JVM](/blog/theory/es-memory-01-jvm-heap), [GC Algorithms and Stop-the-World](/blog/theory/es-memory-02-gc), [JVM Off-heap and Direct Memory](/blog/theory/es-memory-03-off-heap), [Why OS Page Cache Decides ES Performance](/blog/theory/es-memory-04-page-cache)) **into the single Elasticsearch context**.
...
- `Xms` and `Xmx` **must be set to the same value** (*"The minimum and maximum values must be the same."*).
- Two reasons to keep it **at most 50% of RAM**:
  1. **off-heap buffers** (Netty network buffers etc.) — [JVM Off-heap and Direct Memory](/blog/theory/es-memory-03-off-heap)
  2. **filesystem cache** — [Why OS Page Cache Decides ES Performance](/blog/theory/es-memory-04-page-cache)
...
4. The number of values representable in 32 bits is 2^32 ≈ 4.2 billion, and each value points to an 8-byte-unit address, so:
    ...
    2^32 × 8 byte = 32 GByte
    ```
    is the maximum addressable heap. ([OpenJDK HotSpot — CompressedOops](https://wiki.openjdk.org/display/HotSpot/CompressedOops))
...
|---|---|---|
| **Term dictionary** (`.tim`, `.tip`) | **mmap** | Lots of random access, repeatedly accessed → big win from sitting in Page Cache |
| **Doc values** (`.dvd`, `.dvm`) | **mmap** | Column-wise random access for sort/aggregation |
| **Norms** (`.nvd`, `.nvm`) | **mmap** | Hit constantly during scoring |
...
This is the maximum number of VMAs (Virtual Memory Areas) you can create with mmap. Default on most Linux distros is in the 60K range (e.g., 65530), and large indexes / many shards quickly run out. ES officially requires `262144` or higher. ([Elastic — Virtual memory check](https://www.elastic.co/guide/en/elasticsearch/reference/current/vm-max-map-count.html))
...
The values below are confirmed directly against the current Elasticsearch reference docs. ([Elastic — Circuit breaker settings](https://www.elastic.co/guide/en/elasticsearch/reference/current/circuit-breaker.html))

| Breaker | Default limit | Purpose |
|---|---|---|
| **Parent** | **95%** of JVM heap (real memory mode, default) / 70% (non-real memory) | Upper bound across all child breakers |
| **Field data** | **40%** of JVM heap | When loading fielddata for `text` field sort/aggregation |
| **Request** | **60%** of JVM heap | Memory needed by a single request (intermediate aggregation results, etc.) |
| **In-flight requests** | **100%** of JVM heap (overhead 2) | Requests in-flight or queued |
| **EQL sequence** | **50%** of JVM heap | Memory used while running EQL sequence queries |
| **Machine learning** | **50%** of JVM heap | ML jobs only |
| **Script compilation** | **150 / 5 min** (not a ratio) | Prevents script compilation from running away |
| **Regex** | based on `script.painless.regex.limit-factor` | Caps regex complexity |
| **Synonym** | follows the parent breaker limit | When loading synonym analysis |
...
In modern ES operations, `true` is the default and is called the **real memory circuit breaker**. Elastic blog: [Improving Node Resiliency with the Real Memory Circuit Breaker](https://www.elastic.co/blog/improving-node-resiliency-with-the-real-memory-circuit-breaker).
...
![Memory allocation on a single Elasticsearch node — JVM Heap, off-heap, OS Page Cache](/uploads/theory/es-memory/host-ram-breakdown.svg)
...
- [Elastic — Advanced configuration (JVM heap, 50% rule, compressed oops threshold)](https://www.elastic.co/guide/en/elasticsearch/reference/current/advanced-configuration.html)
- [Elastic — Index store settings (hybridfs, mmapfs, niofs)](https://www.elastic.co/guide/en/elasticsearch/reference/current/index-modules-store.html)
- [Elastic — Circuit breaker settings](https://www.elastic.co/guide/en/elasticsearch/reference/current/circuit-breaker.html)
- [Elastic — Circuit breaker errors (troubleshooting)](https://www.elastic.co/docs/troubleshoot/elasticsearch/circuit-breaker-errors)
- [Elastic — High JVM memory pressure (troubleshooting)](https://www.elastic.co/docs/troubleshoot/elasticsearch/high-jvm-memory-pressure)
...
- [A Heap of Trouble: Managing Elasticsearch's Managed Heap (compressed oops principle)](https://www.elastic.co/blog/a-heap-of-trouble)
- [Improving Node Resiliency with the Real Memory Circuit Breaker](https://www.elastic.co/blog/improving-node-resiliency-with-the-real-memory-circuit-breaker)
- [Elasticsearch heap size usage and JVM garbage collection (Elastic Labs)](https://www.elastic.co/search-labs/blog/elasticsearch-heap-size-jvm-garbage-collection)
- [Elasticsearch caching deep dive](https://www.elastic.co/blog/elasticsearch-caching-deep-dive-boosting-query-speed-one-cache-at-a-time)
...
**Previous**: [Why OS Page Cache Decides ES Performance](/blog/theory/es-memory-04-page-cache)

### Recommended companion reads

- [Generational Heap Structure on the JVM](/blog/theory/es-memory-01-jvm-heap) — Young/Old Gen, Eden/Survivor starting point
- [GC Algorithms and Stop-the-World on the JVM](/blog/theory/es-memory-02-gc) — G1 / ZGC / Shenandoah comparison
- [JVM Off-heap and Direct Memory](/blog/theory/es-memory-03-off-heap) — memory Netty/Lucene use outside the heap
- [Why OS Page Cache Decides ES Performance](/blog/theory/es-memory-04-page-cache) — the Linux kernel angle
- [Understanding the JVM and Garbage Collection](/blog/theory/jvm-and-gc) — textbook-style write-up of the broader JVM architecture
