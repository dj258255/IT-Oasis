...
처음에는 각각 별개의 용어처럼 보입니다. 그런데 성능 테스트나 장애 분석을 해보면 이 숫자들은 대부분 연결되어 있습니다.

예를 들어 API가 느려졌다고 해봅시다.
...
그래서 각각의 정의를 외우는 것보다 **어디에서 이 지표를 만나고 다른 지표와 어떤 관계가 있는지** 이해하는 쪽이 오래 남습니다. 이 글은 `Latency → Throughput → Concurrency → Connection Pool → Cache → CPU → Availability`를 그 관계로 이어 정리합니다.

## 1. Latency: 요청 하나가 얼마나 걸립니까?
...
이 요청의 latency는 120ms입니다.

Spring Boot Actuator를 붙이면 Spring MVC와 WebFlux 요청에 대해 `http.server.requests`라는 metric이 기본으로 생성됩니다. Grafana나 Prometheus를 다루다 보면 latency라는 개념을 특히 자주 만나게 됩니다.
...
`p50 = 30ms`, `p95 = 120ms`, `p99 = 850ms`라면 대부분의 요청은 빠르지만 일부 요청에서 상당한 지연이 생긴다는 뜻입니다.

### Spring 개발자는 어디서 만납니까?
...
그래서 "API가 느리다"는 말을 들으면 첫 질문은 보통 이것이 됩니다.

> 전체 latency 중 **어디에서 시간이 소비되고 있는가?**

## 2. Throughput: 얼마나 많이 처리할 수 있습니까?
...
여기서 중요한 점이 하나 있습니다. **Latency가 낮다고 throughput이 반드시 높지는 않다.** 요청 하나를 10ms에 처리하지만 동시에 하나밖에 못 하는 서버와, 요청 하나에 50ms가 걸려도 수백 개를 동시에 처리하는 서버는 특성이 전혀 다릅니다.

그래서 성능 테스트에서 "응답 시간이 100ms입니다"만 기록하면 부족합니다. 최소한 이 정도는 함께 봐야 합니다.
...
## 3. Latency와 Throughput은 언제 충돌할까요?

여기서부터 트레이드오프가 시작됩니다. 대표적인 예가 batching입니다.
...
## 4. Concurrency와 Parallelism은 다릅니다

**Concurrency** 는 여러 작업이 같은 시간 구간에 **진행 중인 상태**입니다. **Parallelism** 은 여러 작업이 실제 같은 순간에 **동시에 실행되는 것**입니다.
...
왼쪽은 parallelism입니다. 오른쪽은 하나의 CPU에서도 여러 작업을 번갈아 처리하는 concurrency입니다.

Spring에서는 이 개념이 아주 자주 등장합니다.
...
> 동시 처리량을 계속 높이면 성능도 계속 좋아질까요?

그렇지 않습니다.

## 5. Concurrency를 계속 높이면 무슨 일이 생길까요?

처음에는 concurrency를 높이면 CPU나 DB가 놀던 시간을 채울 수 있습니다. 그런데 어느 순간 시스템이 처리할 수 있는 한계에 도달합니다.

![동시성을 높이면 처리량은 어느 지점부터 평평해지고 지연은 그 뒤로 치솟는 모습](/uploads/theory/reading-performance-metrics/perf-concurrency-knee.svg)

처음에는 concurrency 증가와 함께 throughput도 증가합니다. 자원이 포화되면 throughput은 더 이상 크게 늘지 않습니다. 기다리는 요청만 늘어나면서 latency가 급격히 오릅니다.

그래서 "스레드를 100개에서 1,000개로 늘렸습니다"는 그 자체로 성능 개선이 아닙니다. **어떤 자원을 기다리는 1,000개의 작업이 되었는가**를 봐야 합니다.

## 6. DB Connection과 Connection Wait

Spring + JPA/JDBC 개발자가 정말 자주 만나는 사례입니다. Spring Boot는 DataSource에 대해 active, idle, maximum, minimum connection gauge를 제공하고 HikariCP를 쓰면 `hikaricp` metric도 별도로 제공합니다.
...
여기서 **connection wait**가 등장합니다. pool이 maximum size에 도달하고 idle connection이 없으면 `getConnection()` 호출은 connection이 반환될 때까지 기다립니다. `connectionTimeout`을 넘기면 timeout됩니다.
...
Controller 코드를 아무리 최적화해도 큰 효과가 없습니다. 요청이 CPU를 쓰는 자리가 아니라 **DB connection을 기다리는 자리**에 걸려 있기 때문입니다. 커넥션 풀 자체를 더 깊이 보려면 [DB 커넥션 풀](/blog/theory/db-connection-pool) 편을 참고하면 됩니다.
...
결국 대기열의 **위치만 옮긴 것**일 수도 있습니다. HikariCP의 공식 pool sizing 문서도 connection을 무작정 늘리는 접근을 경계하면서 실제 workload로 크기를 측정하라고 권합니다.

그래서 봐야 하는 것은 `connection = 30` 하나가 아닙니다.
...
## 8. Cache Hit Rate: DB까지 가지 않은 요청은 얼마나 됩니까?
...
그중 Redis에서 90번 찾았다면 `Cache Hit Rate = 90%`입니다. 캐시를 붙인 이유가 DB 부하를 줄이기 위한 것이었다면 중요한 지표입니다.
...
이렇게 연결될 수 있습니다. 그래서 장애 상황에서 "DB가 갑자기 느려졌습니다"라고 할 때 DB 자체가 원인이 아닐 수 있습니다. 캐시 hit rate가 먼저 떨어졌고 그 결과 DB traffic이 늘었을 수도 있습니다. 캐시 계층을 더 보려면 [Redis 캐싱](/blog/theory/redis-caching-guide) 편을 참고하면 됩니다.

## 9. Hit Rate가 높으면 무조건 좋습니까?

이것도 아닙니다. `Hit Rate 99%`라도 캐시 value 하나가 매우 크면
...
두 단어는 반드시 구분해서 알아야 합니다.

**Utilization** 은 CPU가 얼마나 사용되고 있는지를 나타냅니다. `CPU Utilization = 70%`처럼 봅니다.

**Saturation** 은 CPU를 쓰고 싶은 작업이 있는데 CPU가 부족해 **기다리는 상태**를 말합니다.
...
CPU는 열심히 일하고 있습니다. 중요한 것은 아래쪽입니다. **CPU를 원하는 작업이 줄을 서기 시작했다.** Brendan Gregg의 USE Method도 자원을 볼 때 `Utilization`, `Saturation`, `Errors`를 함께 확인하라고 정리합니다. Saturation은 처리할 수 없는 추가 작업이 queue에 쌓인 정도로 설명됩니다.

## 11. CPU 70%니까 CPU 문제는 아닙니까?

이렇게 판단하면 위험합니다. 모니터링 시스템이 5분 평균을 보여준다고 해봅시다.
...
몇 초 동안 CPU가 100%에 도달하며 queue가 만들어졌어도 평균값에서는 70%로 보일 수 있습니다. Brendan Gregg도 낮아 보이는 평균 utilization이 짧은 burst의 saturation을 숨길 수 있다고 말합니다.
...
**Availability** 는 사용자가 서비스를 **사용할 수 있는가**에 초점을 둡니다.

서비스가 정상적으로 요청을 받을 수 있는 상태입니다.

**Reliability** 는 서비스가 일정 기간 동안 **의도한 동작을 올바르게 수행하는가**에 가깝습니다. 결제 서버를 생각해봅시다.
...
HTTP 서버 자체는 살아 있습니다. 그런데 사용자가 기대한 결제 처리는 올바르게 수행되지 않았습니다. 그래서 운영에서는 단순 uptime 외에도
...
## 13. 결국 이 지표들은 하나의 장애에서 만납니다

상품 조회 API의 p95 latency가 갑자기 `100ms → 1,800ms`가 되었다고 해봅시다.

처음 발견한 것은 latency입니다. 그다음 traffic을 확인합니다.
...
CPU가 충분히 남아 있습니다. 그런데 HikariCP를 보니
...
여기서 "Connection Pool이 부족합니다. 30에서 100으로 올리자"라고 바로 결론 내리면 안 됩니다. DB를 확인해보니
...
API latency는 **결과**였고 connection pool saturation은 **중간 증상**이었습니다. 더 앞쪽의 원인은 cache miss 증가였습니다. 이렇게 지표를 연결해서 보는 것이 성능 분석의 핵심입니다.

## 14. Spring 백엔드에서 어디를 먼저 봅니까?

두 가지 틀을 같이 기억하면 편합니다. 서비스에서는 RED를 봅니다.
...
Spring API라면 자연스럽게 `RPS`, `HTTP 5xx`, `p50 / p95 / p99`로 연결됩니다. 문제가 아래 계층으로 내려갔다면 CPU, Memory, Disk, Network, Connection Pool, Thread Pool 같은 자원에서 `Utilization`, `Saturation`, `Errors`를 확인합니다. Brendan Gregg도 RED를 서비스 관점에서 Request Rate, Errors, Duration으로, USE를 자원 관점에서 Utilization, Saturation, Errors로 정리합니다.
...
CPU를 기다리습니까? DB connection을 기다리습니까? DB lock을 기다리습니까? 외부 API를 기다리습니까? Redis 응답을 기다리습니까? Thread Pool에서 실행 순서를 기다리습니까?

이 질문을 따라가면 "서버가 느립니다"에서 훨씬 구체적인 원인으로 내려갈 수 있습니다.

## 16. 성능 개선에서 숫자 하나만 보면 안 되는 이유

예를 들어 최적화 후 "CPU 사용률이 53% 감소했다"고 합시다. 이것만으로는 좋은 결과인지 알 수 없습니다. Throughput까지 절반으로 떨어졌을 수도 있습니다.
...
같은 1,000 RPS를 처리하면서 CPU 사용량, CPU 대기, DB connection 대기, tail latency가 동시에 내려간 것을 확인하고 나서야 설명이 강해집니다.

## 17. 결국 성능 튜닝은 트레이드오프를 고르는 일입니다

성능 용어를 공부하면서 크게 느낀 점은 **좋은 숫자 하나를 만드는 것이 목적이 아니라는 것**입니다. 거의 모든 결정에 반대쪽 비용이 있습니다.

![값을 올리면 무엇을 얻고 무엇을 내는지 여섯 쌍. 초록이 얻는 쪽이고 빨강이 대가다](/uploads/theory/reading-performance-metrics/perf-tradeoff-chains.svg)

그래서 성능 테스트에서 물어야 할 질문은 "빨라졌는가?" 하나가 아닙니다.
...
부터 시작합니다. 이 두 질문을 기준으로 보면 latency, throughput, concurrency, cache hit rate, connection wait, CPU saturation 같은 용어가 따로 떨어진 개념으로 보이지 않습니다. 하나의 요청이 시스템을 통과하면서 남기는 **연결된 신호**로 보이기 시작합니다.
