...
> 근거 등급: `E1·축소`
> 출처: [SEC, Securities Exchange Act Release No. 69655 (2013-05-29), Admin. Proc. File No. 3-15339, In the Matter of The NASDAQ Stock Market, LLC](https://www.sec.gov/litigation/admin/2013/34-69655.pdf) · [Nasdaq Equity Trader Alert #2012-20](https://www.nasdaqtrader.com/TraderNews.aspx?id=ETA2012-20)
...
명령서 문단 17에 숫자가 하나 있습니다. 그날 가격과 수량 계산에 검증까지 더해 20밀리초가 걸렸고, 이것이 평소 IPO 크로스의 1~2밀리초보다 훨씬 길었다는 것입니다. 원문은 "The time that had elapsed during the price/volume calculation and validation check was 20 milliseconds, which is significantly longer than usual for an IPO cross, which usually takes 1 to 2 milliseconds"라고 적습니다. 평소의 1~2밀리초였다면 그 사이에 취소가 끼어들 확률이 훨씬 낮았을 것이고, 20밀리초로 늘어난 창이 루프의 조건이었습니다.

건수는 층위를 나눠 읽어야 합니다. 문단 27에 따르면 크로스에 포함되지 못한 38,000건이 넘는 주문은 오전 11시 11분부터 11시 30분 9초 사이에 들어온 것입니다. 크로스 계산이 11시 11분까지 받은 주문과 취소를 근거로 돌았기 때문에, 그 뒤 19분치가 통째로 빠졌습니다. 그중 약 8,000건은 11시 30분 연속매매가 개시될 때 시장으로 들어갔고, 남은 3만여 건이 나스닥이 말하는 stuck 주문입니다. 38,000과 30,000은 같은 수를 두 번 센 것이 아니라 층위가 다릅니다. 38,000은 크로스에서 빠진 전체이고, 30,000은 그중 갈 곳이 없어 묶여 있던 나머지입니다.

사건 서술의 출처는 1차 문서이고 재현은 축소판이라 등급을 `E1·축소`로 적었습니다. 사실 관계는 위 두 문서에서만 가져왔습니다. 자주 "주문 정정 때문"이라고 옮겨지는데 원문은 일관되게 취소(cancellation)입니다. "38,000건 이상"도 미체결이 아니라 크로스 미포함입니다.
...
처음에는 이것을 라이브락이라고 불렀습니다. 그 이름은 물립니다. 라이브락은 스레드들이 서로의 상태 변화에 반응하면서 함께 헛도는 상태인데, 이 재현에는 그 되먹임 고리가 없습니다. 취소 유입 스레드는 [app/Cross.java](https://github.com/dj258255/incident-lab/blob/main/sessions/F07-nasdaq-ipo-livelock/app/Cross.java)의 `feed()`에서 `next += intervalNanos`로 다음 발사 시각을 절대 시간으로 밀어 놓고 그때까지 자는 고정 타이머입니다. 계산 스레드가 재계산을 몇 번을 돌든 취소가 오는 속도는 달라지지 않습니다. 한쪽이 다른 쪽에 반응하지 않으니 라이브락의 정의에 맞지 않습니다.
...
실제 코드에는 저 `continue` 뒤에 상한 검사가 한 줄 더 있습니다. 위 조각은 규칙만 보이려고 그 줄을 뺐고, 전체는 [app/Cross.java](https://github.com/dj258255/incident-lab/blob/main/sessions/F07-nasdaq-ipo-livelock/app/Cross.java)에 있습니다.
...
이 글에서 말하는 "확정 실패"의 뜻을 여기서 못박아 둡니다. 영원히 확정되지 않는다는 뜻이 아니라 `compose.yml`이 준 `-DcapMs=1000`에 걸렸다는 뜻입니다. 같은 파일의 `-DmaxRecalc=200`은 세 회차를 통틀어 한 번도 걸리지 않았습니다. 상한을 10초나 60초로 늘렸을 때 어떻게 되는지는 7절에서 따로 쟀습니다. 아래 24회는 "1초 안에 확정하지 못한 횟수"로 읽어야 정확합니다. 그러니 아래 24회는 "1초 안에 확정하지 못한 횟수"로 읽어야 정확합니다.
...
여기서 오해할 만한 우연을 하나 끊어 둡니다. 이 세션의 라운드 1회(R)는 8~22ms 범위에서 움직이고, SEC 명령서가 적은 그날의 계산 20밀리초와 자릿수가 같습니다. 이것은 우연입니다. 원문의 20밀리초를 목표로 잡고 맞춘 것이 아니고, 나스닥의 평소 값인 1~2밀리초를 재현한 것도 아닙니다. 이쪽 R은 주문 8,000건에 400틱을 전부 훑는 이중 루프가 2 vCPU 공유 호스트에서 얼마나 걸리는지의 값일 뿐입니다. 두 숫자를 나란히 놓고 "비슷하게 나왔다"고 읽으면 안 됩니다.

1회차 실행의 버그 구간 출력 전체입니다. 8개 구간을 모두 싣습니다.
...
![F07 버그 실행 화면](/uploads/incident/nasdaq-ipo-livelock/01-livelock.png)
...
3회 실행을 합치면 1초 안에 확정하지 못한 시도가 72회 중 24회입니다. 유입 간격별로는 2ms 7/9, 4ms 9/9, 8ms 6/9, 12ms 2/9이고 16ms 이상은 0/9입니다. 실패한 24회는 전부 1,000ms 시간 상한에 걸린 것이고 재계산 200회 상한에 걸린 시도는 한 번도 없었습니다. 그동안 돈 재계산은 2ms 구간에서 회차별 중앙값 62회, 112회, 84회, 4ms 구간에서 58회, 92회, 100회였습니다. 백 번 가까이 같은 계산을 다시 하고도 그 1초 동안에는 크로스가 하나도 확정되지 않았다는 뜻입니다. 명령과 출력 원문은 [reproduce.md](https://github.com/dj258255/incident-lab/blob/main/sessions/F07-nasdaq-ipo-livelock/reproduce.md)에 그대로 남겼습니다.
...
마지막 줄은 근거로 세우면 안 되는 칸이라 따로 뺐습니다. 32ms와 48ms 구간의 시도는 첫 라운드에서 바로 확정돼 9~13ms 만에 끝납니다. 그러면 측정 구간이 유입 간격보다 짧으니 취소가 거의 도착하지 않습니다. 같은 유입 간격의 해소 A 실행에서 3회 시도 합계 취소 도착이 32ms에서 2건, 48ms에서 0~1건이었고, 버그 3회차 32ms는 아예 한 건도 도착하지 않아 실측비율 칸이 `-`로 찍혔습니다. 표본 0~1건으로 나눈 값이 2.55나 3.55로 나온 것이라, 이 칸의 비율은 유입률의 측정치가 아니라 "라운드 하나가 끝날 때까지 취소가 오지 않았다"를 다르게 적은 것에 가깝습니다. 이 여섯 칸을 "비율이 높아서 안전했다"는 칸과 한데 묶으면 순환논증이 됩니다. 비율이 높아서 성공한 것이 아니라, 성공해서 일찍 끝났고 그래서 비율이 높게 계산됐기 때문입니다.
...
이 표의 마지막 줄을 사건의 규모로 옮겨 읽으면 안 됩니다. 여기서 스냅샷 동결이 얼려 두는 창은 라운드 하나, 이 호스트에서 10~20ms입니다. 그 창에 걸린 취소가 24회 시도를 합쳐 71건, 41건, 43건입니다. 같은 선택을 실제 나스닥이 했을 때 창은 라운드 하나가 아니라 오전 11시 11분부터 11시 30분 9초까지 19분이었고, 그 창에 걸린 것이 1절에서 버그의 피해로 적은 바로 그 38,000건입니다. 두 수치는 다른 사건의 수치가 아니라 같은 지표를 다른 창 길이로 잰 값입니다. 1절의 38,000건을 버그가 낸 손해로만 읽고 4절의 71건을 해소의 대가로 읽으면 앞뒤가 맞지 않습니다. 검증을 들어내 크로스를 확정시킨 순간부터 38,000건은 버그의 피해가 아니라 해소 A의 청구서입니다.

그래서 이 세션이 잰 71건을 "스냅샷 동결의 대가는 수십 건"으로 기억하면 자릿수를 세 개 잘못 짚습니다. 대가의 크기를 정하는 것은 방식이 아니라 입력을 얼마나 오래 얼려 두느냐입니다. 이 재현이 71건에서 끝난 이유는 설계가 안전해서가 아니라 라운드가 20ms 안에 끝났기 때문입니다.

차이가 어디서 벌어지는지도 분명합니다. 이 세션은 스냅샷 동결을 처음부터 설계로 넣어서, 얼리는 시점과 확정하는 시점 사이가 라운드 하나입니다. 나스닥은 루프가 이미 19분을 돈 뒤에 응급 조치로 같은 선택을 했고, 그때 확정에 쓰인 입력은 이미 19분 묵은 것이었습니다. 같은 방식이라도 언제 켜느냐에 따라 청구서가 10ms치가 되기도 하고 19분치가 되기도 합니다.
...
![F07 해소 실행 화면](/uploads/incident/nasdaq-ipo-livelock/02-fixed.png)

*그림 2. 해소 실행 화면입니다. 버그가 상한에 걸리던 2ms, 4ms, 8ms 구간에서 두 방식 모두 확정 실패 0회입니다. 노란 숫자가 대가입니다.*

![크로스 확정 실패 횟수 전후 비교](/uploads/incident/nasdaq-ipo-livelock/03-commit-failures.png)
...
"확정 실패는 영원히 확정되지 않는다는 뜻이 아니라 `-DcapMs` 상한에 걸렸다는 뜻입니다. 상한을 늘렸을 때 결국 확정되는지는 재지 않았습니다"라고 적어 두었습니다. 쟀습니다.
...
**상한을 60배로 늘리니 확정 실패가 0이 됩니다.** 이 현상은 수렴 자체가 안 되는 것이 아니라 **유한하지만 매우 긴 수렴**이었습니다.

### 실패는 전부 한 구간에 몰려 있습니다

취소 유입 간격 여덟 구간 가운데 **2ms 구간 하나에서만** 실패가 납니다. 나머지 일곱 구간은 세 조건 모두 0회입니다.
...
앞의 두 조건에서 **재계산 횟수가 `maxRecalc` 값과 정확히 같습니다.** 200과 2,000입니다. `capMs`가 1,000ms인데 경과가 755ms이므로 시간 상한에는 닿지도 않았습니다. 10,000ms 조건도 6,440ms에서 멈췄습니다. **둘 다 시간이 아니라 횟수에 걸렸습니다.**

세 번째 조건에서는 12,000회 상한에 닿기 전 1,517회에 확정됐습니다. 그때 걸린 시간이 5,126ms입니다. 취소가 한 건도 오지 않는 라운드를 **1,500번 넘게 시도한 끝에 얻은 것**입니다.

### 그러면 이름은 어떻게 되는가

앞 절들은 이것을 라이브락이 아니라 기아 또는 수렴하지 않는 재시도 루프라고 불렀습니다. 그 서술을 한 칸 좁혀야 합니다. **수렴하지 않는 것이 아니라 수렴이 아주 느립니다.**

취소 유입이 고정 타이머라 되먹임 고리가 없다는 지적은 그대로입니다. 실효간격이 라운드 1회보다 짧은 구간(비율 0.54~0.62)에서만 이 일이 벌어진다는 결론도 그대로입니다. 달라지는 것은 "영원히"가 아니라 "1,500회 남짓"이라는 크기입니다.

실무로 옮기면 오히려 더 무섭습니다. **재시도 상한을 넉넉히 잡아 두면 언젠가는 확정되지만, 그 사이 5초가 흐릅니다.** 나스닥의 그날은 그 5초가 아니라 19분이었습니다.

### 이 절의 한계

`capMs`와 `maxRecalc`를 같은 비율로 함께 올렸습니다. 그래서 어느 상한이 결정적이었는지는 앞의 두 조건에서만 갈립니다(재계산이 상한값과 같으므로 횟수 쪽). 세 번째 조건은 둘 다 안 걸렸습니다. 하나씩만 올리는 조건은 만들지 않았습니다.

조건마다 1회 실행이고, 이 호스트의 버그 구간 실패율 자체가 회차마다 흔들립니다. 앞 절의 값은 72회 중 24회이고 이 회차의 1,000ms 조건은 24회 중 3회입니다.
...
실제 나스닥 매칭엔진이 아니라 SEC 문서가 서술한 규칙 하나를 축소 구현한 것입니다. 사건의 시각과 건수를 재현하지 않았고, 재현할 수 있다고 주장하지도 않습니다. 이 글의 이름표도 한 번 물렸습니다. 처음 제목과 본문은 이 현상을 라이브락이라고 불렀는데 재현 코드에 되먹임 고리가 없어 그 규정을 내렸습니다. 주소와 그림 파일 이름에는 `livelock`이 그대로 남아 있습니다. 링크를 깨지 않으려고 두었을 뿐이고, 그 낱말이 이 세션의 결론은 아닙니다. 크로스 가격 결정은 체결 가능 수량 최대화와 동가 시 저가 선택까지만 구현했고, 실제 단일가 규칙의 불균형 최소화나 기준가 근접 규칙은 넣지 않았습니다. 한계 가격의 부분 체결 배분도 다루지 않아, 취소가 반영되지 않은 주문은 "체결됐다"가 아니라 "확정된 크로스 가격에서 체결 조건을 만족한다"까지만 셌습니다.
...
그래서 조치의 절반이 조직 처방입니다. 기술변경 프로세스에 법무와 규제 부서를 참여시키고, Change Approval Board 역할을 명문화하고, 거래시스템 통합 테스트 전담 QA 조직을 신설했습니다. CEO가 이행을 서면 인증하고 이사회가 수리하는 절차까지 붙었습니다.

제재는 censure와 민사 제재금 **1000만 달러**입니다. 거래소 대상으로는 당시 사상 최대였습니다.

**그리고 이 사건이 업계 규칙이 됐습니다.** SEC가 2014년 채택한 Regulation SCI의 채택 릴리스 각주 291이 이 사건을 근거로 명시합니다. SCI 엔티티에 용량·무결성·복원력에 관한 정책 수립, 시스템 문제 발생 시 SEC 통지, BCP 테스트, 연간 검토를 의무화했습니다.

회원 보상은 6200만 달러 프로그램이었는데, 같은 Reg SCI 릴리스 각주가 **UBS 단독 손실을 3억 5600만 달러**로 적고 있습니다. 자릿수가 다릅니다.


### 회차를 반복해 흔들림의 크기를 쟀습니다

본문이 "이 호스트의 버그 구간 실패율 자체가 회차마다 흔들립니다"라고 적었는데 얼마나 흔들리는지는 안 적혀 있었습니다. 상한 스윕 전체를 3회 반복했습니다.

| 값 | 회차별 | 폭 |
|---|---|---|
| 확정 실패 건수 (24회 중) | 1, 2, 3 | **100%** |
| `maxRecalc` 상한에 닿은 건수 | 1, 2, 3 | 100% |
| 조건별 실패 (0또는1) | 0, 0, 1 | 100% |

**같은 조건을 세 번 돌리면 확정 실패가 1건에서 3건까지 나옵니다.** 24회 시행 중 값이라 비율로는 4%에서 12.5%입니다.

이 폭이 뜻하는 것이 분명합니다. **조건 사이의 차이가 3건 미만이면 그 비교는 성립하지 않습니다.** 본문 8절의 다섯 조건에서 실패 건수가 0건과 1건으로 갈린 자리가 있는데, 그 차이는 이 폭 안입니다. 방향만 읽어야 한다고 적어 둔 것이 맞았고, 이제 그 근거가 서술이 아니라 숫자입니다.

반대로 **폭이 안 겹치는 비교는 인용할 수 있습니다.** 앞 절에서 실측비율 0.64 이하 8개 구간이 모두 실패를 냈고 1.22 이상 4개 구간이 모두 무사한 것은 이 폭으로 설명되지 않습니다. 회차 폭은 실패 건수를 흔들지만 실패가 나느냐 마느냐를 뒤집지는 않았습니다.

## 못 한 것

- **8절의 다섯 조건은 각각 1회 실행입니다.** 유입 간격 8구간을 3회씩 시도한 값입니다. 조건 전체를 3회 반복한 결과는 위 절에 있고, 확정 실패 건수의 회차 폭이 100%라 3건 미만의 차이는 비교할 수 없습니다.
- **실제 나스닥 엔진을 재현하지 않았습니다.** 크로스 규칙, 호가 배분, 이중화 전환 절차는 다루지 않았고 사건의 수치(11시 5분 10초, 38,000건)를 재현한 것도 아닙니다.
- **라운드 1회 8~22ms가 SEC 명령서의 20밀리초와 자릿수가 같은 것은 우연입니다.** 원문의 값을 목표로 맞춘 것이 아니고 나스닥의 평소 값 1~2밀리초를 재현한 것도 아닙니다. 두 숫자를 나란히 놓으면 안 됩니다.
- **2 vCPU 공유 호스트에서 쟀습니다.** 취소 유입이 없을 때의 계산 1회가 회차별로 19.4ms, 9.4ms, 9.9ms로 벌어졌습니다. 유입 간격을 절대 시간으로 읽으면 안 되고 그 시도에서 실제로 잰 라운드 시간과 함께 봐야 합니다.
- **검증을 유지한 채 수렴시키는 해소는 재지 않았습니다.** 해소 A와 B는 둘 다 검증할 대상을 줄이는 쪽입니다. 검증을 그대로 두고 재계산 횟수에 상한을 걸어 그 시점의 계산을 확정해 버리는 설계나, 취소를 우선순위 큐에 넣어 오래 기다린 것부터 반영하는 설계는 구현하지 않았습니다.

---

재현에 쓴 compose 파일과 실행 출력 원문은 [incident-lab 저장소의 F07 세션](https://github.com/dj258255/incident-lab/tree/main/sessions/F07-nasdaq-ipo-livelock)에 있습니다.
