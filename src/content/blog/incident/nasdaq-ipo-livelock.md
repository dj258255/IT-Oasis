...
이 글에서 말하는 "확정 실패"의 뜻을 여기서 못박아 둡니다. 영원히 확정되지 않는다는 뜻이 아니라 `compose.yml`이 준 `-DcapMs=1000`에 걸렸다는 뜻입니다. 같은 파일의 `-DmaxRecalc=200`은 세 회차를 통틀어 한 번도 걸리지 않았습니다. 상한을 10초나 60초로 늘렸을 때 결국 확정되는지, 아니면 그때도 못 하는지는 재지 않았습니다. 그러니 아래 24회는 "1초 안에 확정하지 못한 횟수"로 읽어야 정확합니다.
...
- **어느 상한이 결정적이었는지 하나씩 갈라 재지 않았습니다.** 7절은 `capMs`와 `maxRecalc`를 같은 비율로 함께 올렸습니다. 앞의 두 조건은 재계산이 상한값과 같아 횟수 쪽이 걸린 것이 분명하지만, 시간 상한만 올린 조건은 만들지 않았습니다.
