...
**채번 방식을 바꿉니다.** `AUTO` 대신 `IDENTITY`를 쓰면 MySQL의 `AUTO_INCREMENT`를 그대로 쓰므로 별도 커넥션이 사라집니다. 잃는 것은 JDBC 배치 INSERT입니다. 원 사례처럼 대량 적재가 목적이면 이 대가가 큽니다. 같은 시리즈의 [JPA 목록 API의 세 함정](/blog/incident/jpa-list-api)에서 `saveAll`이 `IDENTITY` 때문에 느려지는 쪽을 다뤘습니다.

세 번째를 이 세션에서 측정하지 않았습니다. 아래 "못 한 것"에 적었습니다.
