...
그래서 두 번째 커넥션은 50건에 한 번만 필요합니다. 데드락 조건이 상시로 성립하지 않고 간헐적으로만 성립합니다. 첫 요청만 40ms이고 이후 3~8ms인 것도 여기서 나옵니다. 첫 요청이 채번 커넥션을 여는 비용을 냅니다.
...
해소책 첫 번째의 대가는 인스턴스 수만큼 곱해진다는 것입니다. 풀 24는 인스턴스 하나를 기준으로 계산된 값이라, 앱 컨테이너를 3대까지 띄워 봤습니다.
...
**즉시 조치와 자동화된 감시는 공개 글에 없습니다.** 원인 분석과 처방 중심이고 장애 당시 대응 절차는 적혀 있지 않습니다.
