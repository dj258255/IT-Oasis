...
![ERROR 1503](/uploads/incident/timeseries-partition/fig-1503.png)
...
![INSERT 피해](/uploads/incident/timeseries-partition/chart-oltp-impact.png)
...
지운 공간은 같은 테이블의 새 행이 재사용할 수는 있지만 OS로 돌아가지는 않습니다. 돌려받으려면 `OPTIMIZE TABLE`로 테이블을 다시 만들어야 하는데, 이건 사실상 전체 복사라 원본만 한 여유 공간과 그동안의 부하를 각오해야 합니다. 이 세션에서는 `OPTIMIZE TABLE`을 돌려 보지 않았고, 그래서 회수량도 소요 시간도 재지 않았습니다. 디스크 사용률 경보가 울리는 상황이라면 DELETE 한 문장은 답이 되지 않는다는 것까지가 이 실험이 보인 것입니다.
...
![MDL 대기 행렬](/uploads/incident/timeseries-partition/fig-mdl.png)

(이 그림의 제목에 `LOCK=NONE`이 적혀 있는데, 아래에 적듯 실제로 실행한 문장에는 그 절이 없습니다. 그림을 다시 만들기 전까지 어긋난 채로 둡니다.)
...
프루닝이 깨지는 게 함수 래핑만의 문제가 아닙니다. 더 근본적인 조건이 있습니다. MySQL의 파티셔닝에는 전체 파티션을 하나로 덮는 글로벌 인덱스가 없습니다. `KEY idx_live_created (live_id, created_at)`를 만들어도 그것은 파티션마다 따로 존재하는 로컬 인덱스입니다.

그래서 `WHERE live_id = ?`처럼 파티션 키가 조건에 없는 조회는 프루닝을 못 받고, 파티션 수만큼 인덱스 탐색이 곱해집니다. 파티션 15개면 B-트리 루트에서 리프까지 내려가는 일을 15번 하고 그 결과를 합칩니다. 파티션을 잘게 쪼갤수록 삭제는 싸지고 이런 조회는 비싸집니다. 일 단위로 자르면 1년 보관에 파티션이 365개가 됩니다.
...
금액은 계산하지 않았습니다. 이 세션에 인스턴스 클래스도 스토리지 타입도 정하지 않았고, 로컬 NVMe에서 잰 시간을 클라우드 요금으로 환산할 근거가 없습니다. 구조만 적습니다.
...
### 관측 창을 480초로 맞춰 다시 쟀습니다

원래 측정은 DELETE 480초, DROP 120초였습니다. "삭제 후" 평균에 섞이는 정상 구간의 길이가 달라 같은 조건의 값으로 비교할 수 없었습니다. 둘 다 480초로 맞춰 다시 쟀습니다.
...
**창을 맞추자 근거가 옮겨집니다.** 앞에서 "삭제 후" p95 8.8ms 대 5.3ms를 근거로 삼았는데 그 값은 창이 450초 대 110초라 성립하지 않았습니다. 같은 창에서 재니 **삭제 후가 5.0ms와 5.1ms로 사실상 같습니다.**

차이는 삭제 후가 아니라 **삭제 중**에 있습니다. DELETE는 22.42초 동안 p95가 6.8ms에서 15.9ms로 2.3배가 되고, DROP은 0.09초 만에 끝나 그 구간에 203건밖에 안 잡힙니다. **29,971 대 203이라는 관측 건수가 이 표의 핵심입니다.** DELETE는 249배 긴 시간 동안 부하를 물립니다.

이 글의 결론은 바뀌지 않습니다. 다만 근거가 "삭제 후에도 오래 아프다"에서 **"삭제 중에 훨씬 오래 아프다"**로 정확해졌습니다.
...
프루닝 질의는 다섯 조건 모두 5,479행을 돌려줍니다. 결과 집합이 같으므로 나란히 놓을 수 있습니다.

### 프루닝이 듣는 질의는 파티션 수와 무관합니다

0.95ms부터 1.06ms까지, 파티션이 없을 때와 365개일 때가 사실상 같습니다. `created_at`에 인덱스가 있으면 파티션이 없어도 그 하루를 바로 찾아갑니다.
**"파티션이 조회를 빠르게 한다"는 통념이 이 조건에서는 성립하지 않습니다.**

### 파티션 키가 없는 조회는 파티션 수에 비례해 나빠집니다

단건 조회가 0.11ms에서 1.23ms로 **11.2배**입니다. `EXPLAIN`의 `partitions` 컬럼이 365개를 그대로 찍습니다. 파티션 키(`created_at`)가 조건에 없으므로 어느 파티션에 있는지 모르고 전부 뒤져야 합니다.

PK를 `(id, created_at)`로 둔 것이 그 원인입니다. MySQL은 파티션 키가 모든 유니크 키에 포함되기를 요구하므로, 일 단위 RANGE 파티션을 쓰면 PK에 날짜가 끌려 들어옵니다.
...
`watch_log_plain`에는 `created_at` 단독 인덱스가 없습니다. PK는 `id`이고 보조 인덱스는 `(live_id, created_at)`뿐입니다. 그래서 `scripts/exp-chunked.sh`가 도는 `DELETE FROM watch_log_plain WHERE created_at < '2026-07-28' LIMIT 10000`은 어디부터 읽을지 알려 주는 인덱스가 없어 매번 테이블 앞에서부터 다시 훑습니다. 앞쪽에는 직전 청크가 이미 지운(그리고 아직 퍼지되지 않은) 행이 쌓여 있으므로, 청크가 진행될수록 버려야 하는 잔해가 길어집니다. 재스캔이 누적되는 구조입니다.

본문 앞에서 인용한 pt-archiver는 이렇게 돌지 않습니다. 인덱스 순서를 따라가며 직전 청크가 끝난 자리부터 이어서 훑는 방식이라 같은 재스캔이 생기지 않습니다. 그래서 이 46초를 "청크 삭제는 느리다"의 근거로 쓰면 안 됩니다. 인덱스 없이 자른 청크 삭제가 느린 것입니다. pt-archiver를 직접 돌려 비교하지는 않았습니다.
...
원인을 설명하지 못했습니다. 처음에는 "파티션 키가 NULL인 행은 가장 낮은 파티션에 들어가므로 범위 비교로 뺄 수 없다"고 적었는데, 이 글의 데이터로 반증됩니다. `schema.sql`의 `created_at`은 `DATETIME(3) NOT NULL`이라 NULL인 행이 존재할 수 없습니다. 자기 스키마가 자기 설명을 부정하는 상태였습니다.

지금 걸려 있는 가설은 파티션 식이 컬럼이 아니라 `TO_DAYS(created_at)` 함수라는 점입니다. 옵티마이저가 함수를 통과시켜 경계를 계산할 때 보수적으로 한 칸 더 남긴 것으로 보입니다. 다만 이건 추정입니다. 파티션 식을 바꿔 가며 비교하는 실험을 돌리지 않았고, 실행으로 특정하지 못했습니다. 원인을 단정하지 않고 관측만 남깁니다.
...
4절은 "열린 트랜잭션 뒤에 DDL이 서면 그 뒤의 SELECT까지 갇힌다"를 보였는데, **DDL 없이 열린 트랜잭션만 있는 조건**을 재지 않았습니다. 그러면 갇힌 원인이 DDL인지 열린 트랜잭션 자체인지 갈리지 않습니다. 셋을 나란히 놓았습니다.
...
**A가 89ms입니다.** 열린 트랜잭션이 25초를 붙잡고 있어도 다른 SELECT는 지나갑니다. 읽기끼리는 MDL이 공유이기 때문입니다. B의 14,994ms와 168배 차이이고, 그 차이를 만드는 것이 대기 행렬에 낀 배타 MDL 하나입니다. **이제 4절의 결론에 대조군이 붙습니다.**
...
**빈 테이블을 재구축한 205ms입니다.** DELETE가 남긴 공간을 되찾는 비용이 아닙니다. 이 값을 인용하면 안 됩니다. 건진 것은 MySQL이 돌려준 note 하나입니다.

```
Table does not support optimize, doing recreate + analyze instead
```

InnoDB는 `OPTIMIZE TABLE`을 그대로 지원하지 않고 `ALTER TABLE ... FORCE`로 바꿔 실행합니다. 재구축이므로 온라인 DDL이 적용되고, 그래서 그동안 SELECT가 72ms로 지나갔습니다. **DELETE 뒤 공간 회수에 드는 실제 비용은 데이터가 든 상태에서 다시 재야 합니다.**

## 9. MDL 반복 측정과 데이터가 든 OPTIMIZE TABLE
...
### 파티션 수의 비용이 `table_open_cache`에 좌우되는지 쟀습니다

본문의 11.2배는 조건마다 1회씩이었고 `table_open_cache` 조합은 안 쟀다고 적었습니다. 파티션마다 테이블스페이스 파일이 하나씩 붙으니 이 설정이 배수를 정할 수 있습니다. 조건마다 새 컨테이너를 띄우고 3회씩 쟀습니다.

| 파티션 | `table_open_cache=400` | `table_open_cache=4000` |
|---|---|---|
| 1개 | 54ms | 59ms |
| 32개 | 67ms | 69ms |
| 256개 | 122ms | 124ms |

**같은 파티션 수에서 `table_open_cache`를 열 배로 늘려도 안 달라집니다.** 400과 4000이 전 구간에서 회차 폭 안입니다.

**갈리는 것은 파티션 수 쪽입니다.** 1개에서 256개로 가면 54ms가 122ms로 2.3배입니다.

그러니 본문의 배수는 파일 핸들 캐시가 만든 것이 아닙니다. 파티션이 늘면 플래너가 프루닝을 해도 검토할 대상이 늘고, 그 비용이 남습니다. PostgreSQL 문서가 "each partition requires its metadata to be loaded into the local memory of each session"이라고 적은 것과 같은 층의 비용입니다.

`table_open_cache`를 올려서 해결되는 문제가 아니라는 것이 이 표의 결론입니다.


### `OPTIMIZE TABLE`과 MDL 대기를 3회 반복했습니다

9절의 `OPTIMIZE TABLE`이 규모 하나에서 1회 실행이었습니다. 전 과정을 3회 돌렸습니다.

| 값 | 중앙 | 회차별 | 폭 |
|---|---|---|---|
| `OPTIMIZE TABLE` 소요 | 2,979ms | 2,968 / 2,979 / 4,278 | **44.0%** |
| 조건 A(열린 트랜잭션만) SELECT | 66ms | 58 / 66 / 86 | **42.4%** |

**둘 다 폭이 40%를 넘습니다.** 본문의 `OPTIMIZE TABLE` 소요를 소수점까지 인용하면 안 되고, 4,278ms와 2,968ms 사이 어디쯤으로 읽어야 합니다.

**그래도 결론은 안 바뀝니다.** 이 세션이 `OPTIMIZE TABLE`로 말하려던 것은 "`DELETE`는 파일을 안 줄이고 `OPTIMIZE TABLE`은 줄이지만 그동안 테이블을 통째로 다시 쓴다"입니다. 3초든 4.3초든 그 사이 테이블이 잠긴다는 사실은 같고, `DROP PARTITION`의 0.12초와는 자릿수가 다릅니다.

**조건 A의 SELECT 58ms도 마찬가지입니다.** 이 값이 86ms까지 흔들려도, 같은 표에서 열린 트랜잭션과 DDL이 겹쳤을 때의 20.7초와는 비교가 안 됩니다. **회차 폭이 결론을 뒤집는지가 기준이고, 여기서는 안 뒤집습니다.**

같은 회차에서 UUID 세션도 3회 돌렸는데 그쪽은 결과 파일이 회차마다 완전히 같았습니다. 벤치가 고정 시드로 도는 구조라 반복해도 새로 재는 것이 없습니다. **그런 스크립트는 반복 대상이 아니고, 반복했다고 적으면 안 됩니다.**

## 못 한 것

- **9절의 `OPTIMIZE TABLE`은 규모 하나입니다.** 200만 행 하나이고 회수 시간이 행 수에 어떻게 붙는지 곡선은 안 그렸습니다. 3회 반복 결과는 위 절에 있고 회차 폭이 44%입니다.
...
- **파티션 수 실험은 조건마다 1회씩입니다.** 단건 조회의 11.2배는 `table_open_cache`와 `innodb_open_files` 설정에 좌우될 수 있는데 그 조합은 재지 않았습니다.
...
재현에 쓴 compose 파일과 실행 출력 원문은 [incident-lab 저장소의 R17 세션](https://github.com/dj258255/incident-lab/tree/main/sessions/R17-timeseries-partition)에 있습니다.
