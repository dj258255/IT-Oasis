...
- **페이지 충전율을 직접 재지 않았습니다.** 이 글의 공간 효율은 `DATA_LENGTH`를 행 수로 나눈 값에서 역산한 것이라 리프 페이지 안의 실제 채움 비율이 아닙니다. 페이지를 한 장씩 열어 보는 도구를 쓰지 않았으므로, 무너진 공간 중 얼마가 리프 페이지의 빈자리이고 얼마가 비리프 페이지와 미사용 할당분인지는 가르지 못했습니다.
