...
본문은 첫 실행에서 활성 세션의 21%를 분류하지 못했고, 재실행에서는 미분류가 0건이라 그 조건을 재현하지 못했다고 적었습니다. 후보를 셋으로 갈라 각각 만들어 봤습니다.

| 조건 | 활성 세션 합 | 미분류 | 비율 |
|---|---|---|---|
| 소비자 켬 / 행 락 대기 부하 | 100 | 20 | **20%** |
| 소비자 끔 / 행 락 대기 부하 | 100 | 100 | 100% |
| 소비자 켬 / CPU만 쓰는 부하 | 100 | 100 | 100% |

**소비자가 정상으로 켜진 상태에서도 20%가 미분류입니다.** 본문의 21%와 같은 자리입니다.

세 조건이 서로 다른 것을 말합니다.

**소비자를 끄면 100%입니다.** 이건 본문 뒤쪽에서 다룬 함정이고 화면이 통째로 거짓말하는 경우입니다. 21%는 이게 아닙니다.

**CPU만 쓰는 부하도 100%입니다.** 대기 이벤트가 없으니 당연하고, 이 경우는 **미분류가 아니라 CPU로 읽는 것이 맞습니다.** Performance Insights도 같은 해석을 합니다. 샘플러가 이 둘을 구분하지 않으면 CPU 구간이 통째로 "모름"이 됩니다.

**남는 20%가 진짜 미분류입니다.** `threads`에서는 활성인데 같은 순간 `events_waits_current`에 진행 중인 행이 없는 세션입니다. 대기가 막 끝나 `END_EVENT_ID`가 채워지는 찰나이거나, 두 테이블을 한 문장 안에서 조인해도 스냅숏이 정확히 같은 순간이 아니어서 생깁니다.

**그러니 미분류는 고칠 수 있는 결함이 아니라 샘플링의 구조적 성질입니다.** 매초 한 번 찍는 도구는 찍는 순간과 대기 상태가 바뀌는 순간을 맞출 수 없습니다. 실제 Performance Insights의 AAS 계산도 같은 성질을 갖고, 그래서 표본 사이에서 벌어진 일은 정의상 없는 것으로 처리됩니다.

본문이 "이름을 남기도록 고쳤지만 재실행에서는 미분류가 0건"이라고 적었는데, **0건이 나온 것은 그 실행의 부하가 대기를 길게 잡는 종류여서 찰나에 걸릴 확률이 낮았기 때문으로 보입니다.** 부하의 성격이 미분류 비율을 정합니다.
