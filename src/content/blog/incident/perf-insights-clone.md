...
### 대기 분해가 전부 `cpu`인 것이 이 글의 한계를 다시 보입니다

핫 로우 UPDATE 458 샘플이 전부 `cpu` 입니다. 락 경합이 있는 워크로드인데
`wait/lock` 으로 잡히지 않습니다. 앞에서 "락인지 진짜 IO 인지 갈리지 않는다"고 적은
...
**앞의 둘은 같은 값을 줍니다.** 21과 28로 완전히 같습니다. `sys` 뷰가 `data_lock_waits`를 포함한 여러 테이블을 조인한 것이니 당연합니다. 갈리는 것은 비용이고, **2.6배 비쌉니다.**
...
비용이 비슷해 보이지만 형태가 다릅니다. 텍스트가 중앙 10,327바이트이고 매번 파싱해야 합니다. 이번 실행에서는 잘림이 0/40회였지만 트랜잭션이 많으면 잘립니다.
