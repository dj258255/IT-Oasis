...
| blocked_until만 target=100 | 0.21초 | 209 |
| 네 컬럼 모두 target=1000 | 0.74초 | **1,152** |
| blocked_until만 target=1000 | 0.71초 | **275** |

**`ANALYZE` 소요는 두 조건이 같습니다.** target=1000에서 0.74초와 0.71초입니다. 표본 크기가 같으니 스캔 비용도 같습니다.

**저장되는 값의 개수만 갈립니다.** 1,152개와 275개로 4.2배입니다. 네 컬럼을 함께 올리면 컬럼마다 MCV와 히스토그램을 최대 1,000개씩 저장하고, 한 컬럼만 올리면 그 컬럼만 저장합니다. **컬럼을 고르는 것으로 아끼는 것은 스캔이 아니라 통계의 양과 그것을 읽는 플래너의 시간입니다.**
...
- **플래너 시간을 직접 재지 않았습니다.** 7절에서 저장되는 값이 1,152개와 275개로 갈리는 것은 확인했지만, 그 차이가 실제 플래닝 시간에 얼마인지는 안 쟀습니다.
