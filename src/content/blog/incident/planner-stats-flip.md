...
![A09 재현 실행 화면](/uploads/incident/planner-stats-flip/01-flip.png)

*그림 1. statistics target 1에서 ANALYZE를 돌린 직후입니다. null_frac이 1로 잡히고, 같은 컬럼의 실제 non-null은 2,400행이며, 추정 1행을 믿은 플랜이 7.7초를 씁니다.*

## 3. 내부 원리

PostgreSQL의 ANALYZE는 테이블 전체를 읽지 않고 표본을 뜹니다. 표본 크기는 `300 x statistics target`이고, 중요한 것은 이 값이 컬럼별이 아니라 ANALYZE 한 번에 대해 하나로 정해진다는 점입니다. 분석 대상 컬럼들의 target 중 최댓값이 표본 크기를 결정합니다. 그래서 문제 컬럼 하나만 낮추면 표본은 줄지 않습니다. 실측으로 확인했습니다. `blocked_until`만 target 1로 두고 나머지를 기본값 100으로 남기면 표본은 여전히 30,000행이고, null_frac은 0.9998, 추정 행수는 2,400으로 정확하게 나옵니다([results/09-one-column.txt](https://github.com/dj258255/incident-lab/blob/main/sessions/A09-planner-stats-flip/results/09-one-column.txt)).
...
실측이 그렇게 나옵니다. target 100이면 곱은 30,000 x 0.0002 = 6건입니다. 단순 무작위 표본이었다면 여섯 건이 통째로 빠져 null_frac이 1로 잡히는 것은 좀처럼 보기 어려운 쪽입니다. 그런데 [results/41-sampling-summary.txt](https://github.com/dj258255/incident-lab/blob/main/sessions/A09-planner-stats-flip/results/41-sampling-summary.txt)의 실측은 10회 중 1회입니다. 자릿수가 다릅니다. 정확한 확률은 이 세션에서 계산하지 않았지만 방향은 분명합니다. 곱이 예측하는 것보다 훨씬 자주 틀립니다.
...
![표본 크기별 오판 빈도](/uploads/incident/planner-stats-flip/04-sampling.png)
...
값은 공짜가 아닙니다. 같은 테이블에서 target별 ANALYZE 소요 시간을 3회씩 쟀습니다([results/60-analyze-cost.txt](https://github.com/dj258255/incident-lab/blob/main/sessions/A09-planner-stats-flip/results/60-analyze-cost.txt)).
...
![A09 해소 재계측 실행 화면](/uploads/incident/planner-stats-flip/02-fixed.png)
...
![조인 쿼리 실행 시간 전후](/uploads/incident/planner-stats-flip/03-exec-time.png)
...
## 7. null_frac=1의 확률, 컬럼 하나만 올리기, 자동 analyze
...
non-null이 들어 있는 블록 = 17개
```

**2,400행이 단 17블록에 몰려 있습니다.** 시드가 순차로 넣기 때문입니다. 전체 블록의 0.026%입니다.
...
**세 값이 자릿수는 물론 소수점까지 가깝습니다.** `target=1`은 표본이 300행인데 non-null 비율이 0.02%라 기대 포획 수가 0.06건입니다. **한 건도 못 잡는 것이 정상이고 잡는 쪽이 드뭅니다.** 40회 중 39회로 잰 앞 절의 사다리와도 같은 비율입니다.

### 이 글이 지적한 이상은 다른 target에서 나온 것입니다

3절이 "곱이 예측하는 것보다 훨씬 자주 나온다"고 적은 **10회 중 1회는 `target=100` 값입니다.** 표본이 30,000행이고 기대 포획 수가 6건이므로 단순 무작위라면 `e^-6 = 0.25%`입니다. 실측 10%는 그보다 **40배** 잦습니다.

**이 실험은 그 자리를 답하지 못했습니다.** `target=1`을 200회 쟀고, 거기서는 이론과 실측이 맞습니다. 이상이 있는 곳은 `target=100`이고 그 조건은 시행을 늘려 다시 재야 합니다.

블록 뭉침으로 설명하려던 가설도 이 데이터로는 안 맞습니다. `target=100`이면 64,868블록 중 30,000개를 읽으므로 17개 블록을 전부 놓칠 확률이 `(1-30000/64868)^17`, 약 0.0034%입니다. 단순 무작위보다 **더 작습니다.** **뭉침은 반대 방향으로 작용합니다.** 원인은 여전히 열려 있습니다.

### 컬럼을 고르면 스캔이 아니라 통계의 양이 줄어듭니다

`ANALYZE`의 표본 크기는 컬럼별이 아니라 테이블 단위로 `300 × (대상 컬럼 target의 최댓값)`입니다. 그러면 한 컬럼만 올려도 표본은 같아집니다.
...
**`ANALYZE` 소요는 두 조건이 같습니다.** target=1000에서 0.71초와 0.73초입니다. 표본 크기가 같으니 스캔 비용도 같습니다.

**저장되는 값의 개수만 갈립니다.** 1,161개와 266개로 4.4배입니다. 네 컬럼을 함께 올리면 컬럼마다 MCV와 히스토그램을 최대 1,000개씩 저장하고, 한 컬럼만 올리면 그 컬럼만 저장합니다. **컬럼을 고르는 것으로 아끼는 것은 스캔이 아니라 통계의 양과 그것을 읽는 플래너의 시간입니다.**

### 자동 analyze는 관측 창 안에 안 걸렸습니다

`autovacuum_analyze_scale_factor=0.001`, `threshold=50`, `naptime=5s`로 낮추고 5,000행씩 스물네 묶음을 넣으며 기다렸습니다. **`last_autoanalyze`가 끝까지 바뀌지 않았습니다.**

1,200만 행 테이블에서 `scale_factor=0.001`이면 문턱이 12,050행입니다. 넣은 것은 최대 12만 행이라 넘겼어야 하는데 안 걸렸습니다. `naptime`을 5초로 줄였어도 autovacuum 런처가 그 테이블을 집는 데 더 걸리는 것으로 보이지만 확인하지 못했습니다. **이 항목은 여전히 못 한 것으로 남습니다.**
...
### 자동 analyze가 왜 안 걸렸는지 찾았습니다

본문 7절은 `scale_factor`를 0.001로 낮추고 12만 행을 넣었는데 `last_autoanalyze`가 안 바뀌었고, 이유를 확인하지 못했다고 적었습니다. 조건을 셋으로 갈라 다시 재니 답이 나왔습니다.

| 조건 | 대기 | 돌았는가 |
|---|---|---|
| `naptime=60s`(기본) `scale=0.001` | 60초 | 돌았음 |
| `naptime=1s` `scale=0.001` | 5초 | 돌았음 |
| `naptime=1s` `scale=0` `threshold=0` | 5초 | 돌았음 |

**설정이 문제가 아니라 관측 창이 짧았습니다.** `autovacuum_naptime`의 기본값이 60초입니다. 런처가 60초마다 한 바퀴 돌면서 대상 데이터베이스에 워커를 띄웁니다. 갱신 직후 몇 초 안에 `last_autoanalyze`를 보면 워커가 아직 한 번도 안 돈 상태라 그대로인 것이 당연합니다.

기본값 그대로 두고 60초를 기다리면 걸립니다. **7절이 "임계를 더 낮춰야 하나"를 의심했는데 낮출 필요가 없었습니다.** 재현 실험에서 autovacuum을 다루려면 `naptime`을 먼저 봐야 합니다.

## 못 한 것

- **8절의 플래너 시간은 질의 하나로만 쟀습니다.** 조인 두 개짜리 질의 하나이고, 조인이 많은 질의에서 target이 어떻게 붙는지는 안 봤습니다.
- **`target=100`의 이상은 여전히 설명되지 않습니다.** 7절에서 `target=1`은 이론과 실측이 맞는 것을 200회로 확인했지만, 3절의 10회 중 1회는 `target=100` 값이고 단순 무작위 계산의 40배입니다. 블록 뭉침은 반대 방향으로 작용해 설명이 안 됩니다.
- **원 사건의 규모와 트래픽을 재현하지 않았습니다.** 1,200만 행으로 축소했고 Clerk의 90분 복구 시간과 이 세션의 값을 직접 비교할 수 없습니다.
- **자동 analyze를 관측 창 안에 걸지 못했습니다.** 7절에서 `scale_factor`를 0.001로 낮추고 12만 행을 넣었는데도 `last_autoanalyze`가 안 바뀌었습니다. 이유는 확인하지 못했습니다.
- **2코어 호스트에서 한 조건씩 쟀습니다.** ANALYZE 소요 시간의 절대값은 이 장비에서만 성립합니다.

---

재현에 쓴 compose 파일과 실행 출력 원문은 [incident-lab 저장소의 A09 세션](https://github.com/dj258255/incident-lab/tree/main/sessions/A09-planner-stats-flip)에 있습니다.
