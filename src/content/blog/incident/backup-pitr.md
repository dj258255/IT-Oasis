...
**14절이 만든 파이프라인이 이 구조의 축소판입니다.** 복원해 보고, 행 수와 체크섬으로 대조하고, 소요가 예산 안인지 봅니다. 빠진 것은 14절 끝에 적은 소유자 하나입니다.

## 못 한 것

한계 둘은 본문 안에 적었습니다. 절대 시간을 인용할 수 없다는 것이 13절 끝이고, 파이프라인에 소유자가 없다는 것이 14절 끝입니다. 둘 다 그 자리에서 봐야 무엇의 한계인지 보이므로 여기로 옮기지 않았습니다.

여기 더할 것은 하나입니다. **초기 회차는 호스트 사양을 안 남겼습니다.** `uname -srm`과 `nproc`을 안 찍어서 어느 장비에서 돌린 회차인지 확인되지 않습니다. 지금 다시 찍어도 그 실행의 장비라는 보장이 없어 비워 뒀고, 뒤에 붙인 반복 측정부터 `meta/HOST.md`에 남깁니다.
