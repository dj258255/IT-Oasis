...
| 사고 직전 시각을 정확히 적으면 되나 | 안 됩니다. 네 엔진 모두 벽시계가 초 단위라 그 초에 커밋된 것이 에러 없이 빠집니다 | 엔진별 실측 (7절) |
| 논리와 물리 중 무엇이 빠른가 | 규모가 정합니다. 교차점이 50만과 100만 행 사이이고 물리 쪽 고정비 3초가 그 위치를 정합니다 | 네 규모 측정 (10절) |
| 백업 검사를 만들면 끝인가 | 아닙니다. 일부러 망가뜨린 백업을 함께 넣어야 그 검사가 실제로 걸리는지 알 수 있습니다 | 파이프라인 (12절) |
