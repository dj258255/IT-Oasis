...
- **엔진**: MySQL 8.4.3 · PostgreSQL 17.5 · SQL Server 2022 · Oracle 26ai. 전부 Docker 단일 노드이고 자원 한도는 안 걸었습니다
- **규모**: 1,500행에서 400만 행(덤프 525MB)까지. 사건은 PostgreSQL 300GB라 같은 메커니즘만 축소했습니다
- **측정 환경**: 2026-07-31 재측정 회차 기준 macOS 26.3.1, Apple M2 Pro 12코어 32GB, 컨테이너 VM 12코어 7.7GB. Oracle과 SQL Server는 ARM 에뮬레이션이라 시간 대신 판정을 3회 반복했습니다
- **RTO 예산**: 정해 두지 않고 기울기를 쟀습니다. 쓰는 법은 [4절](#4-논리와-물리-교차점은-규모가-정합니다)에 있습니다
...
절차대로 하면 되는 줄 알았는데 첫 명령에서 두 번 막혔습니다. 둘 다 공식 PITR 문서만 읽고 따라가면 그대로 만납니다.

![복구를 시작조차 못 하게 만드는 두 가지](/uploads/incident/backup-pitr/fig-blocked.png)

- **사고 난 서버에 덤프를 되돌리면 첫 줄에서 막힙니다.** `mysqldump`가 기본으로 넣는 `SET @@GLOBAL.GTID_PURGED`가 이미 실행된 GTID와 겹쳐 `ERROR 3546`으로 중단됩니다. 사고 복구는 대개 원본 서버에서 하려 하므로 여기서 걸립니다. 새 인스턴스에 복원하거나 `--set-gtid-purged=OFF`로 다시 뜨는 것이 해법입니다
- **공식 `mysql:8.4` 이미지에 `mysqlbinlog`가 없습니다.** 문서의 PITR 절차는 전부 이 도구로 쓰여 있는데 이미지에 안 들어 있습니다. 사고 당일에 알면 복구가 그 자리에서 멈춥니다. 이 세션은 `percona-server` 컨테이너를 따로 띄워 우회했습니다

나머지 셋(재적용 조용한 스킵, binlog 만료, 파이프가 컨테이너에 안 닿음)은 [전체 기록](https://github.com/dj258255/incident-lab/blob/main/sessions/A23-backup-pitr/README.md)에 있습니다.
...
복구 시간을 미리 알려면 남의 벤치마크가 아니라 **자기 장비의 기울기**가 필요합니다. 네 규모로 재면 그 기울기가 나옵니다.

| 행 수 | 덤프 크기 | 복원 | 크기당 |
|---|---|---|---|
| 1,500 | 1MB | 0.23초 | 참고값 |
| 200,000 | 27MB | 1.12초 | 42초/GB |
| 1,000,000 | 131MB | 4.29초 | 34초/GB |
| 4,000,000 | 525MB | 16.88초 | 33초/GB |

**100만 행을 넘으면 크기당 시간이 33초/GB 근처로 수렴합니다.** 작은 규모에서 값이 큰 것은 서버 기동 같은 고정비가 섞여서이고, 데이터가 커질수록 그 몫이 희석됩니다.

수렴한 뒤로는 곱셈입니다. **덤프 크기 × 기울기**가 예상 복원 시간이고, GitLab 규모인 300GB를 이 기울기로 늘리면 논리 복원만 2시간 45분입니다. 그 정도 시간이 걸리는 복구 수단밖에 없으면 "여섯 시간 전 스냅숏"이 현실적인 선택지가 되는 이유가 여기 있습니다.

**다만 33초라는 값 자체는 인용하면 안 됩니다.** 이 호스트의 로컬 디스크이고 네트워크 전송이 없으며 논리 복원 기준입니다. 이 절이 말하는 것은 숫자가 아니라 절차입니다. **자기 장비에서 두세 규모만 재면 기울기가 나오고, 그때부터 RTO는 추정이 아니라 계산이 됩니다.** 체크리스트 마지막 항목이 "리허설에서 실측한다. 추정하지 않는다"인 이유입니다.
...
- **XtraBackup을 처음엔 안 썼습니다.** "MySQL 8.4용 이미지를 못 구했다"고 적어 뒀는데 찾아보니 `percona/percona-xtrabackup:8.4`가 있었습니다. 안 찾아보고 못 한다고 적은 것이라 다시 쟀고, 서버를 멈추지 않고 뜬 백업으로 복원까지 됩니다. 대가는 도구입니다. `mysqldump`는 딸려 오지만 이쪽은 따로 설치해야 하고 **복원이 같은 메이저 버전으로만 됩니다**
- **네 엔진을 네 벌 자동화하지 않았습니다.** 검증 파이프라인은 MySQL 하나입니다. 이 절이 보이려는 것은 "통과만 하는 검사는 아무것도 안 지키고 있다"이고 그건 한 번 보이면 증명됩니다. 엔진마다 명령어가 다를 뿐 검사의 구조는 같습니다
- **벽시계 대신 SCN을 쓰는 쪽으로 옮겼습니다.** 처음에는 네 엔진 모두 시각으로 지점을 지정했는데 SQL Server가 회차마다 결과가 갈렸습니다. 초 단위 반올림 때문이라 `STOPATMARK`로 바꿨고, 같은 성질을 Oracle에서 다시 만나 3절이 나왔습니다
