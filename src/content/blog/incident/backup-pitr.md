...
**GitLab은 WAL 아카이빙 자체를 안 쓰고 있었습니다.** 백업 다섯 겹이 전부 안 들었다는 이야기 앞에, 시점 복구를 할 재료가 애초에 없었다는 사실이 있습니다. 사고 뒤에야 도입을 검토 항목(#1097)으로 올렸고, 이 글이 재현한 것이 그들이 그때 갖고 있지 않던 능력입니다.

**벽시계로 지점을 가리키면 네 엔진 모두 경계에서 샙니다.** MySQL `--stop-datetime`, PostgreSQL `recovery_target_time`, SQL Server `STOPAT`, Oracle `SET UNTIL TIME`이 전부 초 단위이고, 같은 초에 커밋된 것이 에러 없이 빠집니다. 안전한 대안으로 남는 것은 위치·LSN·마크·SCN처럼 로그 안의 한 항목을 정확히 가리키는 지정입니다.

그리고 복원해 보지 않은 백업은 백업이 아닙니다. **통과만 하는 검사도 마찬가지입니다.** 12절 파이프라인이 일부러 망가뜨린 백업을 함께 넣는 이유가 그것입니다.
...
**RTO 분해**: 이 실행에서 백업 0.075초, 덤프 복원 0.078초, binlog 추출·적용 0.947초입니다. 로그 원문은 각각 `.075473000` / `.077972000` / `.947246000`초입니다. 대상은 1,500행짜리 테이블 하나이고 덤프가 40K, 이어 붙인 binlog가 192K입니다. 이 규모에서는 복구 시간의 대부분이 binlog 구간을 뽑아 재적용하는 데 들어갔습니다. 백업 주기를 늘리면 그만큼 이어 붙일 binlog가 늘어나므로 RPO와 RTO는 백업 주기 하나로 맞물립니다. **다만 이 비율은 규모가 커지면 뒤집힙니다.** 11절의 20만 행 반복 측정에서 복원이 79.3%, binlog 적용이 10.5%로 갈리고, 10절에서 100만 행까지 올리면 로그 비중이 2.5%까지 내려갑니다. 이 절의 "대부분이 binlog"는 1,500행이라는 규모에서만 성립합니다. 세 구간 모두 1회 실행값이라 분산도 모릅니다.
...
8절의 체크리스트 여덟 항목을 실제로 도는 스크립트로 만들었습니다. 체크리스트는 지키는 사람이 있어야 지켜지고, 사람은 사고 당일에만 확인합니다.
...
> **13절과 14절은 근거 등급이 다릅니다.** 이 글의 나머지는 `E1·축소 재현`으로 직접 돌린 것이고, 이 두 절은 `E3·문서 대조`로 **실행이 없습니다.** 계정이 없어 AWS 문서와 GitLab 자료를 인용만 했습니다.

| 항목 | 직접 운영 | RDS |
|---|---|---|
| 복원 위치 | 같은 서버 가능 | **항상 새 인스턴스** |
| RPO 하한 | 로그 주기를 내가 정함 | **5분 고정** |
| 복원본 설정 | 원본 그대로 | **기본 파라미터 그룹**으로 뜸 |
...
- **교차점을 네 규모로 좁혔고 그 사이는 안 찍었습니다.** 50만과 100만 사이라는 것까지입니다. XtraBackup은 100만 행 1회만 쟀고 규모별 곡선은 안 그렸습니다. 10절에서 10만 행과 100만 행 사이에 있다는 것까지 재고 그 사이는 안 찍었습니다.
- **3절의 PITR 시나리오는 데이터 규모가 작습니다.** 세 구간 비율은 10절에서 100만 행까지 재서 그 서술이 어디까지 성립하는지 밝혔습니다. 사고 직전 1,500행, 덤프 40K, binlog 192K입니다. 10절에서 논리 대 물리는 100만 행까지 키워 봤지만, PITR 세 구간(백업·복원·binlog 적용)의 비율이 그 규모에서 어떻게 변하는지는 11절의 20만 행까지입니다.
