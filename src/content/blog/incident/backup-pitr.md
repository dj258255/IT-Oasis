...
| 사고 직전 시각을 정확히 적으면 되나 | 안 됩니다. 네 엔진 모두 벽시계가 초 단위라 그 초에 커밋된 것이 에러 없이 빠집니다 | 엔진별 실측 (8절) |
| 논리와 물리 중 무엇이 빠른가 | 규모가 정합니다. 교차점이 50만과 100만 행 사이이고 물리 쪽 고정비 3초가 그 위치를 정합니다 | 네 규모 측정 (12절) |
| 백업 검사를 만들면 끝인가 | 아닙니다. 일부러 망가뜨린 백업을 함께 넣어야 그 검사가 실제로 걸리는지 알 수 있습니다 | 파이프라인 (14절) |
...
이 사고의 핵심은 **백업이 있다고 믿고 있었다**는 것입니다. 복구가 실패한 것은 그 믿음의 결과입니다. Google SRE Book이 같은 말을 다르게 적습니다.
...
**RTO 분해**: 이 실행에서 백업 0.075초, 덤프 복원 0.078초, binlog 추출과 적용 0.947초입니다. 로그 원문은 각각 `.075473000` / `.077972000` / `.947246000`초입니다. 대상은 1,500행짜리 테이블 하나이고 덤프가 40K, 이어 붙인 binlog가 192K입니다.

이 규모에서는 복구 시간의 대부분이 binlog 구간을 뽑아 재적용하는 데 들어갔습니다. 백업 주기를 늘리면 그만큼 이어 붙일 binlog가 늘어나므로 RPO와 RTO는 백업 주기 하나로 맞물립니다.

다만 이 비율은 규모가 커지면 뒤집힙니다. 13절의 20만 행 반복 측정에서 복원이 79.3%, binlog 적용이 10.5%로 갈리고, 12절에서 100만 행까지 올리면 로그 비중이 2.5%까지 내려갑니다. 세 구간 모두 1회 실행값이라 분산도 모릅니다.

## 4. 복구를 가로막는 함정들

만난 것은 다섯인데 성격이 둘로 갈립니다. **셋은 어느 환경에서든 나오는 데이터베이스 쪽 함정이고, 둘은 이 랩이 컨테이너로 축소했기 때문에 나온 것입니다.** 처음에는 다섯을 한 줄로 세워 적었는데, 그러면 진짜 셋이 나머지 둘에 희석됩니다. 갈라서 적습니다.
...
두 엔진이 같은 방식으로 위험합니다. PostgreSQL 문서가 `archive_command`에 대해 없는 파일 요청에는 반드시 0이 아닌 값을 반환해야 한다고 못 박는 이유이고, 이 실험의 compose에도 `test ! -f`를 앞에 둔 이유입니다.
...
**MySQL도 같습니다.** 경계 규칙만 보려고 3행짜리 마이크로 실험을 따로 만들었습니다. 이 글의 다른 MySQL 측정과 스케일이 완전히 다르고, 여기서 볼 것은 시간이 아니라 **같은 초의 커밋이 들어오는가 빠지는가** 하나입니다. 1회 실행입니다.
...
**교차점의 위치를 정하는 것은 재기동 비용입니다.** 물리 복원에는 서버를 다시 띄우는 6초 남짓이 고정비로 붙고, 논리 복원에는 그 비용이 없는 대신 행마다 `INSERT`가 듭니다. 데이터가 작으면 6초가 커 보이고 커지면 묻힙니다. 기동이 빠른 환경이라면 교차점이 왼쪽으로 옮겨 갑니다.
...
그래서 RTO를 줄이려면 복원을 봐야 합니다. binlog 적용을 아무리 최적화해도 12% 안에서 움직입니다. 물리 백업으로 바꾸는 것이 이 축에서 나오는 선택지인데, 12절에서 본 대로 이 규모에서는 물리 복원이 더 느립니다. **규모가 커져 논리 복원의 SQL 재실행이 지배하기 시작하는 지점이 그 교환이 뒤집히는 자리입니다.**
...
10절의 체크리스트 여덟 항목을 실제로 도는 스크립트로 만들었습니다. 체크리스트는 지키는 사람이 있어야 지켜지고, 사람은 사고 당일에만 확인합니다.
...
그래서 **데이터 대조가 마지막 방어선입니다.** 이때 행 수를 "0보다 큰가"로 보면 안 되고 원본과 정확히 같은지 봐야 합니다. 12절이 그 자리를 밟았습니다. GTID 충돌로 복원이 중간에 멈췄는데 종료 코드가 0 이라 "복원 0.06초"라는 아주 좋아 보이는 값이 남았습니다.
...
> **15절과 16절은 근거 등급이 다릅니다.** 이 글의 나머지는 `E1·축소 재현`으로 직접 돌린 것이고, 이 두 절은 `E3·문서 대조`로 **실행이 없습니다.** 계정이 없어 AWS 문서와 GitLab 자료를 인용만 했습니다.
...
- 교차점을 네 규모로 좁혔고 그 사이는 안 찍었습니다. 50만과 100만 사이라는 것까지입니다. XtraBackup은 100만 행 1회만 쟀고 규모별 곡선은 안 그렸습니다. 12절에서 10만 행과 100만 행 사이에 있다는 것까지 재고 그 사이는 안 찍었습니다.
- 3절의 PITR 시나리오는 데이터 규모가 작습니다. 세 구간 비율은 12절에서 100만 행까지 재서 그 서술이 어디까지 성립하는지 밝혔습니다. 사고 직전 1,500행, 덤프 40K, binlog 192K입니다. 12절에서 논리 대 물리는 100만 행까지 키워 봤지만, PITR 세 구간(백업·복원·binlog 적용)의 비율이 그 규모에서 어떻게 변하는지는 13절의 20만 행까지입니다.
- 파이프라인에 소유자가 없습니다. 실패했을 때 누구를 깨우고 안 받으면 다음이 누구인지는 코드 밖의 문제입니다. GitLab의 5 Whys 마지막 답이 그 자리였는데 이 세션은 그 절반만 만들었습니다.
