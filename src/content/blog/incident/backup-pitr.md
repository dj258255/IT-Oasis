...
### 네 판정을 여러 회차로 돌리는 데는 이르지 못했습니다

`UNTIL TIME`의 초 단위 경계는 확정했고 `RMAN-20207`도 원문으로 확인했습니다. 그런데 **한 회차 안에서 네 판정을 이어 돌리는 설계가 취약합니다.** 판정마다 `ALTER DATABASE OPEN RESETLOGS`가 들어가고, 그 한 줄이 인케네이션이라는 전역 상태를 바꿉니다. 앞 판정이 만든 계보 때문에 다음 판정의 기준점이 사라집니다.

판정마다 기준 인케네이션으로 되돌리는 것까지 넣었지만, 두 번째 판정 이후 데이터베이스가 열리지 않는 회차가 남았습니다. 제대로 하려면 **판정 하나에 회차 하나**를 배정하고 사이에 전체 복원으로 초기화해야 하는데, 이 컨테이너가 ARM 에뮬레이션이라 한 사이클이 수십 분입니다.

그래서 이 절이 인용하는 것은 **판정 하나(사고 직전 복구가 700행을 되살린다)와 에러 원문 셋**까지입니다. 네 판정의 회차별 안정성은 확인하지 못했습니다.
...
- **Oracle 쪽 판정 반복은 회차 수가 모자랍니다.** `UNTIL TIME`의 초 단위 경계와 `RESETLOGS` 뒤 인케네이션 제약은 확인했지만, 네 판정 전체를 여러 회차로 안정되게 돌리는 데는 이르지 못했습니다.
