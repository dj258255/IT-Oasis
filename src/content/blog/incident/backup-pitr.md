...
![백업만 되돌리면 500건이 사라지고 binlog를 이어 붙이면 0건이 된다](/uploads/incident/backup-pitr/fig-pitr.png)
...
교차점 오른쪽에서 물리가 빠른데 그 속도에 값이 붙습니다. 100만 행에서 논리 덤프는 78.9MB이고 물리 백업은 540.9MB입니다. **6.9배입니다.** 물리 백업은 데이터 파일뿐 아니라 시스템 테이블스페이스와 리두 로그까지 통째로 담기 때문이고, 10만 행에서는 7.8MB 대 237.3MB로 30배까지 벌어집니다. 데이터가 작을수록 그 고정분의 비중이 큽니다.

보관 기간을 곱하면 이 차이가 그대로 저장 비용이 됩니다. 100만 행짜리를 하루 한 번 떠서 30일 보관하면 논리는 2.4GB, 물리는 16.2GB입니다. **복원 1.4초를 벌려고 13.8GB를 상시 지불하는 구조입니다.** 교차점 오른쪽이라고 물리로 가는 것이 아니라 RTO 예산과 저장 예산을 같이 놓고 정할 일입니다.

binlog가 또 한 축입니다. **보관 기간이 곧 PITR 창이라 그 기간이 되돌아갈 수 있는 범위와 저장 비용을 동시에 정합니다.** 백업 주기를 늘리고 binlog로 그 사이를 메우는 쪽이 싼지, 백업을 자주 뜨는 쪽이 싼지가 여기서 갈립니다. 100만 행에서 잰 로그 적용 시간은 전체 복구의 2.5%였고 이 값은 테이블 크기가 아니라 백업 주기에만 달려 있습니다. **주기를 늘려도 복구 시간은 거의 안 늘고 저장 비용만 줄어드는 구간이 있다는 뜻입니다.**
...
다섯 중 셋(재적용 스킵, binlog 만료, `-i` 누락)이 에러 없이 실패합니다. 나머지 둘(GTID 겹침, 도구 부재)은 시끄럽게 실패하는데, 역설적으로 그쪽이 낫습니다. ![종료 코드 0, 에러 없음, 그런데 반영은 0건](/uploads/incident/backup-pitr/fig-silent-skip.png)

**복구에서 가장 위험한 것은 실패가 아니라 성공처럼 보이는 실패입니다.** GitLab 사고에서 cron 실패 알림이 DMARC로 막혀 몇 달간 아무도 몰랐던 것과 같은 종류입니다.
