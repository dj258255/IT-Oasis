...
Oracle은 처음 정리할 때 빼 뒀습니다. "라이선스 때문에 불가능"이 아니라 "아직 안 했다"가 정확한 표현이라고 적어 뒀는데, 뒤에 `RMAN` 의 `UNTIL TIME` 과 아카이브 로그 모드 전환을 둘 다 실행해 채웠습니다.
...
**갈리는 자리는 길이가 아니라 단계입니다.** 이 랩은 데이터가 1,500행이라 12초의 대부분이 기동 시간이고, 운영 규모에서는 `SHUTDOWN IMMEDIATE` 가 열린 트랜잭션의 롤백을 기다리므로 더 길어집니다. 그보다 중요한 것은 Oracle 이 데이터베이스를 `MOUNT` 상태에 두는 단계를 하나 더 요구한다는 점입니다. 열린 채로는 바꿀 수 없습니다. PostgreSQL 은 `archive_mode` 를 켜고 재기동하면 되고, MySQL 은 `log-bin` 을 켜고 재기동하면 됩니다. **백업 전략을 나중에 바꾸려 할 때 이 한 단계가 정지 시간의 성격을 가릅니다.**
...
**예상과 반대입니다.** 논리 복원이 0.88초, 물리 복원이 3.93~4.29초로 논리가 4.5배 빠릅니다. 이 글이 "직접 재면 독자적 기여가 되는 자리"라고 적은 곳에서 나온 답입니다.

이유는 물리 복원의 비용이 데이터가 아니라 **서버 재기동**에 있기 때문입니다. 컨테이너를 멈추고 tar를 풀고 다시 띄우는데, 그 기동이 4초 남짓입니다. 10만 행짜리 SQL을 다시 실행하는 0.88초보다 비쌉니다.

백업 크기도 갈립니다. 논리는 7.8MB, 물리는 237.3MB로 **30배**입니다. 물리 백업은 데이터 파일뿐 아니라 시스템 테이블스페이스와 리두 로그까지 통째로 담습니다.

**규모가 커지면 뒤집힙니다.** 논리 복원은 행 수에 비례해 늘고 물리 복원의 재기동 비용은 대체로 고정입니다. 이 랩은 그 교차점 아래에 있습니다. 어디서 뒤집히는지는 아래 이유로 재지 못했습니다.

### 100만 행 조건은 논리 쪽이 전부 실패했습니다

100만 행에서는 논리 회차 세 번이 모두 복원 후 0행으로 나와 버렸습니다. 물리 쪽은 세 회차 모두 정상입니다.

`mysqldump`와 복원 사이에서 무언가 어긋나는데 원인을 짚지 못했습니다. **행 수를 확인해 버리는 가드가 있어서 그 회차가 "아주 빠른 복원"으로 표에 들어가지는 않았습니다.** 가드가 없던 앞선 실행에서는 실제로 "논리 복원 0.06초"라는 값이 남아 물리보다 75배 빠른 것처럼 보였습니다.

10만 행 조건도 논리는 세 회차 중 첫 회차만 성공했습니다. **위 표의 논리 복원 0.88초는 1회 관측입니다.**
...
- 재기동 비용이 지배하는 규모라 **RTO를 이 값으로 일반화하면 안 됩니다.** 말할 수 있는 것은 "백업 시간만 보고 고르면 안 되고 복원 쪽을 따로 재야 한다"까지입니다.
...
- **논리 복원의 유효 관측이 한 번입니다.** 10절에서 10만 행의 첫 회차만 성공했고 100만 행은 세 회차 모두 복원 후 0행이었습니다. 원인을 짚지 못했습니다.
