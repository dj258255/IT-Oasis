...
**이 랩의 절대 시간은 인용하면 안 됩니다.** 규모가 작고, Oracle과 SQL Server는 ARM 에뮬레이션이라 그 숫자가 이 호스트의 것도 아닙니다. 초기 회차는 호스트 사양조차 안 남겨서 어느 장비였는지 확인되지 않습니다. RTO를 주제로 하는 글에서 이건 구멍이고, 그래서 두 엔진은 시간 대신 **판정을 반복**했습니다. 세 회차가 같은지만 봅니다.

**파이프라인에 소유자가 없습니다.** 실패했을 때 누구를 깨우고 그 사람이 안 받으면 다음이 누구인지는 코드 밖의 문제입니다. GitLab의 5 Whys 마지막 답이 그 자리였는데 이 세션은 그 절반만 만들었습니다.
