...
### Oracle을 판정 중심으로 다시 재려다 막혔습니다

리뷰에서 "Oracle이 얇다"는 지적을 받았습니다. `UNTIL TIME`을 1회 돌린 수준인데 아카이브 모드 전환은 3회나 쟀으니 비중이 반대라는 것이고, 맞는 지적입니다.

그래서 판정 넷을 반복해서 재는 실험을 짰습니다. 사고 직전 시점 복구가 그 시점 행 수를 되살리는가, 사고 이후로 주면 사고까지 함께 오는가, `RESETLOGS` 뒤 옛 시점으로 못 가는가, `RESET DATABASE TO INCARNATION`으로 되돌아갈 수 있는가입니다.

**여섯 번 고쳤는데 유효 회차 2개를 못 만들었습니다.** 밟은 순서대로 적습니다.

| 막힌 지점 | 내용 |
|---|---|
| ORA-65096 | 멀티테넌트 루트는 `CREATE USER`에 `C##` 접두사를 요구합니다 |
| `UNTIL TIME`이 안 감 | 백업 뒤 생긴 리두를 아카이브하지 않으면 백업 시점에 멈춥니다 |
| ORA-01109 | `RESETLOGS`로 열면 CDB만 열리고 PDB는 `MOUNTED`로 남습니다 |
| 다음 회차 연쇄 실패 | 앞 실행이 `MOUNTED`로 끝나면 다음 실행이 기동 실패로 봅니다 |
| PDB 복구 경로 | 테이블을 CDB 루트로 옮겨도 `UNTIL TIME`이 여전히 백업 시점에 멈춥니다 |

마지막 것이 안 풀렸습니다. `ALTER SYSTEM ARCHIVE LOG CURRENT`를 넣고 CDB 루트로 옮겼는데도 복구가 백업 이후의 200행을 안 가져옵니다.

**스크립트는 이 상태를 결과로 내지 않습니다.** 유효 회차가 둘 미만이면 "반복했다고 말할 수 없습니다. 1회 값과 다를 바 없으므로 이 표는 인용하면 안 됩니다"를 찍고 끝냅니다. 회차 하나만 나온 것을 표로 만들면 그게 이 랩이 계속 경계하는 유형이 됩니다.

**그러니 이 글의 Oracle 절은 여전히 얇습니다.** 고치려다 못 고쳤고, 못 고친 자리가 어디인지까지가 지금 말할 수 있는 전부입니다. 이 컨테이너가 ARM 에뮬레이션이라 회차당 수십 분이 걸리는 것이 반복 시도를 어렵게 만든 조건이기도 합니다.
...
- **Oracle 쪽은 여전히 얇습니다.** 판정 넷을 반복해 재려고 여섯 번 고쳤지만 유효 회차 2개를 못 만들었습니다. 무엇에 막혔는지는 위 절에 적었습니다.
