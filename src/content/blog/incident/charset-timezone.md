---
title: 'utf8은 UTF-8이 아니고 CONVERT_TZ는 조용히 NULL을 준다'
 ...
 **이것은 장애 하나를 재현한 기록이 아닙니다.** 사건 재현이라면 하나의 원인과 하나의 타임라인이 있어야 하는데, 여기 담은 것은 서로 독립된 함정 여섯 개입니다. 사건 재현이 아니라 **함정 카탈로그**라고 부르는 편이 정확합니다. 근거 등급을 E2로 둔 것도 그래서고, 뒤에 나오는 전후 수치가 지연이나 처리량이 아니라 각 함정의 실패 출력과 해소 출력의 대조인 것도 그래서입니다.
...
| 호스트 사양 | 2026-07-31 재측정 회차는 macOS 26.3.1, Apple M2 Pro 12코어, 32GB에서 돌았고 컨테이너 런타임 VM은 12코어 7.7GB입니다(`results/00-host.txt`). 그 이전 회차의 호스트는 기록이 없어 확인되지 않습니다 |
| 자원 상한 | 걸지 않았습니다. 성능 측정이 아니라 동작 재현입니다 |
| 클라이언트 | mysql CLI, 접속 문자셋을 `--default-character-set=utf8mb4`로 고정 (지뢰 6이 그 이유입니다) |
| 일시 | 2026-07-29 |

호스트를 기록하지 않은 것은 이 시리즈에서 반복되는 결함이지만, **이 세션에 한해서는 결론에 영향을 주지 않습니다.** 아래 여섯 실험에 지연도 처리량도 없기 때문입니다. 시간 수치는 "실험 전체가 약 1분" 하나뿐이고 그것도 비교 대상이 없습니다. 이 글의 관측은 전부 에러 번호와 출력 문자열이라 장비가 바뀌어도 같은 결과가 나옵니다. 반대로 말하면, 이 세션에서 소요 시간을 근거로 무언가를 주장하려 했다면 그 주장은 성립하지 않습니다.
...
![strict vs non-strict](/uploads/incident/charset-timezone/fig-strict.png)
...
![CONVERT_TZ NULL](/uploads/incident/charset-timezone/fig-convert-tz.png)
...
![이중 인코딩 착시](/uploads/incident/charset-timezone/fig-mojibake.png)
...
그런데도 이 절이 필요합니다. 여섯 지뢰 중 셋(타임존 테이블, 문자셋 기본값, 전환 비용)은 **관리형이냐 아니냐에 따라 원인도 처방도 갈리고**, 나머지 셋(TIMESTAMP의 계약, 언어별 생존, 접속 문자셋)은 엔진과 클라이언트의 성질이라 어디서 돌리든 같습니다. 이 구분을 하지 않으면 이 글이 R 트랙에 있을 이유가 없습니다.

### 타임존: RDS의 위험은 "비어 있음"이 아니라 "낡음"이다

2절이 만든 상태부터 정확히 적겠습니다. mysql:8.4.3 도커 이미지는 `time_zone_name`이 1,792행 채워진 채로 옵니다. 비어 있는 상태는 기본값이 아니라 `TRUNCATE`로 만든 것입니다. "도커라서 NULL이 난다"는 말은 틀립니다.
...
RDS 절에 구조만 적고 수치를 못 적었던 세 가지를 실제로 재봤습니다.

### utf8mb4 전환을 실제로 실행했습니다
...
**`INPLACE`를 에러 1846으로 거부합니다.** [INT PK 고갈 편](/blog/incident/int-pk-exhaustion)의 `ALTER TYPE`과 같은 자리이고, 같은
문구입니다(`Cannot change column type INPLACE. Try ALGORITHM=COPY`). 문자셋 변경도 컬럼
타입 변경으로 취급되므로 테이블을 통째로 다시 씁니다. **COPY 구간에는 쓰기가 통과하지 못한다는 그 글의 결론이 그대로 적용됩니다.**
...
## 8. 전환 중 쓰기, 두 tzdata, 자바의 DST

### 전환 중 쓰기는 막히는 것이 아니라 기다립니다

`utf8mb4` 전환이 4.6초 걸리는 동안 쓰기가 어떻게 되는지 안 쟀습니다. 100만 행 테이블에 초당 30건 남짓의 쓰기를 넣고(스크립트의 `sleep 0.02`가 정하는 명목 상한은 50건입니다) `ALGORITHM=COPY` 전환을 걸었습니다.
...
**실패가 0건입니다.** 쓰기가 거부된 것이 아니라 기다렸습니다. 전환 창 안의 표본은 2건이고 9ms와 4,124ms입니다. 표본이 둘뿐이라 중앙값으로 요약하지 않고 원값을 적습니다. 긴 쪽 4,124ms가 `ALTER` 소요 4,193ms와 거의 같습니다. **전환이 시작될 때 들어온 쓰기는 전환이 끝날 때까지 통째로 멈춥니다.**

창 안의 시도가 2건뿐인 것도 같은 이야기입니다. 기준선에서는 4.2초에 130건 남짓 들어갔을 부하인데, 한 건이 4.1초를 붙잡고 있으니 그 사이 두 건밖에 시작하지 못합니다.

**에러 카운터만 보는 모니터링은 이 4.2초를 정상으로 기록합니다.** `ALGORITHM=COPY`는 원본에 공유 락을 잡아 문서상 읽기를 통과시키고 쓰기만 멈추므로 조회 기반 헬스체크는 통과할 것으로 보입니다. 다만 이 랩의 부하 루프는 INSERT만 던지고 SELECT 프로브가 없어 읽기 쪽은 재지 않았습니다. 락 대기 타임아웃(`innodb_lock_wait_timeout`, 기본 50초)보다 `ALTER`가 길어지면 그때부터 에러 1205로 바뀝니다. 이 랩은 4.2초라 그 전입니다.

### 두 tzdata를 한 서버에 두면

규칙 변경 전후의 오프셋이 갈린다는 것까지만 보였습니다. 이름이 다른 두 타임존을 한 서버에 적재해 같은 질의에 나란히 넣었습니다.

| UTC 시각 | 옛 규칙 | 새 규칙 | 차이 |
|---|---|---|---|
| 2023-07-15 12:00:00 | 2023-07-15 **07:00:00** | 2023-07-15 **06:00:00** | 1시간 |

**같은 UTC 시각인데 규칙에 따라 한 시간이 갈립니다.** 멕시코는 2022년 10월 하계 시간제를 폐지했고, tzdata 2022b 이전 데이터에는 그 폐지가 없어 2023년 여름 오프셋이 -5시간으로 남습니다.

서버 두 대의 tzdata가 다르면 이 두 열이 **서로 다른 서버의 답**이 됩니다. 로그의 시각과 DB의 시각이 한 시간 어긋나는 사고가 여기서 나옵니다. 한 서버 안에서 재현했으므로 "버전이 다르면 갈린다"가 아니라 **"규칙이 다르면 갈린다"**를 직접 보인 것입니다.

이 규칙은 IANA 데이터를 그대로 옮긴 것이 아니라 이 실험이 보이려는 차이만 담은 최소 규칙입니다.
...
**예외 없이 03:30으로 옮겨집니다.** `LocalDateTime.of(2026,3,8,2,30).atZone(ny)`가 조용히 한 시간 뒤를 돌려줍니다. MySQL이 `03:00`과 같은 순간을 돌려준 것과 같은 성격입니다. **두 계층 모두 존재하지 않는 시각을 거부하지 않습니다.**
...
**`atZone`의 기본값은 앞의 것(EDT)입니다.** 뒤의 것을 고르려면 `withLaterOffsetAtOverlap`을 명시해야 합니다. 안 쓰면 **매년 이 한 시간에 들어온 요청이 한 시간 앞으로 기록됩니다.**
...
**오프셋을 버리면 두 순간이 한 값으로 뭉갭니다.** MySQL의 `DATETIME`이 정확히 이 상태이고, `TIMESTAMP`는 UTC로 저장해 구별합니다. 이 글이 DB 쪽에서 본 것과 애플리케이션 쪽에서 본 것이 같은 그림입니다.
...
WordPress가 2015년 4.2에서 기존 사이트를 utf8mb4로 옮긴 절차가 공개돼 있습니다. **어려웠던 것이 인코딩이 아니라 767바이트 인덱스 벽이었습니다.**

이 세션이 2절에서 잰 그 벽입니다. WordPress는 이렇게 적습니다.
...
**191이라는 숫자가 지금도 코어 소스에 주석과 함께 박혀 있습니다.**
...
옮길 수 없는 사이트를 위해서는 **두 번째 저장 표현**을 유지했습니다. "If a site can't be upgraded to utf8mb4, we convert emoji to their HTML-encoded equivalent, and store that, instead."(utf8mb4로 못 올리면 이모지를 HTML 인코딩 형태로 바꿔 대신 저장한다) 적용 대상은 글 제목, 본문, 요약, 사이트 제목, 설명으로 한정했습니다.
...
인덱스가 너무 커도 MySQL이 항상 에러를 내지는 않으니 자동 테스트에 기대지 말라는 것입니다. **조용한 실패를 벤더가 직접 경고한 자리입니다.**

**대가는 곧 청구됐습니다.** Trac #32649의 제목이 "Index length optimization in 4.2 made one of the core queries unable to use it" 입니다. 인덱스를 191로 줄이자 코어 쿼리 하나가 그 인덱스를 못 쓰게 됐습니다.

그리고 **10년 뒤인 6.9(2025-11)에서야 신규 설치의 기본 `DB_CHARSET`이 utf8mb4가 됐습니다.** 4.2는 기존 사이트를 옮겼을 뿐 기본값은 한동안 그대로였습니다.


### 전환 중 쓰기가 1205로 바뀌는 지점을 찾았습니다

본문 8절은 전환 창 안의 표본이 두 건뿐이었고, 락 대기 타임아웃을 넘기는 규모에서 1205로 바뀌는 지점은 재지 않았다고 적었습니다. 행 수를 키우며 전환 중 0.5초 간격으로 쓰기를 시도했습니다.

**`lock_wait_timeout = 5초`**

| 행 수 | 전환 소요 | 시도 | 성공 | 1205 |
|---|---|---|---|---|
| 10만 | 0.3초 | 5 | 5 | 0 |
| 50만 | 1.2초 | 6 | 6 | 0 |
| 200만 | 4.0초 | 6 | 6 | 0 |

**`lock_wait_timeout = 1초`**
...
**경계는 행 수가 아니라 전환 소요와 `lock_wait_timeout`의 대소입니다.** 200만 행에서 전환이 4.1초인데, 타임아웃이 5초면 전부 기다렸다가 성공하고 1초면 두 건이 1205를 받습니다. 같은 테이블 같은 전환인데 설정 하나로 갈립니다.

**그리고 타임아웃 안에서는 실패가 아니라 지연으로 나타납니다.** 성공한 6건도 최대 4초를 기다렸습니다. 애플리케이션 쪽 타임아웃이 그보다 짧으면 DB는 성공을 돌려주는데 클라이언트는 이미 끊은 상태가 됩니다.

`lock_wait_timeout`의 기본값이 31536000초, 곧 1년이라는 것이 여기서 걸립니다. **기본값 그대로면 1205가 영원히 안 나오고 대신 무한정 기다립니다.** 전환 작업 전에 이 값을 짧게 잡고 재시도를 붙이는 것이 [MDL 폭풍 편](/blog/incident/mdl-storm)의 처방과 같은 이유입니다.
...
- **최소 규칙으로 재현했습니다.** 8절의 두 타임존은 IANA 데이터를 그대로 옮긴 것이 아니라 이 실험이 보이려는 차이만 담은 규칙입니다. 실제 tzdata 두 버전을 적재해 대조하지는 않았습니다.
- **전환 창 안의 표본이 두 건입니다.** 8절에서 쓰기가 통째로 멈추는 바람에 창 안에 시도가 두 건밖에 안 들어갔습니다. 락 대기 타임아웃을 넘기는 규모에서 1205로 바뀌는 지점은 재지 않았습니다.
- **최초 회차(2026-07-29)의 호스트 사양 기록이 없습니다.** 뒤에 붙인 6·8절 회차부터는 `results/00-host.txt`에 남겼고 2절이 그것을 인용합니다.
- **타임존 사고의 E1 사례를 찾지 못했습니다.** 문자셋 쪽은 WordPress 사고가 있지만 타임존 쪽은 공식 문서 경고(E2)까지만 확보했습니다.

---

재현에 쓴 compose 파일과 실행 출력 원문은 [incident-lab 저장소의 R14 세션](https://github.com/dj258255/incident-lab/tree/main/sessions/R14-charset-timezone)에 있습니다.
