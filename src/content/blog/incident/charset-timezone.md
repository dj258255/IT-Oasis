...
**에러 카운터만 보는 모니터링은 이 4.2초를 정상으로 기록합니다.** `ALGORITHM=COPY` 는 원본에 공유 락을 잡아 문서상 읽기를 통과시키고 쓰기만 멈추므로 조회 기반 헬스체크는 통과할 것으로 보입니다. 다만 이 랩의 부하 루프는 INSERT 만 던지고 SELECT 프로브가 없어 읽기 쪽은 재지 않았습니다. 락 대기 타임아웃(`innodb_lock_wait_timeout`, 기본 50초)보다 `ALTER`가 길어지면 그때부터 에러 1205로 바뀝니다. 이 랩은 4.2초라 그 전입니다.
