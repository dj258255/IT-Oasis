...
**실패가 0건입니다.** 쓰기가 거부된 것이 아니라 기다렸습니다. 전환 창 안의 표본은 2건이고 9ms 와 4,124ms 입니다. 표본이 둘뿐이라 중앙값으로 요약하지 않고 원값을 적습니다. 긴 쪽 4,124ms 가 `ALTER` 소요 4,193ms 와 거의 같습니다. **전환이 시작될 때 들어온 쓰기는 전환이 끝날 때까지 통째로 멈춥니다.**

창 안의 시도가 2건뿐인 것도 같은 이야기입니다. 기준선에서는 4.2초에 130건 남짓 들어갔을 부하인데, 한 건이 4.1초를 붙잡고 있으니 그 사이 두 건밖에 시작하지 못합니다.

**에러 카운터만 보는 모니터링은 이 4.2초를 정상으로 기록합니다.** `ALGORITHM=COPY`가 원본에 공유 락을 잡아 문서상 `ALGORITHM=COPY` 는 읽기를 통과시키고 쓰기만 멈추므로 조회 기반 헬스체크는 통과할 것으로 보입니다. 다만 이 랩의 부하 루프는 INSERT 만 던지고 SELECT 프로브가 없어 읽기 쪽은 재지 않았습니다. 락 대기 타임아웃(`innodb_lock_wait_timeout`, 기본 50초)보다 `ALTER`가 길어지면 그때부터 에러 1205로 바뀝니다. 이 랩은 4.2초라 그 전입니다.
