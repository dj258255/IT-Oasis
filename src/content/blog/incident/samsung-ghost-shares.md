...
> 근거 등급: `E1·축소`
> 출처: 금융위원회의 검사 결과와 제재 의결이 공개 기록으로 사건의 실재를 뒷받침합니다. **의결서 원문 링크는 확보하지 못했습니다.** 주문을 낸 22명과 실제 체결 물량은 다른 수치인데 체결 쪽은 이 글에서 따로 확인하지 않았습니다.

## 1. 유명한 이유

2018년 4월 6일 삼성증권은 우리사주 배당을 지급하면서 주당 1,000원 대신 주당 1,000주를 입고했고, 그렇게 총 28.1억 주가 직원 계좌에 찍혔습니다. 발행주식총수를 수십 배 넘는 수량인데 원장이 받아들였습니다. 직원 22명이 31분 동안 그 유령주식의 매도 주문을 냈고 그 가운데 일부가 체결돼 주가가 장중 -11.7%까지 밀렸으며, 금융위원회는 검사 후 업무 일부정지 6개월 등을 부과했습니다. 
...
**매도가 35~39% 싸집니다.** 매도는 합계를 줄이므로 검증이 필요 없는데 현재 설계는 매번 전체 `SUM`을 돕니다. `WHEN` 절이나 전이 테이블로 걸러 내면 그 스캔이 사라집니다.

매수 쪽은 네 설계가 20.1~30.9ms로 비슷합니다. 검증을 어차피 해야 하는 경우라 방식의 차이가 작습니다. **D만 20.1ms로 눈에 띄는데, `SUM` 대신 한 행을 갱신하기 때문입니다.**
...
**교차행 불변식이 한 행 안으로 들어와 `CHECK` 범위가 됩니다.** 3절이 "`CHECK`로는 표현할 수 없다"고 적은 것은 `holdings` 여러 행에 걸친 합계를 그 테이블의 `CHECK`로 쓰려 할 때이고, 합계를 `company` 한 행에 물질화하면 표현이 가능해집니다.
...
**단일 실행에서 가장 빨랐던 B가 동시 실행에서 가장 느립니다.** 20.8ms였던 것이 3,102~6,131ms입니다. C도 22.1ms에서 1,095~2,113ms로 벌어집니다. 반대로 A와 D는 76ms로 단일 실행 총 소요의 2~3배에 그칩니다.

이 역전의 원인은 짚지 못했습니다. 매도만 던지므로 B의 `WHEN` 절과 C의 델타 검사가 둘 다 검증을 걸러 내야 하고, 그러면 advisory lock도 안 잡혀야 합니다. 그런데도 느립니다. 행 단위 트리거의 행별 평가 비용이나 전이 테이블을 만드는 비용이 동시 실행에서 드러나는 것으로 의심하지만 확인하지 않았습니다. 관측만 적고 원인은 열어 둡니다.

말할 수 있는 것은 이것입니다. **단일 문장 비용으로 설계를 고르면 안 됩니다.** 증감 구분이 매도 한 건을 39% 싸게 만들지만, 동시 부하에서는 그 이득이 뒤집힐 수 있습니다. D는 두 축에서 모두 나쁘지 않은데, 대가는 모든 입출고가 `company` 한 행을 갱신한다는 점입니다. 이 실험의 부하에서는 그 직렬화가 아직 병목이 아닙니다.

조건마다 1회 실행이고 동시성은 네 세션 하나입니다. 이 표를 3회 반복한 결과는 아래에 있고, **시간 값은 회차 폭이 98%에서 4,116%라 인용할 수 없습니다.** 실패 건수만 전 회차 0으로 안정적입니다.
...
금융위원회가 2018년 5월 28일 「배당사고 재발방지 및 신뢰회복을 위한 주식 매매제도 개선방안」을, 7월 26일 제재를 발표했습니다. **해소의 무게중심이 검증 로직이 아니라 경로 분리에 있습니다.**

가장 확실한 조치가 이것입니다.

> "증권사가 자사 우리사주조합원에 현금배당시 **은행전산망을 통한 입금처리를 의무화**하여 주식입고 가능성을 배제
> \* 금융결제원 등을 거쳐 현금만 입금할 수 있어 주식입고는 원천 불가"

**잘못된 값을 잡아내는 것이 아니라 그 값이 도달할 수 있는 경로를 지웠습니다.** 여기에 "투자자 주식 관리 부서는 우리사주조합 현금배당 업무에서 배제하고, 증권사 내 우리사주조합에 대한 현금배당과 주식배당 시스템을 분리(화면, 처리권한 등)"가 붙습니다. 사고는 현금배당 화면에서 "7.일괄대체입금"을 골라야 하는데 "1.우리사주"를 고른 것이었습니다.

검증 쪽도 들어갔지만 발행주식총수를 하드 리밋으로 박는 방식이 아닙니다. "수작업이 수반되는 입출고의 경우 금액·수량 등을 기준으로 사전 회당 처리한도를 설정하고 한도 초과시 통제 (예) **발행주식 총수 대비 5%, 종가 기준 60억원 이상**" 이고, 장 개시 전에 예탁결제원 통보 수량과 투자자계좌부 총량과 투자자별 수량 **셋을 자동 대사**합니다.

**금감원 검사 결과가 짚은 원인 중 하나가 이 세션의 주제와 정확히 겹칩니다.**

> "발행주식 총수(8천 9백만주) 이상의 주식이 입고되었음에도 오류 검증 및 입력 거부가 되지 않음
> \* \`18.1월 주전산시스템 교체 시 우리사주 배당시스템 오류검증 테스트 미실시"

나머지 절반은 조직 층입니다. 당일 실패가 이렇게 적혀 있습니다.
...
그래서 **「비상 버튼 시스템」**이 생겼습니다. 준법감시부서가 전체 임직원 계좌의 매매를 한 번에 차단하고, "상황의 긴급성 등을 감안하여 담당자에게 선조치 권한을 부여" 합니다. 37분이 걸리던 것을 버튼 하나로 만든 것입니다.

제재는 업무 일부정지 6개월과 과태료 1억 4천4백만 원입니다. 근거는 지배구조법의 내부통제기준·위험관리 비상계획 마련 의무와 전자금융거래법의 선관주의 의무입니다.

**한 가지 후퇴가 보고돼 있습니다.** 개선방안의 실시간 모니터링 시스템에서 **사전 차단 기능이 빠졌습니다.** 당시 금융위원장이 "전산시스템 상으로 불가능한 것은 아니지만 너무 많은 노역이 들어가기 때문에 그렇게 할 필요가 없다"고 말한 것으로 보도됐습니다. 사전 차단이 사후 적발로 내려간 셈입니다. **이 대목은 2차 자료 단독이라 그대로 읽으면 안 됩니다.**

이 세션은 교차행 불변식을 스키마에 넣는 쪽을 쟀습니다. 실제 해소는 그것을 포함하되 **경로를 지우는 쪽과 조직의 대응 속도 쪽에 무게가 더 실려 있습니다.**


### 세션 수를 늘려 물질화 합계의 한계를 찾아봤습니다

본문 7절은 네 세션 하나였고, 물질화 합계(D)가 76ms로 문제없다고 적었습니다. 그 값이 세션 수를 늘려도 유지되는지 재 봤습니다. 조건마다 재시작해 캐시 상태를 맞췄습니다.

| 세션 | 트랜잭션 | 총 소요 | 요청당 | 합계 검사 |
|---|---|---|---|---|
| 2 | 80 | 759ms | 9.5ms | 맞음 |
| 4 | 160 | 754ms | 4.7ms | 맞음 |
| 8 | 320 | 857ms | 2.7ms | 맞음 |
| 16 | 640 | 1,575ms | 2.5ms | 맞음 |
| 32 | 1,280 | 3,373ms | 2.6ms | 맞음 |

**32세션까지 요청당 시간이 안 늘어납니다.** 2세션에서 9.5ms인 것은 총 트랜잭션이 80건뿐이라 기동 비용이 섞인 것이고, 8세션 이후로는 2.5에서 2.7ms에 붙어 있습니다.

**한 행의 잠금이 직렬화 병목이 되는 지점을 이 범위에서는 못 찾았습니다.** 총 소요는 세션 수에 비례해 늘지만 그건 일의 양이 늘어서이고, 요청당으로 나누면 평평합니다. `company` 한 행에 대한 `UPDATE`가 짧아서 잠금 보유 시간이 트랜잭션 전체에서 차지하는 몫이 작기 때문으로 보입니다.

전 조건에서 `company.held`와 `holding` 합계가 일치했습니다. **방어가 동시성을 올려도 뚫리지 않습니다.** 더 높은 동시성이나 더 긴 트랜잭션에서 갈리는지는 여전히 안 쟀습니다.


### 네 설계 비교를 3회 반복했더니 시간을 못 쓰게 됐습니다

본문 7절의 설계 비교표가 조건마다 1회 실행이었습니다. 전 과정을 3회 돌렸습니다.

| 설계 | 가장 빠른 세션 | 가장 느린 세션 | 실패 |
|---|---|---|---|
| A. 문장 단위 + advisory lock | 92ms (74~1,087) **폭 1,101%** | 2,115ms (84~3,105) 폭 143% | 0 |
| B. 행 단위 + WHEN | 1,104ms (1,090~4,097) 폭 272% | 1,113ms (1,108~4,101) 폭 269% | 0 |
| C. 전이 테이블 | 1,108ms (84~5,108) 폭 453% | 2,121ms (94~7,121) 폭 331% | 0 |
| D. 물질화 합계 + CHECK | 74ms (69~3,115) **폭 4,116%** | 4,123ms (2,102~6,128) 폭 98% | 0 |

**시간은 인용할 수 없습니다.** D의 가장 빠른 세션이 69ms에서 3,115ms까지 나옵니다. 45배입니다. 본문이 "네 세션까지는 D가 76ms로 문제없었다"고 적었는데, 그 76ms는 운 좋은 회차의 값입니다.

네 설계의 시간 차이가 각자의 회차 폭 안에 통째로 들어갑니다. **그러니 "어느 설계가 빠른가"는 이 표로 말할 수 없습니다.**

**반대로 실패 건수는 전 회차 전 설계에서 0입니다.** 네 방어 모두 발행총수 초과를 막았고 그 판정은 흔들리지 않습니다.

이 갈림이 이 세션에서 인용할 수 있는 것과 없는 것을 정합니다. **정합성 판정은 쓰고 성능 비교는 못 씁니다.** 앞 절의 동시성 스윕에서 요청당 시간이 32세션까지 평평했던 것과 견주면, 그쪽은 같은 작업을 반복해 평균을 낸 값이고 이쪽은 세션 하나의 최댓값이라 성질이 다릅니다.

왜 이렇게 흔들리는지도 짚어 둡니다. 이 실험은 세션 넷을 동시에 띄워 서로 막게 만든 것이라, **어느 세션이 먼저 락을 잡느냐가 순전히 스케줄링 운입니다.** "가장 빠른 세션"은 그 운의 최댓값이고, 최댓값 통계는 회차마다 크게 튑니다. 앞서 MDL 편에서 초당 최대 지연이 폭 2,560%까지 벌어진 것과 같은 성질입니다.

## 못 한 것

- **의결서 원문 링크를 확보하지 못했습니다.** 금융위원회의 검사 결과와 제재 의결이 사건의 실재를 뒷받침하지만 원문을 직접 인용하지 못했습니다. 주문을 낸 22명과 실제 체결 물량도 다른 수치인데 체결 쪽은 확인하지 않았습니다.
- **물질화 합계의 직렬화 한계를 찾지 못했습니다.** 7절에서 네 세션까지는 D가 76ms로 문제없었지만, 세션을 늘려 `company` 한 행의 잠금이 병목이 되는 지점은 재지 않았습니다.
- **증감 구분이 동시 실행에서 왜 느려지는지 못 짚었습니다.** 7절에서 B와 C가 단일 실행에서는 빠른데 동시 네 세션에서는 30배에서 60배 느립니다. 관측만 있고 원인이 없습니다.
- **동시성이 네 세션 하나입니다.** 7절은 세션 수를 바꿔 가며 재지 않았습니다.
- **삼성증권의 원장에 이 불변식이 있었는지 없었는지는 공개 문서로 확인되지 않습니다.** 재현 대상은 그 회사의 원장이 아니라 조건입니다.
- **한 번의 고정 타이밍 실행입니다.** 난수도 동시성도 없어 반복 측정의 대상이 아니지만, 동시 입고가 섞이는 조건은 만들지 않았습니다.
