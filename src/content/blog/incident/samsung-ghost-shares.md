...
- **합계 컬럼을 물질화하는 대안을 구현하지 않았습니다.** `company`에 잔고 합계 컬럼을 두고 `total_shares <= issued_shares`를 걸면 CHECK 범위에 들어오지만, 모든 입출고 경로에서 그 컬럼을 정확히 갱신할 책임이 새로 생깁니다. 트리거 쪽으로 갔습니다.
- **트리거가 증감을 구분하지 않습니다.** `AFTER INSERT OR UPDATE OF shares`는 매도 차감도 검증 함수를 지나갑니다. 행 단위 트리거로 내리거나 전이 테이블로 판정하는 두 방법 모두 구현하지 않았습니다.
- **직렬화 비용을 재지 않았습니다.** 지금 구조에서는 모든 매도가 잠금 하나에 줄을 서는데 그 비용이 얼마인지 모릅니다.
