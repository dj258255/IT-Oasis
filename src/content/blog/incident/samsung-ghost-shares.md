---
title: '삼성증권 유령주식 축소 재현: 행 단위 CHECK는 잔고 합계를 막지 못한다'
titleEn: "Reproducing the Samsung Securities Ghost Shares at Reduced Scale: Row-Level CHECKs Cannot Cap a Sum"
description: '2018년 삼성증권이 우리사주 배당에서 주당 1,000원 대신 주당 1,000주를 입고했습니다. 총 28.1억 주가 직원 계좌에 찍혔고, 직원 22명이 31분 동안 매도 주문을 냈으며 그 가운데 일부가 체결돼 주가가 장중 -11.7%까지 밀렸습니다. 삼성증권의 내부 원장이 어떻게 생겼는지는 공개 기록으로 확인하지 못했습니다. 그래서 이 글이 재현한 것은 사건이 아니라 조건입니다. 제약이 행 단위 CHECK뿐인 발행총수 1,000,000주짜리 축소 원장에 같은 구조의 원/주 단위 착오를 넣었더니 아무것에도 걸리지 않고 그대로 커밋돼 잔고 합계가 발행총수의 30.66배가 됐습니다. 이 배수는 시드 구성이 정한 값이라 사건의 규모를 재현한 수치가 아닙니다. 문장 단위 총량 트리거로 롤백시켰더니 서로 다른 계좌 동시 입고 2건에서 1,010,000주로 다시 뚫렸고, advisory lock으로 검증을 직렬화해 990,000주에서 막았습니다. 이 해소가 성립하는 조건은 격리 수준이 아니라 검증 함수가 VOLATILE이라는 점입니다. STABLE로 선언하면 잠금을 잡고도 낡은 합계를 보고 조용히 통과시킵니다.'
descriptionEn: "In 2018 Samsung Securities credited 1,000 shares per share instead of 1,000 won per share in an employee stock dividend, putting 2.81 billion shares into employee accounts; 22 employees entered sell orders over 31 minutes, some of which filled, and the stock fell as much as 11.7% intraday. What Samsung's own ledger looked like is not something public records establish, so this session reproduces a condition rather than the incident. Injecting the same won-versus-share unit error into a reduced ledger whose only constraints are row-level CHECKs, nothing stopped it: the batch committed and total holdings reached 30.66x the 1,000,000 issued shares, a ratio fixed by the seed design rather than calibrated to the incident. A statement-level total-cap trigger rolled the batch back, but two concurrent credits to different accounts broke the invariant again at 1,010,000 shares, and serializing the check with an advisory lock held the line at 990,000. That fix rests on the check function being VOLATILE, not on the isolation level; declared STABLE, it reads a stale sum and passes while still holding the lock."
date: 2026-07-28
tags:
  - PostgreSQL
  - Trigger
...
![F02 버그 시나리오 실행 화면](/uploads/incident/samsung-ghost-shares/01-bug-commit.png)
...
12주를 보유한 계좌에는 12,012주가 찍혔습니다. 이어서 실보유 55주인 계좌가 유령주 50,000주 매도(잔고 차감)를 넣었는데 이것도 통과했습니다. 잔고가 55,055주로 부풀려져 있어 차감 후에도 5,055주가 남으니 음수 잔고 CHECK에 걸릴 일이 없습니다. 명령과 출력 원문은 [reproduce.md](https://github.com/dj258255/incident-lab/blob/main/sessions/F02-samsung-ghost-shares/reproduce.md)에 있습니다.
...
![F02 해소 시나리오 실행 화면](/uploads/incident/samsung-ghost-shares/02-fix-rollback.png)
...
![착오 입고 후 잔고 합계 배율 전후](/uploads/incident/samsung-ghost-shares/03-ratio.png)
...
재현에 쓴 compose 파일과 실행 출력 원문은 [incident-lab 저장소의 F02 세션](https://github.com/dj258255/incident-lab/tree/main/sessions/F02-samsung-ghost-shares)에 있습니다.
