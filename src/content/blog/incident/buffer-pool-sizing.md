---
title: '버퍼 풀을 얼마로 잡을까, 그 값으로 옮길 때 드는 정지 시간'
...
![조회당 디스크 읽기, 균등](/uploads/incident/buffer-pool-sizing/08-perquery-uniform.png)
![조회당 디스크 읽기, 핫셋](/uploads/incident/buffer-pool-sizing/09-perquery-hot.png)
...
![리사이즈 직후와 나머지 구간의 최대 지연](/uploads/incident/buffer-pool-sizing/06-resize-stall.png)
...
마지막 열은 단위를 정정한 값입니다. `docker stats`의 BlockIO를 측정 창의 처음과 끝에 찍어 뺐는데, `O_DIRECT` 조건은 3.32GB에서 5.69GB로 2.37GB 늘었습니다. `docker stats`가 쓰는 GB는 2진이 아니라 10진이라 2.37GB는 약 2,260MiB입니다. `results/08-report.txt`에 적힌 2,427MB는 `scripts/report.py`가 GB에 1024를 곱해서 나온 값이라 이 글에서는 정정해 씁니다. InnoDB 쪽 `Innodb_data_read`는 바이트를 1024로 두 번 나눈 값이라 처음부터 MiB이고, 두 열을 같은 단위로 놓아야 비교가 됩니다.

![측정 창 동안 블록 디바이스에서 읽은 양](/uploads/incident/buffer-pool-sizing/10-blockio.png)
...
비용은 재지 않았습니다. 이 세션은 로컬 컨테이너 한 대라 인스턴스 등급도 금액도 나오지 않습니다. 그래도 사이징 결정이 클라우드에서 무엇으로 바뀌는지는 적어 둘 만합니다. 버퍼 풀을 키우려면 인스턴스 메모리를 키워야 하고, 메모리는 등급에 묶여 있으니 결국 등급을 올리는 결정이 됩니다. 무릎이 512M인 이 부하에서 2G를 잡으면 메모리를 네 배 쓰고 처리량은 17% 더 얻습니다. 그 17%가 메모리 네 배 값을 하는지는 요금표를 봐야 알 수 있고, 이 글은 그 계산을 하지 않았습니다. 세션이 내놓는 재료는 무릎의 위치까지입니다. 무릎 왼쪽에서 아낀 메모리는 지연으로 되갚고, 무릎 오른쪽에서 더 쓴 메모리는 요금만 늘립니다.
...
- **캐시가 다 찬 구간은 CPU에 눌려 있습니다.** 2코어라 4,200 q/s 근처가 천장입니다. 1536M과 2G의 차이 같은 것은 이 2코어 서버에서는 드러나지 않습니다. 코어가 많은 장비에서 다시 재면 됩니다. 아직 안 했습니다.
- **O_DIRECT 조건의 블록 I/O가 InnoDB가 센 읽기보다 2.0배 큽니다**(약 2,260MiB 대 1,122MiB). 리드어헤드나 파일시스템 계층을 의심했지만 확인하지 못했습니다. 실험 3의 결론은 `fsync` 쪽이 0이라는 데 기대고 있어서 이 격차와 무관하지만, 숫자를 그대로 남깁니다.
...
| 경로 | 내용 |
|---|---|
| [compose.yml](https://github.com/dj258255/incident-lab/blob/main/sessions/A08-buffer-pool-sizing/compose.yml) | MySQL 8.4.3과 부하 컨테이너. 설정마다 왜 껐는지 주석 |
| [schema.sql](https://github.com/dj258255/incident-lab/blob/main/sessions/A08-buffer-pool-sizing/schema.sql) | `orders` 한 장 |
| [scripts/seed.py](https://github.com/dj258255/incident-lab/blob/main/sessions/A08-buffer-pool-sizing/scripts/seed.py) | 700만 행 적재 |
| [scripts/workload.py](https://github.com/dj258255/incident-lab/blob/main/sessions/A08-buffer-pool-sizing/scripts/workload.py) | 부하 생성과 계측 |
| [scripts/run-sweep.sh](https://github.com/dj258255/incident-lab/blob/main/sessions/A08-buffer-pool-sizing/scripts/run-sweep.sh) | 실험 1 |
| [scripts/run-resize.sh](https://github.com/dj258255/incident-lab/blob/main/sessions/A08-buffer-pool-sizing/scripts/run-resize.sh) | 실험 2 |
| [scripts/run-pagecache.sh](https://github.com/dj258255/incident-lab/blob/main/sessions/A08-buffer-pool-sizing/scripts/run-pagecache.sh) | 실험 3 |
| [scripts/report.py](https://github.com/dj258255/incident-lab/blob/main/sessions/A08-buffer-pool-sizing/scripts/report.py) | 표 출력과 `results/render.json` 생성 |
| [reproduce.md](https://github.com/dj258255/incident-lab/blob/main/sessions/A08-buffer-pool-sizing/reproduce.md) | 실행 명령과 출력 원문 |

---

재현에 쓴 compose 파일과 실행 출력 원문은 [incident-lab 저장소의 A08 세션](https://github.com/dj258255/incident-lab/tree/main/sessions/A08-buffer-pool-sizing)에 있습니다.
