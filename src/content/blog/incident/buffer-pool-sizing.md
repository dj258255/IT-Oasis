...
**2G 조건에서만 갈립니다.** 워밍업 전 상태가 히트율 100%와 디스크 읽기 0페이지를 만들고 있었다는 것이 확인됐습니다. 콜드로 출발하면 99.33%에 17,945페이지입니다. **"큰 풀 조건의 0페이지는 그 스캔이 만들어 준 값"이라고 적어 둔 것이 맞았습니다.**

다만 처리량 영향은 2.7%뿐입니다. 워밍업 30초가 대부분을 다시 채우기 때문입니다. **곡선의 모양과 무릎의 위치는 바뀌지 않습니다.**
...
|---|---|---|---|
| 읽기 전용 (회차 1) | 3.14초 | 49.3ms | 0에 가까움 |
| 읽기 전용 (회차 3) | 7.08초 | 42.8ms | 0에 가까움 |
| **쓰기 20%** | 3.14초 | **79.5ms** | **1,760** |

더티 페이지가 0 근처에서 1,760개로 늘고 최대 지연이 42.8~49.3ms에서 79.5ms로 1.6배에서 1.9배 커집니다. **"3.5초는 하한으로 봐야 한다"고 적은 것의 근거가 생겼습니다.** 축소 시간 자체는 3.14초로 읽기 전용 회차와 같은 범위 안에 있으므로, 이 조건에서 늘어난 것은 시간이 아니라 **그동안 조회가 겪는 지연**입니다.
...
코어를 3배로 늘리자 처리량이 2.6배로 올랐습니다. **CPU 천장은 확실히 풀렸습니다.** 그런데도 두 크기가 여전히 구별되지 않고, 6코어에서는 1536M이 오히려 근소하게 높습니다.
...
512M 조건은 **8을 요청했는데 실제로는 1입니다.** MySQL은 버퍼 풀이 1GB 미만이면 인스턴스를 자동으로 1로 되돌립니다. 그래서 이 줄의 15,403.7 대 14,855.7은 인스턴스 효과가 아니라 실행 편차입니다.

## 못 한 것

- **축소의 관측이 셋입니다.** 7절의 축소는 3.48초와 3.14초와 7.08초이고 그중 하나는 다른 장비입니다. 한 회차는 기록이 실패했습니다. 폭이 2.3배라 중앙값을 말할 표본이 아닙니다.
- **콜드 조건도 워밍업 30초를 그대로 둡니다.** 8절은 임포트 시점의 풀 스캔만 없앴고 측정 전 워밍업은 양쪽 다 있습니다. 워밍업까지 없앤 진짜 콜드 스타트는 재지 않았습니다.
- **쓰기 비율은 20% 하나만 썼습니다.** 8절이 더티 1,760개에서 최대 지연 79.5ms를 보였지만, 비율을 올렸을 때 어디까지 가는지는 재지 않았습니다.
