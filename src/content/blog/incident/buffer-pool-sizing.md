...
| 조건 | 원 실행(다른 호스트) | run2 | run3 |
|---|---|---|---|
| 확대 128M → 2G, 안정까지 | 0.4초 | 0.13초 | 0.13초 |
| 축소 2G → 128M, 안정까지 | 3.48초 | 기록 실패 | 7.08초 |
| 축소 구간 최대 지연 | 374.9ms | | 42.8ms |
| 대조군(리사이즈 없음) 최대 지연 | | 7.0ms | 5.1ms |

**확대는 두 회차가 0.13초로 같습니다.** 확대는 새 청크를 붙이는 일이라 옮길 것이 없습니다.

**축소는 3.48초와 7.08초로 두 배 차이가 납니다.** 축소가 확대보다 훨씬 오래 걸린다는 방향은 유지되고 그 배수는 54배까지 벌어지지만, **3.5초라는 값을 상수처럼 인용하면 안 됩니다.** 축소는 버리려는 블록에서 데이터를 빼내야 하고, 그 시점에 무엇이 올라와 있느냐에 따라 달라집니다.

**정지 374.9ms도 인용할 수 없습니다.** 이 호스트에서 축소 구간의 최대 지연은 42.8ms였고 대조군은 5.1ms였습니다. 리사이즈가 스파이크를 만든다는 방향은 8배 차이로 남지만, 374.9ms는 다른 장비에서 한 번 나온 값입니다.
...
- **축소의 관측이 아직 둘입니다.** 7절에서 확대는 두 회차가 0.13초로 모이지만, 축소는 3.48초와 7.08초 둘뿐이고 그중 하나는 다른 장비입니다. 대조군 최대 지연도 이 호스트에서 두 회차입니다.
