---
title: 'DB는 1분에 복구됐는데 앱만 계속 죽어 있다'
...
![복구 곡선](/uploads/incident/failover-dns-cache/chart-failover.png)
...
이 세션은 JVM 캐시 TTL을 0으로 두어 23.8초를 5.9초로 줄였고, `maxLifetime`과 `keepaliveTime`을 함께 줄인 조건은 5.9초로 같았습니다. **그 두 번째 결과를 벤더 문서가 설명해 줍니다.**

HikariCP 위키의 문장입니다.

> "HikariCP is designed to recover from database restart or network partition events. However, **the pool can only do so for connections currently under its control**."

**페일오버 순간 애플리케이션 스레드가 이미 빌려 간 커넥션은 풀 밖입니다.** `maxLifetime`도 `keepaliveTime`도 그 커넥션에 닿지 않습니다. 그것을 끊는 것은 드라이버의 `socketTimeout`이나 OS의 TCP keepalive입니다. 이 세션에서 추가 개선이 안 나온 이유가 여기 있습니다.

위키의 권고도 그쪽입니다. "the driver-level socket timeout be set to (at least) **2-3x the longest running SQL transaction, or 30 seconds**, whichever is longer."

AWS가 Aurora PostgreSQL 문서에서 커넥션 풀 옵션이 아니라 **TCP keepalive를 1초**로 잡으라고 하는 것도 같은 이유입니다.
...
같은 문서의 JDBC 파라미터는 `connectTimeout=2`, `socketTimeout=60`, `tcpKeepAlive=true`, `loginTimeout=2`입니다.

### 버전에 따라 기본값이 뒤집힙니다

`keepaliveTime`은 **HikariCP 4.0.0에서 도입됐고 6.2.1에서 기본값이 바뀌었습니다.**

| 버전 | `keepaliveTime` 기본값 |
|---|---|
| 4.0.0에서 6.1.x | `0` (비활성) |
| 6.2.1 이상 | `120000` (2분) |

**Spring Boot 3.x가 묶어 쓰는 HikariCP 5.x에서는 기본으로 꺼져 있습니다.** 설정을 안 건드렸다면 그 조건입니다. 그리고 문서가 조건을 하나 더 답니다. "A 'keepalive' will **only occur on an idle connection**." 유휴 커넥션에만 돕니다.

### DNS TTL 권고 숫자가 서비스마다 다릅니다

| 항목 | RDS Multi-AZ | Aurora |
|---|---|---|
| 페일오버 시간 | 60에서 120초 | 보통 30초 이내 |
| DNS 존 TTL | 명시 없음 | 5초 |
| 권장 JVM TTL | 60초 이하 | 1초 |

RDS 문서의 경고가 이 세션이 잰 것과 같은 말입니다.

> "On some Java configurations, the JVM default TTL is set so that it **never refreshes DNS entries until the JVM is restarted**."

Aurora 핸드북의 표현이 더 직설적입니다. "JVMs are **notorious for caching DNS indefinitely** unless configured otherwise."

**이 세션이 잰 30초는 보안 관리자가 없을 때의 값이고, 있으면 무한입니다.** 어느 쪽이든 DNS TTL 5초와 무관합니다.

AWS의 커넥션 풀 권고 세 줄도 HikariCP 옵션과 거의 1:1로 대응합니다. 빌려올 때 검증, 안 빌려와도 주기 검증, **"Don't let connections remain in the pool indefinitely. Recycle connections by closing and reopening them periodically (for example, every 15 minutes)"**입니다. HikariCP `maxLifetime` 기본값 30분보다 짧습니다.

### RDS Proxy는 다른 층입니다

제품 페이지의 주장은 "reduces failover times for Aurora and RDS databases by **up to 66%**"이고 "**preserving application connections**"입니다. 이 세션이 잰 문제를 애플리케이션 밖으로 옮기는 방법입니다.

제약이 붙습니다.

- 복제 구성에서 **라이터에만** 붙일 수 있고 리드 리플리카에는 못 붙입니다
- "Any statement with a text size greater than **16 KB** causes the proxy to **pin the session** to the current connection." 피닝되면 멀티플렉싱 이득이 사라집니다
- SSL 호스트명 검증을 쓰면 커스텀 DNS와 함께 못 씁니다
- MySQL `init_connect`로 세션 변수를 설정하면 기대대로 안 될 수 있습니다

## 못 한 것

- **기본값 조건만 3회이고 나머지는 1회 실행입니다.** 기본값은 처음 실행분에 30초짜리 시계 불연속이 있어 3회 다시 돌렸고(23.65~23.78초로 모입니다), 폐기한 실행분은 `results/superseded-default.jsonl`로 남겨 뒀습니다. `ttl=0` 계열과 6절의 세 절은 각각 1회이고 읽기 전용 창은 25초 하나만 썼습니다.
- **프로브 간격이 장애 구간에서 3.6초까지 벌어집니다.** 명목은 0.5초인데 커넥션이 안 되면 `curl --max-time 4`가 그만큼 물립니다. 그래서 3·4절의 시점 값은 전부 앞 표본까지가 불확실 구간이고, 배수로 요약하면 그 폭이 커집니다.
...
재현에 쓴 compose 파일과 실행 출력 원문은 [incident-lab 저장소의 R02 세션](https://github.com/dj258255/incident-lab/tree/main/sessions/R02-failover-dns-cache)에 있습니다.
