...
> | 무엇 | 등급 | 확인한 것 |
> |---|---|---|
> | 장애가 반복해서 일어났다는 사실과 건수 | **E1·축소** | 의원실 제출 자료를 언론이 보도한 것까지. **금융감독원 공표 원문은 확인 못 했습니다** |
> | 원인이 커넥션 풀 고갈이라는 것 | **E2** | 각 사의 내부 원인 분석이 비공개라 벤더 문서와 업계 통념 수준의 가설입니다 |
>
> 메리츠증권 건도 1차 자료를 확인하지 못했습니다. **이 글이 재현한 것은 확인된 원인이 아니라 원인 가설입니다.**
> 출처: [굿모닝경제 2026-04-14](https://www.goodkyung.com/news/articleView.html?idxno=285848) · [NSP통신 2026-04-14](https://www.nspna.com/news/?mode=view&newsid=809679) · [머니투데이 2026-05-06](https://www.mt.co.kr/stock/2026/05/06/2026050611250058757)
> 장애 건수는 의원실이 각 사에서 받은 자료를 언론이 보도한 것이고, 금융감독원이 같은 내용을 공표한 원문은 확인하지 못했습니다. 각 사의 내부 원인 분석은 공개되지 않아 이 세션이 재현한 커넥션 풀 고갈은 확인된 원인이 아니라 원인 가설입니다.
...
장애가 반복해서 일어났다는 사실과 그 건수까지는 위 자료로 확인됩니다. 각 사의 내부 원인 분석은 공개되지 않아 이 세션은 특정 사의 원인을 그대로 재현하지 않습니다. 장 시작 급증에서 백엔드가 무너지는 대표 메커니즘 하나, 커넥션 풀 고갈을 골라 규모를 줄여 재현합니다. 여기서 재현한 커넥션 풀 고갈은 실제 장애의 확인된 원인이 아니라 원인 가설이고, 가설로서의 근거는 벤더 문서와 업계 통념 수준인 E2입니다. 시세 조회 한 건이 DB 커넥션을 잠깐 잡는데, 그 조회가 초당 수백 건씩 몰리면 풀이 마르고 뒤따르는 요청이 커넥션을 기다리다 무너지는 과정입니다.
...
같은 k6 스크립트를 부하 차단이 있는 `/quote/fixed`에 돌렸습니다. 그런데 두 경로가 실제로 받은 부하는 같지 않았습니다. 이 절의 수치를 읽기 전에 그 이야기부터 해야 합니다.

k6 설정은 양쪽 다 초당 400건을 겨냥한 도착률 모델입니다. **이번에는 그 모델이 양쪽에서 유지됐습니다.** 목표 10,000회 가운데 9,999회가 발사되고 중단된 반복이 0건입니다. 버그 경로도 해소 경로도 같습니다.

1차에서는 그렇지 않았습니다. [1차 버그 경로](https://github.com/dj258255/incident-lab/blob/main/sessions/F03-market-open-connection-storm/results/buggy-k6.txt)는 `http_reqs 6598`에 `dropped_iterations 3401`이었습니다. 응답이 3초에서 5초씩 걸리는 바람에 미리 잡아 둔 VU 900개가 모자라 목표 도착률의 3,401회분을 쏘지도 못했습니다. 해소 경로는 9,959건이라 앱에 도달한 요청 수가 51% 차이 났고, 그래서 "같은 스파이크를 그대로 줬다"는 서술을 거뒀습니다.

VU를 5,000으로 올려 그 결함을 없앴습니다. 느린 응답이 도착률을 되끌어 내리지 못하게 VU를 넉넉히 잡는 것이 열린 모델을 유지하는 조건이었습니다. 이제 두 경로의 처리량 비교가 성립합니다.

![F03 해소 경로 부하 화면](/uploads/incident/market-open-connection-storm/02-fixed-shed.png)

*그림 3. 부하 차단을 넣은 해소 경로입니다. 응답 중앙값이 52ms로 돌아오고, 풀 여력을 넘은 5,245건은 503으로 흘려보내며, 500은 0건입니다.*
...
2초 커넥션 타임아웃으로 인한 500은 0건입니다. 풀이 마르지 않았기 때문입니다. 앱 안에서 잰 커넥션 획득 시간이 중앙값 0.01ms입니다. 버그 경로의 962.97ms와 견주면 9만 배 차이입니다. 획득 대기 큐 최댓값도 1건이고 스레드를 기다리는 요청은 76건입니다. 버그 경로의 191건과 4,335건에서 내려온 값입니다.

503이 얼마나 빨리 나갔는지도 이번에는 쟀습니다. 중앙값 1ms, 평균 29.63ms, p95 189ms, 최소 370.8마이크로초, 최대 453.4ms입니다. 1차 본문에 "1ms 만에 돌려보낸다"고 썼다가 근거가 없어 지웠던 자리인데, 중앙값으로는 맞고 평균과 p95는 그보다 큽니다. 램프업 구간에서 몇 건이 늦게 나갔습니다.
...
첫째, 풀 고갈의 첫 증상은 에러가 아니라 지연이었습니다. 풀이 마르면 500이 쏟아질 것이라 생각했는데, 커넥션 획득 제한 시간이 2초로 넉넉하다 보니 대부분의 요청은 실패하지 않고 몇 초씩 기다렸습니다. 500은 전체 9,999건 가운데 761건, 7.61%에 그쳤습니다. 그런데 중앙값 지연은 11.42초였습니다. 제한 시간을 짧게 잡으면 실패가 빨리 드러나고, 길게 잡으면 지연으로 숨습니다. 모니터링을 500 에러율만 보면 이 장애를 놓치고, p95 지연을 함께 봐야 잡힌다는 뜻입니다.

둘째, 부하 시험의 부하 모델이 결과를 바꿨습니다. 처음에는 고정 사용자 수를 반복시키는 닫힌 모델로 200명이 요청하게 했는데, 해소 경로에서 503이 즉시 떨어지자 그 사용자들이 곧바로 재요청을 퍼부어 초당 수천 건이 됐고, 재현하려던 커넥션 풀 고갈이 아니라 Tomcat의 연결 수락 큐 고갈이 재현됐습니다. connection refused가 2만 건 넘게 찍혔습니다. 그래서 도착률을 고정하는 열린 모델로 바꿨습니다. 다만 그 전환이 1차에서는 의도대로 끝나지 않았습니다. 응답이 느려 VU 900개가 모자랐고 목표 도착률의 3,401회분이 발사되지 못했으니, 열린 모델을 설정하고도 유지하지는 못한 셈입니다. VU를 5,000으로 올린 이번 측정에서 양쪽 다 9,999회가 발사되어 그 결함이 사라졌습니다. 풀이 마른 것은 획득 대기 큐 191과 커넥션 타임아웃 예외 761건으로 확인되지만, 3절에서 실측한 대로 지연의 91%는 풀 앞단에서 쌓였으므로 병목이 풀 계층 하나에 고정됐다고는 말할 수 없습니다. 빠른 거부가 오히려 재시도 폭풍을 부른다는 것은, 실제 대기열 설계에서 클라이언트 백오프가 필요한 이유이기도 합니다.
...
단일 인스턴스에서 시세 조회 하나만 재현했습니다. 실제 MTS의 주문·체결·실시간 시세 스트림, 여러 인스턴스와 로드밸런서, 그 앞단의 API 게이트웨이는 다루지 않았습니다. 조회의 DB 점유 시간은 `pg_sleep(0.05)`로 모델링한 값이라 실제 쿼리 프로파일과 다르고, 이 글의 초당 200건 상한도 그 모형에서 나온 산술입니다.
...
`max-threads` 스윕은 조건마다 한 번씩 돌렸습니다. 표의 방향, 곧 스레드를 늘리면 500이 늘고 스레드 대기 큐가 줄어든다는 것은 네 조건이 단조롭게 움직여 편차로 보기 어렵지만, 각 칸의 값 자체는 반복하지 않았습니다.
...
이 글로 말할 수 있는 것은 이렇습니다. 버그 경로에서 풀이 마르고 커넥션 획득 타임아웃이 761건 났으며 그 건수가 앱 예외 수와 정확히 같다는 점, 부하 차단을 넣은 경로에서는 그것이 0건이고 획득 대기가 중앙값 0.01ms라는 점, 받은 요청의 중앙값 지연이 11.42초에서 52ms로 돌아왔다는 점, 그리고 그 11.42초의 91%가 커넥션 풀보다 앞단에서 쌓였다는 점입니다. 마지막 항목이 이번 회차에서 새로 갈린 것이고, 그래서 이 글이 가리키는 병목은 풀 하나가 아니라 풀이 정한 상한 앞에 줄 선 계층 전체입니다. CATALOG가 제시한 가상 대기열과 조회·주문 경로 분리는 부하 차단 한 겹으로 축소했습니다.

## 현업은 어떻게 해소했는가

**이 사건군은 다른 편과 성격이 다릅니다. 아직 닫히지 않았습니다.**

개별 사고의 회사 공식 사후 보고서는 거의 없습니다. 1차로 확인되는 회사 사례는 토스증권 기술블로그 하나입니다. 미국 정규장 개시에 트래픽이 20배 이상 뛰면서 브로커 응답이 5000ms 를 넘던 문제에 이렇게 대응했다고 밝혔습니다.

- Resilience4j `RateLimiter` 로 브로커 전송 주문 TPS 를 제한하고, 예약주문 배치는 파라미터로 TPS 를 동적 조절
- Grafana 알림과 웹훅을 붙여 브로커 이상을 자동 감지하고 Kafka 이벤트로 전파, CRITICAL 이면 서브 브로커로 자동 전환
- Oracle 날짜 파티셔닝, 계좌 단위 조회는 MongoDB 로 분리

**증설이 아니라 유입 속도 제어와 자동 페일오버로 흡수하는 접근입니다.** 그리고 **병목이 자사 시스템 밖이었다**는 점이 이 세션과 다릅니다. 이 세션은 자기 풀이 마르는 것을 쟀는데, 토스가 실제로 만난 것은 외부 브로커의 응답 지연이었습니다.

감독 층에서는 금융감독원이 2023년 11월 「금융IT 안전성 강화를 위한 가이드라인」을 냈습니다. 성능관리 항목이 이 세션의 주제와 겹칩니다.

> "전산자원 사용량 임계치를 4단계(정상→주의→경계→심각)로 구분하고, 경계 및 심각 징후 발생시 즉각 설비 증설 추진"
> "비정상적 트래픽 발생시 **동시접속자를 통제**하고, CPU·메모리 등 전산자원을 즉각 증설할 수 있는 체계 마련"

문서에 실린 검사 사례가 이 세션이 잰 것과 같은 모양입니다.

> "◈ A증권사는 전산자원 증설 기준과 대응방안을 수립하지 않아, '23.6월 경계 수준의 사용량(CPU 사용량 65%↑)에 도달하였음에도 조치 없이 운영하다 '23.7월 MTS 중단"

**그런데 이 가이드라인은 법규가 아니라 협회 자율 규범입니다.** 문서 자신이 "각 회사별 상황에 따라 취지를 벗어나지 않는 범위내에서 조정 가능" 이라고 적습니다. 정식 행정지도로 발령된 문건은 찾지 못했습니다.

**효과가 확인되지 않습니다.** 2차 자료 기준으로 한 증권사의 장애는 2022년 4건, 2023년 9건, 2024년 11건, 2025년 13건으로 **가이드라인 시행 이후에도 늘었습니다.** 2022~2025년 9개 증권사 168건, 배상 53억 원입니다.

**다른 편들은 "고치면 이렇게 된다"를 보여 주는데 이 편은 "고치지 않으면 이렇게 된다"에 머물러 있습니다.** 이 세션이 잰 것은 풀 크기와 스레드 수를 바꿨을 때의 효과이고, 현실에서 걸린 것은 그 조정을 할 조직과 예산이 있느냐 쪽입니다.

## 못 한 것

- **7절의 풀 스윕은 해소 경로, 스레드 스윕은 버그 경로입니다.** 두 표를 같은 경로에서 나란히 놓지는 않았습니다.
- **각 사의 내부 원인 분석이 공개되지 않았습니다.** 그래서 여기서 재현한 커넥션 풀 고갈은 확인된 원인이 아니라 원인 가설이고, 가설로서의 근거는 벤더 문서와 업계 통념 수준입니다.
- **금융감독원 공표 원문을 확인하지 못했습니다.** 장애 190건은 의원실 자료를 언론이 보도한 것입니다. 메리츠증권 건도 1차 자료를 확인하지 못했습니다.
- **단일 인스턴스에서 시세 조회 하나만 재현했습니다.** 실제 MTS의 주문·체결·실시간 시세 스트림, 여러 인스턴스와 로드밸런서, API 게이트웨이는 다루지 않았습니다.
- **병목을 풀 계층 하나에 고정하지 못했습니다.** 풀이 마른 것은 획득 대기 큐 191과 커넥션 타임아웃 761건으로 확인되지만, 지연의 91%는 풀 앞단에서 쌓였습니다.
