...
- **풀 크기를 바꿔 가며 재지 않았습니다.** 풀 확대를 택하지 않은 것은 측정 결과가 아니라 HikariCP 문서의 권고를 따른 판단입니다. 게다가 이 재현의 초당 200건은 `pg_sleep(0.05)`가 정의한 값이라 풀을 키웠을 때 어디서 무너지는지를 이 모형으로는 알 수 없습니다. 풀 크기를 바꿔 가며 재는 것과 모형을 실제 쿼리로 바꾸는 것 둘 다 가능한데 아직 안 했습니다.
- **단일 인스턴스에서 시세 조회 하나만 재현했습니다.** 실제 MTS의 주문·체결·실시간 시세 스트림, 여러 인스턴스와 로드밸런서, API 게이트웨이는 다루지 않았습니다.
- **`max-threads` 스윕은 조건마다 한 번씩입니다.** 네 조건이 단조롭게 움직여 방향은 편차로 보기 어렵지만 각 칸의 값 자체는 반복하지 않았습니다.
