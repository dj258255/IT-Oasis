...
개별 사고의 회사 공식 사후 보고서는 거의 없습니다. 1차로 확인되는 회사 사례는 토스증권 기술블로그 하나입니다. 미국 정규장 개시에 트래픽이 20배 이상 뛰면서 브로커 응답이 5000ms를 넘던 문제에 이렇게 대응했다고 밝혔습니다.

- Resilience4j `RateLimiter`로 브로커 전송 주문 TPS를 제한하고, 예약주문 배치는 파라미터로 TPS를 동적 조절
- Grafana 알림과 웹훅을 붙여 브로커 이상을 자동 감지하고 Kafka 이벤트로 전파, CRITICAL 이면 서브 브로커로 자동 전환
- Oracle 날짜 파티셔닝, 계좌 단위 조회는 MongoDB로 분리
