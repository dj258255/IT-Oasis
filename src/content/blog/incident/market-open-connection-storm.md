...
k6 설정은 양쪽 다 초당 400건을 겨냥한 도착률 모델입니다. 그런데 [results/buggy-k6.txt](https://github.com/dj258255/incident-lab/blob/main/sessions/F03-market-open-connection-storm/results/buggy-k6.txt)를 보면 버그 경로는 `http_reqs 6598`에 `dropped_iterations 3401`입니다. 응답이 3초에서 5초씩 걸리는 바람에 미리 잡아 둔 VU가 모자라, 목표 도착률의 3,401회분을 쏘지도 못했다는 뜻입니다. 해소 경로는 [results/fixed-k6.txt](https://github.com/dj258255/incident-lab/blob/main/sessions/F03-market-open-connection-storm/results/fixed-k6.txt)에서 `http_reqs 9959`에 `dropped_iterations 40`입니다. 앱에 도달한 요청 수가 51% 차이 납니다.

그래서 "같은 스파이크를 그대로 줬다"는 서술을 거둡니다. 도착률을 고정하는 열린 모델을 쓴다고 했지만, 버그 경로에서는 그 열린 모델이 유지되지 않았습니다. 느린 응답이 도착률을 되끌어 내려 사실상 닫힌 모델처럼 굴었습니다. 따라서 이 글은 두 경로의 처리량을 비교하지 않습니다. 비교가 성립하는 것은 각 경로가 실제로 받은 요청의 지연, 그리고 커넥션 타임아웃이 났는지 여부까지입니다.
...
그런데 이 실행에는 200도 503도 아닌 실패가 1,324건 더 있습니다. `http_reqs`가 9,959건인데 200(3,759)과 503(4,876)을 더하면 8,635건이라, 남는 1,324건은 응답을 아예 받지 못한 요청입니다. 세션 기록은 이것을 램프업 첫 6초에 몰린 connection refused라고 적었습니다. 그리고 같은 요약의 `http_req_duration`에 `min=0s`가 찍혀 있습니다. 0ms짜리 실패가 지연 분포에 섞여 있다는 뜻이라, 위의 중앙값 51ms는 그만큼 낮게 나온 값입니다. 성공 응답만 따로 본 `{ expected_response:true }` 쪽은 중앙값 52.16ms, p95 542.32ms입니다. "커넥션 타임아웃 0건"이라는 문장은 이 1,324건과 같이 읽어야 합니다.
