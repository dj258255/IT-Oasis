...
- **풀 크기를 바꿔 가며 재지 않았습니다.** 풀 확대를 택하지 않은 것은 측정 결과가 아니라 HikariCP 문서의 권고를 따른 판단입니다. 게다가 이 재현의 초당 200건은 `pg_sleep(0.05)`가 정의한 값이라 풀을 키우면 어디서 무너지는지를 이 모형으로는 답할 수 없습니다.
