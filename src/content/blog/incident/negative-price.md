...
입력은 코드로 만든 하강 틱 10건입니다. 20.00, 10.00, 5.00, 1.00, 0.10, 0.01, 0.00, -1.00, -10.00, -37.63 순서이고, 마지막 -37.63만 CFTC 문서의 수치이며 나머지 9건은 합성값입니다. 실제 그날의 체결 경로가 아닙니다. 환경은 Docker 위 postgres:16-alpine과 eclipse-temurin:21-jdk-alpine(JDK 21.0.11), 2 vCPU 공유 호스트입니다. compose 파일에 `cpus`도 `mem_limit`도 없고 JVM에 `-Xmx`도 주지 않았습니다. 이 세션은 로직 재현이라 처리량이나 지연 시간을 하나도 싣지 않으므로 자원 한도가 결과를 바꾸지 않지만, 그래서 이 글의 어떤 수치도 성능 수치로 읽으면 안 됩니다. 명령과 출력 원문은 [reproduce.md](https://github.com/dj258255/incident-lab/blob/main/sessions/F09-negative-price/reproduce.md)에 그대로 남겼습니다.

한 가지 미리 밝힐 것이 있습니다. 세 계층은 따로 재현했을 뿐 아니라 서로 연결돼 있지도 않습니다. compose 파일의 `sql` 컨테이너와 `app` 컨테이너는 완전히 분리돼 있고, 자바 코드에는 JDBC가 한 줄도 없습니다. Postgres가 남긴 최종가 0.01과 증거금 엔진이 쓰는 0.01은 같은 값이지만 같은 경로로 흘러간 값이 아니라, 앞의 실행 결과를 보고 뒤의 코드에 `STALE = 0.01`이라는 상수로 옮겨 적은 것입니다. 아래에서 "저장 계층이 멈춘 값을 증거금 계층이 읽는다"고 쓸 때, 그 전파는 재현한 것이 아니라 두 실행 결과를 사람이 이어 붙인 서술입니다.

표시 방식도 하나 밝혀 둡니다. 아래에서 실행 출력을 실은 코드 블록은 전부 필요한 줄만 뽑은 발췌입니다. 남긴 줄의 글자는 손대지 않았지만 여러 줄을 덜어냈으므로 화면에 찍힌 그대로는 아니고, 전체 출력은 위에 링크한 reproduce.md에 있습니다.
...
![F09 저장 계층 거부 화면](/uploads/incident/negative-price/01-storage-reject.png)
...
![F09 파싱 계층 재현 화면](/uploads/incident/negative-price/02-parse-buggy.png)
...
아래는 실행 출력에서 행을 덜어내 다시 짠 표입니다. 콘솔 원문이 아닙니다. 뺀 행은 20.00, 10.00, 1.00, -37.63이고 전체 출력은 [reproduce.md](https://github.com/dj258255/incident-lab/blob/main/sessions/F09-negative-price/reproduce.md)와 바로 아래 그림 3에 그대로 있습니다.
...
![F09 증거금 계층 재현 화면](/uploads/incident/negative-price/03-margin-buggy.png)
...
![F09 해소 실행 화면](/uploads/incident/negative-price/04-fixed-run.png)
...
![가격이 내려갈 때 계약당 요구증거금](/uploads/incident/negative-price/05-margin-curve.png)
...
![가격 0.10달러에서 열 수 있는 계약 수](/uploads/incident/negative-price/06-contracts-before-after.png)
...
- **3,764만 달러는 이 세션 모델이 낳은 수입니다.** 명령서에는 시스템이 마지막 양수 틱에 고정됐다는 메커니즘도, 과대계상 폭의 수치도 없습니다. 진입가와 계약 수를 고르는 순간 결정되는 값이라 실제 사건의 금액에 대해 아무것도 말해 주지 않습니다.
- **실제 주문 게이트웨이와 거래소 마켓 룰 설정을 재현하지 않았습니다.** 화면 렌더링, 여러 계약과 여러 통화가 섞인 포트폴리오 마진, 장중 실시간 청산 엔진도 다루지 않았습니다.
- **변동성 기반 증거금이 실제 모형과 다릅니다.** 관측 변동폭에 상수를 곱하는 수준이고 SPAN이나 VaR와는 다릅니다.
- **부하와 지연 시간을 측정하지 않았습니다.** 이 기록에 시간 수치가 하나도 없어 컨테이너 자원 한도도 걸지 않았습니다. 뒤집어 말하면 이 세션의 결과는 자원 한도를 걸어야 비교할 수 있는 종류가 아닙니다.

---

재현에 쓴 compose 파일과 실행 출력 원문은 [incident-lab 저장소의 F09 세션](https://github.com/dj258255/incident-lab/tree/main/sessions/F09-negative-price)에 있습니다.
