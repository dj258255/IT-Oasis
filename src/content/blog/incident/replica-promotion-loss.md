---
title: '복제 지연 중 승격, 성공 응답을 받은 커밋이 사라진다'
...
다만 **초당 커밋 수만은 호스트와 무관하게 설명됩니다.** 쓰기 클라이언트가 커밋 사이에 20밀리초를 고정으로 쉬기 때문입니다. 이 대기만으로 상한이 초당 50건이고, 실측 커밋 간격 중앙값이 27밀리초라 나머지 약 7밀리초가 INSERT와 커밋에 쓰였습니다. 그래서 뒤에 나오는 초당 37건은 **하드웨어의 처리 능력이 아니라 클라이언트가 정한 속도**입니다. 이 글의 어떤 수치도 성능 벤치마크로 읽으면 안 됩니다.
...
![비동기 유실](/uploads/incident/replica-promotion-loss/fig-async-loss.png)
...
![거짓 0](/uploads/incident/replica-promotion-loss/fig-false-zero.png)
...
이 세션은 둘 다 해 보지 않았습니다. 병합은 승격본이 이미 같은 키에 새 값을 쓴 경우의 충돌 처리가 필요한데 그 시나리오를 만들지 않았고, 백업은 주기도 복원 시간도 재지 않았습니다. 그래서 이 글이 수치로 말할 수 있는 것은 "얼마나 잃는가"까지입니다. **둘 중 병합 경로는 6절에서 실제로 재서 유실 50행을 전부 회수했습니다.** 여기서 여전히 재지 않은 것은 백업 복원 쪽입니다. 백업 주기를 RPO 논의에 끌어들이려면 그 복원을 실제로 재는 세션이 따로 있어야 합니다.
...
쓰기를 멈추고 따라잡기를 기다리라는 절차입니다. **프라이머리가 이미 죽어서 하는 페일오버는 이 절차를 밟을 수 없습니다.** 그 상태가 실험 A이고, 밟지 못한 대가가 555건입니다. 같은 문서가 이 용도를 조건부로만 인정하는 것도 그래서입니다. "If you are aware of the ramifications and limitations of asynchronous replication and you still want to use read replica promotion for data recovery, you can."
...
혼동이 잦은 이유는 둘 다 "다른 AZ에 사본이 있고 장애 시 넘길 수 있다"로 보이기 때문입니다. 갈리는 지점은 하나입니다. 커밋이 사본의 응답을 기다렸는가. 실험 A와 B가 그 한 가지 차이를 555건과 0건으로 갈라 놓았습니다.

### 실험 B·C가 대응하는 것: Multi-AZ DB 클러스터

AWS 문서는 Multi-AZ DB 클러스터를 "a semisynchronous, high availability deployment mode"라고 정의하고, "requires acknowledgment from at least one reader DB instance in order for a change to be committed"이라고 적습니다. 실험 B·C의 `AFTER_SYNC` 계약과 같은 모양입니다. RDS for MySQL이 흐름 제어 파라미터로 `rpl_semi_sync_master_target_apply_lag`을 노출하는 것도 같은 계열임을 보여줍니다.

대가는 지연만이 아니라 대수입니다. Multi-AZ DB 클러스터는 라이터 1대와 리더 2대, 세 AZ에 걸쳐 3대입니다. 이 세션은 인스턴스 클래스도 금액도 계산하지 않았습니다.

확인하지 못한 것도 있습니다. 실험 C가 보인 조용한 강등이 관리형 환경에서 어떤 지표로 노출되는지 조사하지 않았습니다. 자체 구성에서는 `Rpl_semi_sync_source_status`를 직접 읽을 수 있었지만, 관리형에서는 그 상태를 볼 수 있는지부터가 별개 문제입니다.

Aurora는 아예 다른 얘기입니다. AWS 문서가 "Multi-AZ DB clusters aren't the same as Aurora DB clusters"라고 못 박습니다. 이 세션의 범위 밖입니다.

## 6. AFTER_COMMIT 대조, 평상시 지연, 복구 경로

앞 절들이 남긴 세 질문을 재봤습니다. 반동기가 평상시에 얼마를 물리는지, `AFTER_COMMIT`은 `AFTER_SYNC`와 무엇이 다른지, 유실을 실제로 되돌릴 수 있는지입니다.
...
- **초기 회차의 호스트 사양을 기록하지 않았습니다.** `uname -srm`, `nproc`, `free -g`를 남기지 않아 이 세션이 어느 장비에서 돌았는지 확인되지 않습니다. 다른 세션과 절대 시간을 나란히 놓을 수 없습니다. 반복 측정을 하면 그때 기록이 남으므로 이 항목은 반복 측정과 함께 해소됩니다.
- **반동기의 평상시 지연 비용은 이 환경에서 잴 수 없습니다.** 커밋 자체를 100건씩 재도 세 조건이 구별되지 않습니다. 가용영역을 나눈 환경이 필요합니다.
- **RDS에서 검증하지 않았습니다.** 5절의 대응 관계는 AWS 문서와 대조한 것이고, 실제 리드 리플리카 승격이나 Multi-AZ DB 클러스터 페일오버를 돌려 본 것이 아닙니다.
- **백업 복원 경로는 실행하지 않았습니다.** 차집합 병합은 재봤지만 옛 소스가 아예 못 뜨는 경우의 복구는 다루지 않았습니다.
...
- **읽기를 복제본으로 분산했을 때의 값 차이는 다루지 않았습니다.** 여기서 잰 것은 승격 시점의 유실이지 평상시 복제본이 돌려주는 값의 신선도가 아닙니다. 집계값을 복제본에서 읽을 때 그 값이 지연만큼 뒤처지는 문제는 같은 시리즈의 [라이브 후원 카운터의 핫 로우를 슬롯 카운터로 풀었다](/blog/incident/slotted-counter)에서 한계로 적어 뒀습니다.

---

재현에 쓴 compose 파일과 실행 출력 원문은 [incident-lab 저장소의 R01 세션](https://github.com/dj258255/incident-lab/tree/main/sessions/R01-replica-promotion-loss)에 있습니다.
