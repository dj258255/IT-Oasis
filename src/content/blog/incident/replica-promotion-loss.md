...
이유가 둘입니다. 두 노드가 같은 호스트의 컨테이너라 복제 왕복 자체가 짧고, 계측 장치가 반복마다 접속을 새로 맺어 그 비용이 왕복보다 큽니다. **반동기의 평상시 비용은 이 환경에서 잴 수 없다는 것이 결론입니다.** 실제 가용영역 사이에서는 왕복이 밀리초 단위로 붙으므로 다른 그림이 나옵니다.

### `AFTER_COMMIT`은 읽힌 값을 잃습니다

복제망을 끊어 ack가 오지 않는 상태에서 커밋을 던지고, 그 커밋이 매달린 동안 **다른 세션이 그 값을 읽을 수 있는지** 봅니다. 그다음 소스를 죽이고 복제본을 승격합니다.
...
`AFTER_SYNC`는 복제본이 ack 할 때까지 엔진 커밋을 미룹니다. 그래서 다른 세션이 읽을 수 있게 된 값은 이미 복제본에 있고, **읽히지 않았으니 사라질 것도 없습니다.**

`AFTER_COMMIT`은 먼저 엔진에 커밋하고 그 뒤에 ack를 기다립니다. 그 창에서 다른 세션이 값을 읽었고, 소스가 죽자 **읽힌 값이 사라졌습니다.** 사용자 화면에 이미 뜬 후원이 없어지는 것이 이 한 줄의 뜻입니다.
