...
이 세션은 지연이나 처리량을 재지 않습니다. 서버가 내놓는 문구와 임계값, 상태 전이 세 가지만 봅니다. 호스트 사양이 결론에 영향을 주지 않습니다.
...
5만 행 규모인 이 랩에서 원인만 제거하고 40초 기다리자 wraparound 방지 autovacuum이 `template0`까지 전부 처리했습니다. 수동 VACUUM도, 단일 사용자 모드도 쓰지 않았습니다. 접속이 막힌 `template0`도 긴급 autovacuum은 들어갑니다.
...
- **`vacuum_failsafe_age` 대조.** 기본값 16억과 무력화 조건을 나눠 재지 않았습니다. failsafe가 발동한 것은 관측했지만 그것이 없을 때 얼마나 더 걸리는지는 재지 않았습니다. Sentry 병목을 정량화하려면 이 대조가 필요합니다.
- **13과 17 비교.** 임계값과 HINT 문구가 갈리는 것을 같은 시나리오로 나란히 보이지 못했습니다. 소스와 커밋 이력으로만 확인했습니다.
- **거대 테이블.** Sentry의 병목은 인덱스가 많은 거대 테이블이었습니다. 이 랩의 `sponsor`는 5만 행이라 VACUUM이 즉시 끝납니다. failsafe의 실효를 시간으로 못 보였습니다.
