...
## 6. failsafe가 아끼는 것과 대신 내는 것

5절에서 failsafe가 발동한 것은 관측했지만 그것이 없을 때 얼마나 더 걸리는지는 재지 않았습니다. `sponsor`가 5만 행이라 VACUUM이 즉시 끝나 차이가 시간으로 드러나지 않았기 때문입니다. 테이블을 키워 두 조건을 갈랐습니다. `big` 500만 행 946MB, 보조 인덱스 5개, 20% 삭제로 죽은 튜플 100만 개입니다.
...
## 7. 13과 17 대조, 멀티XID 경로, failsafe 반복

### failsafe 대조를 네 번 재서

6절은 조건마다 한 번씩이었습니다. 세 번 더 돌렸습니다.
...
**버퍼 수가 네 회차 모두 정확히 같습니다.** 4와 5,007입니다. 이 값이 6절의 결론을 떠받칩니다. failsafe가 인덱스 정리를 건너뛰면 그 인덱스가 정리되지 않은 채 남고, 같은 조회가 페이지를 5,007개 읽습니다. **버퍼 수는 시간과 달리 실행 편차가 없습니다.**
...
**갈리는 것은 임계값이 아니었습니다.** 동결 관련 GUC 여섯 개의 기본값이 두 버전에서 완전히 같습니다. 갈리는 것은 **failsafe 두 항목의 존재 여부**입니다. 13에는 그 설정이 아예 없고, 따라서 6절이 잰 "인덱스 정리를 건너뛰는 응급 모드"가 **13에서는 일어나지 않습니다.** wraparound가 임박해도 VACUUM이 인덱스를 다 훑고 갑니다.

그러면 6절의 거래 조건이 버전마다 다릅니다. 17은 "빨리 끝내는 대신 조회를 220배 느리게 만드는" 선택지가 자동으로 켜지고, 13은 그 선택지가 없어 느리게라도 제대로 정리합니다. **어느 쪽이 나은지는 그 순간 무엇이 급한지가 정합니다.**
...
- **7절의 반복은 같은 호스트 네 회차입니다.** 버퍼 수 4와 5,007은 네 회차가 같지만 VACUUM 시간은 2.18초에서 4.24초로 폭이 있습니다.
- **13과 17의 경고 문구는 여전히 못 봤습니다.** 7절은 설정과 카탈로그 대조까지이고, age를 임계값 위로 올리는 시나리오를 두 버전에서 같이 돌리지는 않았습니다.
- **6절의 946MB도 Sentry 규모는 아닙니다.** 인덱스 정리를 건너뛰면 시간이 줄어든다는 방향은 확인됐지만, 3시간이 걸리는 규모에서 비율이 유지되는지는 재지 않았습니다.
...
- **멀티XID를 소진까지 태우지 않았습니다.** 7절은 멀티XID가 붙는 것까지 보였고, 그 공간이 차서 wraparound에 이르는 과정은 만들지 않았습니다.
