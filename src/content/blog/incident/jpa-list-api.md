...
**행당 쿼리는 계속 내려가는데 시간은 100에서 멈춥니다.** 100에서 5000까지 행당 쿼리가 1.021에서 1.001로 줄지만 소요는 8.81초에서 8.79초로 같습니다. 남은 0.02건이 아끼는 왕복이 전체에서 차지하는 몫이 이미 무시할 수준이기 때문입니다.

**1은 배치를 아예 끄는 것과 같습니다.** 행당 쿼리가 2.001인데, 삽입 한 건마다 문장 하나에 커밋 관련 왕복이 하나 더 붙기 때문입니다. 소요가 1.75배입니다.
...
- **무작위 `id`로는 재지 않았습니다.** 9절에서 200만 행이 든 테이블도 같은 비용임을 확인했지만, 그것은 `id`가 순증하는 조건입니다.
