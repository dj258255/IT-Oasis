...
description: 'WebSocket으로 시세를 뿌리는 서버에 초당 32KB만 읽는 느린 구독자 한 명을 붙였습니다. 그 상태로 팬아웃 구현 열한 가지를 각각 3회씩 쟀습니다. 브로드캐스트 스레드가 직접 전송하면 발행량이 초당 2,909건에서 142건으로 떨어지고 생산자 스레드가 45초 중 44.0초를 막혀 있었습니다. 무제한 큐로 바꾸면 생산자는 살아나지만 느린 구독자 몫이 힙 128MB 중 95.58MB를 차지하며 86초에 OOM이 났습니다. 종목별 최신값으로 접고 한도를 넘으면 끊는 조합은 발행량 2,936건에 느린 구독자 몫 0.01MB였습니다.'
descriptionEn: "A WebSocket market-data server was given five healthy subscribers plus one slow subscriber that reads only 32KB per second, and eleven fan-out implementations were measured three times each. Sending directly from the broadcast thread dropped throughput from 2,909 to 142 messages per second, with the producer thread blocked for 44.0 of the 45 measured seconds. An unbounded per-subscriber queue kept the producer running but let the slow subscriber's backlog take 95.58MB of a 128MB heap, ending in OOM at 86 seconds. Conflating the queue to the latest tick per symbol and closing sessions past a limit held 2,936 messages per second with only 0.01MB queued."
date: 2026-06-19
tags:
  - WebSocket
  - Spring Boot
