...
- **6절의 복제 대조는 조건마다 1회 실행입니다.** 0.07초와 40.65초가 그 값이고, 조건마다 3회 반복한 것은 10절에 있습니다. 단일 행 쪽이 회차마다 0.07~2.18초로 31배 흩어지므로 6절 값 하나를 인용하면 안 됩니다. 복제본이 하나이고 지연 복제나 병렬 적용(`replica_parallel_workers`)은 다루지 않았습니다. 복제 지연이 평소에는 0으로 보이다가 승격에서 드러나는 방식은 같은 시리즈의 [복제 지연 중 승격, 성공 응답을 받은 커밋이 사라진다](/blog/incident/replica-promotion-loss)에서 다룹니다.
- **10절의 슬롯 스윕은 조건마다 1회 실행입니다.** 복제 대조만 3회입니다. 방송당 최대 슬롯은 앞선 회차에서 uniform 이 16·4·1 이었고 이번 회차가 8·2·1 입니다. zipf 는 두 회차 다 전부 64 입니다.
