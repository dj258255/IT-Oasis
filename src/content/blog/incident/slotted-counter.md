...
![적재 데이터 감사](/uploads/incident/slotted-counter/chart-data-audit.png)
...
라이브 후원은 예시일 뿐이고, 구조는 종목별 체결수량과 체결대금 누적에 그대로 있습니다. 한 종목의 누적 행에 체결이 몰리고, 거래는 인기 종목 몇 개에 쏠립니다. 이 글이 씌운 zipf 분포가 그 모양입니다. 계좌 잔고 갱신도 한 행에 갱신이 몰린다는 점에서는 같습니다. 다만 잔고에는 슬롯을 그대로 얹기 어렵습니다. 후원 누적은 더하기만 하면 되지만 잔고는 "음수가 되면 안 된다"를 갱신하는 그 순간 확인해야 하고, 값이 슬롯 N개에 흩어져 있으면 그 확인이 매번 `SUM` 한 번을 요구하기 때문입니다. 슬롯이 잘 맞는 자리에는 "쓰기만 몰리고 읽기는 드문 누적값"이라는 조건이 붙습니다.
...
![zipf 변형별 비교](/uploads/incident/slotted-counter/chart-zipf.png)
...
이 50에는 근거가 없습니다. `OPTIMISTIC_RETRY` 기본값으로 정한 임의값이고, 백오프도 없이 실패하면 곧바로 다시 시도합니다. 상한과 백오프를 바꾸면 실패율과 처리량이 같이 움직이므로, 낙관적 락 행은 "이 설정에서의 값"으로만 읽어야 합니다.

![낙관적 락 되감기](/uploads/incident/slotted-counter/fig-retry.png)

### 원인 지표: 행 락 대기

처리량은 결과이고 락 대기가 원인입니다. 측정 구간의 `Innodb_row_lock_waits`와 `Innodb_row_lock_time` 증가분을 받아 적었습니다.

![락 평균 대기](/uploads/incident/slotted-counter/chart-lock-zipf.png)
...
표의 세 행은 각 변형의 중앙값 실행 하나에서 가져온 값이라 요청 수와 대기 횟수가 같은 실행에서 나옵니다. 실행별로 따로 나눠도 방향은 같습니다. 요청당 대기 횟수는 원자적 UPDATE가 0.314~0.316회, 슬롯 16이 0.139~0.144회, 슬롯 64가 0.036~0.037회로 범위가 겹치지 않습니다.
...
쓰기와 다른 조건입니다. VU 50으로 30초 동안 방송 1번의 총액만 반복 조회했습니다. 앞서 30초 동안 같은 zipf 부하로 데이터를 채운 뒤 읽기만 쟀고, 그 시점에 방송 1번의 슬롯은 각각 16행과 64행이 모두 차 있었습니다. 채우는 시간은 같았지만 변형별 쓰기 처리량이 달라 원장 크기는 4.7만~12.8만 건으로 달랐습니다. 조회 쿼리는 원장을 읽지 않습니다.

![읽기 비용](/uploads/incident/slotted-counter/chart-read.png)
...
![인스턴스 2대 비교](/uploads/incident/slotted-counter/fig-multi.png)
...
![원장 복구](/uploads/incident/slotted-counter/fig-recovery.png)
...
이 세션은 컨테이너 MySQL에서 쟀지만, 이 글이 속한 트랙의 전제는 RDS와 Aurora입니다. 관리형으로 옮기면 네 가지가 달라집니다.
...
읽기를 복제본으로 돌리는 구성을 붙였습니다. 슬롯을 꺼내는 이유가 원본의 쓰기 처리량을 올리는 것이므로, 복제본이 따라가야 할 양도 같이 늘어납니다. 그 지연이 실제로 얼마나 벌어지는지 쟀습니다.

원본과 복제본을 GTID 기반(`SOURCE_AUTO_POSITION=1`)으로 붙이고, 복제본은 `read-only`로 두었습니다. 같은 부하(VU 100, 30초, 방송 1,000개 zipf)를 두 조건에 줍니다.

![따라잡기까지 걸린 시간](/uploads/incident/slotted-counter/fig-repl-catchup.png)
...
16과 64를 고정으로 재 봤을 뿐 방송 인기도에 따라 조절하는 방식은 없었습니다. 만들어 재봤습니다.

규칙은 단순합니다. 방송별 누적 쓰기가 문턱(200건)을 넘을 때마다 그 방송의 슬롯 수를 두 배로 키우고, 상한은 64입니다. 줄이지는 않습니다. 이미 값이 든 슬롯 행을 지울 수 없고, 읽기가 `SUM`이라 남겨 두어도 정합성에 영향이 없기 때문입니다. 높은 슬롯에 더 쓰지 않기만 하면 됩니다.

![슬롯 테이블 행 수](/uploads/incident/slotted-counter/fig-auto-rows.png)
...
zipf 분포라 인기 방송 하나만 상한 64를 받았고 959개는 슬롯 하나로 끝났습니다. **행 수를 11.4배 줄이면서 처리량 92.5%를 유지합니다.**

처리량이 7.5% 낮은 것은 정직하게 적어 둡니다. 요청마다 방송별 누적 쓰기를 하나 올리는 비용이 붙고, 슬롯 하나만 받은 방송은 경합이 생길 수 있습니다. 이 부하는 zipf라 그 959개에 트래픽이 거의 없어서 드러나지 않았습니다. **트래픽이 고르게 퍼지는 부하에서는 자동 쪽이 더 불리해질 것으로 보이고, 그 조건은 재지 않았습니다.**

행 수가 줄어드는 것이 왜 중요한지는 6절과 이어집니다. 슬롯 행이 적으면 복제본이 적용할 이벤트도 적고, 읽기 쪽 `SUM`이 훑는 행도 적습니다.

## 8. 인스턴스 비교에서 힙 혼입을 없앰

5절의 인스턴스 2대 비교는 인스턴스마다 힙을 1GB씩 줘서 총 힙이 2GB였습니다. 1대(1GB)와 나란히 놓으면 처리량 차이에 힙 차이가 섞입니다. 총 힙을 1,024MB로 고정하고 다시 쟀습니다. 2대일 때는 512MB씩 나눠 줍니다.
...
**결론이 바뀌지 않습니다.** JVM 안의 자물쇠는 그 JVM만 지키므로 2대로 늘리면 유실이 다시 생깁니다. 이제 그 판정에 힙 차이가 섞여 있지 않습니다.

처리량이 1대 698.9건에서 2대 816.7건으로 오른 것도 이제 읽을 수 있는 값이 됐습니다. 힙을 반으로 나눠도 인스턴스를 늘린 쪽이 16.9% 빠릅니다. 다만 그 이득은 유실 7,426건과 함께 옵니다.
...
![커넥션 풀 없는 파이프라인](/uploads/incident/slotted-counter/fig-nopool.png)
...
**슬롯 합만 보면 속습니다.** 문턱 200에서 zipf 2,115와 uniform 2,010이 거의 같습니다. 비율로는 0.95배라 "조절이 안 된다"로 읽힐 값입니다.

**판정은 방송당 최대 슬롯입니다.** 같은 문턱 200에서 zipf는 어떤 방송이 상한 64개를 다 받고, uniform은 최대 4개입니다. 문턱 1000에서는 uniform이 **1개**, 즉 자동 조절이 아예 안 걸립니다. 방송이 1,000개이므로 합이 1,000인 것도 그 뜻입니다.

**자동 조절이 실제로 조절을 합니다.** 쏠린 방송에는 상한까지 주고 고른 부하에는 거의 안 줍니다. 합이 비슷한 것은 zipf에서 소수가 64개씩 갖고 나머지가 1개씩인 반면 uniform은 전부 4개씩이기 때문입니다. **분포가 완전히 다른데 합이 우연히 가깝습니다.**

문턱을 올릴수록 두 조건 다 슬롯이 줄어듭니다. uniform은 50에서 16개, 200에서 4개, 1000에서 1개로 문턱에 반비례합니다.

처리량은 네 조건이 4,300~4,457 tps로 3.6% 안에 있습니다. **슬롯을 덜 주는 것이 처리량을 깎지 않습니다.** 고른 부하에서는 애초에 경합이 없어 슬롯이 필요 없습니다.
...
- **복제 대조를 한 번씩만 쟀습니다.** 6절의 0.53초와 41.83초는 각각 1회 실행입니다. 복제본이 하나이고 지연 복제나 병렬 적용(`replica_parallel_workers`)은 다루지 않았습니다. 복제 지연이 평소에는 0으로 보이다가 승격에서 드러나는 방식은 같은 시리즈의 [복제 지연 중 승격, 성공 응답을 받은 커밋이 사라진다](/blog/incident/replica-promotion-loss)에서 다룹니다.
- **`zipf` 문턱 50 조건이 빠졌습니다.** 9절에서 그 조건만 앱이 뜨지 않았습니다. 그리고 조건마다 1회 실행입니다.
- **5절 표의 인스턴스 2대 값은 총 힙 2GB에서 잰 것입니다.** 8절에서 총 힙을 고정해 다시 쟀고 결론은 같았습니다. 5절 표의 절대값은 그 혼입이 있는 상태로 남겨 두었습니다.

---

재현에 쓴 compose 파일과 실행 출력 원문은 [incident-lab 저장소의 R13 세션](https://github.com/dj258255/incident-lab/tree/main/sessions/R13-slotted-counter)에 있습니다.
