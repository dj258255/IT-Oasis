---
title: "정산 배치가 풀 스캔을 돌리면 실시간 조회가 버퍼 풀에서 밀려난다"
titleEn: "When a Settlement Batch Full Scan Evicts Live Queries from the Buffer Pool"
description: "전역 히트율만 보는 대시보드는 정산 배치가 돌 때마다 잘못된 경보를 냅니다. 그 지표에는 스캔 자신의 미스가 섞여 있어서, 워킹셋이 멀쩡해도 숫자는 무너집니다. MySQL 8.4에서 핫 테이블 약 0.3GB와 콜드 테이블 약 2.0GB를 버퍼 풀 1GB 위에 올려놓고 innodb_old_blocks_time을 0과 1000ms로 나눠 풀 스캔 배치를 걸었습니다. Percona가 2011년에 본 330에서 2 req/s로의 붕괴는 NVMe 환경에서 재현되지 않았습니다. 점조회 p95는 2.6ms에서 2.9ms 사이에 머물렀고, 스캔 구간의 전역 히트율은 7%까지 떨어졌습니다. 워킹셋이 다쳤는지는 스캔이 끝난 뒤의 회복 곡선으로 갈렸습니다. 히트율이 99%로 돌아오는 데 방어를 끄면 13.7~15.6초, 기본값이면 0.9~1.6초가 걸렸습니다. 관리형 DB에서는 같은 미스가 돈입니다. 콜드 테이블 2,056MiB를 한 번 훑으면 Aurora Standard 기준 read I/O가 약 13만 1천 건이고, 로컬에서 2.71초 걸린 이 스캔은 gp3 기본 처리량 125MiB/s에서 16.5초가 됩니다."
descriptionEn: "A dashboard that watches only the global buffer pool hit rate cries wolf every time a settlement batch runs, because that metric includes the scan's own misses and collapses even when the working set is untouched. This session places a 0.3GB hot table and a 2.0GB cold table behind a 1GB buffer pool on MySQL 8.4 and runs the batch with innodb_old_blocks_time set to 0 and to the 1000ms default. The 330-to-2 req/s collapse Percona measured in 2011 did not reappear on NVMe, where point-query p95 stayed between 2.6ms and 2.9ms even though the global hit rate fell to 7% during the scan. What separated a damaged working set from an intact one was the recovery curve after the scan, since the hit rate climbed back to 99% in 0.9 to 1.6 seconds with the default and took 13.7 to 15.6 seconds with the defense turned off. On managed databases the same misses cost money: one pass over the 2,056MiB cold table is roughly 131,008 billed read I/Os on Aurora Standard, and the scan that took 2.71 seconds locally would take 16.5 seconds at the 125MiB/s gp3 baseline throughput."
date: 2026-04-01
tags:
  - MySQL
  - InnoDB
...
![OLTP p95](/uploads/incident/batch-cache-pollution/chart-oltp.png)
...
![히트율](/uploads/incident/batch-cache-pollution/chart-hitrate.png)
...
5절의 지속 스캔이 60초 동안 몇 번 돌았는지를 파일로 남기지 않아 총 read I/O를 계산하지 못했습니다. 남기게 고치고 다시 돌렸습니다.
...
**페이지 수로 옮기면 규모가 보입니다.** 60초에 284만에서 323만 페이지입니다. 16KB 페이지이므로 44GiB에서 50GiB이고, 초당 733MiB에서 833MiB입니다.

gp3의 기본값이 125MiB/s이므로 **이 부하는 그 상한의 다섯 배를 넘게 요구합니다.** 로컬 NVMe라 여기서는 통과했지만 관리형 환경이라면 스로틀에 걸립니다. 이 글이 "스토리지 상한 환경을 만들지 못했다"고 적은 자리의 크기가 이 값입니다.

`old_blocks_time=1000` 쪽이 오히려 스캔을 세 번 더 돌았습니다. 미드포인트 삽입이 스캔 페이지를 젊은 쪽으로 못 올리게 막으므로 스캔 자체는 캐시를 덜 쓰고 지나갑니다. **보호 장치가 스캔을 느리게 만들지 않습니다.**

이 값은 테이블 크기 곱하기 스캔 횟수의 추정입니다. 프리페치나 스캔 도중의 버퍼 재사용은 반영되지 않으므로 상한으로 읽어야 합니다.
...
지연 쪽은 네 조건이 다 같습니다. 2.86ms부터 3.17ms 사이이고 평시 대비 1.0~1.1배입니다. 6절에 적은 대로 이 호스트의 NVMe에서는 미스가 싸서, 오염이 일어나도 점조회가 안 아픕니다.
...
### 그 간극을 직접 재 보려다 실패했습니다

매뉴얼은 효과가 작다고 하고 Percona는 165배를 쟀습니다. 조건 차이라면 만들어 볼 수 있으니 재 봤습니다. 버퍼 풀 256M을 고정하고 배치가 훑을 콜드 테이블과 OLTP의 핫셋 크기를 바꿔 가며, 조건마다 3회씩 돌렸습니다.

**핫셋 22.5MB, 콜드 1,341MB** (핫셋이 풀에 넉넉히 들어감)

| 조건 | OLTP 중앙 | 3회 값 |
|---|---|---|
| 배치 없음 | 1,992ms | 2324, 1992, 1909 |
| `time=0 pct=37` | 2,222ms | 2222, 2256, 2185 |
| `time=1000 pct=37` (기본) | 2,209ms | 2306, 2198, 2209 |
| `time=1000 pct=5` | 2,336ms | 2301, 2378, 2336 |

**핫셋 180MB, 콜드 670MB** (핫셋이 풀을 거의 채움)

| 조건 | OLTP 중앙 | 3회 값 |
|---|---|---|
| 배치 없음 | 25,519ms | 31364, 25519, 25117 |
| `time=0 pct=37` | 68,904ms | 66142, 73170, 68904 |
| `time=1000 pct=37` (기본) | 83,366ms | 85031, 83366, 81491 |
| `time=1000 pct=5` | 77,031ms | 77085, 75678, 77031 |

**두 조건 어디서도 방어가 이기지 못했습니다.** 첫 조건에서는 회차 폭 안이라 차이를 말할 수 없고, 둘째 조건에서는 방어를 켠 쪽이 오히려 21% 느립니다.

억지로 맞추지 않고 왜 못 잰 것인지를 적습니다.

**첫째, 지표가 둘을 못 가릅니다.** `Innodb_buffer_pool_reads`는 전역 카운터라 OLTP가 읽은 것과 배치가 읽은 것이 섞입니다. 실제로 방어를 조일수록 이 값이 늘었는데(706쪽에서 3,581쪽), 이건 배치의 페이지가 빨리 밀려나 **배치 자신이 디스크를 더 읽은 것**입니다. 방어가 의도한 대로 동작한 증거일 수도 있는데, 이 카운터로는 확정할 수 없습니다.

**둘째, 부하의 모양이 다릅니다.** Percona의 OLTP는 sysbench이고 배치는 `mysqldump`의 순수 순차 읽기입니다. 이 재현의 OLTP는 무작위 키를 담은 probes 테이블과의 조인이라, OLTP 자신이 150만 행짜리 순차 읽기를 함께 합니다. **방어해야 할 쪽이 방어 대상과 같은 접근 패턴을 섞어 쓰고 있습니다.**

**셋째, 둘째 조건은 핫셋이 이미 안 들어갑니다.** 배치 없이도 39만 쪽을 읽습니다. 방어는 "들어가던 것이 밀려나는 것"을 막는 장치인데, 처음부터 안 들어가면 막을 것이 없습니다.

그래서 이 절이 말할 수 있는 것은 하나입니다. **매뉴얼의 유보가 이 랩의 경험과 맞습니다.** "harder to predict, relatively small, and varies more with the workload"이고, 실제로 조건을 두 번 바꿔 봤는데 두 번 다 다른 방향이 나왔습니다. 이 파라미터를 만지기 전에 자기 워크로드로 재라는 매뉴얼의 마지막 문장이 빈말이 아닙니다.
...
재현에 쓴 compose 파일과 실행 출력 원문은 [incident-lab 저장소의 R16 세션](https://github.com/dj258255/incident-lab/tree/main/sessions/R16-batch-cache-pollution)에 있습니다.
