...
5절과 6절의 값은 시드 하나에서 나온 것입니다. 시드를 서른 개로 늘려 같은 실험을 다시 돌렸고, 걸음마다의 반올림 방향도 함께 셌습니다.

### 걸음마다 어느 쪽으로 갔는가

지수 갱신 60,000회에서 매 걸음의 반올림 방향입니다(시드 42, 본문과 같은 조건).
...
**절삭은 올림이 0건입니다.** 값이 양수인 한 항상 0 쪽으로 깎으므로 한 방향으로만 움직이고, 그래서 오차가 상쇄 없이 쌓입니다. HALF_EVEN은 내림과 올림이 137건 차이로 거의 반반입니다. 최종 오차가 작은 이유는 걸음마다 작아서가 아니라 방향이 갈려 상쇄되기 때문입니다. 같은 1,511분의 1이라는 배수를 최종값이 아니라 과정으로 보면 이렇게 나옵니다.
...
**절삭 오차는 서른 개 시드가 전부 음수입니다.** 폭도 -30.086에서 -29.805 사이로 좁습니다. 구조적 편향이라 시드가 정하는 것은 소수점 아래 흔들림뿐이고 방향은 안 바뀝니다.

**HALF_UP 편향은 서른 개 시드가 전부 정확히 +50,000원입니다.** 시드가 정하는 것이 하나도 없습니다. 수수료 10만 건이 전부 x.5원 경계에 걸리도록 조건을 만들었으므로 HALF_UP은 예외 없이 위로 보내고, 건당 +0.5원 곱하기 10만 건이 그대로 값이 됩니다. 5절이 "이 값은 시드와 무관하게 결정된다"고 적은 것이 서른 개 시드로 확인됐습니다.

**HALF_EVEN 편향은 부호가 갈립니다.** 서른 개 중 열일곱 개가 음수이고 -106원에서 +99원 사이에 흩어집니다. 그리고 **5절과 6절이 인용한 +99원은 이 서른 개 중 최대값입니다.** 중앙값은 -11원이고 평균은 -9.03원입니다. 그러니까 +99원은 HALF_EVEN이 남기는 전형적인 잔여가 아니라 흔들림의 위쪽 끝입니다. 6절이 "표본 하나의 잔여로만 읽어야 한다"고 적어 둔 것은 맞았고, 그것이 어느 위치의 표본이었는지가 이제 나왔습니다.

같은 자리를 다르게도 읽을 수 있습니다. HALF_EVEN의 잔여는 최악을 잡아도 100원대이고 HALF_UP의 편향은 시드와 무관한 50,000원입니다. 자릿수가 셋 차이 납니다. 결론은 그대로이고 근거만 단단해졌습니다.
...
- **실제 VSE 산식과 22개월치 데이터를 재현하지 않았습니다.** 지수 갱신을 현재값에 무작위 증감을 더하는 모형으로 단순화했고 횟수도 60,000회입니다. 이 세션의 29.9포인트를 사건의 수백 포인트 격차와 직접 비교할 수 없습니다.
- **횟수를 사건 규모로 늘려도 사건에 가까워지지 않습니다.** `RoundingMode.DOWN`은 0 쪽으로 버리는 모드라 지수가 음수 구간에 들어가면 편향이 위로 뒤집힙니다. 100만 회대에서는 이 모형의 전제 자체가 깨지므로 편향이 선형으로 쌓이는 구간만 잘랐습니다.
- **1차 보도의 지면 원문을 확인하지 못했습니다.** 서지사항은 전부 재인용이고, 정정 직전 지수도 524.811과 520.638로 자료마다 다릅니다.
- **곱셈과 나눗셈의 반올림 방향은 재지 않았습니다.** 7절이 센 것은 덧셈 경로의 걸음별 방향이고, 요율을 곱하거나 비율을 나누는 자리의 방향 분포는 다루지 않았습니다.
- **시드 서른 개는 지수 갱신과 수수료 두 실험에만 돌렸습니다.** 7절의 분포는 그 둘의 것이고, 1절과 4절의 다른 실험은 여전히 고정 시드 한 판입니다.
