...
위 실패 기록에 원인이 하나 분명하게 적혀 있습니다. `sb-sp70` 조건에서 클라이언트 넷이 **겹치는 행 락 70개**를 잡아 서로 막았고 쓰기가 초당 0.68건이었습니다. `ASSIGNMENT` 레코드가 나올 수 있는 유일한 조건이었는데 정작 쓰기량이 없었습니다.

고칠 지점이 하나였습니다. **클라이언트마다 자기 키 범위를 줍니다.** 그러면 서브트랜잭션 200개를 쓰면서도 서로 안 막습니다.

프라이머리 쓰기량을 조건 사이에 맞추고(폭 5%), 조건마다 두 컨테이너를 재시작해 캐시 상태를 같게 한 결과입니다.

| 조건 | 스탠바이 조회 | 프라이머리 UPDATE | WAL `ASSIGNMENT` | 스탠바이 `pg_subtrans` 히트 |
|---|---|---|---|---|
| A 기준선 | 1,276회 | 66,000 | 0 | 0 |
| B 롱TX만 | 1,114회 | 66,000 | 0 | 0 |
| C 서브TX만 | 1,286회 | 62,400 | 936 | 72,788 |
| **D 롱TX + 서브TX** | **1,006회** | 63,600 | 1,890 | **115,420,156** |

**D에서만 `pg_subtrans` 조회가 1억 1,542만 건입니다.** C의 1,585배입니다. 위 표에서 모든 조건이 0이었던 그 칸입니다.

`ASSIGNMENT` 레코드도 936건과 1,890건으로 잡힙니다. **위에서 "이 레코드가 안 나와서 스탠바이가 오버플로를 통보받지 못한다"고 분석한 그 경로가 이번에는 열렸습니다.** 분석은 맞았고, 그 조건을 못 만든 것이 실패의 내용이었습니다.

스탠바이 처리량은 1,276회에서 1,006회로 **21% 내려갑니다.** 롱TX만 있을 때(1,114회)나 서브TX만 있을 때(1,286회)로는 안 내려가고 둘이 겹쳐야 내려갑니다. GitLab이 적은 조합 조건과 같습니다.

**다만 21%는 GitLab이 겪은 규모가 아닙니다.** 그들은 에러율이 급증해 서비스가 흔들렸습니다. 이 재현은 메커니즘이 도는 것과 방향까지이고, 절벽의 깊이는 이 규모에서 나오는 값입니다. 200,000행에 클라이언트 6개이고 GitLab은 그보다 훨씬 큽니다.

정정할 것이 하나 더 있습니다. 위에서 든 세 가지 조건 중 첫째("쓰기량이 없어서 조건을 못 만들었다")는 이번에 해소됐습니다. 둘째(버전 12 대 17)와 셋째(같은 호스트의 컨테이너)는 그대로 남습니다.
...
PostgreSQL 16과 17.5를 같은 부하로 나란히 돌렸습니다. 두 버전 다 `subtransaction_buffers`가 32입니다. 16 에는 그 GUC 자체가 없고 값이 32로 고정입니다. 17에서 32로 되돌려 맞췄습니다. 버퍼 수가 같으므로 남는 차이는 SLRU 락 구조입니다. 조건마다 3회 돌렸습니다.
...
최솟값이 29,000 대이므로 한 회차가 통째로 느렸다는 뜻이고, 같은 호스트에서 다른 컨테이너와 겹친 구간으로 봅니다. **개별 회차 값은 인용하면 안 되고 중앙값만 씁니다.** 17.5 쪽은 폭이 5% 안이라 그 조건이 없었습니다.

폭이 이렇게 넓은데도 중앙값 배수는 1회 실행 때와 같습니다. 앞서 1회로 잰 값이 1.00 / 1.01 / 1.00 / 0.98 / 1.16배였고 3회 중앙값이 1.00 / 1.01 / 1.00 / 1.01 / 1.15배입니다.
...
떨어집니다. 두 조건 다 SLRU 읽기가 0 이므로 이 계단은 디스크 접근이 아니라 PGPROC 캐시 64칸을 넘긴 비용 자체입니다. **"활성 수를 64 미만으로 유지하면 회복된다"가 처리량 축에서도 확인됩니다.** 2절에서 `ROLLBACK TO`가 캐시를 비운다는 것만 봤는데, 비운 상태의 처리량이 실제로 none에 가깝습니다.

절벽은 63과 64 사이가 아니라 10,000과 500,000 사이입니다. 0.93배와 0.60배 사이가 훨씬 큽니다. 64를 넘기는 것 자체는 7% 손해이고, SLRU가 캐시에 안 들어갈 만큼 불어나는 것이 40% 손해입니다.

17로 올려도 sub500k는 여전히 none의 0.70배입니다. **락 구조 개선은 손해를 40%에서 30%로 줄이지 그 조건을 없애지 않습니다.**
