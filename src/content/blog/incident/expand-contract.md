...
- **백필 중 조회 지연의 원인에 양성 증거가 없습니다.** 락이 아니라는 배제까지는 했지만 디스크 경쟁 쪽은 `pg_stat_io`도 `EXPLAIN (ANALYZE, BUFFERS)`도 `iostat`도 재지 않았습니다. 2 vCPU 한 대에서 백필과 조회가 같은 디스크를 쓴다는 것은 가설로 남아 있습니다.
- **35배는 5회차 한 실행 안에서 나온 비율입니다.** 앞선 회차는 기준선을 따로 옮겨 적지 않아 같은 방식으로 계산하지 못했습니다.
- **백필 뒤 `VACUUM` 계획을 세워 재지 않았습니다.** 3단계가 437MB로 부푼 것과 죽은 튜플 2,999,730개는 관측했지만, autovacuum이 언제 얼마나 회수하는지는 재지 않았습니다.
