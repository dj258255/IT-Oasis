...
**기준선의 read 블록이 세 회차에서 114,668~114,676으로 사실상 같습니다.** 같은 조회가 같은 테이블을 세는 것이니 당연합니다. 그런데 백필이 도는 동안 같은 조회의 read 블록이 **170,528에서 194,692로 1.5배에서 1.7배 늘어납니다.**
...
- **autovacuum 이 언제 도는지는 재지 않았습니다.** 7절은 `VACUUM` 을 손으로 돌린 값이고, autovacuum 이 자기 임계값으로 이 테이블을 언제 집는지는 다루지 않았습니다.
