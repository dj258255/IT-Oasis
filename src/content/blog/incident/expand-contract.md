---
title: '빠른 DDL과 안전한 DDL은 다르다'
titleEn: 'Fast DDL Is Not Safe DDL'
description: "PostgreSQL 11부터 상수 기본값 컬럼 추가는 테이블을 재작성하지 않아 300만 행에서도 3 ms에 끝납니다. 그런데 같은 문장이 12초짜리 트랜잭션 뒤에 줄을 서니 락이 충돌하지도 않는 평범한 SELECT 세 건이 각각 7초에서 9초씩 밀렸습니다. postgres:16-alpine 하나로 volatile 기본값의 테이블 재작성(5회 실행에서 11.3초부터 28.0초)과 그 뒤의 락 큐잉을 만들어 봤습니다. 3단계 expand-contract는 가장 긴 ACCESS EXCLUSIVE 보유를 3.436 ms로 줄이지만 그 락을 잡는 문장이 하나에서 셋으로 늘어나므로, 세 문장 모두에 lock_timeout과 재시도가 따라와야 합니다."
descriptionEn: 'Since PostgreSQL 11, adding a column with a constant default no longer rewrites the table, so it finishes in 3 ms even on 3 million rows. Yet the same statement, once it queues behind a 12-second transaction, stalls three ordinary SELECTs for 7 to 9 seconds each even though their locks never conflict. Using a single postgres:16-alpine container, this session reproduces the table rewrite caused by a volatile default (11.3 to 28.0 seconds across five runs) and the lock queueing behind it. A three-step expand-contract migration cuts the longest ACCESS EXCLUSIVE hold to 3.436 ms, but it also turns one exclusive-lock window into three, so every one of those statements needs lock_timeout and a retry.'
date: 2026-02-05
tags:
  - PostgreSQL
  - DDL
...
이 11,316.601 ms는 같은 스크립트를 다섯 번 돌려 얻은 값 가운데 가장 작은 것입니다. 다섯 회차의 관측 범위는 11,316.601 ms에서 27,989.225 ms이고, 회차별 값은 27,135.545 / 27,989.225 / 20,440.788 / 12,117.905 / 11,316.601입니다. 다시 말해 11.3초는 이 서버가 보여 준 최선의 경우이고 나쁠 때는 28초였습니다. 아래에서 11.3초라고 적은 자리는 전부 이 범위의 아래끝이라고 읽으시면 됩니다.
...
![B43 재현 실행 화면](/uploads/incident/expand-contract/01-volatile-alter.png)
...
![B43 락 큐잉 실행 화면](/uploads/incident/expand-contract/02-lock-queue.png)

*그림 2. 장기 트랜잭션(316)은 ACCESS SHARE만 쥐고 있는데도 전체가 멈췄습니다. 막고 있는 pid 칸을 보면 SELECT 세 건을 막는 것은 316이 아니라 그 앞에 끼어든 ALTER(325)입니다.*

명령과 출력 원문은 [reproduce.md](https://github.com/dj258255/incident-lab/blob/main/sessions/B43-expand-contract/reproduce.md)에 그대로 남겼습니다.
...
![B43 해소 재계측 실행 화면](/uploads/incident/expand-contract/03-fixed-run.png)
...
![SELECT가 막힌 시간 전후](/uploads/incident/expand-contract/04-select-wait.png)

*그림 4. 같은 300만 행에서 다른 세션의 SELECT count(*)가 막힌 시간입니다. 락 없을 때 기준선은 0.16초에서 0.31초 사이입니다.*

![최대 락 보유 시간 전후](/uploads/incident/expand-contract/05-lock-hold.png)
...
**기준선의 read 블록이 세 회차에서 114,664~114,672로 사실상 같습니다.** 같은 조회가 같은 테이블을 세는 것이니 당연합니다. 그런데 백필이 도는 동안 같은 조회의 read 블록이 **185,040에서 194,424로 1.6배에서 1.7배 늘어납니다.**
...
**죽은 튜플은 사라지는데 파일 크기는 그대로입니다.** `VACUUM`은 죽은 튜플을 재사용 가능으로 표시할 뿐 파일을 줄이지 않습니다. 줄이려면 `VACUUM FULL`이 필요하고 그것은 `ACCESS EXCLUSIVE`를 잡습니다.

**그래서 백필의 대가는 시간이 아니라 되찾기 어려운 공간입니다.** 2.51초짜리 `VACUUM`으로 다음 쓰기가 그 자리를 재사용할 수 있게는 되지만, 디스크에서 711MB가 줄어들지는 않습니다. 스토리지가 자동 확장되고 축소되지 않는 관리형 환경에서는 이 차이가 그대로 요금이 됩니다.
...
본문 8절이 표 하나에 1회 실행이었습니다. 80만 행으로 규모를 맞춘 뒤 시드부터 끝까지 3회 돌렸습니다.

| 구간 | 회차별 (ms) | 중앙 | 폭 |
|---|---|---|---|
| `ADD COLUMN` 상수 기본값 | 0.975 / 1.023 / 0.890 | 0.975 | 13.6% |
| `ADD COLUMN` 두 번째 | 14.898 / 14.633 / 14.675 | 14.68 | 1.8% |
| vacuum | 88.698 / 80.555 / 79.780 | 80.56 | 11.1% |
| 백필 | 15.866 / 14.512 / 14.842 | 14.84 | 9.1% |
| `ADD CONSTRAINT NOT VALID` | 0.080 / 0.088 / 0.105 | 0.088 | 28.4% |
| `VALIDATE CONSTRAINT` | 0.085 / 0.067 / 0.077 | 0.077 | 23.4% |

**결론이 걸린 구간은 전부 폭 30% 안입니다.** `ADD COLUMN`이 밀리초에 끝난다는 것도, `NOT VALID`가 그보다 한 자릿수 더 빠르다는 것도 회차마다 같습니다.

**본문의 "PG 11 이후로 대형 테이블 ADD COLUMN 조심이 옛날 지식이 됐다"는 결론이 반복으로 확인됩니다.** 0.975ms가 1.023ms가 되어도 결론은 같은 자리에 있습니다.

**흔들리는 것은 락 대기 쪽입니다.** 같은 로그에서 `Time: 132.590 ms`로 찍힌 줄이 회차마다 5.592ms에서 132.6ms까지 나옵니다. 폭 1,877%입니다. 락 대기는 어느 세션이 먼저 잡느냐에 달린 값이라 이 랩의 다른 최댓값 통계와 같은 성질입니다. 본문 8절에서 autovacuum이 DDL 앞을 막아 1,010ms가 걸린 회차를 적었는데, **그 1,010ms는 그 회차의 값이고 다음 회차에는 안 나올 수 있습니다.**

이 실험을 돌리는 데 네 번 걸렸습니다. 실행 비트가 없었고, 호스트에서 `psql`을 못 찾았고, 러너 컨테이너가 상주하지 않아 `docker exec`가 종료 코드 137로 끊겼습니다. 세 번째를 처음에는 OOM으로 봤는데, **행 수를 300만에서 80만으로 줄여도 같은 지점에서 죽는 것이 단서였습니다.** 규모 문제였으면 줄였을 때 달라졌어야 합니다.

## 못 한 것

- **8절: 표 하나, 1회 실행입니다.** 300만 행에서 vacuum이 64.3초였는데 표 크기를 바꿔 가며 그 길이가 어떻게 가는지는 안 봤습니다. 80만 행으로 전 과정을 3회 반복한 결과는 위 절에 있고, 결론이 걸린 구간은 폭 30% 안입니다.
- **카탈로그 주장의 앞부분을 재현하지 않았습니다.** "컬럼 추가와 삭제 순서가 뒤집히면 배포 중 없는 컬럼을 참조한다"는 애플리케이션 배포와 스키마 변경의 순서 문제라 이 세션의 무대인 락 관리자와 다른 층이고, 조사에서 출처도 확보하지 못했습니다.
- **락 큐잉을 명시한 공식 문서 문장을 찾지 못했습니다.** Explicit Locking 문서는 "충돌한다"까지만 말합니다. 3절의 설명은 실측한 `pg_locks` 대기 사슬과 `pg_blocking_pids()`와 서버 로그로만 뒷받침합니다.
- **버퍼 경쟁의 몫을 정량화하지 않았습니다.** 7절이 read 블록 증가로 양성 증거를 잡았지만, 그 증가분 가운데 백필이 밀어낸 몫과 체크포인트가 쓴 몫을 가르지는 않았습니다.
- **35배는 여전히 5회차 한 실행 안의 비율입니다.** 7절의 반복은 백필 구간이고 DDL 구간의 35배는 기준선 계산 방식을 맞춰 다시 재지 않았습니다.

---

재현에 쓴 compose 파일과 실행 출력 원문은 [incident-lab 저장소의 B43 세션](https://github.com/dj258255/incident-lab/tree/main/sessions/B43-expand-contract)에 있습니다.
