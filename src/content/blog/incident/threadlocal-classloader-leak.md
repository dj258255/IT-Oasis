...
여기까지는 웹앱 배포 한 번을 `URLClassLoader` 하나로 흉내 낸 것이었습니다. 이번에는 실제 Tomcat에 서블릿 하나짜리 WAR를 열 번 올렸다 내리며 매번 `/leak`을 스무 번씩 찔렀습니다. 그 서블릿은 `ThreadLocal`에 자기 클래스로더가 적재한 객체를 넣고 `remove()`를 부르지 않습니다.
