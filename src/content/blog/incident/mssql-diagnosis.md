...
앞의 두 편은 전부 **무엇이 문제인지 아는 상태**에서 출발했습니다. 락 승격을 재려고
...
스크립트와 결과는 [incident-lab](https://github.com/dj258255/incident-lab) 의
`sessions/A26-mssql-diagnosis` 에 있습니다.
...
단위로 쌓여 있습니다. 그런데 `sys.dm_os_wait_stats` 는 **인스턴스가 뜬 뒤로 계속
더해진 값**입니다.
...
상위를 전부 차지해 사고가 만든 `LCK_M_IS` 가 3위 밖으로 밀려 있었습니다. 둘 다
시스템이 놀 때 쌓는 대기라 사고와 무관합니다. **제가 만든 무시 목록에 그 둘이
빠져 있었습니다.**
...
그것을 죽여도 뿌리가 그대로라 다시 같은 일이 납니다. 재귀로 사슬을 따라가면
뿌리가 나옵니다.
...
앞 절의 방법에는 전제가 있습니다. **계획이 캐시에 남아 있어야 합니다.** 사고가 지나간
...
계획의** 통계이고, 계획이 없어지면 통계도 함께 없어집니다. Query Store 는 디스크에
쓰기 때문에 그대로 있습니다.
...
블로킹 사슬은 Query Store 로 못 찾습니다. 지금 누가 누구를 막고 있는지는 저장되는
값이 아니기 때문입니다.
...
"두 번 떠서 차분"에는 **사고가 진행 중이어야 한다**는 전제가 있었습니다. 신고가 늦으면
기준으로 삼을 앞 스냅샷이 없습니다. 수집기를 만듭니다. 누적값을 회차마다 그대로 적고
**차분은 읽을 때 냅니다.**
...
가져다 쓰게 고쳤습니다. 규칙을 문서에만 적어 두면 다음 사람이(그것이 자기 자신이어도)
또 만듭니다.
...
같은 함수를 3편에서도 썼는데 거기서는 화면이 아니라 `RESTORE ... STOPAT` 의 인자로
