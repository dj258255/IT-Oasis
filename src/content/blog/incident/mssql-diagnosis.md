---
title: '느려졌다는 신고 하나로 시작할 때, MSSQL 장애 진단 순서'
description: "앞의 세 편은 무엇이 문제인지 아는 상태에서 출발했습니다. 운영은 반대라 느려졌다는 신고 하나로 시작합니다. 대기 통계는 인스턴스가 뜬 뒤로 누적된 값이라 지금을 못 보고, 블로킹 사슬의 뿌리는 sleeping 이라 실행 중인 것만 보는 도구에는 안 나오며, 계획 캐시를 총 논리 읽기로 정렬하면 리터럴로 흩어진 범인이 순위에 못 옵니다. 세 자리를 실측으로 확인하고 순서로 묶었습니다."
date: 2026-08-09
tags:
  - SQL Server
  - MSSQL
...
세 자리에서 **정석대로 하면 틀리는** 지점이 나왔습니다.

- 대기 통계를 그냥 보면 이번 사고가 아니라 인스턴스가 뜬 뒤 전부를 봅니다
- 막은 세션을 죽이면 그것도 피해자일 수 있고, 진짜 뿌리는 `sleeping` 이라 안 보입니다
- 계획 캐시를 총 논리 읽기로 정렬하면 부하의 절반이 순위에서 사라집니다
...
- **계획이 캐시에서 밀려나는 것을 안 봤습니다.** `query_stats` 는 계획이 사라지면 통계도 함께 사라집니다. 사고가 지나간 뒤에 들여다보면 범인이 이미 없을 수 있는데 그 상황을 만들지 않았습니다.
- **대기 유형이 하나뿐입니다.** 락 대기만 만들었습니다. `PAGEIOLATCH`, `WRITELOG`, `RESOURCE_SEMAPHORE` 처럼 다른 원인을 가리키는 대기는 재현하지 않았습니다.
- **시간을 재지 않았습니다.** ARM 에뮬레이션이라 "진단에 몇 분 걸리는가"는 이 랩에서 답할 수 없습니다.
- **자동 수집을 만들지 않았습니다.** 사고가 난 뒤에 스냅샷을 뜨면 이미 늦습니다. 주기적으로 떠 두는 수집기가 있어야 하는데 만들지 않았습니다.
- **Query Store 를 안 썼습니다.** 2016 이후로는 그쪽이 표준에 가까운데 DMV 만 다뤘습니다.

이 시리즈는 여기서 닫습니다. 탐지(1편) · 락(2편) · 보정(3편) 은 전부 원인을 아는
상태에서 출발했고, 이 편이 그 앞에 붙는 자리입니다. 실제 순서는 진단이 먼저입니다.
