...
그런데 그 서술과 이 세션의 재현 경로는 정확히 겹치지 않습니다. 이 시리즈의 카탈로그에는 이자율을 `잔여일/365`가 아니라 `잔여일/0`으로 입력한 것이 발단이라고 적어 두었는데, 분자가 0이 아니면 그 나눗셈의 결과는 Infinity입니다. 그리고 뒤에서 보듯 Infinity는 상한 비교에 걸려 거부됩니다. 사건에서 어떤 값이 어떤 검증을 어떻게 빠져나갔는지는 공개된 기록만으로 확정할 수 없습니다. 이 세션이 재현한 것은 한맥의 코드가 아니라, 분모가 0이 되어 만들어진 값이 상하한 검증을 빠져나가는 일반 메커니즘입니다. 실제 거래소와 옵션 이론가 모델은 재현하지 않았습니다.
...
입력 `@Valid` 검증은 120건 모두 통과했습니다. 이건 발견이 아닙니다. `OrderRequest`에 붙은 제약은 `@NotNull` 세 개가 전부라, 값이 비어 있지만 않으면 통과하도록 만들어 둔 구성입니다. 저장소 주석에도 검증이 도는 것을 보이려고 넣었다고 적어 두었습니다. 그러니 여기서 확인되는 것은 `@Valid`가 무력하다는 사실이 아니라, 입력 검증이 보는 자리와 위험한 값이 만들어지는 자리가 다르다는 배치입니다. `@Valid`가 보는 것은 클라이언트가 보낸 입력이고, 문제의 값은 서버가 그 뒤에 계산한 파생값입니다. 코드는 [spring/](https://github.com/dj258255/incident-lab/tree/main/sessions/F01-hanmac-divide-by-zero/spring/)에 있고, 버그 경로의 수치는 최소 재현과 맞물립니다.
...
출력에 찍힌 "비정상 0건"이라는 숫자 자체도 근거가 약합니다. 그 자리에 들어가는 `fixBad` 변수는 코드 어디에서도 증가하지 않아, 무엇을 넣고 돌리든 0으로 출력됩니다. 이 글에서는 성과 귀속을 정정하는 데까지만 하고 다시 재지 않았습니다. 가드 단독 검증은 남은 과제입니다.
