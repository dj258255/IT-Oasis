...
![중복 키 에러](/uploads/incident/int-pk-exhaustion/fig-1062.png)
...
![전환 중 쓰기 지연](/uploads/incident/int-pk-exhaustion/chart-migration.png)
...
원 사례는 MySQL이지만 같은 함정이 PostgreSQL에도 있습니다. 다른 자리가 두 곳 나옵니다. 하나는 고갈 지점의 진단 난이도이고, 다른 하나는 위험한 DDL을 누가 막아 주느냐입니다. 환경은 PostgreSQL 17.5, 같은 300만 행(191MB)입니다.

### 무엇이 고갈됐는지 알려 주는 쪽은 PostgreSQL입니다

![PostgreSQL의 고갈 지점](/uploads/incident/int-pk-exhaustion/fig-pg-exhaust.png)

| 선언 | 막는 쪽 | 에러 |
|---|---|---|
| `serial` | 시퀀스 MAXVALUE | `nextval: reached maximum value of sequence "..." (2147483647)` |
| `integer` 컬럼에 `bigint` 시퀀스 | 컬럼 캐스트 | `integer out of range` |
| `GENERATED BY DEFAULT AS IDENTITY` | 시퀀스 MAXVALUE | 첫째와 같은 문구 |
| MySQL `INT AUTO_INCREMENT` | 유니크 인덱스 | `Duplicate entry '2147483647' for key 'sponsor_int.PRIMARY'` |

PostgreSQL은 세 경우 모두 문구가 원인을 말해 줍니다. MySQL은 중복키 에러입니다. 상한에서 같은 값을 재발급하다 유니크 인덱스에 걸린 것인데, 로그만 보면 애플리케이션이 같은 키를 두 번 넣은 것과 구별되지 않습니다. **2절에서 이 함정을 다룬 이유가 이것이고, PostgreSQL 쪽은 같은 함정이 없습니다.**

`serial`과 IDENTITY가 같은 문구인 이유는 IDENTITY도 뒤에 `integer` 시퀀스를 두기 때문입니다. 두 번째 경우는 시퀀스를 `AS bigint`로 따로 만들어 붙였을 때만 나옵니다.

### 위험한 DDL을 미리 막아 주는 쪽은 MySQL입니다

3절에서 MySQL은 요청을 거부했습니다.
...
PostgreSQL에는 이 사전 거부가 없습니다. `ALTER TABLE ... ALTER COLUMN id TYPE bigint`를 그대로 받아들이고 `AccessExclusiveLock`을 쥔 채 테이블을 다시 씁니다.
...
![전환 중 통과한 쓰기](/uploads/incident/int-pk-exhaustion/fig-pg-migration.png)
...
3절과 결론이 같습니다. 실패 0건이 무중단의 근거가 되지 못하고, 1.6초짜리 전환에서 한 건이 1.5초를 매달려 있었습니다. 서버 로그의 1,497ms 대기가 같은 사건을 서버 쪽에서 찍은 것입니다. **MySQL이 에러로 알려 준 것을 PostgreSQL에서는 직접 재봐야 알 수 있습니다.**
...
`ADD COLUMN`이 88배 차이 납니다. PostgreSQL 11부터 기본값 없는 컬럼 추가는 카탈로그만 고치므로 테이블 크기와 무관합니다.

**3단계 잔여 건수는 두 값을 나란히 놓으면 안 됩니다.** 경계를 다른 데 그었습니다. MySQL 스크립트는 청크가 2만 미만을 고치면 그 자리에서 벌크를 끝내므로 부하가 넣은 행이 대부분 2단계에 흡수됐고, PostgreSQL 쪽은 시작 시점의 최대 id를 워터마크로 잡아 벌크를 그 범위로 묶었으므로 부하가 넣은 3,193건 전부가 3단계로 떨어집니다. 총 작업량은 같고 회계만 다릅니다.

벌크 청크가 양쪽 151회로 같은 것이 이 비교의 전제입니다. 처음에는 PostgreSQL 루프를 `WHERE id_new IS NULL`로만 돌렸는데, 그러면 부하가 방금 넣은 20~30행짜리 청크를 계속 쫓아 수렴하지 않았습니다. 벌크 151회 뒤에 1,400회가 넘게 이어졌습니다. **쓰기가 계속되는 동안 도는 백필은 종료 조건을 스스로 갖지 못합니다.** 시작 시점의 워터마크로 묶어야 끝납니다.

## 6. `INT UNSIGNED`로 미루는 선택지

상한이 눈앞이면 `BIGINT`까지 가지 않고 `INT UNSIGNED`로 2배만 벌 수도 있습니다. 300만 행에서 같은 테이블을 두 목표로 각각 전환해 봤습니다.
...
**전환 비용의 성격이 같습니다.** 둘 다 `ALGORITHM=INPLACE`를 같은 에러 1846으로 거부당하고 COPY로만 됩니다. 소요도 4.5초와 4.7초로 이 규모에서는 구별되지 않습니다. 3절에서 본 "COPY 구간에는 쓰기가 통과하지 못한다"도 양쪽에 똑같이 적용됩니다.

`INT UNSIGNED`는 같은 값을 치르고 2배만 사는 선택입니다. 21억을 채우는 데 걸린 시간과 같은 시간이 지나면 42억도 채웁니다. 그때 다시 같은 COPY를 해야 하고, 그 사이에 테이블은 더 커져 있습니다.
...
통과시키고 그중 하나가 4.6초를 매달려 있었습니다. 두 도구는 기준선의 88.5%와 96.5%를 유지합니다.

### 두 도구가 변경분을 따라가는 방식이 다릅니다

`pt-online-schema-change`는 원본에 INSERT/UPDATE/DELETE 트리거를 겁니다. 복사가 도는 동안 들어오는 모든 쓰기가 그 트리거를 한 번씩 더 지납니다.
...
`Copy`는 기존 행 복사이고 `Applied`는 그동안 들어온 변경분입니다. **둘이 분리돼 있고 백로그가 1,000짜리 큐로 관리됩니다.** 원본 쓰기 경로에는 아무것도 얹히지 않으므로 p95가 7ms로 셋 중 가장 낮습니다.

대가는 시간입니다. 27.5초로 pt-osc의 2.3배입니다. binlog를 읽어 따라가는 만큼 복사를 느리게 하고, 지연이 늘면 스스로 속도를 줄입니다.

### 셋 다 어딘가에서 한 번은 멈춥니다

`gh-ost`의 최대 지연 998ms는 마지막 컷오버 구간입니다. 새 테이블로 이름을 바꾸는 그 순간에는 어떤 방식이든 잠깐 멈춥니다. **차이는 멈추느냐가 아니라 그 멈춤이 4,576ms 냐 998ms 냐입니다.**
...
2. **흔한 우회인 `mysql_native_password`가 MySQL 8.4 에는 없습니다.**
   `Plugin 'mysql_native_password' is not loaded`. 8.0에서 deprecated 였고 **8.4에서 제거됐습니다.** 남는 길은 `--mysql_ssl 1`입니다.

3. **`gh-ost`는 공개 컨테이너 이미지가 없습니다.** `ghcr.io/github/gh-ost`는 접근이
   거부되고 Docker Hub 에도 없습니다. 소스에서 빌드했고 `go.mod`가 Go 1.25.12 이상을 요구해 1.24 로는 안 됩니다. 빌드 절차는 저장소의 `tools/gh-ost/README.md`에 적었습니다.
...
컨테이너를 띄우자마자 스크립트를 시작해 `CREATE TABLE`이 실패했고, 그 뒤 `ALTER`가 없는 테이블을 대상으로 즉시 실패했습니다. **실패한 회차가 아주 빠른 회차로 보입니다.** 표에 그대로 들어가면 "한 방 ALTER가 12.8초에서 0.16초까지 갈린다"는 틀린 결론이 됩니다. 스크립트에 준비 대기와 적재 행 수 확인을 넣어 실패하면 멈추도록 고쳤습니다. run1의 방법 B는 정상이라 살립니다.
...
소요는 114.1초에서 119.4초, p95는 3.6ms에서 4.0ms입니다. **발행된 값을 그대로 인용해도 됩니다.** 백필 청크도 네 회차 모두 151회로 같습니다.
...
소요는 12.71초에서 12.86초로 붙어 있습니다. 그런데 **통과한 쓰기의 지연은 회차마다 자릿수가 다릅니다.** 12,602ms와 5.9ms는 3천 배 차이입니다.

이유는 표본입니다. 통과 쓰기가 9건에서 11건뿐이고, 그중 **ALTER가 락을 잡은 뒤에 시작한 문장이 있느냐 없느냐**로 최댓값이 정해집니다. 원본 회차는 한 건이 락 뒤에 줄을 서서 12.6초를 기다렸고, 나머지 두 회차는 그런 건이 안 걸렸습니다. 4절이 이미 "표본이 11건이라 p95가 백분위수가 아니라 최댓값"이라고 적어 두었는데, 반복해 보니 그 최댓값 자체가 회차마다 나오기도 하고 안 나오기도 합니다.

**그래서 12,602ms를 한 방 ALTER의 대기 시간으로 읽으면 안 됩니다.** 정확한 서술은 이렇습니다. 12.8초 동안 쓰기가 열 건 남짓만 통과하고, 그 창에 걸린 문장은 ALTER가 끝날 때까지 기다립니다. 걸릴 확률은 쓰기 유입 속도와 창 길이가 정하고, 이 랩의 유입 속도에서는 회차의 3분의 1에서 걸렸습니다.

결론은 그대로입니다. **실패는 네 회차 모두 0건**이고, 초당 통과량이 66.5건에서 0.9건으로 떨어지는 것이 방법 A의 대가입니다. 에러 카운터만 보는 모니터링이 이 구간을 정상으로 기록한다는 4절의 서술도 유지됩니다.
...
본문의 12.8초와 119.4초는 각각 1회 실행이라 관측 범위가 없었습니다. 회차마다 새 컨테이너를 띄워 버퍼 풀과 페이지 캐시를 같은 상태에서 출발시키고 5회씩 다시 쟀습니다.

| 방법 | 중앙 | 최소 | 최대 | 폭 |
|---|---|---|---|---|
| 한 방 ALTER (COPY) | 5.2초 | 5.1초 | 5.9초 | 0.8초 |
| expand-contract | 37.7초 | 37.4초 | 41.8초 | 4.4초 |

**두 방법의 중앙값 차이 32.5초가 회차 폭 최대 4.4초보다 큽니다.** 그러니 "expand-contract가 총 시간을 더 쓴다"는 비교는 인용할 수 있습니다.

절대값은 본문과 다릅니다. 본문은 12.8초와 119.4초였습니다. 배수로 보면 본문이 9.3배, 이번이 7.3배로 같은 자리에 있습니다. **절대 시간은 호스트와 그날의 상태를 타고 배수는 덜 탑니다.** 본문의 두 숫자를 다른 세션의 숫자와 나란히 놓지 말라고 적어 둔 이유입니다.

## 못 한 것

- **9절의 표는 조건마다 1회 실행입니다.** 크기는 실행 간 편차가 거의 없는 값이라 반복하지 않았습니다.
- **초기 회차의 호스트 사양을 기록하지 않았습니다.** 컨테이너 할당량만 있고 어느 장비인지 확인되지 않아, 이 글의 절대 시간은 다른 세션의 절대 시간과 나란히 놓을 수 없습니다. 반복 측정을 하면 그때 기록이 남으므로 이 항목은 반복 측정과 함께 해소됩니다.
...
재현에 쓴 compose 파일과 실행 출력 원문은 [incident-lab 저장소의 A01 세션](https://github.com/dj258255/incident-lab/tree/main/sessions/A01-int-pk-exhaustion)에 있습니다.
