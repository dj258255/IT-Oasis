...
- **2절부터 5절까지는 읽기 부하만입니다.** 쓰기 부하는 6절에서만 쟀고, 그쪽도 조건 1회 실행입니다.
- **6절의 풀 실험은 최소 구현입니다.** `queue.Queue` 기반 풀이라 HikariCP 의 `maxLifetime`, `leakDetectionThreshold`, 하우스키핑 같은 동작이 없습니다. 실제 풀에서 같은 비율이 나오는지는 재지 않았습니다. 획득 타임아웃도 기본값 30초가 아니라 2초로 줄여 잡았습니다.
