...
1. **병목은 사라지지 않고 리뷰로 옮겨간다.** 반복 확인까지 사람이 읽으면 생성이 빨라져도 검토 부담은 남는다.
2. **사람이 봐야 할 면적을 줄이는 쪽으로 간다.** 더 빨리 읽는 쪽은 상한이 금방 온다.
3. **AI가 실수해도 사고가 안 나는 경계를 아키텍처로 먼저 만든다.** 감사 로그와 격리 수준은 프록시 계층이 맡는다.
4. **그 경계를 문서 대신 테스트로 박는다.** 다만 막히는 것은 그 검사가 탐지할 수 있는 위반까지다.
5. **규칙을 미리 다 적을 필요는 없다.** 위반이 나올 때마다 한 건씩 박으면 계층 나누기의 암묵지 문제가 줄어든다.
6. **근거는 changelog 대신 산출물에서 만든다.** 다만 JAR diff가 답하는 범위는 API와 바이너리 호환성까지다.
7. **그 근거의 사각지대를 따로 잰다.** 바이트코드 검사가 못 보는 위반이 실제로 있었다.
8. **그래도 도메인 판단은 남는다.** 처음 하는 도메인이면 그 판단조차 어렵다.

이 글은 설계 기록이다. 아래 구성 가운데 실제로 만들어 재본 것은 아직 없고, 무엇을 먼저 만들어 어떤 결과로 판단할지까지가 범위다.
...
**반복해서 확인하는 규칙까지 매번 사람이 읽어야 한다면, 코드가 빨리 만들어져도 검토 부담은 그대로 남는다.**
...
대신 확인할 수 있는 것에서 출발한다. 읽어야 할 양이 읽을 수 있는 양을 넘으면, 사람은 읽는 시간을 늘리다가 어느 지점부터 안 읽고 넘긴다. 이건 수치를 인용하지 않아도 리뷰를 해 본 사람이면 안다. 그러면 답은 사람이 봐야 할 면적을 줄이는 쪽이다.
...
앞 절들이 다룬 조각은 여럿이지만, 한 번에 만들면 무엇 때문에 효과가 있었는지 알 수 없다. 그래서 첫 실험의 범위를 하나로 좁힌다. **팀 규칙을 실행 가능한 검사로 옮기고, 그 검사를 고정한 위반 샘플로 시험하는 데까지다.**

Java와 Gradle을 쓰는 작은 프로젝트에서 시작한다. 사용 흐름은 이렇게 잡았다.

1. 검사할 패키지와 금지할 의존 관계를 설정한다
2. 로컬과 CI에서 같은 명령으로 검사한다
3. 실패하면 위반 위치와 규칙의 이유를 Markdown과 JSON으로 남긴다
4. 규칙을 바꿀 때 정상·위반 샘플을 다시 돌려 오탐과 누락을 확인한다

반복 적용이 되면 실행 코드를 Gradle 플러그인으로 묶고, 패키지 구조가 다른 두 번째 저장소에 붙여 공통 부분을 뽑는다.
...
규칙 개선에 쓴 샘플과 마지막 평가에 쓴 샘플은 나눠 둔다. 평가 샘플을 보고 규칙을 고쳤다면 그 샘플은 개발용으로 옮기고 새 평가 사례를 만든다.

나머지는 실제로 막히는 곳에서 고른다. 실패 원인 파악에 시간이 들면 근거 수집을, 같은 호환성 문제를 반복해 만나면 증분 문서를, 테스트 준비 시간이 병목으로 측정되면 이미지 사전 빌드를 붙인다. 순서를 미리 정하지 않는 이유는 어느 것이 병목인지 아직 재보지 않았기 때문이다.

다음 글에는 첫 규칙을 붙인 결과와 검사가 놓친 사례를 남긴다.

---

원 발표는 토스 프론트엔드 챕터의 라이브 세션이다. 발표 내용은 요약해서 인용했고, 백엔드 이식과 코드 예시는 전부 필자의 해석이다. 토스 백엔드 챕터의 실제 방식과는 무관하다.

[Codesai의 두 글](https://codesai.com/posts/2026/06/architectural-mutants)은 원문을 읽고 정리했으며 직접 재현하지는 않았다. 상수 인라이닝 사례도 그 글에서 보고된 것이고 내가 확인한 것이 아니다. [ArchUnit](https://www.archunit.org/userguide/html/000_Index.html), [japicmp](https://siom79.github.io/japicmp/), [datasource-proxy](https://github.com/jdbc-observations/datasource-proxy)의 기능 범위는 각 문서를 근거로 적었다.
