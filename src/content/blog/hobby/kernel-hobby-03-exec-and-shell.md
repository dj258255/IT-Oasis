...
- **exec**: 자기가 올라타 있는 주소공간을 갈아엎는 문제. 우리는 satp를 유지하고 코드 페이지만 remap한 뒤 인라인 `mv sp; sret`으로 진입했고, 정통 satp 교체 경로는 `userret_to`가 레지스터만으로 처리합니다. 실패 안전성의 핵심은 **새 상태를 끝까지 성공시킨 뒤에야 옛 코드를 `kfree`**하는 순서입니다.
