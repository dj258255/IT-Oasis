...
> QEMU에 `-smp 3`을 줬으니 코어가 3개입니다. 그런데 전원이 들어왔을 때 커널로 점프하는 건 **딱 하나**뿐이에요. 나머지 둘은 어디 있고, 누가, 어떻게 데려와야 할까요?

부팅 직후의 상황을 정확히 그려볼게요.
QEMU virt에서 OpenSBI가 M-mode 셋업을 끝내고 우리 커널을 `0x8020_0000`으로 점프시키는데, 그 점프를 받는 건 **부팅 하트(boot hart) 하나**뿐이에요.
나머지 코어들은 아직 OpenSBI(펌웨어) 안에서 *"누가 나를 깨워주길"* 기다리며 멈춰 있어요.

그러니 멀티코어로 가는 첫걸음은 명확해요 — 부팅 하트가 초기화를 다 끝낸 뒤, **나머지를 명시적으로 깨워서** 우리 커널 코드로 데려오는 거예요.

### 배경 개념 — SBI HSM (Hart State Management)

> 한 코어가 다른 코어에게 "이 주소에서, 이 모드로 실행을 시작해라"라고 어떻게 명령할까요?

이걸 우리가 직접 하려면 IPI(코어 간 인터럽트)를 쏘고 깨운 코어의 초기 상태를 맞춰주는 저수준 작업을 해야 하는데, RISC-V는 이걸 **SBI HSM**(Hart State Management) 확장으로 표준화해 뒀어요.
`hart_start`라는 호출 하나로 *"hartid 번 코어를, 이 주소에서, S-mode로 시작해라"* 를 `ecall`로 펌웨어에 부탁하면 돼요.
...
깨어난 코어가 `_entry`로 들어오면 가장 먼저 할 일은 **자기 스택을 잡는 것**이에요.
이게 왜 중요하냐면 — 두 코어가 같은 스택을 쓰면, 한 코어가 `push`한 값을 다른 코어가 덮어쓰면서 호출 프레임이 통째로 박살나거든요.
그래서 `entry.S`에서 hartid로 스택 영역을 나눠 잡아요.
...
처음엔 자연스럽게 *"hartid가 0이면 부팅 하트, 아니면 보조 코어"* 로 분기했어요.
그런데 돌려보니 OpenSBI가 **부팅 하트를 2번으로** 고른 적이 있었어요.

> **흔한 오해 정정**: *"hartid 0번이 항상 먼저 부팅된다"* 는 보장은 어디에도 없어요. 어느 하트를 부팅 하트로 쓸지는 **OpenSBI(펌웨어)가 비결정적으로** 정합니다. 0번을 가정하고 분기하면, 펌웨어가 2번을 골랐을 때 부팅 코드가 통째로 안 돌아요.

그래서 "hartid 0"이라는 **번호**가 아니라 **"가장 먼저 진입한 하트가 부팅 하트"** 라는 **순서**로 기준을 바꿨어요.
원자적 플래그 하나로 첫 진입자를 가려내요 — 여러 코어가 동시에 `kmain`에 들어와도, `__sync_lock_test_and_set`이 *"내가 0을 1로 바꾼 첫 번째 코어인가"* 를 한 명령으로 판정해 줘요.
...
부팅 하트는 트랩·페이징·디스크·파일시스템·셸을 **혼자 다** 세운 뒤, 마지막에 보조 코어들을 깨워요.
깨어난 보조 코어는 `boot_taken`이 이미 1이라 `hart_main`으로 빠지고, 거기서 자기 코어의 satp·트랩·타이머만 잡고 곧장 스케줄러에 합류해요.
...
> **핵심 교훈**: 멀티코어 부팅의 골자는 *"초기화는 한 코어가 다 하고, 나머지는 깨워서 곧장 스케줄러에 밀어 넣는다"* 예요. 초기화를 코어마다 중복으로 돌리면 디스크·페이지 테이블이 두 번 만들어져 깨지니까요.
...
구체적인 레이스를 하나 볼게요.
페이지 할당기의 프리리스트에서 페이지 하나를 떼는 `kalloc`은 본질적으로 이런 두 줄이에요.
...
코어 A와 코어 B가 동시에 들어왔다고 해봐요.
A가 (1)에서 `freelist`(페이지 X)를 읽고, B도 (1)에서 **같은 X**를 읽어요.
둘 다 X를 반환하고, 둘 다 `freelist`를 `X->next`로 옮겨요.
결과 — **두 코어가 같은 물리 페이지 X를 자기 것이라 믿고** 페이지 테이블이든 스택이든 거기에 써요. 곧 메모리가 박살나죠.

이런 *"읽고-고치고-쓰는(read-modify-write)"* 구간을 **임계구역(critical section)** 이라 하고, 여기에 **한 번에 하나의 코어만** 들어가게 막는 장치가 스핀락이에요.
...
스핀락에서 정작 까다로운 건 `amoswap`이 아니라 `push_off()`/`pop_off()`예요.

> **흔한 오해 정정**: *"락은 다른 코어를 막는 거니까, 내 코어의 인터럽트는 켜둬도 되지 않나?"* — 안 됩니다. 락을 쥔 채로 인터럽트가 들어와 그 핸들러가 **같은 락**을 또 잡으려 하면, 자기 자신이 풀어주길 영원히 기다리는 **자기 자신과의 데드락**에 빠져요. 그래서 락을 쥔 동안엔 그 코어의 인터럽트를 꺼야 해요.

문제는 락이 **중첩**될 수 있다는 거예요(락 A를 쥔 채 락 B를 잡는 경우).
이때 안쪽 락 B를 놓자마자 인터럽트를 켜버리면, 아직 바깥 락 A를 쥐고 있는데 인터럽트가 들어와 위험해져요.
그래서 `push_off`/`pop_off`가 인터럽트 끄기를 **중첩 카운트(noff)** 로 관리해요 — *가장 바깥에서 끄고, 가장 바깥에서만 켠다.*
...
여기서 `mycpu()`가 `cpus[r_tp()]`로 **자기 코어의** 카운터를 찾는다는 점, 그리고 그게 `r_tp()`(=hartid)에 전적으로 의존한다는 점을 기억해 두세요 — 4절에서 `r_tp()`가 틀어지면 이 카운터까지 통째로 엉뚱한 코어 것을 만지게 됩니다.

## 3. 멀티코어 스케줄러 — 락 baton

이게 이번 글의 진짜 핵심이에요.
스핀락으로 *"가만히 있는 자료구조"* 는 지킬 수 있게 됐는데, 가장 어려운 건 **"프로세스를 컨텍스트 스위치하는 그 찰나"** 거든요.
...
이런 레이스를 상상해보세요.
코어 A의 프로세스 P가 yield하면서 자기 상태를 RUNNABLE로 바꾸고 **P의 스택에서 스케줄러 스택으로 빠져나가는 도중**이에요.
바로 그 찰나에 코어 B의 스케줄러가 proctable을 훑다 *"어, P가 RUNNABLE이네"* 하고 P를 RUNNING으로 점유한 뒤 **P의 컨텍스트로 swtch해 들어가요.**

그러면 **두 코어가 같은 P의 같은 커널 스택을** 동시에 쓰게 돼요.
A는 아직 그 스택에서 빠져나가는 중이고, B는 그 스택으로 들어오는 중이라, P의 스택이 그대로 박살나요.
스핀락만으로는 이게 안 막혀요 — 전환은 *"자료구조를 잠깐 만지는 일"* 이 아니라 *"스택과 PC를 통째로 갈아타는 일"* 이라 임계구역이 `swtch` 양쪽으로 걸쳐 있거든요.
...
이 커널은 **항상 스케줄러를 경유**해요(xv6와 같음).
P는 자기가 다음 프로세스를 고르지 않고, 무조건 스케줄러 컨텍스트(`cpu_sched[id]`)로 돌아간 뒤 스케줄러가 다음을 골라요.
이러면 전환의 한쪽 끝이 **항상 스케줄러**라, 락을 "스케줄러 ↔ proc" 한 쌍 사이에서만 주고받으면 돼서 규칙이 단순해져요.

### 해결 — 락을 swtch를 가로질러 든다 (baton)

핵심 아이디어는 이거예요 — **`pt_lock`(proctable 보호 락)을 컨텍스트 스위치를 가로질러 쥔 채로 넘긴다.**

마치 **배턴을 주고받듯**, 락이 swtch의 양쪽으로 넘어가요.

- 스케줄러가 `pt_lock`을 **잡고**, P를 RUNNING으로 점유한 뒤, **락을 든 채로** P로 `swtch`해 들어가요.
- P는 (첫 실행 지점이나 yield 지점에서) 그 **락을 놓아요.**
- P가 다시 양보할 땐 **락을 잡고** `swtch`로 스케줄러에 돌아오고, 스케줄러가 그 **락을 놓아요.**

이 baton 동안엔 `pt_lock`이 한순간도 풀리지 않으니, **다른 코어가 P를 절대 못 집어요.** P가 전환을 완전히 끝낼 때까지 P는 락 뒤에 숨어 있는 셈이죠.
...
반대편(proc 쪽)에서 `yield`는 거울상이에요 — 락을 **잡고** swtch로 나가고, 스케줄러가 잡아줬던 락을 다음에 스케줄될 때 자기가 풀어요.
...
여기 함정이 하나 있어요.
이미 한 번 돌던 프로세스는 `yield` 안의 `swtch`에서 멈췄다가 거기서 깨어나니, 깨어난 직후의 `release(&pt_lock)`가 baton을 받아 풀어줘요.
그런데 **갓 만들어진 프로세스가 처음 실행될 때**는 yield 지점이 아니라 **진입 함수의 맨 처음**에서 시작해요. 그래서 그 진입 함수들이 스케줄러가 쥐여 준 락을 직접 놓아줘야 baton이 끊기지 않아요.

이 커널엔 진입 함수가 셋 있고, 셋 다 첫 줄이 `release(&pt_lock)`이에요.
...
새 프로세스의 `context.ra`를 이 함수들 중 하나로 세팅해 두니(예: `p->context.ra = (uint64)enter_user`), 첫 `swtch`가 곧장 진입 함수로 점프하고, 그 첫 줄이 baton을 받아 락을 풀어요.

### sleep/wakeup도 같은 락 위에서 — 잃어버린 wakeup 막기

`pt_lock`은 스케줄링만 지키는 게 아니에요. `sleep`/`wakeup`도 이 같은 락 위에서 돌아요.
이게 왜 중요하냐면, **잃어버린 wakeup(lost wakeup)** 이라는 고전적 레이스를 막기 때문이에요.

`read()`가 콘솔 입력을 기다리는 상황을 봐요.
...
아직 잠들기 전이라 그 wakeup은 **아무도 못 받고 허공으로 사라지고**, 그 직후 (2)에서 잠들어 **영원히 안 깨어나요.**

해결은 *"검사와 sleep을 같은 락 아래 원자적으로 묶는 것"* 이에요.
호출자가 `pt_lock`을 **쥔 채로** `sleep`에 들어오고, `sleep`은 그 락을 든 채로 swtch해요. `wakeup`도 같은 락을 쥐어야 하니, 검사~sleep 구간엔 wakeup이 끼어들 수 없어요.
...
`proc_wait`가 좋은 예시예요 — `acquire(&pt_lock)` 한 번 잡은 뒤, 그 락 안에서 *"종료한 자식이 있나"* 검사하고, 없으면 같은 락을 든 채로 `sleep(p)` 합니다. 자식이 `proc_exit`에서 `wakeup(부모)`을 부르는 것도 같은 락 아래라, 그 사이에 깨움을 흘릴 일이 없어요.

> **핵심 교훈**: 멀티코어 스케줄러의 정확성은 *"`pt_lock` 하나가 컨텍스트 스위치를 가로질러 끊김 없이 넘어간다"* 는 단 하나의 불변식에 달려 있어요. 스케줄러가 잡고 → proc이 놓고 → proc이 잡고 → 스케줄러가 놓고. 이 baton이 한 번이라도 끊기면 두 코어가 같은 proc을 밟습니다.
...
폴트 로그를 따라가 보니 `proc_fork`의 한 줄이었어요.

```
[trap] EXCEPTION  scause=0xd  sepc=0x80201e10  stval=0x00000000000000a8
```

`scause=0xd`는 **load page fault**, `stval=0xa8`(168)은 폴트가 난 주소예요.
거의 null인 포인터의 168번째 바이트를 읽다 죽은 거죠.
디스어셈블해 보니 `current_proc()`가 **null**을 반환하고 있었어요.

### 원인 — tp는 그냥 hartid가 아니다

`current_proc()`를 보면 원인이 보여요.
...
`r_tp()`(=hartid)를 인덱스로 *"이 코어가 지금 돌리는 proc"* 을 꺼내요.
그런데 `r_tp()`가 엉뚱한 값을 반환하면, `cpu_proc[엉뚱한 인덱스]`가 0(null)이 나오고, 그 null의 168번째 바이트(`proc` 구조체의 어떤 필드)를 읽다 폴트가 나는 거예요.

원인은 정말 미묘했어요.

> **흔한 오해 정정**: *"`tp` 레지스터는 그냥 hartid다"* 라고 믿었던 게 함정이었어요. `tp`는 ABI상 **유저 프로그램의 스레드 포인터**라, 유저가 자기 용도로 언제든 덮어쓸 수 있는 레지스터예요. 우리가 거기에 hartid를 넣어 뒀다고 해서 유저가 안 건드린다는 보장은 전혀 없습니다.

우리 커널은 부팅 때 `entry.S`에서 `tp`에 hartid를 담아 뒀어요(`mv tp, a0`).
그런데 유저 프로그램이 U-mode에서 돌다 `tp`를 자기 스레드 포인터로 덮어써요.
그 상태에서 트랩(타이머·시스템콜)이 나면 커널로 들어오는데, **커널 코드가 도는 동안에도 `tp`엔 여전히 유저가 써 놓은 값**이 들어 있어요.
...
"단일 코어에선 우연히 맞던 값이 멀티코어에서 진실을 드러낸다" — 멀티코어 버그의 전형이죠.
...
`kernelvec`은 트랩 당한 코드의 `tp`를 일단 스택 프레임에 저장한 뒤(`sd tp, 16(sp)`), 곧바로 `csrr tp, sscratch`로 **커널이 쓸 `tp`를 hartid로 되돌려** 놔요.
그래야 그 뒤로 호출되는 `kerneltrap` → `current_proc()` → `mycpu()`가 전부 올바른 코어 번호를 보죠.
복귀할 땐 `trapret`이 프레임에 저장해 둔 유저의 `tp`(`ld tp, 16(sp)`)를 그대로 되돌려 주니, 유저는 자기가 써 둔 값을 그대로 받아요.

이 두 줄로 크래시가 깨끗이 사라졌어요.
*"레지스터 하나의 의미를 누가 소유하는가"* — 유저 ABI의 `tp`와 커널의 hartid가 같은 레지스터를 두고 충돌하는 문제였고, 그 경계를 트랩 진입/복귀에서 명확히 그어 준 게 답이었어요.

## 5. 나머지 공유 자원 잠그기

스케줄러가 안정되고 `tp`까지 바로잡으니, 남은 공유 자원들도 차례로 락을 채웠어요.
원칙은 하나예요 — *"두 코어가 동시에 이 줄을 밟으면?"* 이라는 질문을 모든 공유 지점에 던지는 것.

- **페이지 할당기**(`kalloc`/`kfree`) — 2절 도입부의 그 프리리스트 레이스를 막으려고 전용 `kmem_lock`으로 보호해요. 여러 코어가 동시에 페이지를 떼고 돌려줘도 같은 페이지가 두 번 나가지 않아요.
- **콘솔 버퍼** — 입력 버퍼와 `sleep`/`wakeup`을 스케줄러 락(`pt_lock`)으로 함께 보호해요. 3절의 잃어버린 wakeup을 막는 게 핵심이에요.
- **UART 출력** — 락이 없으면 두 코어의 출력이 **글자 단위로 섞여요**(보조 코어가 *"[ok] hart ..."* 를 찍는 사이 다른 코어가 끼어드는 식). 메시지 단위로 직렬화하는 `pr_lock`을 둬서 한 줄이 통째로 나가게 했어요.

### 락 순서 — 데드락을 막는 규칙

여기서 **락 순서(lock ordering)** 가 중요해져요.
두 락을 중첩해서 잡을 때, 코어 A가 `pt_lock → uart_lock` 순서로 잡는데 코어 B가 `uart_lock → pt_lock` 순서로 잡으면, A는 uart를 B는 pt를 쥔 채 서로를 기다리는 **데드락**에 빠져요.

규칙은 단순해요 — **모든 코어가 항상 같은 순서로 락을 잡는다.**
`pt_lock`을 쥔 채로 `uart_lock`을 잡는 경로는 있어도, 그 반대는 절대 없게 했어요.
UART 락을 항상 **가장 안쪽(leaf)** — 그 락을 쥔 동안엔 다른 락을 더 잡지 않는 위치 — 에 두면 이 규칙이 자연히 지켜져요.

## 정리

이 글은 단일 코어 커널을 **3개 코어가 함께 도는 SMP 커널**로 키웠어요.
다섯 조각이 한 방향으로 쌓입니다.

- **멀티코어 부팅** — 부팅 하트 하나가 전체 초기화를 끝내고, SBI HSM `hart_start`로 나머지를 깨워 곧장 스케줄러에 합류시켜요. 부팅 하트 번호는 비결정적이라 **원자 플래그(`boot_taken`)로 첫 진입자**를 부팅 하트로 삼습니다.
- **스핀락** — `amoswap`(`__sync_lock_test_and_set`)으로 임계구역에 한 코어만 들이고, `__sync_synchronize` 배리어로 가시성을 보장해요. 락을 쥔 동안엔 `push_off`/`pop_off`가 **중첩 카운트**로 인터럽트를 꺼요(자기 자신과의 데드락 방지).
- **멀티코어 스케줄러(락 baton)** — `pt_lock`을 **컨텍스트 스위치를 가로질러** 든 채로 넘겨, 전환의 찰나에 다른 코어가 같은 proc을 못 집게 해요. 첫 실행은 `enter_user`/`forkret`/`thread_start`가 락을 놓고, `sleep`/`wakeup`도 같은 락으로 잃어버린 wakeup을 막아요.
- **`tp`/`sscratch` 버그** — 유저가 `tp`를 덮어 `r_tp()`가 틀어지던 간헐적 크래시를, hartid를 `sscratch`에 두고 **트랩 진입 때 `csrr tp, sscratch`로 복구**해 해결했어요. 단일 코어에선 우연히 0이 맞아 안 터졌던 버그예요.
- **나머지 공유 자원** — 페이지 할당기·콘솔·UART에 락을 채우고, **락 순서**(UART를 leaf로)로 데드락을 막았어요.

이제 `-smp 3`으로 부팅하면 **3개 코어가 공유 proctable에서 프로세스를 가져다 동시에** 돌려요.
