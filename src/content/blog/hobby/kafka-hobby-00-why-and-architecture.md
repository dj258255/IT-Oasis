...
가장 먼저 짚고 넘어가야 했어요. **Kafka는 "메시지 큐"가 아니에요.** (정확히는, 그것보다 훨씬 단순하면서 강력한 무언가예요.)
...
생산도 소비도 하기 전에, 모든 클라이언트가 **맨 처음** 하는 일이 있어요 — 브로커에 붙어서 *"너 어떤 API를 어느 버전까지 지원해?"*라고 묻는 것. 이게 `ApiVersions` 요청, 프로토콜의 **핸드셰이크**예요.
