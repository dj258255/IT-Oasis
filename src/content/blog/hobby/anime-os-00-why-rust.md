...
> 갱신 안내: 이 연재(#0~#2)는 **처음 Rust/x86으로 만든 기록**이에요. 이후 OS를 더 깊이 공부하려고 학습용 커널을 **C/RISC-V로 전면 재작성**했습니다(커널의 정석 언어가 C이고, 참고서 xv6도 C/RISC-V라 마찰이 적어서). 현재 저장소는 C/RISC-V 버전이에요. 아래 내용은 Rust 시절의 여정으로 읽어주세요.
