...
> **흔한 오해 정정**: SUM=1로 고정하는 건 *"그냥 켜두면 편하니까"* 가 아니에요. 그건 제가 4절에서 **트랩을 유저 스택 위에서 처리**하기로 한 결정에서 강제로 따라 나오는 결과예요. xv6처럼 별도 커널 스택을 쓰면 SUM은 평소 0이어야 맞아요 — 커널이 유저 메모리를 *실수로* 건드리는 걸 잡아주니까요. SUM을 어떻게 다룰지는 트랩 프레임을 어디 두느냐에 종속됩니다.
