...
기능이 쌓인 뒤 방향을 뒤집어 동시성·기종별 정확성·보안·수명주기 네 축으로 코드를 정독했습니다. 혼자 훑으면 자기 코드에 관대해지니 축을 갈라 병렬로 보고 OWASP·CWE·벤더 문서와 대조해 결함 20건 이상을 찾았습니다.

- **전부 고치지 않고 FIX/SKIP을 근거와 함께 갈랐습니다.** "안 고쳐도 되는 걸 고치는 것도 부채"라, 재검증 후 SKIP도 이유를 남겼습니다(MySQL 5.7 비호환은 8.0+ 명시 지원으로 SKIP, Oracle `v$` vs `gv$`는 데모가 단일 인스턴스라 SKIP). 수정에는 신규 단위 테스트 28건이 붙었습니다.
- **가장 심각한 결함은 XXE였습니다.** 데드락·실행계획 XML을 파싱하는 세 곳이 외부 DTD 로드 차단만 걸어놨는데, OWASP가 "불충분"이라 명시한 조합입니다. 악성 대상 DB가 조작된 XML을 주면 DBTower 호스트가 외부로 요청을 날리는 블라인드 SSRF가 가능했습니다. 같은 저장소의 DeepAnalyzer는 이미 DOCTYPE 선언 자체를 거부하고 있었으니, 설계를 몰랐던 게 아니라 나중에 만든 파서에 기준을 안 옮긴 일관성 누락이었습니다. 세 곳을 통일하고 XXE 페이로드가 거부되는 것을 단위 테스트로 고정했습니다.
- **감사 결론도 실측으로 한 번 더 확인해야 했습니다.** "slow_log는 마이크로초를 저장 안 해 복구 불가"라는 제 감사 결론을 원본 테이블 실측이 뒤집었습니다(`00:00:00.600594` 저장 확인). `TIME_TO_SEC`가 정수를 반환해 1초 미만이 0으로 찍히던 건 맞았지만, 마이크로초가 애초에 저장 안 된다는 가정은 이 버전에선 틀렸습니다.
...
### 장기 분석 창고와 손잡아 오탐을 지웠습니다

이 관측 데이터를 만료 전에 받아 가는 상대가 곧 소개할 **DBTower-lakehouse**입니다. lakehouse가 수개월 이력으로 계산한 요일×시간대 베이스라인을 DBTower가 받아, 14일 단기 창의 이상 감지 오탐을 지웁니다.
