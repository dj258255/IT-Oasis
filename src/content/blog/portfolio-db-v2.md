...
다섯 기종은 쿼리 통계를 주는 방식부터 시간 단위(피코초·밀리초·마이크로초)까지 전부 다르다. 경계를 SQL 문법이 아니라 운영 행위(통계 조회·실행계획·백업)에 그으면 SQL이 없는 MongoDB까지 같은 틀에 들어온다. 성격이 정반대인 Oracle과 MongoDB를 실제로 추가해 수집·비교·감지·콘솔·MCP **코어 경로 0줄 수정**을 실증했고, 이 경계는 문서가 아니라 빌드(Spring Modulith 순환 의존 실패)가 지킨다.

### 판단 2. 옵티마이저가 속은 이유까지 진단한다

EXPLAIN은 추정이라 "풀스캔을 택했다"까지만 보여준다. 실제 실행계획을 5기종 각자의 경로로 받아 추정 vs 실제 행수 괴리를 계산했고, 추정 300행 vs 실제 1행의 **괴리 300배**에서 암시적 형변환을 지목해 원클릭 재진단으로 Index lookup 전환까지 확인했다. 고치고 보니 수정 전의 "1행"은 `'012345'`가 잘못 매칭된 행이라 성능 문제이기 전에 정확성 문제였다. 진단이 대상에 부하를 주지 않도록 SELECT 전용·타임아웃·ADMIN 인가를 기능보다 먼저 설계했다.

이 도구를 자기 자신에게 먼저 썼다. 메타 DB를 관리 대상으로 등록하자 자기 스냅샷 쿼리가 슬로우 쿼리로 떴고, 자기 진단이 지목한 인덱스를 추가해 **21.269 → 0.062ms(343배)**. 개선을 측정한 도구가 곧 내가 만든 기능이라는 순환이 플랫폼이 실제로 동작한다는 증거다.

### 판단 3. 도구의 선은 판정·기록까지, 실행은 사람 뒤에

DBA 손이 반복해서 가는 다섯 지점(설정 드리프트·스키마 리뷰 게이트·인덱스 판정·인시던트·월간 리포트)을 자동화하되, 전부 읽고 판정·기록까지만 하고 대상 DB는 바꾸지 않는다. 대상을 실제로 바꾸는 경로는 gh-ost 온라인 스키마 변경 하나뿐이고 ADMIN 전용·기본 dry-run·AI 미노출로 봉인했다. AI 채널(MCP 도구 16종)도 read-only 화이트리스트만 열어 kill·backup·DDL은 아예 등록하지 않았다.
...
| 보존 정리 | 월별 파티셔닝으로 블로트 404MB → 0 · 만료 삭제 **147배**(1.9s→12.8ms) |
...
반쪽 파티션 위의 "조용히 틀린 랭킹"을 막으려고 4축 fail-closed 게이트(정합·완결성·신선도·스키마)를 dbt 앞에 세웠다. 파티션 20,158행을 지워 주입하자 게이트가 FAIL을 내고 dbt가 실행되지 않았다. 반대 방향도 감사했다. backfill 시 멱등 재적재(delete-first)가 보존 창 밖에서는 세상에 남은 유일본을 지우고 exit 0으로 끝나는 경로를 재현했고, "지울 자격은 다시 쓸 수 있는 자에게만" 원칙으로 삭제를 거부하게 고쳤다. 성공의 부재는 deadman이 잡는다. 실제로 수집기가 21시간 조용히 멈췄는데 알림이 0통이었던 경험이 이 장치를 만들었다.

### 판단 2. 최적화는 실측이 요구할 때만 한다

3일치 데이터로는 규모를 증명할 수 없어 시간축 1년치 **54.5M행**, 인스턴스축 300대 **52.2M행** 두 축을 합성해 쟀다. 병목은 fct 전체 재빌드(407.62s) 하나였고, 증분 전환으로 **407.62s → 4s(약 100배)**, 읽는 파일 2,190개 → 6~12개. 축을 돌려 재보니 외삽해 뒀던 예측 두 개("소파일이 먼저 무너진다")가 실측에서 뒤집혀 운영 가이드 1순위가 바뀌었다. 재보지 않은 예측은 틀린다.

### 판단 3. 창고가 판정을 내리되, 발화는 남에게 맡긴다

DBA가 손으로 하던 판정 여섯(용량 D-day·플랜 회귀·백업 RPO·미사용 인덱스·설정 변경 상관·가용성 SLO)을 매일 컬럼으로 계산한다. 관측이 부족하면 PENDING, 창이 오염되면 AMBIGUOUS로 지어내지 않고, 미사용 인덱스는 삭제 후보까지만 내고 최종 판단은 사람에게 남긴다. 라이브 실측에서 MSSQL 2대의 uptime 63~64% breach를 실제로 검출했다.
