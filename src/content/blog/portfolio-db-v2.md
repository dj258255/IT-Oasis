...
3. **새 기종은 구현체(Operator) 하나만 추가하면 됩니다.** Oracle·MongoDB처럼 성격이 정반대인 기종도 인터페이스가 정한 메소드(통계 조회·실행계획·백업 등)만 구현하면 끝이었습니다. 시점비교·회귀감지·콘솔·MCP는 그 메소드를 호출만 하니 기종이 늘어도 그대로입니다. 이 경계는 문서가 아니라 빌드(Spring Modulith 순환 의존 실패)가 지킵니다.
