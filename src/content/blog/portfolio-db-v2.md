...
- **설정 드리프트 이력**: 파라미터 diff의 공간축("A와 B가 다른가")에 시간축("언제부터 무엇이 바뀌었나")을 붙였습니다. PostgreSQL만 파라미터가 수백 개라 매시간 통째로 저장하면 이력이 폭증하므로, 거울 테이블에 현재 전량을 들고 바뀐 항목만 로그에 append하고 무변경 주기엔 해시 한 줄만 쌓습니다. work_mem 실변경(4096→8192)을 감지해 카드를 쐈고, MongoDB `parameters()`가 응답 gossip 필드를 파라미터로 흘려 매번 오탐 나던 버그도 봉투 필드를 걸러 잡았습니다. "누가" 바꿨는지는 대상 DB가 안 주니 지어내지 않고 감사 로그에서 확인하라고 안내합니다.
- **스키마 변경 리뷰 게이트**: 배포 전 DDL/대량 DML을 JSqlParser 구문 트리로 판정합니다(락 위험·DEFAULT 없는 NOT NULL·DROP·WHERE 없는 대량 변경). 정규식은 문자열·달러 인용 안의 세미콜론을 문장 경계로 오인하지만, 파서는 ADD/DROP 컬럼을 구조로 뽑아 실제 스키마와 대조하고 실제 행수(8,118)로 락 위험을 확정합니다. AI에는 규칙이 짚은 걸 반복하지 말고 배포 순서·롤백만 보태라고 프롬프트에 박았고, **실행은 하지 않고** gh-ost 경로만 안내합니다.
- **인덱스 사용 통계 주기 영속**: "이 인덱스 지워도 되나"는 재시작 누적 카운터의 순간값으론 못 답합니다. "지난주 재기동 이후 0회"와 "분기 내내 0회"가 구분되지 않아 실사용 인덱스를 지우는 사고가 나기 때문입니다. 5기종 스캔 통계를 6시간 주기로 영속하고, 분기 창 장기 판정은 lakehouse 마트가 맡습니다.
- **인시던트·월간 리포트**: 장애 구간을 주면 신규 수집 없이 저장된 신호(시점 비교·설정 변경·플랜 플립·대기·가용성)를 한 장에 재구성하고 AI가 재료 안 사실만으로 요약합니다. 자기 자신 2시간을 뽑으니 AI가 "work_mem 변경은 4분 만에 되돌린 net-zero라 2시간 악화를 설명하기 어렵고, 플랜 플립 0건이라 근거가 부족하다"고 답했습니다. 월간 리포트는 나쁜 순 롤업 한 장으로 자동 발행됩니다.
...
- **함정은 Discord의 Message Content 특권 인텐트였습니다.** 웹훅이 쓴 메시지 본문을 봇이 읽으려면 심사가 필요한 특권 권한이 있어야 하고, 반응 이벤트에는 message_id만 오는데 "어느 인스턴스의 알림인지"는 그 메시지 embed를 읽어야 압니다.
- **그 정보는 애초에 우리가 만든 것이라 특권 인텐트 0개로 풀었습니다.** 발송 시점에 웹훅을 `?wait=true`로 호출해 message_id를 돌려받아 인스턴스에 매핑하고 메타 DB에 영속(재시작 생존 e2e 실측)했습니다. 반응이 오면 조회 한 번으로 대상이 나옵니다.
- **가장 마음에 든 건 죽은 인스턴스의 답글이었습니다.** 진단 도구가 전부 빈 결과인 케이스에서 AI가 수집 5단계를 나열하고 "수치를 지어내지 않겠습니다"로 마감했습니다. 판단 기준을 사람이 문서로 정하는 원칙이 런타임 응답까지 관통한다는 증거라, 화려한 성공 케이스보다 이 답글이 더 좋았습니다.
...
- **남의 팀 인스턴스는 403이 아니라 404로 숨깁니다.** 403을 주면 "당신은 못 보지만 그 id의 인스턴스는 존재한다"는 사실이 새서, 공격자가 id를 순회하며 인프라 지도를 그릴 수 있어 스코프 밖은 미등록과 완전히 같은 404를 받습니다. 강제 지점은 RegistryService 한 곳이라(모든 모듈이 registry를 경유) 컨트롤러 수십 개마다 권한 검사를 뿌리지 않습니다.
- **로그인 잠금 카운터도 메타 DB로 옮겼습니다.** 인메모리였다면 LB 뒤에서 임계가 노드 수만큼 벌어져, 공격자가 요청을 분산시키면 실패 허용치가 노드 수배가 됩니다. 노드 A 두 번·B 한 번 틀리자 네 번째가 잠기는 것을 실측했습니다.
- **디스크 포화는 잔량이 아니라 속도로 예측합니다.** 여유 76.8%인데 치명 경보가 뜨는 화면을 실쓰기 부하로 만들었습니다. 초당 17MB씩 줄면 20시간 뒤 장애라 이 경보가 맞습니다. 여유 바이트를 최근 6시간 감소 속도로 나눈 선형 추세 ETA를 쓰되, 문안에 "예측이지 사실이 아님"을 넣었습니다.
- **등록 단위는 DB지만 물리 단위는 서버라, 같은 서버는 그룹으로 묶습니다.** 세션·복제·데드락은 서버 단위 관측이라 같은 서버의 DB 둘이면 같은 알림이 두 배로 갑니다. `host:port`로 서버 그룹을 파생해 그룹당 1회로 줄이되, 위험 귀속(헬스 스코어)은 일부러 dedup하지 않았습니다. 같은 서버의 두 DB는 둘 다 실제로 위험하기 때문입니다.
...
- **전부 고치지 않고, 고칠 것과 안 고칠 것을 근거와 함께 갈랐습니다.** "안 고쳐도 되는 걸 고치는 것도 부채"라고 보고, 그냥 두기로 한 것(SKIP)도 이유를 남겼습니다(예: MySQL 5.7 관련 문제는 8.0 이상만 지원한다고 이미 명시했으니 SKIP). 실제로 고친 것에는 새 테스트 28건을 붙였습니다.
- **가장 심각한 건 XML 파서의 보안 구멍(XXE)이었습니다.** SQL Server·MySQL은 실행계획을 XML 형태로 돌려주는데, 이 XML을 읽는 파서 세 곳이 방어를 절반만 해뒀습니다. 악의적인 DB가 "바깥 주소를 가리키는" XML을 응답으로 주면, DBTower 서버가 그 주소로 대신 요청을 날리게 되는(서버가 공격자의 심부름꾼이 되는) 문제였습니다. 같은 저장소의 다른 파서는 이미 제대로 막고 있었으니, 몰라서가 아니라 나중에 만든 파서에 같은 기준을 안 옮긴 실수였습니다. 세 곳을 같은 방식으로 막고, 공격용 XML이 거부되는지 테스트로 못 박았습니다.
- **제 감사 결론조차 실측으로 한 번 더 확인해야 했습니다.** "이 로그는 1초 미만 시간을 저장하지 않아 복구 불가"라고 감사에서 결론 냈는데, 원본 테이블을 직접 열어 보니 `0.600594`초까지 저장돼 있었습니다. 화면에 0으로 찍히던 건 변환 함수가 소수점을 버려서였지, 데이터가 없어서가 아니었습니다. 감사도 짐작이 섞이면 틀린다는 걸 배웠습니다.

![감사 결론을 뒤집은 실측, sub-second 슬로우 쿼리가 실측 ms(581·750ms)로 표시, 구코드는 전부 0](/uploads/project/dbtower/slowquery-subsecond.png)

### 판단과 트레이드오프

- **데이터 접근은 경로별 비용으로 골랐습니다.** 메타 CRUD는 JPA, 스냅샷 대량 쓰기는 JDBC batch, SQL이 없는 MongoDB는 드라이버 직접입니다. 하나의 ORM으로 통일하는 편함 대신 경로별로 맞췄고, Spring Data도 파생 메서드(정적 단순)·`@Query`(정적 집계·벌크)·Specification(동적 필터) 세 층으로 나눠, 필터 6종이 런타임에 조립되는 감사 로그 검색에서만 Specification을 썼습니다.
- **p95는 소스가 다른 숫자를 섞지 않습니다.** MySQL은 히스토그램을 구간 차분해 실측(누적 0.48ms가 가리던 최근 0.19ms 노출), PostgreSQL은 평균+1.645σ 추정, 원자료 없는 기종은 UNSUPPORTED입니다. 히스토그램을 TRUNCATE하면 깨끗한 최근 창을 즉시 얻지만 대상 상태를 바꾸는 행위라, 내 스냅샷 두 장을 버킷별로 빼 최근 구간을 복원하는 차분을 택했습니다.
- **최소 권한은 추측하지 않고 DB에게 물어 확정했습니다.** 권한 0 계정으로 시작해 DB가 돌려주는 권한 에러 원문을 수집하며 필요한 것만 더했습니다. MongoDB는 clusterMonitor로 충분했고, PostgreSQL은 pg_monitor 전체가 아니라 pg_read_all_stats로 좁혔습니다. 권한이 부족하면 PG가 에러 대신 HTTP 200에 전 행 insufficient privilege를 채워 조용히 저하되는 함정도 이때 실측으로 잡았습니다.
- **MongoDB 타임아웃 정석(CSOT)은 일부러 배제했습니다.** CSOT를 켜면 드라이버가 심층 진단이 명령에 실은 maxTimeMS를 무시하고 자기 예산으로 재계산해, 전역 안전장치가 정밀 안전장치를 덮어쓰는 간섭이 됩니다. 나중에 누가 "왜 정석을 안 썼지?" 하고 CSOT로 바꾸면 심층 진단이 조용히 망가지므로 이유를 코드 주석에 남겼습니다.
...
| 플랜 플립 감지 | 인덱스 드랍을 다음 폴링 주기(60초)에 감지, 동반 레이턴시 회귀 **0.03→6.23ms**(PG e2e, 계획 shape는 5기종) |
...
| 장기 분석 루프 | lakehouse 베이스라인 병합으로 판정 반전 실측(z=7.42·관측 101) · MCP 마트 서빙(카드 143ms 렌더) |
...
### 다음 이야기: 장기 분석 창고와 손잡아 오탐을 지웠습니다

DBTower가 7일 뒤 버리는 관측 데이터를 만료 전에 받아 가는 상대가, 곧 소개할 **DBTower-lakehouse**입니다. lakehouse가 수개월 이력으로 계산한 요일×시간대 베이스라인을 DBTower가 받아, 14일 단기 창의 이상 감지 오탐을 지웁니다.

- **평균·분산만 받아 원시 관측을 통계적으로 복원해 병합합니다.** "매주 월요일 아침 배치 피크"는 14일 창엔 버킷당 관측이 최대 2개라 오탐이 나는데, 장기 요약(n·mean·stddev)에서 충분통계량을 복원해 더하면 원시 관측을 일일이 더한 것과 수학적으로 동일합니다. 근사가 아닙니다.
- **같은 스파이크의 판정이 장기 테이블 내용에 따라 뒤집힙니다.** 장기가 "평소 0.2qps"라 하면 z=7.42로 발화하고 관측 수 101(장기 100+단기 1)이 찍혀, 병합이 실제로 게이트를 넘겼다는 물증이 됩니다. 반대로 평소가 높으면 월요일 피크 오탐이 사라집니다. 테이블이 비면 판정이 병합 전과 완전히 같은 것을 테스트로 고정했습니다.
- **창고 마트를 AI 에이전트에게도 엽니다.** MCP 도구 2종(`lakehouse_query` 읽기 가드·행 상한, `lakehouse_card_create` 격리 컬렉션)으로 자연어 질문에서 마트 질의·차트 생성까지 가고, DELETE는 400으로 거부됩니다. 에이전트가 만든 카드가 bar 차트로 143ms에 렌더되는 것까지 실측했습니다.
