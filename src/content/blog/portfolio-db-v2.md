...
**시점 비교는 "평소 구간"과 "문제 구간"을 같은 화면에서 비교합니다.** CPU 그래프 위에서 평소 구간(초록)과 문제 구간(주황)을 드래그로 고르면, 느려진 원인이 세 갈래 중 무엇인지 가려집니다. 신규 쿼리 유입(NEW 뱃지), 호출량 급증, 호출당 읽는 행수(rows/call) 증가(IN절 폭증류)입니다. 원리는 누적 카운터의 차분입니다.
...
**대상 DB를 실제로 바꾸는 경로는 gh-ost 온라인 스키마 변경 하나뿐이고, 네 겹으로 봉인했습니다.** ADMIN 전용, 기본 dry-run(`execute=true`를 명시해야 적용), 비밀번호는 소유자 전용 임시 conf로 넘기고 실행 직후 삭제, AI/MCP 도구엔 아예 미노출(kill·backup과 함께 화이트리스트 제외)입니다. gh-ost는 그림자 테이블을 만들어 binlog로 따라잡고 원자적으로 이름을 바꾸는 락 없는 도구라, 관제탑이 직접 짠 DDL을 던지는 것과 안전성의 결이 다릅니다.
...
가는 길에 함정도 여럿이었습니다. 하트비트와 진단이 스레드를 공유해 연결이 반복 종료되던 버그는 진단 전용 워커 분리로 재접속 0회가 됐고, 트리거 이모지는 왼쪽 돋보기(U+1F50D)로 등록했는데 실제로 단 건 오른쪽 돋보기(U+1F50E)라 트리거를 집합으로 바꿔 둘 다 받게 했습니다. 반대로 음소거 이모지를 달면 그 인스턴스 알림이 1시간 중지됩니다.
...
별도 프로젝트인 lakehouse가 수개월 이력으로 계산한 요일×시간대 베이스라인을 받아, 14일 단기 창의 이상 감지 오탐을 지웁니다.
