...
### 기능 2. 프로비저닝, 세 도구가 멱등 등록 하나로 수렴합니다
...
### 기능 5. 사람 손이 붙던 다섯 지점을 기능으로 끊었습니다
...
gh-ost는 그림자 테이블을 만들어 binlog로 따라잡고 원자적으로 이름을 바꾸는 락 없는 도구라, 관제탑이 직접 짠 DDL을 던지는 경로에는 없는 안전성을 갖습니다.
...
### 기능 6. 알림에 이모지를 달면 AI가 진단 답글을 답니다
...
### 기능 7. 여럿이 쓰는 관제탑에 팀 경계와 호스트 차원을 넣었습니다
...
**도그푸딩은 DB 내부를 알아야 보이는 버그도 잡아줬습니다.** `sample` DB와 `dbtower` DB가 서로의 쿼리를 자기 화면에서 보고 있었는데, `pg_stat_statements`가 클러스터 전역 뷰라 dbid 필터 없이는 다른 데이터베이스 쿼리까지 섞이기 때문이었습니다. 현재 dbid로 필터해 잡았습니다. 저장 계층을 직접 만들어봤던 경험 덕에 이런 지점이 보였습니다.
...
- **원인**: 직전에 성능을 위해 넣은 수집 병렬화(워커 4)가 경합을 키웠습니다. 자원 경합이 허위 장애 신호로 둔갑하는 구조였습니다
