...
export const hubDomains: HubDomain[] = [
  {
    ...
    posts: [post('ch6-auth-cost'), post('ch34-inflow-control-remeasured')],
    col: 1,
    row: 1,
  },
  {
    ...
    posts: [
      ...
      post('ch52-search-tie-order'),
      post('ch53-korean-query-dictionary'),
      post('ch58-page2-count-queries'),
    ],
    col: 2,
    row: 1,
  },
  {
    ...
    posts: [
      post('ch21-cancel-row-overwrite'),
      post('ch22-parser-drops-refund'),
      post('ch17-db-rejected-it'),
  ...
  },
  {
    ...
    posts: [post('ch8-reconciliation-judgement'), post('ch9-batch-ownership'), post('ch37-split-hypothesis-rejected')],
    col: 4,
    row: 1,
  },
  {
    ...
    posts: [
      post('ch33-reco-measured-not-adopted'),
      post('ch35-cdc-default-was-slow'),
      post('ch39-cache-compression-threshold'),
      post('ch40-overload-policy-coverage'),
      post('ch41-late-events-merge'),
      post('ch43-home-pagination-session'),
      post('ch44-genpage-mini'),
      post('ch49-cdc-running-not-flowing'),
      post('ch50-genpage-empty-body'),
      post('ch51-ab-synthetic-users'),
      post('ch54-admission-observed-throughput'),
      post('ch55-compression-under-concurrency'),
      post('ch56-next-page-purchase-history'),
      post('ch57-overload-real-genpage'),
      post('ch60-genpage-v2-replica'),
      post('ch61-model-input-lost-facts'),
      post('ch62-sold-out-while-generating'),
      post('ch63-heavy-model-overload-budget'),
    ],
    col: 2,
    row: 2,
  },
  {
    id: 'verification',
    name: '검증·CI',
    tagline: '실 DB 테스트와 CI',
    summary: 'PR마다 실제 MySQL·Redis로 통합 테스트를 돌리고, 건너뛴 테스트는 통과로 치지 않습니다. 측정용 테스트는 매일 한 번 돌리고, PR은 바뀐 모듈의 테스트만 고릅니다.',
    modules: ['ci', 'testsupport'],
    doc: { label: 'CI 워크플로', url: `${REPO}/.github/workflows/ci.yml` },
    posts: [post('ch59-ci-integration-time')],
    col: 1,
    row: 2,
  },
  {
